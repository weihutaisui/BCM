...
cleanup() {
	...
	iptables -t nat -D PREROUTING -i $LAN_DEV -p tcp -j REDSOCKS 2> /dev/null
...
####### Configure Iptables to redirect tcp traffic from LAN to redsocks ########
		...
		iptables -t nat -I REDSOCKS -p tcp -d $PROXY_SERVER_ADDR --dport $PROXY_SERVER_PORT -j RETURN
		...
		iptables -t nat -I PREROUTING -i $LAN_DEV -p tcp -j REDSOCKS

		# add rules to tap locally generated tcp traffic to REDSOCKS
		#[ ! -z "$WAN1_DEV" ] && iptables -t nat -A OUTPUT -o $WAN1_DEV -p tcp -j REDSOCKS
		#[ ! -z "$WAN2_DEV" ] && iptables -t nat -A OUTPUT -o $WAN2_DEV -p tcp -j REDSOCKS
