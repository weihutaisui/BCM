...
*    Description:
*
*      OMCI header file for GPON OMCI read/write APIs, OMCI protocol,
*      message types, message format, etc.
*
*****************************************************************************/
...
#define OMCI_PACKET_ALARM_BITMAP_SIZE 28
#define OMCI_PACKET_ALARM_NUM_MAX     223 /* (28 * 8) - 1 */

#define OMCI_PACKET_PRIORITY(p) (UINT8)(((p)->tcId[0] & 0x80) >> 7)
...
#define OMCI_GET_OVERHEAD         3 /* attributes in the Get response start in byte 3 */
#define OMCI_AVC_OVERHEAD         2 /* attributes in the AVC start in byte 3 */
...
/* OMCI operation related definitions. */

#define IS_ONE_BIT_SET(b) (b && !(b & (b - 1)))
...
struct omciPacket_struct
{
    ...
    /* Not part of OMCI message format, used internally for CMS messages */
    UINT32 src_eid;
...
#define OMCI_PACKET_SIZE(m) \
  ((m)->devId == OMCI_PACKET_DEV_ID_A ? OMCI_PACKET_A_SIZE : \
  (OMCI_PACKET_B_HDR_SIZE + OMCI_NTOHS(&(m)->B.msgLen) + OMCI_PACKET_MIC_SIZE))
#define OMCI_PACKET_MSG(m) \
  ((m)->devId == OMCI_PACKET_DEV_ID_A ? \
  (m)->A.msg : (m)->B.msg)
#define OMCI_PACKET_MSG_LEN(m) \
  ((m)->devId == OMCI_PACKET_DEV_ID_A ? \
  OMCI_PACKET_A_MSG_SIZE : OMCI_NTOHS(&(m)->B.msgLen))

/*
 * Return pointer to CRC for type A, or MIC location, right after B.msgLen bytes,
 * should be set last before sending message.
 */
#define OMCI_PACKET_CRC(m) \
  ((m)->devId == OMCI_PACKET_DEV_ID_A ? \
  &((m)->A.trailer[4]) : &((m)->B.msg[OMCI_NTOHS(&(m)->B.msgLen)]))
