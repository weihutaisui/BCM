...
#
# This script reads the data-model in XML format.
...
# process the data-model.xml from stdin in a single pass.
# Towards the end of this section, we have more complicated functions
# that suck in the data-model.xml and builds an internal data
...
#            print "input=> $oid $rowHash{\"name\"}\n";
            ...
            {
                # since we input both the data-model-merged.xml and
...
# The omci_objectid.h file contains OID for both the IGD and Device data
...
sub output_mdmObjectIdFile
{
    ...
    print $fileRef "#define OMCI_MAX_OID $maxOid\n\n";
}
    ...
    while (parse_row($inputFileRef, \%rowHash))
    {
        if (($rowHash{"type"} =~ /object/i) && ($rowHash{"supportLevel"} ne "NotSupported"))
        {
            ...
            if ($conditionalLine eq "")
            {
                ...
                $handlerFuncsLine = "$rclfunc, $stlfunc},\n";
            }
            else
            {
                # bracket rcl/stl handler functions under ifdef
                $handlerFuncsLine = "#if $conditionalLine\n";
                $handlerFuncsLine = $handlerFuncsLine . "    $rclfunc, $stlfunc},\n";
                $handlerFuncsLine = $handlerFuncsLine . "#else\n";
                $handlerFuncsLine = $handlerFuncsLine . "    NULL, NULL},\n";
                $handlerFuncsLine = $handlerFuncsLine . "#endif\n";
            }

            $oidToHandlerFuncsHash{$oid} = $handlerFuncsLine;
        }
    }
...
sub output_report
{
    ...
    while (parse_row($inputFileRef, \%rowHash))
    {
        if ($rowHash{"type"} =~ /object/i)
        {
            ...
            #
            # dont' print out unsupported objects.
            #
            if ($rowHash{"supportLevel"} ne "NotSupported")
            {
                if ($pure_device2 == 1)
...
sub insert_profile_info
{
    ...
    if ($currProfile eq "Unspecified")
    {
        $currProfile = "Baseline:1";
    }
...
#    print "${$startObjRef}{\"shortObjectName\"} numChildren=$numChildren\n";
    ...
    $lowestObjRef = find_lowest_fake_parent_object($firstChildObjRef);
...
#        print "got deleteObj for $arg1\n";
    ...
    if ($cmd eq "deleteParam")
    {
       ...
       # omci_object.h is generated, so the objects must have the
       # same definition!
...
#        print "process_merge_cmd: filename=$arg1 \n";
        ...
        $lowestObjRef = find_lowest_fake_parent_object($srcObjRef);
...
#
# Output the valid strings array to data-model-merged.xml, and 
...
# 
# Output the objects and parameters to data-model-merged.xml
#
...
sub output_merged_data_model
{
    ...
    print $fileoutRef "<xmlMandatorySingleRootNode copyright=\"Broadcom, 2017\" ";
...
sub output_mdmParams
{
    ...
    $fileRef = open_filehandle_for_output("$basedir/inc/omci_params.h");

    autogen_warning($fileRef);

    print $fileRef "#ifndef __OMCI_PARAMS_H__\n";
    print $fileRef "#define __OMCI_PARAMS_H__\n\n";

    doxygen_header($fileRef, "omci_params.h");
    ...
    print $fileRef "#endif /* __OMCI_PARAMS_H__ */\n";
...
sub output_mdm
{
    ...
    print $fileRef "#include \"omci_rcl.h\"\n";
    print $fileRef "#include \"omci_stl.h\"\n";
...
if ($ARGV[0] eq "objectid")
{
    ...
    $outputFileRef = open_filehandle_for_output("$build_dir/inc/omci_objectid.h");

    print $outputFileRef "#ifndef __OMCI_OBJECTID_H__\n";
    print $outputFileRef "#define __OMCI_OBJECTID_H__\n\n\n";

    autogen_warning($outputFileRef);

    doxygen_header($outputFileRef, "omci_objectid.h");

    output_mdmObjectIdFile($outputFileRef);

    print $outputFileRef "\n\n#endif /* __OMCI_OBJECTID_H__ */\n";
...
elsif ($ARGV[0] eq "oidinfo" || $ARGV[0] eq "oidinfo2")
{
    ...
    {
        $outputFileName = "omci_oidInfoArray.c";
        $baseProfile = "defined(BRCM_OMCI)";
        $arrayName = "oidInfoArray_omci";
        $arrayDecl = "const _MdmOidInfoEntry oidInfoArray_omci[]= { \n";
    }

    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile");
    $outputFileRef = open_filehandle_for_output("$build_dir/schema/$outputFileName");

    autogen_warning($outputFileRef);

    print $outputFileRef "#if $baseProfile\n\n";
    print $outputFileRef "#include \"mdmlite_defs.h\"\n";
    print $outputFileRef "#include \"omci_rcl.h\"\n";
    print $outputFileRef "#include \"omci_stl.h\"\n\n";
    ...
    print $outputFileRef "\n};\n";
    print $outputFileRef "\nint cnt_$arrayName = (sizeof($arrayName) / sizeof(_MdmOidInfoEntry));\n\n";

    print $outputFileRef "void mdm_getPtrs_$arrayName(const _MdmOidInfoEntry **begin, const _MdmOidInfoEntry **end)\n";
    print $outputFileRef "{\n";
...
elsif ($ARGV[0] eq "object")
{
    ...
    $fileRef = open_filehandle_for_output("$build_dir/inc/omci_object.h");

    print $fileRef "#ifndef __OMCI_OBJECT_H__\n";
    print $fileRef "#define __OMCI_OBJECT_H__\n\n\n";

    autogen_warning($fileRef);

    doxygen_header($fileRef, "omci_object.h");

    print $fileRef "#include \"os_defs.h\"\n\n";
...
elsif ($ARGV[0] eq "report")
{
    ...
    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile");
    $outputFileRef = open_filehandle_for_output("$build_dir/data-model/OMCI-supported-parameters-report.txt");
    ...
    $spreadsheet1Done = 0;
    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile");
    $outputFileRef = open_filehandle_for_output("$build_dir/data-model/OMCI-supported-profiles-report.txt");
...
elsif ($ARGV[0] eq "report2")
{
    ...
    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile");
    $outputFileRef = open_filehandle_for_output("$build_dir/data-model/OMCI-supported-parameters-report2.txt");
    ...
    $spreadsheet1Done = 0;
    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile");
    $outputFileRef = open_filehandle_for_output("$build_dir/data-model/OMCI-supported-profiles-report2.txt");
...
elsif ($ARGV[0] eq "mdmparams")
{
    ...
    # input the second data model file (data-model-merged2.xml)
...
elsif ($ARGV[0] eq "validstrings")
{
    ...
    $fileRef = open_filehandle_for_output("$build_dir/inc/omci_validstrings.h");

    autogen_warning($fileRef);

    print $fileRef "#ifndef __OMCI_VALIDSTRINGS_H__\n";
    print $fileRef "#define __OMCI_VALIDSTRINGS_H__\n\n";

    output_validstrings($fileRef);

    print $fileRef "#endif /* __OMCI_VALIDSTRINGS_H__ */\n";
...
elsif ($ARGV[0] eq "prototypes")
{
    ...
    $fileRef1 = open_filehandle_for_output("$build_dir/inc/omci_rcl.h");
    $fileRef2 = open_filehandle_for_output("$build_dir/inc/omci_stl.h");

    autogen_warning($fileRef1);
    autogen_warning($fileRef2);

    print $fileRef1 "#ifndef __OMCI_RCL_H__\n";
    print $fileRef2 "#ifndef __OMCI_STL_H__\n";

    print $fileRef1 "#define __OMCI_RCL_H__\n\n";
    print $fileRef2 "#define __OMCI_STL_H__\n\n";

    print $fileRef1 "#include \"omci_objectid.h\"\n";
    print $fileRef1 "#include \"omci_object.h\"\n\n";

    print $fileRef2 "#include \"omci_objectid.h\"\n";
    print $fileRef2 "#include \"omci_object.h\"\n\n";
    ...
    print $fileRef1 "#endif /* __OMCI_RCL_H__ */\n";
    print $fileRef2 "#endif /* __OMCI_STL_H__ */\n";
...
elsif ($ARGV[0] eq "skeletons")
{
    ...
    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile");
    $fileRef1 = open_filehandle_for_output("$build_dir/schema/rcl_skel.c");
    $fileRef2 = open_filehandle_for_output("$build_dir/schema/stl_skel.c");
