...
CmsRet qdmSystem_getDeviceInfoLocked_dev2(char *oui, char *serial, char *productClass)
{
   ...
   {
      cmsLog_debug("cmsObj_get, Dev2DeviceInfoObject success: oui %s, serialNumber %s productClass %s",
                   deviceInfoObj->manufacturerOUI,deviceInfoObj->serialNumber,deviceInfoObj->productClass);

      strcpy(oui,deviceInfoObj->manufacturerOUI);
      strcpy(serial,deviceInfoObj->serialNumber);
      strcpy(productClass,deviceInfoObj->productClass);
      cmsObj_free((void **) &deviceInfoObj);
   } 
   else
   {
      cmsLog_debug("cmsObj_get, MDMOID_DEV2_DEVICE_INFO ret %d",ret);
   }
   return ret;
}
