...
/** Destroy any state in the ODL from previous set's.
 *
 * If the PHL enounters an error in the middle of an array of parameter name /
 * values which it was setting, prior to returning, it must call this function
 * to clean up any state that had built up in the ODL.
 */
 ...
 * @return CmsRet enum.
 */
CmsRet odl_get(const MdmPathDescriptor *pathDesc,
               char **value);
