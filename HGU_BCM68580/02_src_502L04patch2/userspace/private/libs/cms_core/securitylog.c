...
CmsRet cmsLog_security(CmsSecurityLogIDs id, CmsSecurityLogData * pdata, const char *pFmt, ... )
{
   ...
   cmsLck_getInfo(&lockInfo);
   if (lockInfo.locked &&
       (lockInfo.lockOwner == getpid()))
   {
      /* We already have the lock, must be web access */
      cmsLog_debug("already have lock");
      needLock = 0;
   }

   if(needLock &&
      ((ret = cmsLck_acquireLockWithTimeout(CMSLOG_LOCK_TIMEOUT)) != CMSRET_SUCCESS ))
   {
