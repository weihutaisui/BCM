          ...
          (hasAnyParamsOrAttrsChanged(objNode, iidStack, parentAttr) && !(objNode->flags & OBN_PRUNE_WRITE_TO_CONFIG_FILE)))
      {
         ...
         if (indentBuf2 != NULL) {
            ...
            *idx += (wrote < max - (*idx)) ? wrote : max - (*idx);
            ...
            if (/* (currAttr->ownerId != parentAttr->ownerId) && */
                (currAttr->ownerId != 0))
            {
               wrote = snprintf(&buf[*idx], max - (*idx), " %s=\"%d\"", CONFIG_FILE_ATTR_OWNERID, currAttr->ownerId);
               *idx += (wrote < max - (*idx)) ? wrote : max - (*idx);
            }
                                        ...
                                        void *cbContext)
{
   ...
   if (mdmLibCtx.dumpAll || ctx->changed)
   {
      if (ctx->buf != NULL)
      {
         ...
         if ((attrDesc != NULL) &&
             (attrDesc->nodeAttr.ownerId != 0))
         {
            wrote = snprintf(&(ctx->buf[ctx->idx]), ctx->max - ctx->idx, " %s=\"%d\"", CONFIG_FILE_ATTR_OWNERID, attrDesc->nodeAttr.ownerId);
            ctx->idx += (wrote < ctx->max - ctx->idx) ? wrote : ctx->max - ctx->idx;
         }
