                              ...
                              UINT32 len)
{
       ...
       (compatChildObjNode != NULL))
   {
      ...
      if (nxmlCtx.loadMdm)
      {
          ...
          */
         if (nxmlCtx.attr.accessBitMaskChange || nxmlCtx.attr.notificationChange || nxmlCtx.attr.ownerId)
         {
            cmsLog_debug("set sub-tree attr notif=(%d)%d access=(%d)0x%x ownerId=%d",
                         nxmlCtx.attr.notificationChange,
                         nxmlCtx.attr.notification,
                         nxmlCtx.attr.accessBitMaskChange,
                         nxmlCtx.attr.accessBitMask,
                         nxmlCtx.attr.ownerId);
            ...
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("setSubTree attr failed for %s %s notification=(%d)%d accessBitMask=(%d)0x%x ownerId=%d ret=%d",
                            nxmlCtx.objNode->name,
   ...
   else if ((paramNode = mdm_getParamNode(nxmlCtx.objNode->oid, buf)) != NULL)
   {
       ...
       */
      if ((nxmlCtx.loadMdm) &&
          (nxmlCtx.attr.accessBitMaskChange || nxmlCtx.attr.notificationChange || nxmlCtx.attr.ownerId))
      {
         cmsLog_debug("set sub-tree attr notif=(%d)%d access=(%d)0x%x ownerId=%d",
                      nxmlCtx.attr.notificationChange,
                      nxmlCtx.attr.notification,
                                      ...
                                      UINT32 len)
{
   ...
   else if (!strcmp(CONFIG_FILE_ATTR_OWNERID, buf))
   {
      nxmlCtx.currXmlAttr = MDM_CONFIG_XMLATTR_OWNERID;
   }
   else
   {
                                      ...
                                      SINT32 more)
{
   ...
   case MDM_CONFIG_XMLATTR_OWNERID:
      {
         UINT32 ownerId;
         CmsRet ret;

         ret = cmsUtl_strtoul(buf, NULL, 0, &ownerId);
         if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("invalid ownerId %s", buf);
            nxmlCtx.ret = CMSRET_INVALID_ARGUMENTS;
         }
         else
         {
            nxmlCtx.attr.ownerId = ownerId;
         }
      ...
      else
      {
         ...
         else
         {
                ...
                (nxmlCtx.attr.accessBitMaskChange || nxmlCtx.attr.notificationChange || nxmlCtx.attr.ownerId))
            {
               ...
               cmsLog_debug("set single param attr notif=(%d)%d access=(%d)0x%x ownerId=%d",
                            nxmlCtx.attr.notificationChange,
                            nxmlCtx.attr.notification,
                            nxmlCtx.attr.accessBitMaskChange,
                            nxmlCtx.attr.accessBitMask,
               ...
               if (r2 != CMSRET_SUCCESS)
               {
                  cmsLog_error("setSingle attr failed for %s %s notification=(%d)%d accessBitMask=(%d)0x%x ownerId=%d ret=%d",
                               nxmlCtx.paramNode->name,
                               cmsMdm_dumpIidStack(&(nxmlCtx.iidStack)),
                               nxmlCtx.attr.notificationChange,
                               nxmlCtx.attr.notification,
