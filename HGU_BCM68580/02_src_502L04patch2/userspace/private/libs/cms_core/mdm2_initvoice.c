...
CmsRet mdm_adjustForVoiceHardware(void)
{
   ...
   cmsLog_debug("mdm_adjustForVoiceHardware 1: maxPhysEndpt %d, maxVoipEndpt %d, numFxoEndpt %d, maxCodecs %d\n",
   ...
   ret = mdm_getObject(MDMOID_VOICE, &pathDesc.iidStack, (void**)&voiceObj);
   if( ret == CMSRET_SUCCESS )
   {
       REPLACE_STRING_IF_NOT_EQUAL(voiceObj->X_BROADCOM_COM_BoundIfName, EPON_VOICE_WAN_IF_NAME);
       mdm_setObject((void **)&voiceObj, &pathDesc.iidStack, FALSE );
   }
   mdm_freeObject((void **)&voiceObj);
#endif

   /* VoiceService.{i}.Capabilities.MaxCallLogCount*/
   ret = mdm_getObject(MDMOID_VOICE_CAP, &pathDesc.iidStack, &obj);
   if( ret == CMSRET_SUCCESS )
   {
      pathDesc.oid = MDMOID_VOICE_CAP;
      ((VoiceCapObject *)obj)->maxCallLogCount = 20;
      mdm_setObject((void **)&obj, &pathDesc.iidStack, FALSE );
   }
