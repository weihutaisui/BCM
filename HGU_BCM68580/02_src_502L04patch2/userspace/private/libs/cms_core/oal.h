 ...
 *
 * @param timeoutMs (IN) If a blocking call is desired, then set 
 *                       the timeoutMs pointer to NULL.  Otherwise, it points
 *                       to a UINT32 which is the number of milliseconds to
 *                       wait for the lock.  If timer expires, the return
 *                       value will be CMSRET_TIMED_OUT.
 *
 * @return CmsRet enum.
 */
