...
PostActiveNodeInfo postMdmInfo[] =
    {
#ifdef BCM_PON
        /* Filter entry need to waiting for wan and wlan port ready */ 
        {MDMOID_FILTERS_CFG,               MDM_POST_ACT_TYPE_FILTER,    1},
        {MDMOID_INGRESS_FILTERS_DATA,      MDM_POST_ACT_TYPE_FILTER,    1},  
#else
        /* No Post activating data */
        {MDM_MAX_OID+1,                    MDM_POST_ACT_TYPE_MAX,       0},  
#endif
    };

#define PostMdmNodeNum          (sizeof(postMdmInfo)/sizeof(PostActiveNodeInfo))
...
CmsRet cmsMdm_initWithAcc(CmsEntityId eid, UINT32 acc, void *msgHandle, SINT32 *shmId)
{
    ...
    * RCL handler function may call RUT function, which then may call
    * a cmsObj_xxx function.  cmsObj_xxx functions require a lock, even though
    * there are no apps that could possibly be accessing the MDM at this point.
    */
   cmsLog_debug("Activating objects, acquire lock...");
   
   cmsLck_acquireLock();
   
   mdmLibCtx.entryCount = 1;
   mdmShmCtx->inMdmInit = TRUE;
   
   INIT_INSTANCE_ID_STACK(&iidStack);
   mdm_activateObjects(mdmShmCtx->rootObjNode, &iidStack, FALSE);
   
   mdmShmCtx->inMdmInit = FALSE;
   mdmLibCtx.entryCount = 0;
   ...
   cmsLog_debug("Done activating objects, first attach init done, ret=%d", ret);
...
CmsRet mdm_adjustForHardware_igd(void)
{
   ...
   if (ret == CMSRET_SUCCESS)
   {
      ret = mdm_addDefaultWanCellularObjects();
   }
   if (ret == CMSRET_SUCCESS)
   {
      ret = mdm_addDefaultCellularAPNObjects();
   }
#endif
   ...
   if (ret == CMSRET_SUCCESS)
   {
      ret = mdm_addDefaultCellularIntfObjects();
   }
#endif
...
void cmsMdm_postHandle(UINT32 subType)    
{
   ...
   for (i = 0 ; i < PostMdmNodeNum; i++)
   {
      if (postMdmInfo[i].subType == subType)
      {
         postMdmInfo[i].entCunt--;
         if (postMdmInfo[i].entCunt == 0)
         {
            if ((objNode = mdm_getObjectNode(postMdmInfo[i].oid)) == NULL)
            {
               cmsAst_assert(0);
            }        
            
            INIT_INSTANCE_ID_STACK(&iidStack);                          
            mdm_activateObjects(objNode, &iidStack, TRUE);
         }
      }
   }       
   mdmLibCtx.entryCount = 0;
                         ...
                         UBOOL8 isSingleNode)
{
   ...
   UINT32 i;
   UBOOL8 postMdm = FALSE;   
   CmsRet ret;

   for (i = 0; i < PostMdmNodeNum; i++)
   {
      if ( (objNode->oid == postMdmInfo[i].oid) &&
           (postMdmInfo[i].entCunt != 0))
      {
         postMdm = TRUE;
      }
   }
            ...
            (DEPTH_OF_IIDSTACK(iidStack) + 1 == objNode->instanceDepth))
   {
      ...
      while (instDesc != NULL)
      {
         ...
         mdm_activateObjects(objNode, &subTreeIidStack, isSingleNode);
   ...
   for (i=0; (i < objNode->numChildObjNodes) && (!isSingleNode); i++)
   {
      mdm_activateObjects(&(objNode->childObjNodes[i]), iidStack, isSingleNode);
   }
...
UBOOL8 fixNumberOfEntriesBridging()
{
   ...
   cmsObj_free((void **) &l2BridgingObj);
