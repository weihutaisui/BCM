                           ...
                           void ** L2LinkCfgObj)
{
...
#ifdef DMP_X_BROADCOM_COM_CELLULARWAN_1
   else if (!cmsUtl_strcmp(wanCommIntf->WANAccessType, MDMVS_X_BROADCOM_COM_CELLULAR))
   {
      isCellularWan = TRUE;
   }              
#endif
   ...
   if (isCellularWan)
   {
      ...
      if ((ret = cmsObj_getAncestorFlags(MDMOID_WAN_CELLULAR_INTERFACE_CFG, 
                                  wanConnOid,
                                  &wanConnIid, 
                                  0,
                                  (void **) &linkCfg)) != CMSRET_SUCCESS)
      {
         cmsLog_error("Failed to get cellularIntfObj from %s", cmsMdm_dumpIidStack( (void *) &wanConnIid));
         return ret;
      }
      cmsLog_debug("Got cellularIntfObj."); 
   }
...
#ifdef DMP_X_BROADCOM_COM_CELLULARWAN_1 
      case MDMOID_WAN_CELLULAR_INTERFACE_CFG:
      {
         WanIpConnObject *ipConnObj = NULL;         
         InstanceIdStack ipConnIid = EMPTY_INSTANCE_ID_STACK;
 
         if (cmsObj_getNextInSubTree(MDMOID_WAN_IP_CONN, iidStack, &ipConnIid, (void **)&ipConnObj) != CMSRET_SUCCESS)
         {
             cmsLog_error("Cannot get cellular MDMOID_WAN_IP_CONN!!");
             ret = CMSRET_INTERNAL_ERROR;
         }
         else
         {
            *isLayer2LinkUp = cmsNet_isInterfaceExist(ipConnObj->X_BROADCOM_COM_IfName);
            cmsUtl_strcpy(l2IfName, ipConnObj->X_BROADCOM_COM_IfName);
            *connMode = cmsUtl_connectionModeStrToNum(MDMVS_DEFAULTMODE);
         }
      }
      break;      
#endif /* DMP_X_BROADCOM_COM_CELLULARWAN_1 */
