...
#define DHCP_OPTION_REPORT_LIST  "43_60" /* Pls connect multi codes with "_" */
#define DHCP_OPTION_REQUEST_LIST  "122"  /* Pls connect multi codes with "_" */
#endif
    ...
    UBOOL8 op212)
{
   ...
   if (!IS_EMPTY_STRING(leasedTime) != 0)
   {
    ...
    rutDhcp_mkCfgDir(DHCP_V4, ifName);
#if defined(BRCM_PKTCBL_SUPPORT)
    create_dhcpOption43(ifName, duid);
    create_dhcpOption60(ifName);
    sprintf(&cmdLine[strlen(cmdLine)], " -R %s", DHCP_OPTION_REPORT_LIST);
    sprintf(&cmdLine[strlen(cmdLine)], " -Q %s", DHCP_OPTION_REQUEST_LIST);
#endif
...
CmsRet rutWan_initPPPoE_igd(const InstanceIdStack *iidStack,  void *obj)
{
   ...
   {
      snprintf(cmdLine, sizeof(cmdLine), "%s -o %d", cmdLine,  newObj->idleDisconnectTime);
   }
...
CmsRet rutWan_getAvailableConIdForCellular(SINT32 *outConId)
{
   _WanIpConnObject *ipConn=NULL;
   CmsRet ret = CMSRET_SUCCESS;
   SINT32 conId;
   SINT32 conIdArray[IFC_WAN_MAX+1];
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   InstanceIdStack linkIidStack = EMPTY_INSTANCE_ID_STACK;

   memset(conIdArray, 0, sizeof(conIdArray));
