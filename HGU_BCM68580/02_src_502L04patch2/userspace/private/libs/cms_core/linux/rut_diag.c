...
void rutEthOam_set3ahService(UBOOL8 enabled)
{
   ...
   if(enabled && ieee8023ahCfg->enabled)
   {
      ...
      else
      {
         ...
         if ((ret = odl_setObjectNoRclCallback((void *)ieee8023ahCfg, &ieee8023ahCfgIidStack)) != CMSRET_SUCCESS)
         {
            cmsLog_error("odl_setObject ieee8023ahCfg returns error. ret=%d", ret);
         }
      }
   }
   else
...
void rutEthOam_set1agService(UBOOL8 enabled)
{
   ...
   {   
      ...
      else
      {
         ...
         if ((ret = odl_setObjectNoRclCallback((void *)ieee8021agCfg, &ieee8021agCfgIidStack)) != CMSRET_SUCCESS)
         {
            cmsLog_error("odl_setObject ieee8021agCfg returns error. ret=%d", ret);
         }
      }
   }
   else
