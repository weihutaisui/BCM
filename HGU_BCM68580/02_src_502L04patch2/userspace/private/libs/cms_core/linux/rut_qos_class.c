                           ...
                           QosCommandType cmdType)
{
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      sprintf(cmd, "%s -t mangle -A OUTPUT %s %s %s %s %s -j MARK --or-mark 0x%x",
              tblStr, egressIfc, proto, sport, dport, dscpCheck, flowId);
   }
   else
   {
      sprintf(cmd, "%s -t mangle -D OUTPUT %s %s %s %s %s -j MARK --or-mark 0x%x 2>/dev/null",
              tblStr, egressIfc, proto, sport, dport, dscpCheck, flowId);
   }
                             ...
                             QosCommandType cmdType)
{
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      sprintf(cmd, "%s -t mangle -A PREROUTING %s %s %s %s %s %s %s %s -j MARK --or-mark 0x%x",
              tblStr, proto, ifcStr, smac, src, dst, sport, dport, dscpCheck, flowId);
   }
   else
   {
      sprintf(cmd, "%s -t mangle -D PREROUTING %s %s %s %s %s %s %s %s -j MARK --or-mark 0x%x 2>/dev/null",
              tblStr, proto, ifcStr, smac, src, dst, sport, dport, dscpCheck, flowId);
   }
...
//   }
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
       ...
       */
      sprintf(cmd, "%s -t mangle -N rule%d", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMark", cmd);

      /* mark priority queue */
      sprintf(cmd, "%s -t mangle -A rule%d -j MARK --or-mark 0x%x", tblStr, clsKey, queueMark);
      rut_doSystemAction("rutQos_roEgressMark", cmd);

      if (!IS_EMPTY_STRING(set_dscp))
      {
         /* mark dscp */
         sprintf(cmd, "%s -t mangle -A rule%d %s", tblStr, clsKey, set_dscp);
         rut_doSystemAction("rutQos_roEgressMark", cmd);
      }
      
      sprintf(cmd, "%s -t mangle -A rule%d -j ACCEPT", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMark", cmd);

      sprintf(cmd, "%s -t mangle -A POSTROUTING %s -m mark --mark 0x%x/0x%x -j rule%d",
              tblStr, egressIfc, flowId, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_roEgressMark", cmd);
   }
   else
   {
      sprintf(cmd, "%s -t mangle -D POSTROUTING %s -m mark --mark 0x%x/0x%x -j rule%d 2>/dev/null",
              tblStr, egressIfc, flowId, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_roEgressMark", cmd);
      sprintf(cmd, "%s -t mangle -F rule%d 2>/dev/null", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMark", cmd);
      sprintf(cmd, "%s -t mangle -X rule%d 2>/dev/null", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMark", cmd);
   }
...
CmsRet rutQos_qMgmtClassDelete_igd(const char *l3Ifcname)
{
   ...
   while ((ret = cmsObj_getNext(MDMOID_Q_MGMT_CLASSIFICATION, &iidStack, (void **)&cObj)) == CMSRET_SUCCESS)
   {
      if (cmsUtl_strcmp(l3Ifcname, cObj->X_BROADCOM_COM_egressInterface) == 0)
      {
         ...
         if ((ret = odl_deleteObjectInstance(&pathDesc)) != CMSRET_SUCCESS)
         {
            cmsLog_error("odl_deleteObjectInstance returns error. ret = %d", ret);
            cmsObj_free((void **)&cObj);
            return ret;
         }
