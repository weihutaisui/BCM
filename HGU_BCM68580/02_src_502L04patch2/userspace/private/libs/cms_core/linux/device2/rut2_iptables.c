...
CmsRet rutIpt_ipFilterInRunIptables_dev2(char *ipver, char action, const char *ifName, char *protocol, char *src, char *sport, char *dst, char *dport,char *target)
{
   ...
   snprintf(cmd, sizeof(cmd), "%s -%c FORWARD -i %s %s %s %s %s %s %s", ipt, action, ifName, protocol, src, sport, dst, dport,target);
   ...
   */
   if (action == 'D')
   {
      cmsUtl_strncat(cmd, sizeof(cmd), " 2>/dev/null");
   }
   ...
   */   
   ...
   {
      snprintf(cmd, sizeof(cmd), "%s -%c INPUT -i %s %s %s %s %s %s %s", ipt, action, ifName, protocol, src, sport, dst, dport,target);
      if (action == 'D')
      {
         cmsUtl_strncat(cmd, sizeof(cmd), " 2>/dev/null");
      }      
...
CmsRet rutIpt_AddfwExceptionforIPDevice_dev2(char *ipIntfFullPath)
{
   ...
   if(!found)
   {
      ...
      cmsObj_set((void *)fwExObj,&iidStack);
      cmsObj_free((void **)&fwExObj);
   }else
   {
...
CmsRet rutIpt_vrtsrvCfg_dev2(const _Dev2NatPortMappingObject *portmapObj, const UBOOL8 add ,char *portmap_ifName)
{
   ...
   do{
         ...
         if( externalPort==0 && internalPort==0)
         {
             ...
             {
                 sprintf(cmd, "iptables -t nat -D PREROUTING -i %s -j DNAT --to-destination %s 2>/dev/null", interface, internalClient);
                 rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
                 sprintf(cmd, "iptables -D FORWARD -i %s -d %s -j ACCEPT  2>/dev/null", interface, internalClient);
                 rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);   

                 if( action == 'A')   
                 {
                    sprintf(cmd, "iptables -t nat -A PREROUTING -i %s -j DNAT --to-destination %s 2>/dev/null",interface, internalClient);
                    rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);

                    sprintf(cmd, "iptables -I FORWARD 1 -i %s -d %s -j ACCEPT 2>/dev/null", interface, internalClient);
                    rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);  
                 }
             }
         ...
         }else
         {
            ...
            while( entry_ptr->name != NULL )
            {
                ...
                {
                    sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -p %s --dport %d -j REDIRECT --to-ports %d 2>/dev/null"
                            ,action
                            ,interface
...
void rutIpt_configNatForIntfGroup_dev2(UBOOL8 isAdd, const char *ifName, const char *bridgeIfName, UBOOL8 isFullCone)
{
...
#endif
   {
      ...
      {
         sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -d %s -p udp --dport 53 -j DNAT --to %s", 
