             ...
             !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_DISCONNECTED))      
         {
            ...
            */
            ...
            if (ret == CMSRET_SUCCESS && newObj->idleDisconnectTime > 0)
            {
               ...
               rutIpt_initNatForPppOnDemand(newObj->name, qdmIpIntf_isFullConeNatEnabledOnIntfNameLocked_dev2(newObj->name));
            }
         }
      }
         ...
         */
                  ...
                  !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_DISCONNECTED))
         {
            ...
            */
            ...
            /* Do we need this ???
            *
            * Adding ip address of PPP interface to IP interface is not really needed
            * but does not seem to be a problem if an extra command such as 
            * "ifconfig ppp0 10.6.37.193 netmask 255.255.255.255 broadcast 10.6.37.255 up".  By doing this
            * IP interface will have the ipv4Adress object which is symmetrical with dynamtic IPoE.
            */
            ...
            */
            {
               ...
               {  
                  /* If lower level is up, do not kill pppd 
                   */
