                                 ...
                                 CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (cmsUtl_strcmp(newObj->diagnosticsState,MDMVS_REQUESTED) == 0)
   {
      ...
      */
      ...
      {
         // add -d option when ping is executed from tr69c application
         sprintf(cmdLine, "-c %d -s %d -q -m -d %d %s", (int)newObj->numberOfRepetitions,
                 newObj->dataBlockSize,EID_TR69C, newObj->host);
      }
                                          ...
                                          CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (buf[0])
   {
      ...
      if (qdmIntf_fullPathToIntfnameLocked(buf, if_name_cmd) == CMSRET_SUCCESS)
         sprintf(buf, "iptables -D INPUT -i %s -p udp --dport %d -j ACCEPT", if_name_cmd, currObj->UDPPort);
   }
   else
      sprintf(buf, "iptables -D INPUT -p udp --dport %d -j ACCEPT", currObj->UDPPort);
   ...
   if (newObj->enable)
   {
      ...
      if (buf[0])
      {
         ...
         if (qdmIntf_fullPathToIntfnameLocked(buf, if_name_cmd) == CMSRET_SUCCESS)
            sprintf(buf, "iptables -A INPUT -i %s -p udp --dport %d -j ACCEPT", if_name_cmd, newObj->UDPPort);
      }
      else
         sprintf(buf, "iptables -A INPUT -p udp --dport %d -j ACCEPT", newObj->UDPPort);
