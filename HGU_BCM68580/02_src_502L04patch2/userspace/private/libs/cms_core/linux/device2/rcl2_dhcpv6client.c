                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj && currObj)
   {
      ...
      /* update MAPT if changed */
      if (!IS_EMPTY_STRING(newObj->maptBRPrefix) &&
          cmsUtl_strcmp(newObj->maptBRPrefix, currObj->maptBRPrefix))
      {
         rutDhcpv6_updateMapt(dhcpcObj->interface, newObj->maptBRPrefix, newObj->maptRuleIPv4Prefix, 
                 newObj->maptRuleIPv6Prefix, newObj->maptEALen, newObj->maptPSIDOffset,
                 newObj->maptPSIDLen, newObj->maptPSID, newObj->maptIsFMR);
      }
