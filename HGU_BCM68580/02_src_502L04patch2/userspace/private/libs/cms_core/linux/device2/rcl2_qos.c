                                    ...
                                    CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      {
         if (currObj->X_BROADCOM_COM_ClassPolicer > 0)
         {
            /* also delete the associated policer */
            rutQos_deletePolicerByInstance_dev2(currObj->X_BROADCOM_COM_ClassPolicer);
         }
                           ...
                           CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj != NULL && currObj != NULL)
   {
      ...
      if (newObj->enable)
      {
         ...
         {
            cmsLog_notice("configuring queue %s for (UP intf) %s",
                  newObj->X_BROADCOM_COM_QueueName, newObj->interface);
            /* Queue is still enabled, and interface is UP, config it */
