                      ...
                      const InstanceIdStack *iidStack __attribute__((unused)))
{
   ...
   if (mdmLibCtx.eid != EID_WLMNGR && mdmLibCtx.eid != EID_SSK )
   {
      ...
      if (wlcsm_mngr_dm_get(radioIndex, 0,  MDMOID_DEV2_WIFI_RADIO, offsetof(Dev2WifiRadioObject, possibleChannels), varValue) != NULL)
      {
         CMSMEM_REPLACE_STRING_FLAGS(obj->possibleChannels, varValue, mdmLibCtx.allocFlags);
      }
      ...
      if (obj->autoChannelEnable)
      {
         ...
         {
            obj->channel = atoi(varValue);
            CMSMEM_REPLACE_STRING_FLAGS(obj->channelsInUse, varValue, mdmLibCtx.allocFlags);
         }
      }

      memset(varValue, 0, sizeof(varValue));
      if (wlcsm_mngr_dm_get(radioIndex, 0,  MDMOID_DEV2_WIFI_RADIO, offsetof(Dev2WifiRadioObject, supportedFrequencyBands), varValue) != NULL)
      {
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->supportedFrequencyBands, varValue, mdmLibCtx.allocFlags);
      }
      
      memset(varValue, 0, sizeof(varValue));
      if (wlcsm_mngr_dm_get(radioIndex, 0,  MDMOID_DEV2_WIFI_RADIO, offsetof(Dev2WifiRadioObject, operatingFrequencyBand), varValue) != NULL)
      {
         if (strcmp(varValue, "5") == 0) 
         {
            REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->operatingFrequencyBand, MDMVS_5GHZ, mdmLibCtx.allocFlags);
         }
         else {
            REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->operatingFrequencyBand, MDMVS_2_4GHZ, mdmLibCtx.allocFlags);
         }
      }
   }
