                             ...
                             UBOOL8 isWan, UBOOL8 isBridge)
{
   ...
   if (isStaticIpIntf)
   {
       ...
       * XXX hmmm, really?  This is just the local subnet right?
       * When did we add this route?  and why don't we
       * have to do that for dynamic IPoE?
       */
      ...
      {
         cmsLog_error("qdmIpIntf_getIpv4AddrAndSubnetByNameLocked_dev2 failed on %s.", ifname);
         return;
      }
