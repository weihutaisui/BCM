...
void rutIpv6Service_start(const char *ipIntfFullPath, const char *ifname, UBOOL8 isWan)
{
   ...
   /* dhcp6c not used in cellular wan */
   if (!cmsUtl_strstr(ifname, CELLULAR_IFC_STR))
#endif
   rutIpv6Service_launchDhcp6c(ipIntfFullPath, ifname);
...
#ifdef SUPPORT_MAPT
      rutMap_maptControl(ipIntfFullPath);
#endif
...
#ifdef SUPPORT_MAPT
      mapt = rutTunnel_isMaptDynamic(ipIntfFullPath);
#endif
