...
void rutNtpd_restart_dev2(void)
{
   ...
   if (cmsObj_get(MDMOID_DEV2_TIME, &iidStack, 0, (void **) &timeObj) == CMSRET_SUCCESS)
   {
      ...
      if (timeObj->localTimeZone && timeObj->X_BROADCOM_COM_LocalTimeZoneName)
      {
         int i = 0;
         char *ptr = timeObj->X_BROADCOM_COM_LocalTimeZoneName;
         FILE *tzFile = NULL;
         bzero(cmd, BUFLEN_1024*4);
         //while (isalpha(*ptr) && i < 6)
         while (isalpha(*ptr) && i < _POSIX_TZNAME_MAX)
         {
            cmd[i++]=*ptr;
            ptr++;
         }
         ptr = timeObj->localTimeZone;
         while (*ptr != '\0')
         {
            if (*ptr == ' ')
               cmd[i++]='-';
