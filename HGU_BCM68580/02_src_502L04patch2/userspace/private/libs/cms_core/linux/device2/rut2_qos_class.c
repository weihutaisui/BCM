...
void rutQos_reconfigAllClassifications_dev2(const char *intfName)
{
   ...
   /* see rclQos_classConfig (basically defaultPolicy for wlan) */
   {
      ...
      {
         if (cmsUtl_strstr(qObj->interface, TR181_WIFI_INTF_PATH) != NULL)
         {
            /* each Wifi.SSID has 8 queues, but we only want to do default
             * policy for each interface once.
             */
            if (strcmp(qObj->interface, qIntf) != 0)
            {
               char wlIfcname[CMS_IFNAME_LENGTH]={0};

               strcpy(qIntf, qObj->interface);

               if ((ret = qdmIntf_fullPathToIntfnameLocked(qObj->interface, wlIfcname)) != CMSRET_SUCCESS)
               {
                  cmsLog_error("Could not convert fullpath %s to intfName, ret=%d",
                                qObj->interface, ret);
               }
               else
               {
                  ...
                  {
                     rutQos_doDefaultWlPolicy(wlIfcname, TRUE);
                  }
                  else
                  {
                     rutQos_doDefaultWlPolicy(wlIfcname, FALSE);
                  }
               }
            }
         }
