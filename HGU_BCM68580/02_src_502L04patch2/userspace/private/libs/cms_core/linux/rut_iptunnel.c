                           ...
                           const char *tunnelName, const char *lanIntf, UBOOL8 add)
{
   ...
   if ( add )
   {
      char ipv6AddrStr[CMS_IPADDR_LENGTH];
...

      {
         ...
         cmsUtl_strncpy(tmp, ipv6str, CMS_IPADDR_LENGTH);
         snprintf(ipv6str, sizeof(ipv6str), "%s/%d", tmp, plen+(32-ipv4MaskLen)); /* Prefix delegation */
      }
