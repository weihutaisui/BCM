...
CmsRet stl_wanCommonIntfCfgObject(_WanCommonIntfCfgObject *obj, const InstanceIdStack *iidStack)
{
   ...
   else if (!cmsUtl_strcmp(obj->WANAccessType, MDMVS_X_BROADCOM_COM_CELLULAR))
   {
#ifdef DMP_CELLULARINTERFACEBASE_1
      Dev2CellularInterfaceObject *cellularIntfObj = NULL;
      InstanceIdStack cellularIidStack;

      INIT_INSTANCE_ID_STACK(&cellularIidStack);
      if ((ret = cmsObj_getNext(MDMOID_DEV2_CELLULAR_INTERFACE, &cellularIidStack, (void **)&cellularIntfObj)) != CMSRET_SUCCESS)
      {
         cmsLog_error("could not get MDMOID_DEV2_CELLULAR_INTERFACE object, ret=%d", ret);
         return ret;
      }
     
      if (!cmsUtl_strcmp(cellularIntfObj->status, MDMVS_UP))
      {
         cmsLog_debug("cellular wan up");
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->physicalLinkStatus, MDMVS_UP, mdmLibCtx.allocFlags);
      }
      else
      {
         cmsLog_debug("cellular wan down");
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->physicalLinkStatus, MDMVS_DOWN, mdmLibCtx.allocFlags);
      }
