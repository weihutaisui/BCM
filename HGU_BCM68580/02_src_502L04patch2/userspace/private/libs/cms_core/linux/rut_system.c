                          ...
                          const char *cp_user __attribute__((unused)))
{
   ...
   */
   if (cmsFil_isFilePresent(loginPasswdFile))
   {
      ...
      while ((fgets(line, BUFLEN_264, fsPwd) != NULL )) 
      {
      ...
      /* remove /var/passwd file */
      unlink(loginPasswdFile);
   }
   fclose(tmp_fp);

   /* rename the temp passwd file to /var/passwd" */
   if (cmsFil_renameFile(tmp_file, loginPasswdFile) != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to rename %s to %s", tmp_file, loginPasswdFile);
      unlink(tmp_file);
      return CMSRET_INTERNAL_ERROR;
   }

   /* Note: The /etc/group part will be dealt later on for appending the lines by other processes */
   fsGrp = fopen("/etc/group", "w");
   if ( fsGrp != NULL ) 
   {
#ifdef SUPPORT_IEEE1905_GOLDENNODE
      fprintf(fsGrp, "root::0:root,admin,ncap\n");
#else
...
void rut_updateLogSOAP(CmsEntityId destEid)
{
   /*
    * logging SOAP oject is changed, 
    * send a TR69C_CONFIG_CHANGED event msg to tr69c,
    */
   CmsMsgHeader msg = EMPTY_MSG_HEADER;
   msg.type = CMS_MSG_TR69C_CONFIG_CHANGED;
   msg.src =  mdmLibCtx.eid;
   msg.dst = destEid;
   msg.flags_event = 1;
...
{
   FILE *pFile;
   LanHostEntryObject *hostEntryObj=NULL;
...
#ifdef DMP_X_BROADCOM_COM_DNSPROXY_1
   {
      ...
      {
         cmsLog_debug("Found lanIpIntf IPAddress=%s", lanIpIntf->IPInterfaceIPAddress);

         fprintf(pFile,"%s\t%s\n", lanIpIntf->IPInterfaceIPAddress, dnsProxyObj->deviceHostName);
         fprintf(pFile,"%s\t%s.%s\n", lanIpIntf->IPInterfaceIPAddress, dnsProxyObj->deviceHostName, dnsProxyObj->deviceDomainName);
   ...
   /*
    * Now write out all the host entries.  It is not clear who else needs this
    * besides dnsproxy.  But if dnsproxy is not running, this entire function won't
    * get called anyways.
    */
   INIT_INSTANCE_ID_STACK(&iidStack);
   while ((ret = cmsObj_getNextFlags(MDMOID_LAN_HOST_ENTRY, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &hostEntryObj)) == CMSRET_SUCCESS)
   {
      /*
       * If a host entry was newly created, it does not have IPAddress and hostname
       * set yet, so skip those.
       */
      if (hostEntryObj->IPAddress != NULL && hostEntryObj->hostName != NULL)
      {
         fprintf(pFile, "%s\t%s\n", hostEntryObj->IPAddress, hostEntryObj->hostName);
      }
      cmsObj_free((void **) &hostEntryObj);
   }
