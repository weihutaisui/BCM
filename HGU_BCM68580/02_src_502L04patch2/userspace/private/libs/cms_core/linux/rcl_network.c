                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   /* If the dns list or static dns ip has been changed (only from user apps).
   * For using DNSIfName, it will reevaluate the list and try to find the active WAN interface
   * from the list.  For using DNSServers, just config the system dns with the static ip.
   */
   if (cmsUtl_strcmp(newObj->DNSIfName, currObj->DNSIfName) ||
