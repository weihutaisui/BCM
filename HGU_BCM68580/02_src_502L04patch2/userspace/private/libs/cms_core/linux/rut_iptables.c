...
#ifdef SUPPORT_SIP 
   {nf_conntrack_sip, "nf_conntrack_sip",  {nf_conntrack, 0}, {NF_DIR}, NULL, FALSE},
#endif   
...
{
   PMODULE_INFO pMod;
   char moduleName[MAX_NAME_LEN], DirPrefix[BUFLEN_128];
...
void rutIpt_createRoutingChain6(void)
{
   ...
   rut_doSystemAction("rtchain", "ip6tables -N rtchain 2>/dev/null");

   sprintf(tblfile, "/var/tmp_file");

   /* Before inserting rtchain to FORWARD chain, check if it already exists */
   snprintf(line, sizeof(line), "ip6tables -n -L FORWARD -v --line-numbers > %s", tblfile);
   ...
   if (!found)
   {
      rut_doSystemAction("rtchain", "ip6tables -I FORWARD -j rtchain 2>/dev/null");
   }
}
...
void rutIpt_configRoutingChain6(const char *prefix, const char *ifName, UBOOL8 add)
{
    ...
    */ 
   sprintf(tblfile, "/var/tmp_file");
   snprintf(line, sizeof(line), "ip6tables -n -L rtchain -v --line-numbers > %s", tblfile);
   rut_doSystemAction("rut", line);
   ...
   if (add)
   {
      if (!ret_found)
      {
         snprintf(line, sizeof(line), "ip6tables -I rtchain -i %s -s %s -j RETURN 2>/dev/null", ifName, prefix);
         rut_doSystemAction("rtchain", line);
      }

      if (!drop_found)
      {
         snprintf(line, sizeof(line), "ip6tables -A rtchain -i %s ! -d ff00::/8 -j DROP 2>/dev/null", ifName);
         rut_doSystemAction("rtchain", line);
      }
   }
   else
   {
      snprintf(line, sizeof(line), "ip6tables -D rtchain -i %s -s %s -j RETURN 2>/dev/null", ifName, prefix);
      rut_doSystemAction("rtchain", line);

      /* Should we remove the drop rule here? */
      if (drop_found && (ret_count == 1))
      {
         snprintf(line, sizeof(line), "ip6tables -D rtchain -i %s ! -d ff00::/8 -j DROP 2>/dev/null", ifName);
         rut_doSystemAction("rtchain", line);
      }
   }
}
...
void rutIpt_mldRules(UBOOL8 add, const char *ifname)
{
   ...
   if (add == TRUE)
   {
      ...
      sprintf(cmd, "ip6tables -t filter -I INPUT 1 -i %s -p icmpv6 "
                   "--icmpv6-type 130 -j ACCEPT 2>/dev/null",
                   ifname );
      rut_doSystemAction("mldRules", &cmd[0]);
     
      sprintf(cmd, "ip6tables -t filter -I FORWARD 1 -i %s "
                   "-d FF00::0/8 -j ACCEPT 2>/dev/null",
                   ifname );
      rut_doSystemAction("mldRules", &cmd[0]);
   }
...
void rutIpt_TCPMSSforIPTunnel(const char *wanIfName, UBOOL8 is6rd)
{
   ...
   snprintf(cmd, sizeof(cmd), "%s -I FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss %d", is6rd?"ip6tables":"iptables", mss);
...
UBOOL8 rutIpt_isRedirectOnNAT(void)
{
   ...
   rut_doSystemAction("rut", "iptables -t nat -L > /var/nat_redirect");
...
{
   char cmd[BUFLEN_128], addr[BUFLEN_128];
    ...
    * sprintf(cmd, "iptables -t nat -D POSTROUTING -p ALL -j MASQUERADE");
    */
    
   snprintf(cmd, sizeof(cmd), "iptables -t nat -D PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to 128.9.0.107 2>/dev/null", addr);
...
static void rut_fakeDnsForwarding(void) 
{
   ...
   snprintf(cmd, sizeof(cmd), "iptables -t nat -A PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to 128.9.0.107 2>/dev/null", addr);
   rut_doSystemAction("fakeDnsForwarding", cmd);
}	
...
static void rut_enableDNSForward(const char *bridgeName, const char *addr, const char *dns)
{
    ...
    */
   sprintf(cmd,
      "iptables -t nat -D PREROUTING -i %s -d %s -p udp --dport 53 -j DNAT --to %s 2>/dev/null",
      bridgeName, addr, dns);
   rut_doSystemAction("enableDnsForward", cmd);
   
   sprintf(cmd, "iptables -t nat -A PREROUTING -i %s -d %s -p udp --dport 53 -j DNAT --to %s", 
      bridgeName, addr, dns);
   rut_doSystemAction("enableDnsForward", cmd);
...
void rutIpt_deleteNatMasquerade(const char *ifName, const char *localSubnet, const char *localSubnetMask)
{
   ...
   snprintf(cmd, sizeof(cmd),  "iptables -t nat -D POSTROUTING -o %s -s %s/%s -j MASQUERADE 2>/dev/null", 
            ifName, localSubnet, localSubnetMask);
   rut_doSystemAction("deleteNatMasq", cmd);
         
   /* Delete possible full cone rule */
   snprintf(cmd, sizeof(cmd),  "iptables -t nat -D POSTROUTING -o %s -s %s/%s -j MASQUERADE --mode fullcone 2>/dev/null", 
            ifName, localSubnet, localSubnetMask);
   rut_doSystemAction("deleteNatMasq", cmd);
...
void rutIpt_insertNatMasquerade(const char *ifName, const char *localSubnet, const char *localSubnetMask, UBOOL8 isFullCone)
{
   ...
   snprintf(cmd, sizeof(cmd),  "iptables -t nat -A POSTROUTING -o %s -s %s/%s -j MASQUERADE",
            ifName, localSubnet, localSubnetMask);
   if (isFullCone)
   {
      cmsUtl_strncat(cmd, sizeof(cmd), " --mode fullcone");
   }
...
/* exexutes commands to setup NAT Masquerade for PPP on demand */
void rutIpt_initNatForPppOnDemand(char *ifcName, UBOOL8 fullCone) 
{
   ...
   */
   ...
   rutIpt_insertNatMasquerade(ifcName, localsubnet, localsubnetmask, fullCone);
...
void rutIpt_initNat(const char *ifcName, UBOOL8 isFullCone)
{
   ...
   if (rutPMap_isWanUsedForIntfGroup(ifcName) == TRUE)
   {
      ...
      {
         cmsLog_error("Fail to get bridge interface name %s.", bridgeIfName);
         return;
      }
...
void rutIpt_executeDPIRules(SINT32 domain, const char *ifName)
{
   ...
   snprintf(cmd, sizeof(cmd), "%s -I FORWARD -m layer7 --l7proto http -j DC", ipt);
                                              ...
                                              UBOOL8 isFirewallEnabled)
{
   ...
   cmsLog_debug("ifName=%s isFullCone=%d idleDisconnect=%d isNat=%d isFirewall=%d",
                ifName, isFullCone, pppIdleDisconnectTime, isNatEnabled, isFirewallEnabled);
...
void rutIpt_setupFirewallForDHCPv6(UBOOL8 unblock, const char *ifName)
{
   ...
   strcpy(fmt, "ip6tables -%c INPUT -p tcp --dport 546 -i %s -j ACCEPT %s");
   ...
   strcpy(fmt, "ip6tables -%c INPUT -p udp --dport 546 -i %s -j ACCEPT %s");
   sprintf(cmd, fmt, 'D', ifName, "2>/dev/null");
   rut_doSystemAction("rut", cmd);
   if (unblock)
   {
   ...
   strcpy(fmt, "ip6tables -%c INPUT -p icmpv6 --icmpv6-type neighbor-solicitation -i %s -j ACCEPT %s");
   sprintf(cmd, fmt, 'D', ifName, "2>/dev/null");
   rut_doSystemAction("rut", cmd);
   if (unblock)
   {
   ...
   strcpy(fmt, "ip6tables -%c INPUT -p icmpv6 --icmpv6-type neighbor-advertisement -i %s -j ACCEPT %s");
   sprintf(cmd, fmt, 'D', ifName, "2>/dev/null");
   rut_doSystemAction("rut", cmd);
   if (unblock)
   {
   ...
   strcpy(fmt, "ip6tables -%c INPUT -p icmpv6 --icmpv6-type router-advertisement -i %s -j ACCEPT %s");
   sprintf(cmd, fmt, 'D', ifName, "2>/dev/null");
   rut_doSystemAction("rut", cmd);
   if (unblock)
   {
...
void rutIpt_initIpSecPolicy(const char *ifName)
{
   ...
   // allow isakmp
   sprintf(cmd, "iptables -A INPUT -p udp --dport 500 -i %s -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);

   // allow esp
   sprintf(cmd, "iptables -A INPUT -p esp -i %s -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);

   // allow ah 
   sprintf(cmd, "iptables -A INPUT -p ah -i %s -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);

   // mark all esp/ah packets
   sprintf(cmd, "iptables -t mangle -p esp -A PREROUTING -i %s -j MARK --set-mark 0x10000000", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);

   sprintf(cmd, "iptables -t mangle -p ah -A PREROUTING -i %s -j MARK --set-mark 0x10000000", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);
   ...
   sprintf(cmd, "iptables -A INPUT ! -p esp -i %s -m mark --mark 0x10000000/0x10000000 -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);
   sprintf(cmd, "iptables -A FORWARD ! -p esp -i %s  -m mark --mark 0x10000000/0x10000000 -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);

   sprintf(cmd, "iptables -A INPUT ! -p ah -i %s -m mark --mark 0x10000000/0x10000000 -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);
   sprintf(cmd, "iptables -A FORWARD ! -p ah -i %s  -m mark --mark 0x10000000/0x10000000 -j ACCEPT", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);
...
void rutIpt_removeIpSecPolicy(const char *ifName)
{
   ...
   sprintf(cmd, "iptables -t mangle -p esp -D PREROUTING -i %s -j MARK --set-mark 0x10000000 2>/dev/null", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);

   // mark all ah packets
   sprintf(cmd, "iptables -t mangle -p ah -D PREROUTING -i %s -j MARK --set-mark 0x10000000 2>/dev/null", ifName);
   rut_doSystemAction("ipsecPolicy", cmd);
...
void rutIpt_initFirewall(SINT32 domain, const char *ifName) 
{
    ...
    */
   sprintf(cmd, "%s -A INPUT -p ALL -i %s -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT", ipt, ifName);
   rut_doSystemAction("initFirewall", cmd);

   if (isOnWanSide)
   {
      sprintf(cmd, "%s -A FORWARD -p ALL -i %s -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT", ipt, ifName);
      rut_doSystemAction("initFirewall", cmd);   
   }
    ...
    */
   sprintf(cmd, "%s -A INPUT -i %s -p tcp --dport %d -j ACCEPT", ipt, ifName, TR69C_CONN_REQ_PORT);
   rut_doSystemAction("initFirewall", cmd);   
#endif
...
#if defined(DMP_UDPECHO_1) || defined(DMP_DEVICE2_UDPECHO_1)
   {
      ...
      if (enable)
      {
         if (fullPathBuf[0])
         {
            ...
            {
               sprintf(cmd, "%s -A INPUT -i %s -p udp --dport %d -j ACCEPT", ipt, intfName, port);
            }
            else
            {
         ...
         {
            sprintf(cmd, "%s -A INPUT -p udp --dport %d -j ACCEPT", ipt, port);
         }
    ...
    */
   sprintf(cmd, "%s -A INPUT -i %s -p tcp --syn -m limit --limit 6/h -j LOG --log-level 1 --log-prefix=\"Intrusion -> \"", ipt, ifName);
   rut_doSystemAction("initFirewall", cmd);
   
   /* diable everything else */
   sprintf(cmd, "%s -A INPUT -i %s -j DROP", ipt, ifName);
   rut_doSystemAction("initFirewall", cmd);

   if (isOnWanSide)
   {
      sprintf(cmd, "%s -A FORWARD -i %s -p tcp --syn -m limit --limit 6/h -j LOG --log-level 1 --log-prefix=\"Intrusion -> \"", ipt, ifName);
      rut_doSystemAction("initFirewall", cmd);
   
      sprintf(cmd, "%s -A FORWARD -i %s -j DROP", ipt, ifName);
      rut_doSystemAction("initFirewall", cmd);   
   }
...
static void removeAllIptRulesByIntfName(SINT32 domain, const char *ifcName, const char *table, const char *chain)
{
   ...
   while (!done)
   {
      ...
      snprintf(line, sizeof(line), "%s -t %s -n -L %s -v --line-numbers > %s", ipt, table, chain, tblfile);
      rut_doSystemAction("rut", line);
      ...
      while (idx >= 0)
      {
         ...
         snprintf(line, sizeof(line), "%s -t %s -D %s %d 2>/dev/null",
...
void rutIpt_upnpConfigStopMulticast(char *wanIfcName, UBOOL8 addRules)
{
   ...
   snprintf(cmd, sizeof(cmd), "iptables -t filter -%c OUTPUT -o %s -d %s -j DROP 2>/dev/null", action, wanIfcName, UPNP_IP_ADDRESS);
...
void rutIpt_ripAddIptableRule(const char *ifcName)
{
   ...
   snprintf(cmd, sizeof(cmd), "iptables -I INPUT 1 -p udp --dport 520 -i %s -j ACCEPT", ifcName);
...
static UBOOL8 removeSingleRipIptableRule(void)
{
   ...
   /* execute iptables command to create iptable file */
   sprintf(line, "iptables -n -L INPUT -v --line-numbers > /var/iptable");
   ...
   while (fgets(line, sizeof(line), fs))
   {
           ...
           cmsUtl_strcmp(comment, "dpt:520") == 0)
      {
         ...
         snprintf(cmd, sizeof(cmd), "iptables -D INPUT %s 2>/dev/null", col[IP_TBL_COL_NUM]);
         rut_doSystemAction("rip", cmd);
         found = TRUE;
         break;
      }
...
void rutIpt_activeUrlFilter(void)
{
   ...
   rut_doSystemAction("UrlFIlter", "iptables -N urlfilter");
   rut_doSystemAction("UrlFIlter", "iptables -A FORWARD -p tcp --tcp-flags SYN,RST,FIN NONE -j urlfilter");  

#ifdef SUPPORT_IPV6
   rut_doSystemAction("UrlFIlter", "ip6tables -N urlfilter");
   rut_doSystemAction("UrlFIlter", "ip6tables -A FORWARD -p tcp --tcp-flags SYN,RST,FIN NONE -j urlfilter");  
#endif
   return;
}
...
void rutIpt_deactiveUrlFilter(void)
{
   ...
   if (cmsObj_get(MDMOID_URL_FILTER_CFG, &iidStack, 0, (void **) &urlCfg) == CMSRET_SUCCESS && urlCfg->enable == TRUE)
   {
      rut_doSystemAction("UrlFIlter", "iptables -D FORWARD -p tcp --tcp-flags SYN,RST,FIN NONE -j urlfilter");  
      rut_doSystemAction("UrlFIlter", "iptables -X urlfilter");
#ifdef SUPPORT_IPV6
      rut_doSystemAction("UrlFIlter", "ip6tables -D FORWARD -p tcp --tcp-flags SYN,RST,FIN NONE -j urlfilter");  
      rut_doSystemAction("UrlFIlter", "ip6tables -X urlfilter");
#endif
...
void rutIpt_urlFilterConfig(void *Obj, UBOOL8 add)
{
   ...
   rut_doSystemAction("UrlFilter", "iptables -F urlfilter");
#ifdef SUPPORT_IPV6
   rut_doSystemAction("UrlFilter", "ip6tables -F urlfilter");
#endif

   for (i=0;ports[i]!=0;i++) 
   {
      snprintf(cmd, sizeof(cmd), "iptables -I urlfilter -p tcp --dport %d -j SKIPLOG", ports[i]);
      rut_doSystemAction("UrlFilter", cmd);

      snprintf(cmd, sizeof(cmd), "iptables -A urlfilter -p tcp --dport %d -j QUEUE", ports[i]);
      rut_doSystemAction("UrlFilter", cmd);
#ifdef SUPPORT_IPV6
      snprintf(cmd, sizeof(cmd), "ip6tables -I urlfilter -p tcp --dport %d -j SKIPLOG", ports[i]);
      rut_doSystemAction("UrlFilter", cmd);

      snprintf(cmd, sizeof(cmd), "ip6tables -A urlfilter -p tcp --dport %d -j QUEUE", ports[i]);
      rut_doSystemAction("UrlFilter", cmd);
#endif
   }
                               ...
                               const char *nonDmzMask)
{
   ...
   snprintf(cmd, sizeof(cmd), "iptables -t nat -I POSTROUTING -s %s/32 -d %s -j ACCEPT", localLanIp, localIp);
   rut_doSystemAction("adDmz", cmd);
   snprintf(cmd, sizeof(cmd), "iptables -t nat -I POSTROUTING -s %s/%s -d %s -j ACCEPT", nonDmzIp, nonDmzMask, localIp);
   rut_doSystemAction("adDmz", cmd);
   snprintf(cmd, sizeof(cmd), "iptables -t nat -A POSTROUTING -s %s/32 -j SNAT --to-source %s", localLanIp, localIp);
   rut_doSystemAction("adDmz", cmd);
   snprintf(cmd, sizeof(cmd), "iptables -t nat -A POSTROUTING -s %s/%s -j SNAT --to-source %s", nonDmzIp, nonDmzMask, localIp);
   rut_doSystemAction("adDmz", cmd);
                               ...
                               const char *nonDmzMask)
{
   ...
   snprintf(cmd, sizeof(cmd), "iptables -t nat -D POSTROUTING -s %s/32 -d %s -j ACCEPT", localLanIp, localIp);
   rut_doSystemAction("adDmz", cmd);
   snprintf(cmd, sizeof(cmd), "iptables -t nat -D POSTROUTING -s %s/%s -d %s -j ACCEPT", nonDmzIp, nonDmzMask, localIp);
   rut_doSystemAction("adDmz", cmd);
   snprintf(cmd, sizeof(cmd), "iptables -t nat -D POSTROUTING -s %s/32 -j SNAT --to-source %s", localLanIp, localIp);
   rut_doSystemAction("adDmz", cmd);
   snprintf(cmd, sizeof(cmd), "iptables -t nat -D POSTROUTING -s %s/%s -j SNAT --to-source %s", nonDmzIp, nonDmzMask, localIp);
   rut_doSystemAction("adDmz", cmd);         
	                                ...
	                                const _WanPppConnPortmappingObject *pppObj, const _WanIpConnPortmappingObject *ipObj, const UBOOL8 add)
{
   ...
   if(add)
   {
      ...
      if ( dmzHost[0] != '\0' ) 
      {  
         sprintf(cmd, "iptables -t nat -D PREROUTING -i %s -j DNAT --to-destination %s", interface, dmzHost);
         rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
         sprintf(cmd, "iptables -D FORWARD -i %s -d %s -j ACCEPT", interface, dmzHost);
         rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);	
      }
   ...
   {
      sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -p 50 -j DNAT --to %s",
              action, interface, internalClient);
      rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
      if ( action == 'A' ) 
      {
         sprintf(cmd, "iptables -I FORWARD 1 -i %s -p 50 -d %s -j ACCEPT",
                 interface, internalClient);
         rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
      } 
      else 
      {
         sprintf(cmd, "iptables -D FORWARD -i %s -p 50 -d %s -j ACCEPT",
                 interface, internalClient);
         rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);		 
      }	 
   } 
   ...
   {
      sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -p 47 -j DNAT --to %s",
              action, interface, internalClient);
      rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
      if ( action == 'A' ) 
      {
         sprintf(cmd, "iptables -I FORWARD 1 -i %s -p 47 -d %s -j ACCEPT",
                 interface, internalClient);
         rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
      }
      else 
      {
         sprintf(cmd, "iptables -D FORWARD -i %s -p 47 -d %s -j ACCEPT",
                 interface, internalClient);
         rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);		 
      }	 
   }
   ...
   {
      sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -p tcp --dport %d -j REDIRECT --to-ports %d 2>/dev/null",
...
/*	  
      ...
      if ( cmsUtl_strcmp(Obj->srvName, "SNMP") == 0 &&
         ...
         sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -p udp --dport %s -j REDIRECT --to-ports %s",
...
*/
   ...
   {
      sprintf(cmd, "iptables -t nat -%c PREROUTING -i %s -p udp --dport %d -j REDIRECT --to-ports %d 2>/dev/null",
   ...
   if(add && dmzHost[0] != '\0' )
   {
      sprintf(cmd, "iptables -t nat -A PREROUTING -i %s -j DNAT --to-destination %s 2>/dev/null", interface, dmzHost);
      rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
      sprintf(cmd, "iptables -I FORWARD 1 -i %s -d %s -j ACCEPT 2>/dev/null", interface, dmzHost);
      rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);	
   }
                              ...
                              char *inPort, char *srvAddress, char *srvPort, char *remoteHost) 
{
   ...
   if ( srvPort[0] == '\0' ) 
   {
      ...
      if(action=='A')
      {
         snprintf(cmd, sizeof(cmd), "iptables -t nat -%c PREROUTING -i %s -p %s%s%s --dport %s -j DNAT --to %s",
                 'D', device, protocol,
                 (remoteHost[0] != '\0'? " --source " :""), (remoteHost[0] != '\0'? remoteHost :""),
                 inPort, srvAddress);
         cmsUtl_strncat(cmd, sizeof(cmd)," 2> /var/rutIpt_vrtsrvRunIptables");
...
*/
      ...
      if(action=='A')
      {
         snprintf(cmd, sizeof(cmd), "iptables -%c FORWARD %c -i %s -p %s -d %s --dport %s -j ACCEPT",
...
*/
   ...
   else 
   {
      if(action=='A')
      {
         snprintf(cmd, sizeof(cmd), "iptables -t nat -%c PREROUTING -i %s -p %s%s%s --dport %s -j DNAT --to %s:%s",
                 'D', device, protocol,
                 (remoteHost [0]!= '\0'? " --source " :""), (remoteHost[0] != '\0'? remoteHost :""),
...
void rutIpt_igmpRules(UBOOL8 add, const char *ifname)
{
   ...
   if (add == TRUE)
   {
      ...
      sprintf(cmd, "iptables -t filter -I INPUT 1 -i %s "
                   "-p 2 -j ACCEPT 2>/dev/null", ifname);
      rut_doSystemAction("igmpRules", &cmd[0]);

      sprintf(cmd, "iptables -t filter -I FORWARD 1 -i %s "
                   "-d 224.0.0.0/4 -j ACCEPT 2>/dev/null", ifname);
      rut_doSystemAction("igmpRules", &cmd[0]);
      
      sprintf(cmd, "iptables -t nat -I PREROUTING 1 -i %s "
                   "-d 224.0.0.0/4 -j ACCEPT 2>/dev/null", ifname);
      rut_doSystemAction("igmpRules", &cmd[0]);
   }
   else if (add == FALSE)
...
void rutIpt_addIpFilterOut(const IpFilterCfgObject *obj, const char *ifName, UBOOL8 add)
{
   ...
   strcpy(fmt, "%s -%c FORWARD -i %s %s %s %s %s %s -j DROP %s");
...
CmsRet rutIpt_ipFilterInRunIptables(char *ipver, char action, const char *ifName, char *protocol, char *src, char *sport, char *dst, char *dport)
{
   ...
   snprintf(cmd, sizeof(cmd), "%s -%c INPUT -i %s %s %s %s %s %s -j ACCEPT", ipt, action, ifName, protocol, src, sport, dst, dport);
   ...
   {
      snprintf(cmd, sizeof(cmd), "%s -%c FORWARD -i %s %s %s %s %s %s -j ACCEPT", ipt, action, ifName, protocol, src, sport, dst, dport);
...
void rutIpt_redirectHttpTelnetPorts(const char *ifName, const char *ipAddress)
{
   ...
   /* Move local HTTP server to port 8080 and redirect the 8080 traffic to
     ...
     */
   snprintf(cmd,  sizeof(cmd), "iptables -t nat -A PREROUTING -i %s -p tcp --dport %d -j DNAT --to-destination %s:%d",
...
void rutIpt_deleteTCPMSSRules(SINT32 domain, const char *ifName)
{
   ...
   snprintf(cmd, sizeof(cmd), "%s -D FORWARD -i %s -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null", ipt,ifName);
   rut_doSystemAction("TCPMSSRules", cmd);   
   snprintf(cmd, sizeof(cmd), "%s -D FORWARD -o %s -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null", ipt,ifName);
   rut_doSystemAction("TCPMSSRules", cmd);
}
...
void rutIpt_insertTCPMSSRules(SINT32 domain, const char *ifName)
{
   ...
   snprintf(cmd, sizeof(cmd), "%s -I FORWARD -i %s -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu", ipt,ifName);
   rut_doSystemAction("TCPMSSRules", cmd);   
   snprintf(cmd, sizeof(cmd), "%s -I FORWARD -o %s -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu", ipt,ifName);
   rut_doSystemAction("TCPMSSRules", cmd);
}
...
void rutIpt_omciIpHostRules(UINT32 oid, UINT8 options, char *ifName)
{
    ...
    {
        // flush all rules
        snprintf(cmd, sizeof(cmd), "iptables -D INPUT -i %s -p icmp --icmp-type 8 -j DROP 2>/dev/null", ifName);
        rut_doSystemAction("omciIpHostRules", cmd);
        snprintf(cmd, sizeof(cmd), "iptables -D OUTPUT -o %s -p icmp --icmp-type 3/3 -j DROP 2>/dev/null", ifName);
        rut_doSystemAction("omciIpHostRules", cmd);
        snprintf(cmd, sizeof(cmd), "iptables -D OUTPUT -o %s -p icmp --icmp-type 11 -j DROP 2>/dev/null", ifName);
        rut_doSystemAction("omciIpHostRules", cmd);

        // block icmp echo request: don't response ping.
        if (!(options & 0x2))
        {
            snprintf(cmd, sizeof(cmd), "iptables -A INPUT -i %s -p icmp --icmp-type 8 -j DROP 2>/dev/null", ifName);
            rut_doSystemAction("omciIpHostRules", cmd);
        }
        ...
        if (!(options & 0x4))
        {
            ...
            snprintf(cmd, sizeof(cmd), "iptables -A OUTPUT -o %s -p icmp --icmp-type 3/3 -j DROP 2>/dev/null", ifName);
            rut_doSystemAction("omciIpHostRules", cmd);
            //drop icmp Time Exceeded
            snprintf(cmd, sizeof(cmd), "iptables -A OUTPUT -o %s -p icmp --icmp-type 11 -j DROP 2>/dev/null", ifName);
            rut_doSystemAction("omciIpHostRules", cmd);
        }
    }
    ...
    {
        // flush all rules
        snprintf(cmd, sizeof(cmd), "ip6tables -D INPUT -i %s -p icmpv6 --icmpv6-type 128 -j DROP 2>/dev/null", ifName);
        rut_doSystemAction("omciIpHostRules", cmd);

        // block icmpv6 echo request: don't response ping.
        if (!(options & 0x8))
        {
            snprintf(cmd, sizeof(cmd), "ip6tables -A INPUT -i %s -p icmpv6 --icmpv6-type 128 -j DROP 2>/dev/null", ifName);
            rut_doSystemAction("omciIpHostRules", cmd);
        }
    }
}
...
void rutIpt_disassociateOtherBridgesFromWanIntf(char action, SINT32 domain, const char *wanIfcName, const char *localSubnet, const char *localSubnetmask)
{
    ...
    {
        snprintf(cmd, sizeof(cmd), "%s -D FORWARD -i %s ! -d %s/%s -j DROP 2>/dev/null", ipt, wanIfcName, localSubnet, localSubnetmask);
        rut_doSystemAction("WanIntfRules", cmd);
        snprintf(cmd, sizeof(cmd), "%s -D FORWARD -o %s ! -s %s/%s -j DROP 2>/dev/null", ipt,wanIfcName, localSubnet, localSubnetmask);
        rut_doSystemAction("WanIntfRules", cmd);
        if ('A' == action)
        {
            snprintf(cmd, sizeof(cmd), "%s -A FORWARD -i %s ! -d %s/%s -j DROP", ipt, wanIfcName, localSubnet, localSubnetmask);
            rut_doSystemAction("WanIntfRules", cmd);
            snprintf(cmd, sizeof(cmd), "%s -A FORWARD -o %s ! -s %s/%s -j DROP", ipt,wanIfcName, localSubnet, localSubnetmask);
            rut_doSystemAction("WanIntfRules", cmd);
        }
    }
    else
