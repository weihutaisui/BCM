...
#define DHCP6C_OPTION_REPORT_LIST   "16_17"  /* Pls connect multi codes with "_" */
#define DHCP6C_OPTION_REQUEST_LIST  "17_37_39"  /* Pls connect multi codes with "_" */
#endif
                              ...
                              UBOOL8 isIpObj __attribute__((unused)))
{
   ...
   cmsLog_debug("delegated address part!");
...
#ifdef SUPPORT_UPNP
      if (rut_isUpnpEnabled() && rut_isApplicationRunning(EID_UPNP) == FALSE)
      {
...
UINT32 rutWan_restartDhcp6c(const char *ifName_info, UBOOL8 dynamicIpEnabled,
                            UBOOL8 pdEnabled, UBOOL8 aftrName, UBOOL8 mapt)
{
   char cmdLine[BUFLEN_128];
...
//   rutIpt_setupFirewallForDHCPv6(TRUE, ifName);
   ...
   rutDhcp_mkCfgDir(DHCP_V6, ifName);
#if defined(BRCM_PKTCBL_SUPPORT)
   create_dhcp6Option17(ifName);
   snprintf(&cmdLine[strlen(cmdLine)], sizeof(cmdLine), " -P");
   snprintf(&cmdLine[strlen(cmdLine)], sizeof(cmdLine), " -R %s", DHCP6C_OPTION_REPORT_LIST);
   snprintf(&cmdLine[strlen(cmdLine)], sizeof(cmdLine), " -Q %s", DHCP6C_OPTION_REQUEST_LIST);
#endif
