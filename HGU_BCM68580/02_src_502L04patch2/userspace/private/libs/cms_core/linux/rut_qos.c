...
void rutQos_doDefaultPolicy(void)
{
   ...
   {
      sprintf(cmd, "iptables -t mangle -D OUTPUT -j MARK --or-mark 0x%x %s 2>/dev/null",
              XTM_QOS_LEVELS-1, defaultClassifications[i]);
      rut_doSystemAction("rutQos_doDefaultPolicy", cmd);

#ifdef SUPPORT_IPV6
      sprintf(cmd, "ip6tables -t mangle -D OUTPUT -j MARK --or-mark 0x%x %s 2>/dev/null",
              XTM_QOS_LEVELS-1, defaultClassifications[i]);
      rut_doSystemAction("rutQos_doDefaultPolicy", cmd);
#endif      
   }
   ...
   sprintf(cmd, "ip6tables -t mangle -D OUTPUT -j MARK --or-mark 0x%x -p udp --dport 546:547 2>/dev/null",
           XTM_QOS_LEVELS-1);
   rut_doSystemAction("rutQos_doDefaultPolicy", cmd);
#endif      
   ...
   for (i = 0; i < sizeOfDefaultClassifications()/sizeof(char *); i++)
   {
      sprintf(cmd, "iptables -t mangle -A OUTPUT -j MARK --or-mark 0x%x %s",
              XTM_QOS_LEVELS-1, defaultClassifications[i]);
      rut_doSystemAction("rutQos_doDefaultPolicy", cmd);

#ifdef SUPPORT_IPV6
      sprintf(cmd, "ip6tables -t mangle -A OUTPUT -j MARK --or-mark 0x%x %s",
              XTM_QOS_LEVELS-1, defaultClassifications[i]);
      rut_doSystemAction("rutQos_doDefaultPolicy", cmd);
#endif      
   }
   ...
   sprintf(cmd, "ip6tables -t mangle -D OUTPUT -j MARK --or-mark 0x%x -p udp --dport 67:68 2>/dev/null",
           XTM_QOS_LEVELS-1);
   rut_doSystemAction("rutQos_doDefaultPolicy", cmd);

   sprintf(cmd, "ip6tables -t mangle -A OUTPUT -j MARK --or-mark 0x%x -p udp --dport 546:547",
           XTM_QOS_LEVELS-1);
   rut_doSystemAction("rutQos_doDefaultPolicy", cmd);
#endif      
...
void rutQos_doDefaultDSCPMarkPolicy(QosCommandType cmdType, SINT32 defaultDSCPMark)
{
   ...
   else if (defaultDSCPMark != QOS_RESULT_NO_CHANGE)
   {
      ...
      if (cmdType == QOS_COMMAND_CONFIG)
      {
         ...
         rut_doSystemAction("rutQos_doDefaultDSCPMarkPolicy", cmd);
         sprintf(cmd, "iptables -t mangle -A POSTROUTING -j DSCP --set-dscp 0x%x", ftosbyte);
         rut_doSystemAction("rutQos_doDefaultDSCPMarkPolicy", cmd);
#ifdef SUPPORT_IPV6
         sprintf(cmd, "ip6tables -t mangle -A POSTROUTING -j DSCP --set-dscp 0x%x", ftosbyte);
         rut_doSystemAction("rutQos_doDefaultDSCPMarkPolicy", cmd);
#endif
      }
      else
      {
         ...
         rut_doSystemAction("rutQos_doDefaultDSCPMarkPolicy", cmd);
         sprintf(cmd, "iptables -t mangle -D POSTROUTING -j DSCP --set-dscp 0x%x 2>/dev/null", ftosbyte);
         rut_doSystemAction("rutQos_doDefaultDSCPMarkPolicy", cmd);
#ifdef SUPPORT_IPV6
         sprintf(cmd, "ip6tables -t mangle -D POSTROUTING -j DSCP --set-dscp 0x%x 2>/dev/null", ftosbyte);
         rut_doSystemAction("rutQos_doDefaultDSCPMarkPolicy", cmd);
#endif
      }
   }
