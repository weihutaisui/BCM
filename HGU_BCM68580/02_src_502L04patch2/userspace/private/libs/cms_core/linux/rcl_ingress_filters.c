		...
		CmsRet *errorCode __attribute__((unused)))
{
	...
	UBOOL8 direction = 0;
	UBOOL8 deleation = 0;
	_IngressFiltersDataObject *otherFilterData = NULL, confObj, confObj2;
	...
	if (ADD_NEW(newObj, currObj) || SET_EXISTING(newObj, currObj))
	{
		...
		if (SET_EXISTING(newObj, currObj) && currObj->ports != NULL)
		{
			memcpy(&confObj2, currObj, sizeof(_IngressFiltersDataObject));
			cmsUtl_strtoul64(confObj2.ports, NULL, 2, &prev_ports);
		}
