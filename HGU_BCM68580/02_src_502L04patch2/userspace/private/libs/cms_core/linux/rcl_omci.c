...
//remove policy route of ipHost
    ...
    if (newObj->currentAddress != currObj->currentAddress)
    {
        ...
        inAddr.s_addr = currObj->currentAddress;
...
static void removeIpv6PolicyRoute(const HEXBINARY address)
{
    ...
    cmsUtl_hexStringToBinaryBuf(address, &buf, &size);
...
CmsRet rutOmci_configIpv4Host(UINT32 meId, char *ifName)
{
    ...
    UBOOL8 releaseLock = FALSE, enblDhcp = FALSE;
    ...
    // Test if lock not already acquired.
    if (mdmLibCtx.locked == FALSE)
    {
        // MDM not previously locked, attempt to lock now.
        cmsResult = cmsLck_acquireLockWithTimeout(6*MSECS_IN_SEC);

        // Signal that this routine has locked the MDM.
        releaseLock = TRUE;
    }
    ...
    if (cmsResult == CMSRET_SUCCESS)
    {
        ...
        while ((!found) &&
               ((cmsResult = cmsObj_getNext(MDMOID_BCM_OMCI_RTD_IP_HOST_CONFIG_DATA,
                                     &iidStack, (void **) &ipHost))
                == CMSRET_SUCCESS))
        {
        ...
        // Test if this function has locked MDM.
        if (releaseLock == TRUE)
        {
            // Release MDM lock since this function has locked it.
            cmsLck_releaseLock();
        }
    }
...
static CmsRet activateIpv4Interface(IpHostConfigDataObject *ipHost)
{
    ...
    inAddr.s_addr = ipHost->currentMask;
    strcpy(mask, inet_ntoa(inAddr));
    inAddr.s_addr = ipHost->currentGateway;
    strcpy(gtwy, inet_ntoa(inAddr));
    ...
    else
    {
                ...
                == CMSRET_SUCCESS))
        {
            ...
            if (found == TRUE && bcmIpHost->interfaceName != NULL)
            {
                ...
                {
                    inAddr.s_addr = ipHost->primaryDns;
                    strcpy(dns1Addr, inet_ntoa(inAddr));
                    inAddr.s_addr = ipHost->secondaryDns;
                    strcpy(dns2Addr, inet_ntoa(inAddr));
                }
   ...
   if (obj != NULL && obj->interfaceName != NULL)
   {
      ...
      if (ia_na || info_only)
      {
         ...
         if ((pid = rutWan_restartDhcp6c(obj->interfaceName, ia_na,
                                         FALSE, FALSE, FALSE)) == CMS_INVALID_PID)
         {
        ...
        if (hasIpHost == TRUE)
        {
                    ...
                    == CMSRET_SUCCESS))
            {
                ...
                if ((hasBcIpHost == TRUE) && (bcmIpv6Host->interfaceName != NULL))
                {
                    ...
                    snprintf(cmd, sizeof(cmd), "iptables -t mangle -o %s -%c POSTROUTING -p %d --dport %d -j DSCP --set-dscp 0x%x 2>/dev/null",
...
CmsRet rutOmci_refreshDnsServer(UINT32 dns1, UINT32 dns2, UINT8 *dns6)
{
    ...
    */
   tmp_file = tempnam("/var/fyi/sys", "dns");
   ...
   fclose(tmp_fp);
   rename(tmp_file, DNS_FYI_FILENAME);
   free(tmp_file);

   return ret;
}

CmsRet rutOmci_reloadDnsProxy(UINT32 dns1 __attribute__((unused)),
  UINT32 dns2 __attribute__((unused)))
{
   CmsRet ret = CMSRET_SUCCESS;
   ...
   if (!rutDpx_isEnabled())
   {
      cmsLog_debug("dnsproxy is not enable and do nothing.");
      return ret;
   }
...
#ifndef DMP_X_BROADCOM_COM_DNSPROXY_1
   rename(tmp_file, DNS_FYI_FILENAME);
#else
   rename(tmp_file, REAL_DNS_FYI_FILENAME);

   //set name server of resolv.conf as 127.0.0.1
   if ((tmp_fp = fopen(tmp_file, "w+")) == NULL)
   {
      cmsLog_error("could not open tmp file %s", tmp_file);
      return CMSRET_INTERNAL_ERROR;
