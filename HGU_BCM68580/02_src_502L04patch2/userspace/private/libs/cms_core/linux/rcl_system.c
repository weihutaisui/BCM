...
CmsRet rcl_cellularappCfgObject( _CellularappCfgObject *newObj,
                const _CellularappCfgObject *currObj,
                const InstanceIdStack *iidStack __attribute__((unused)),
                char **errorParam __attribute__((unused)),
                CmsRet *errorCode __attribute__((unused)))
{   
   if (newObj != NULL && currObj != NULL)
   {
      rut_updateLogLevel(EID_CELLULAR_APP, newObj->loggingLevel);
      rut_updateLogDestination(EID_CELLULAR_APP, newObj->loggingDestination);
   }
