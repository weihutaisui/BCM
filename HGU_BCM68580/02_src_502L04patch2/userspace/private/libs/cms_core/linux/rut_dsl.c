...
void xdslUtil_IntfCfgInit(adslCfgProfile *pAdslCfg, WanDslIntfCfgObject *pDlIntfCfg)
{
    ...
    if((kAdslCfgModAny == (dslCfgParam & kAdslCfgModMask)) && !(pAdslCfg->adsl2Param & kAdsl2CfgAnnexMEnabled)) {
        pDlIntfCfg->X_BROADCOM_COM_AdslModulationCfg = cmsMem_strdup(MDMVS_ADSL_MODULATION_ALL);
    }
    else {
        ...
        if (len > 2) {
           cfgModType[len-2] = '\0';
           pDlIntfCfg->X_BROADCOM_COM_AdslModulationCfg = cmsMem_strdup(cfgModType);
        }
        else {
           /* default will be all */
           pDlIntfCfg->X_BROADCOM_COM_AdslModulationCfg = cmsMem_strdup(MDMVS_ADSL_MODULATION_ALL);
   ...
   /* sra */
    ...
    if (pAdslCfg->adslDemodCapValue & kXdslSRAEnabled)
        pDlIntfCfg->X_BROADCOM_COM_SRA = cmsMem_strdup(MDMVS_ON);
    else
        pDlIntfCfg->X_BROADCOM_COM_SRA = cmsMem_strdup(MDMVS_OFF);
    /* bitswap */
    cmsMem_free(pDlIntfCfg->X_BROADCOM_COM_Bitswap);
    if (pAdslCfg->adslDemodCapValue & kXdslBitSwapEnabled)
        pDlIntfCfg->X_BROADCOM_COM_Bitswap = cmsMem_strdup(MDMVS_ON);
    else
        pDlIntfCfg->X_BROADCOM_COM_Bitswap = cmsMem_strdup(MDMVS_OFF);
    
    cmsMem_free(pDlIntfCfg->X_BROADCOM_COM_PhoneLinePair);
    if(kAdslCfgLineOuterPair == (dslCfgParam & kAdslCfgLinePairMask))
        pDlIntfCfg->X_BROADCOM_COM_PhoneLinePair = cmsMem_strdup(MDMVS_OUTER_PAIR);
    else
        pDlIntfCfg->X_BROADCOM_COM_PhoneLinePair = cmsMem_strdup(MDMVS_INNER_PAIR);
}
...
CmsRet rutDsl_initPPPoA_igd(const InstanceIdStack *iidStack, void * Obj)
{
   ...
   */
   ...
   {
      snprintf(cmdLine + strlen(cmdLine), sizeof(cmdLine) - strlen(cmdLine), " -o %d", newObj->idleDisconnectTime);
   }
