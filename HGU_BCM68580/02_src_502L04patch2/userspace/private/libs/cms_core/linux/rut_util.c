   ...
   */
   ...
   else
   {
      ...
      else
      {
         ...
         */
         if(!cmsUtl_strcmp(cmdArray[0], "ifconfig"))
         {
            cmsUtl_strcpy(programWithPath, "/opt/sbin/");
         }
         else if ( !cmsUtl_strcmp(cmdArray[0], "route") ||
             !cmsUtl_strcmp(cmdArray[0], "insmod") ||
             !cmsUtl_strcmp(cmdArray[0], "rmmod"))
         {
            cmsUtl_strcpy(programWithPath, "/sbin/");
         }
         else if (cmsUtl_strstr(cmdArray[0], "sendarp"))
         {
            cmsUtl_strcpy(programWithPath, "/opt/usr/sbin/");
         }
         else
         {
            cmsUtl_strcpy(programWithPath, "/bin/");         
         }
         cmsUtl_strncat(programWithPath, sizeof(programWithPath), cmdArray[0]);
...
UBOOL8 rut_isApplicationRunning(CmsEntityId eid)
{
   ...
   cmsLog_debug("App. with eid 0X%0X status = %d", eid, isAppRuning);
...
CmsRet rut_intfnameToFullPath(const char *intfname, UBOOL8 layer2, char **mdmPath)
{
...
#ifdef BRCM_WLAN
   else if (strstr(intfname, WLAN_IFC_STR) != NULL)
   {
   ...
   else if (strstr(intfname, CELLULAR_IFC_STR) != NULL)
   {
      /*Cellular wan share same interface for layer2 and layer3*/      
