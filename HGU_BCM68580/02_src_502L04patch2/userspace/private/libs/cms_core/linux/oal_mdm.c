...
CmsRet oalShm_init(UINT32 shmSize, SINT32 *shmId, void **shmAddr)
{
    ...
    */
   libmdmHandle = dlopen(mdmFilename, RTLD_LAZY);
...
UINT32 copyToSharedMem(MdmSharedMemContext *shmCtx, const MdmObjectNode *rootObjNode)
{
    ...
    */
   shmCtx->oidNodePtrTable = (void *) (MDM_SHM_ATTACH_ADDR + sizeof(MdmSharedMemContext));
   ...
   cmsLog_debug("oidNodePtrTable= 0x%x (+0x%x)", shmCtx->oidNodePtrTable, sizeof(MdmSharedMemContext));
               ...
               const char *shmEnd)
{
   ...
   for (i=0; i < objNode->numParamNodes; i++)
   {
      ...
      /*
       * Fix up the offsetInObject field.  This calculation 
       * was previously done in GenObjectNode, but GenObjectNode 
       * does not have knowledge of which profiles are defined so it
       * would not know which parameters in an object are there or not.
       * So we have to the parameter offset calculation at runtime.
       */
      if (shmParamNode->type == MPT_BOOLEAN)
      {
         if (i == 0)
         {
            /*
             * Becuase we have a MdmObjectId (UINT16) as the
             * first field, a structure that starts with a BOOL
             * can pretend there were 2 other BOOLS in front of it.
             * That way, the first real BOOL can be packed into the
             * the first 4 byte word of the structure.
             */
             numConsecutiveBools = 2;
             offsetInObject = 1;
         }

         numConsecutiveBools += 1;
         if (numConsecutiveBools == 1)
         {
            /*
             * this is the first BOOL after non-BOOLS, advance offset by 8 or 4,
             * depending on whether the previous param was a 64 bit param.
             */
            offsetInObject += (isPrevParam64) ? 8 : 4;
         }
         else
         {
            /*
             * This is a bool following one or more bools, so I only need to
             * advance 1 byte since the previous bool took only one byte of space.
             */
            offsetInObject += 1;
         }
         isPrevParam64 = 0;
      ...
      {
         /* parameter is a non-BOOL type */

         if (numConsecutiveBools == 0 || numConsecutiveBools == 1)
         {
            /*
             * previous param was not a bool or just a single bool,
             * advance offset to the "next appropriate boundary".
             * The next appropriate boundary depends on the type of the 
             * previous param and the type of the current param.
             * If the previous param was a 64 bit type, we have to advance
             * by 8, otherwise, advance by 4.  Also on MIPS, if the current
             * param is a 64 bit type, it must be 8 byte aligned,
             * but on Desktop/Intel it does not have to be 8 byte aligned (done below).
             */
         ...
         {
            /*
             * previous param was two or more bools.  Now we have to calculate
             * how much to advance to get to the next 4-byte boundary.
             * Again, on MIPS, if the current param is a 64 bit type,
             * we actaully have to get to the 8 byte boundary (done below).
             */
             if (numConsecutiveBools % 4 == 0)
             {
                offsetInObject += 1;
             }
             else
             {
                /* just need to get to the next 4 byte boundary */
                offsetInObject += 5 - (numConsecutiveBools % 4);
             }
         }

#if !defined(DESKTOP_LINUX) || (defined(DESKTOP_LINUX) && defined(__LP64__))
         {
            UBOOL8 isCurrParam64=FALSE;
            isCurrParam64 = (shmParamNode->type == MPT_LONG64 || shmParamNode->type == MPT_UNSIGNED_LONG64 
#ifdef __LP64__
			|| shmParamNode->type == MPT_STRING || shmParamNode->type == MPT_DATE_TIME
			|| shmParamNode->type == MPT_BASE64 || shmParamNode->type == MPT_HEX_BINARY
#endif
		);
            if ((isCurrParam64) && (offsetInObject % 8 != 0))
            {
               offsetInObject += 4;
            }
         }
         ...
         /* this param is not a bool, so reset numConsecutiveBools */
         numConsecutiveBools = 0;

         /*
          * remember if this param is a 64 bit type, this info is needed
          * when calculating the offset of the next param.
          */
         isPrevParam64 = (shmParamNode->type == MPT_LONG64 || shmParamNode->type == MPT_UNSIGNED_LONG64
#ifdef __LP64__
			|| shmParamNode->type == MPT_STRING || shmParamNode->type == MPT_DATE_TIME 
                	|| shmParamNode->type == MPT_BASE64 || shmParamNode->type == MPT_HEX_BINARY
#endif
...
CmsRet oal_readConfigFlashToBuf(const char *selector, char *buf, UINT32 *len)
{
   ...
   if (isCrc)
   {
      ...
      calculatedCrc = crc_getCrc32((UINT8 *) &(flashBuf[offset]), crcLen, CRC_INITIAL_VALUE);
