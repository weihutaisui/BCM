...
static CmsRet rutQos_cellularQueueConfig(QosCommandType cmdType,
                             char * ifName,
                             UINT32 queueWeight __attribute((unused)),
                             UINT32 queuePrecedence __attribute((unused)),
                             char * schedulerAlgorithm __attribute((unused)),
                             SINT32 shapingRate __attribute((unused)),
                             UINT32 shapingBurstSize __attribute((unused)),
                             SINT32 minBitRate __attribute((unused)),
                             UINT32 queueId __attribute((unused)));
#endif
...
void rutQos_getIntfNumOfCreatedQueues(const char *queueInterface,
                                      UINT32 *numQueues);
...
CmsRet rutQos_qMgmtQueueConfig(QosCommandType cmdType, const void *qObj)
{
       ...
       !cmsUtl_strncmp(qInfo.intfName, MOCA_IFC_STR, strlen(MOCA_IFC_STR)))
   {
      ...
      if ((!qInfo.isWan) && (cmdType == QOS_COMMAND_UNCONFIG))
      {
          ...
          rutQos_getIntfNumOfCreatedQueues(((_QMgmtQueueObject*)qObj)->queueInterface, &qNum);
          if (qNum == 0)
          {
...
#endif   
#ifdef SUPPORT_CELLULAR
   else if (!cmsUtl_strncmp(qInfo.intfName, CELLULAR_IFC_STR, strlen(CELLULAR_IFC_STR)))
   {
      if ((ret = rutQos_cellularQueueConfig(cmdType,
                                       qInfo.intfName,
                                       qInfo.queueWeight,
                                       qInfo.queuePrecedence,
                                       qInfo.schedulerAlgorithm,
                                       qInfo.shapingRate,
                                       qInfo.shapingBurstSize,
                                       qInfo.minBitRate,
                                       qInfo.queueId)) != CMSRET_SUCCESS)
      {
         cmsLog_error("rutQos_cellularQueueConfig returns error. ret=%d", ret);
      }
   }
#endif   
   else if (((cmsUtl_strstr(((_QMgmtQueueObject *) qObj)->queueInterface, "GPON")) != NULL) ||
            ((cmsUtl_strstr(((_QMgmtQueueObject *) qObj)->queueInterface, "Epon")) != NULL) ||
            ((cmsUtl_strstr(((_QMgmtQueueObject *) qObj)->queueInterface, "EPON")) != NULL) ||
...
#ifdef SUPPORT_CELLULAR
/* Check if exist one enabled cellular queue
Sarah: TODO: Later, TR181 implement
*/
static UBOOL8 rutQos_isCellularQueueExist()
{
   ...
   while (!exist &&
          cmsObj_getNext(MDMOID_Q_MGMT_QUEUE, &iidStack, (void **) &qObj) == CMSRET_SUCCESS)
   {
      ...
      if (cmsUtl_strstr(intfname, CELLULAR_IFC_STR) && qObj->queueEnable)
      {
         exist = TRUE;
      }
   ...
   cmsLog_debug("Entered: exist=%d", exist);
   
   return exist;
}

CmsRet rutQos_cellularQueueConfig(QosCommandType cmdType,
                             char * ifName,
                             UINT32 queueWeight __attribute((unused)),
                             UINT32 queuePrecedence __attribute((unused)),
                             char * schedulerAlgorithm __attribute((unused)),
                             ...
                             UINT32 queueId __attribute((unused)))
{
   ...
   cmsLog_debug("cmdType=%d, ifName=%s", cmdType, ifName);

   /* No harm doing duplicate times to reduce complexity */
   sprintf(cmd, "tc qdisc del dev %s root 2>/dev/null", ifName);
   rut_doSystemAction("rutQos_cellularQueueConfig", cmd);

   if (cmdType == QOS_COMMAND_UNCONFIG && rutQos_isCellularQueueExist() == FALSE)
   {
      return CMSRET_SUCCESS;
   }

   sprintf(cmd, "tc qdisc add dev %s root handle 1: htb default 1 2>/dev/null", ifName);
   rut_doSystemAction("rutQos_cellularQueueConfig", cmd);

   /*Hard code upstream rate to 50M*/
   sprintf(cmd, "tc class add dev %s parent 1: classid 1:1 htb rate 51200kbit 2>/dev/null", ifName);
   rut_doSystemAction("rutQos_cellularQueueConfig", cmd);

   /* Always create CELLULAR_QOS_LEVELS bands, classification rules will take care which queue to use */
   sprintf(cmd, "tc qdisc add dev %s parent 1:1 handle 10: prio bands %d priomap %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d 2>/dev/null", 
           ifName, CELLULAR_QOS_LEVELS, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, 
           CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, 
           CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, 
           CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1, CELLULAR_QOS_LEVELS - 1);
   rut_doSystemAction("rutQos_cellularQueueConfig", cmd);
      
   for (i=1; i <= CELLULAR_QOS_LEVELS; i++)
   {
      sprintf(cmd, "tc qdisc add dev %s parent 10:%d handle %d: pfifo limit 10 2>/dev/null", ifName, i, 100+i);
      rut_doSystemAction("rutQos_cellularQueueConfig", cmd);
   
      /*For skb->mark, higher value, higher priority
        For classid, lower value, higher priority   
      */
      queueMark = SKBMARK_SET_Q(queueMark, (CELLULAR_QOS_LEVELS - i));
      sprintf(cmd, "tc filter add dev %s protocol all parent 10:0 prio 1 handle 0x%x/0x%x  fw classid 10:%d 2>/dev/null", ifName, queueMark, SKBMARK_Q_M, i);
      rut_doSystemAction("rutQos_cellularQueueConfig", cmd);
   }   
...
#ifdef SUPPORT_CELLULAR
         else if (!cmsUtl_strncmp(queueIntfName, CELLULAR_IFC_STR, strlen(CELLULAR_IFC_STR)))
         {
            qid = CELLULAR_QOS_LEVELS - queuePrecedence;
         }
#endif		 
#ifdef BRCM_WLAN
...
#ifdef SUPPORT_CELLULAR
   if (!cmsUtl_strncmp(queueIntfName, CELLULAR_IFC_STR, strlen(CELLULAR_IFC_STR)))
   {
      qidMark = qid & 0x1f;
   }
   else
                                      ...
                                      UINT32 *numQueues)
{
   ...
   while ((ret = cmsObj_getNext(MDMOID_Q_MGMT_QUEUE, &iidStack, (void**)&qObj))
      == CMSRET_SUCCESS)
   {
      if (cmsUtl_strcmp(qObj->queueInterface, queueInterface) == 0)
      {
          createdQNum++;
      }
      cmsObj_free((void**)&qObj);
   ...
   if (numQueues != NULL)
   {
      *numQueues = createdQNum;
      cmsLog_debug("qIntf=%s, qnum=%d", queueInterface, createdQNum);
   }
...
#ifdef SUPPORT_CELLULAR
   else if (cmsUtl_strstr(l2IfName, CELLULAR_IFC_STR))
   {
      txqs = MAX_CELLULAR_TRANSMIT_QUEUES;
      maxLevels = CELLULAR_QOS_LEVELS;
   }   
#endif   
   else
   {
...
CmsRet rutQos_tmQueueConfig(QosCommandType cmdType, CmsQosQueueInfo *qInfo)
{
   ...
   if(devType == TMCTL_DEV_ETH)
   {
...

      {
         ...
         /* Add TM queue */
         cmsLog_debug("Add TM queue. ifname=%s qid=%d schedMode=%d qWt=%d shapingRate=%d",
                      qInfo->intfName, qInfo->queueId-1, schedMode, qInfo->queueWeight, kbps);
         ...
         {
            cmsLog_error("Add TM queue. ifname=%s qid=%d priority=%d schedMode=%d qWt=%d shapingRate=%d",
                         qInfo->intfName, qInfo->queueId-1, priority, schedMode, qInfo->queueWeight, kbps);
            ret = CMSRET_INTERNAL_ERROR;
         }
         else if (qcfg.qsize)
         {
