      ...
      if ( cmsUtl_strcmp(newObj->X_BROADCOM_COM_BoundIpAddr, currObj->X_BROADCOM_COM_BoundIpAddr) )
      {
         if ( rutIsVoipRunning() )
         {
            ...
            else
            {
               ...
               msg.type = CMS_MSG_RESTART_VOICE;
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   if ((newObj != NULL && currObj == NULL)) /* new instance creation */
   {
      ...
      else
      {
         ...
         if ( (ret = mdm_setObject( (void **) &vObj, &ancestorIidStack, FALSE )) != CMSRET_SUCCESS )
         {
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   if ((newObj != NULL && currObj == NULL)) /* new instance creation */
   {
      ...
      if ( totalNumLines >= maxLines )
      {
         ...
         mdm_setObject( (void **) &vpObj, &ancestorIidStack, FALSE );
      ...
      else
      {
         ...
         if ( (ret = mdm_setObject( (void **) &vpObj, &ancestorIidStack, FALSE )) != CMSRET_SUCCESS )
         {
            cmsLog_error("Unable to set value to mdm, ret = %d\n", ret);
            cmsObj_free((void **) &vpObj);
            return ret;
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if ( newObj != NULL && currObj != NULL && newObj->resetStatistics )
   {
      ...
      /* Save MDM Object ID before memset */
      tempId = newObj->_oid;
