...
static void policyRoutingAction( const char *prefixedRuleName, const char *srcIfName,
                              ...
                              const char *outIfName, SINT16 idx, UBOOL8 add )
{
   ...
   char ip_ro_subnet_cmd[BUFLEN_128]={0};
   ...
   if ( cmsUtl_strcmp(srcIfName, "") )
   {
      ...
      {
         strncat(eb_cmd, " -i ", sizeof(eb_cmd));
      }
      else
      {
         strncat(eb_cmd, " --logical-in ", sizeof(eb_cmd));
      }
   ...
   if (newIPRoute2)
   {
      ...
      cmsUtl_strncat(ip_rule_cmd, sizeof(ip_rule_cmd)-strlen(ip_rule_cmd)-1, id);
   }
   else
   {
      cmsUtl_strncat(ip_rule_cmd, sizeof(ip_rule_cmd)-strlen(ip_rule_cmd)-1, prefixedRuleName);
   }
   ...
   if (newIPRoute2)
   {
      char id[BUFLEN_8];
      snprintf(id, sizeof(id), "%d ", RT_TABLE_BASE+idx);  
      cmsUtl_strncat(ip_ro_cmd, sizeof(ip_ro_cmd)-strlen(ip_ro_cmd)-1, id);
   }
   else
   {
      cmsUtl_strncat(ip_ro_cmd, sizeof(ip_ro_cmd)-strlen(ip_ro_cmd)-1, prefixedRuleName);
   }
   ...
   if (newIPRoute2)
   {
      char id[BUFLEN_8];
      snprintf(id, sizeof(id), "%d ", RT_TABLE_BASE+idx);  
      cmsUtl_strncat(ip_ro_subnet_cmd, sizeof(ip_ro_subnet_cmd)-strlen(ip_ro_subnet_cmd)-1, id);
   }
   else
   {
      cmsUtl_strncat(ip_ro_subnet_cmd, sizeof(ip_ro_subnet_cmd)-strlen(ip_ro_subnet_cmd)-1, prefixedRuleName);
   }
