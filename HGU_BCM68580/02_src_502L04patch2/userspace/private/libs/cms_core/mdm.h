...
/** Modifies ownerId for an Instance  
 *
 * @param objNode   (IN) The mdmObj which is being queried.
 * @param iidStack (IN) The iidStack of the mdmObj which is being queried.
 * @param ownerId  (IN) ownerId to be set.
 *
 * @return CmsRet enum.
 */
CmsRet mdm_setInstanceOwnerId(const MdmObjectNode *objNode,const InstanceIdStack *iidStack, UINT16 ownerId);


/** Return ownerId for an Instance  
 *
 * @param objNode  (IN) The mdmObj which is being queried.
 * @param iidStack (IN) The iidStack of the mdmObj which is being queried.
 * @param ownerId  (OUT) current ownerId.
 *
 * @return CmsRet enum.
 */
CmsRet mdm_getInstanceOwnerId(const MdmObjectNode *objNode,const InstanceIdStack *iidStack, UINT16 *ownerId);
...
/** Acquire the MDM lock with timeout */
#define MLOCK_WITH_TIMEOUT(p)  oal_lock(p)


/** Release the MDM lock. */
#define MUNLOCK oal_unlock()



/** The main objective of this function is verify that the caller
 *  has the MDM lock before going into the MDM.
 *
 * This function is used by all the entry point functions in the
 * OBJ, PHL, and management layers.
 * 
 * @param where (IN) string of the function calling this prolog function.
 *                   Makes the debug message more useful.
 * @param maxEntries (IN) if this is 1, it means this function cannot be 
 *                        called by STL/RCL handler functions.  Otherwise,
 *                        set it to MDM_MAX_ENTRY_COUNT. 
 *                    
 * @return CmsRet enum.
 */
...
/** This function is the companion to lck_checkBeforeEntry() function.
 * 
 * This function should be called whenever we exit from a function
 * that had lck_checkBeforeEntry() at the beginning of the function.
 * 
 * @param where (IN) string of the function calling this function.
 *                   Makes the debug message more useful.
 */
...
typedef struct
{
   ...
   UINT8          lockFuncCode; /**< 's' for STL, 'r' for RCL */
   UINT8    isDataModelDevice2; /**< 1: root=Device; 0: root=InternetGatewayDevice */
   MdmObjectId    oid;           /**< oid currently being accessed */
...
typedef struct
{
   ...
   UBOOL8  locked;    /**< This process currently has the lock. */
   UINT8 entryCount; /**< How many times have we entered through the OBJ layer.
                          This is used to keep track of whether a cmsObj_
                          function is being called from a management app
                          or from inside the RCL or STL. */ 
...
typedef struct
{
   MdmObjectId    oid;         /**< oid which want to be posted activating */   
   UINT32         subType;     /**< Post activating node type */
   UINT32         entCunt;     /**< Events counter waiting for */
} PostActiveNodeInfo;

/** Maximum times a cmsObj_ function can be called recursively.
 * The variable that tracks the entry count is an UINT8, hence this limit.
 */
#define MDM_MAX_ENTRY_COUNT   255

/** Macro to determine if we are being called from an external management
 * entity or from rcl/rut function using cmsObj_ API.
 */
#define IS_EXTERNAL_CALLER (mdmLibCtx.entryCount == 1) 
