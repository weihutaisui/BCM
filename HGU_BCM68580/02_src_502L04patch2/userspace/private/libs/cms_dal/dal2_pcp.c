...
CmsRet dalPcp_set_dev2(const char *ipIntfFullPath, UINT32 mode,
                       const char *pcpServer)
{
   MdmPathDescriptor pathDesc = EMPTY_PATH_DESCRIPTOR;
   Dev2IpInterfaceObject *obj = NULL;
   CmsRet ret = CMSRET_SUCCESS;

   cmsLog_debug("path<%s> mode<%u> pcpServer<%s>", ipIntfFullPath,
                mode, pcpServer);
   ...
   ret = cmsObj_get(pathDesc.oid, &pathDesc.iidStack, 0, (void **) &obj);
   if (ret != CMSRET_SUCCESS)
   {
   ...
   obj->X_BROADCOM_COM_PCPMode = mode;
   CMSMEM_REPLACE_STRING(obj->X_BROADCOM_COM_PCPServer, pcpServer);

   if ((ret = cmsObj_set(obj, &pathDesc.iidStack)) != CMSRET_SUCCESS)
   {
