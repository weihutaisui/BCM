...
int  wlcsm_dm_load(int idx,int from)
{
    ...
    if(wlcsm_dm_handler)
        return wlcsm_dm_handler->dm_load(idx,from);
    else
        return 1;
}
...
int  wlcsm_dm_save_config(int idx,int from)
{
    ...
    if(wlcsm_dm_handler)
        return   wlcsm_dm_handler->dm_save(idx,from);
    else
        return 1;
}
...
char *wlcsm_dm_mngr_get_value(unsigned int idx,unsigned int sub_idx,char *name,char *varValue,unsigned int pos)
{
    ...
    WLCSM_DM_TYPE data_type;
    WLCSM_MNGR_NAME_OFFSET *obj_mapper= NULL;
    //WLCSM_TRACE(WLCSM_TRACE_DM," name:%s ,oid:%u\r\n",name,pos );
...
int wlcsm_dm_pwrctl(int anyassociated)
{
    if(wlcsm_dm_handler) {
        if(anyassociated)
            return  wlcsm_dm_handler->dm_query_info(WLCSM_DM_QUERY_PWRCTL,(char *)&(anyassociated));
        else
            return  wlcsm_dm_handler->dm_query_info(WLCSM_DM_QUERY_PWRCTL,NULL);
    }
    else
        return -1;
}
...
int wlcsm_dm_select_dm(char *name,unsigned int idx,int from)
{
    ...
    do {
        if(!WLCSM_STRCMP(tempHandler->dm_name,name)) {
            ...
            if(!_wlcsm_dm_init()) return wlcsm_dm_load(idx,from);
            else {
