...
/*max no of milli seconds to wait for lock */
#define UPNP_LOCK_TIMEOUT (10 * 1000)

/*default lease time in seconds for portmappings (4 hours?) */
#define UPNP_DEFAULT_LEASETIME (4 * 3600)
...
struct ServiceTemplate {
    char	*name;		/* Type of the service, e.g. "WANPPPConnection:1" */
...
extern int upnp_main(PDeviceTemplate pdevtmpl, char *ifname, char *wanif,char *wanif_l2);
...
extern char g_lan_ifname[CMS_IFNAME_LENGTH];
extern char g_wan_ifname[CMS_IFNAME_LENGTH];
extern char g_wan_l2ifname[CMS_IFNAME_LENGTH];
extern char g_upnp_wan_proto[BUFLEN_16];
extern int   g_wan0_primary;
extern char *g_upnpAppName;

extern int mode;
extern char pcpsrv[CMS_IPADDR_LENGTH];
extern char pcplocal[CMS_IPADDR_LENGTH];
extern uint8_t pcpextaddr[16];

//PCP TODO
extern void invoke( UFILE *up, PService psvc, PAction ac, pvar_entry_t args, int nargs);
extern bool validate_invoke( PService psvc, PAction ac, pvar_entry_t args, int nargs);
extern void pcp_add_invoke( UFILE *up, PService psvc, PAction ac, pvar_entry_t args, int nargs);
