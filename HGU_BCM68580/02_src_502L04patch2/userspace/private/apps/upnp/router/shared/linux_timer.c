 ...
 *
 * $Id: linux_timer.c,v 1.5.12.2 2003/10/31 21:31:11 mthawani Exp $
 */
...
void init_event_queue(int n)
{
    ...
    g_maxevents = n;
    event_freelist = (struct event *) cmsMem_alloc(n * sizeof(struct event), ALLOC_ZEROIZE);
...
static void check_timer()
{
    ...
    if (timercmp(&(itimer.it_value), &(event_queue->it_value), >)) {
/* TODO: need to have Pavan fix this.
