...
struct net_connection *make_http_socket(struct iface *pif)
{
    ...
    c = (struct net_connection *) cmsMem_alloc(sizeof(struct net_connection), ALLOC_ZEROIZE);
	 if (c == NULL) {
		 UPNP_ERROR(("make_http_socket cannot cmsMem_alloc.\n"));
		 goto error;
	 }
...
void accept_http_connection(caction_t flag, struct net_connection *nc, void *arg)
{
	...
	if (s == -1) {
	    ...
	    hc = (struct http_connection *) cmsMem_alloc(sizeof(struct http_connection), ALLOC_ZEROIZE);
		 if (!hc) {
			 UPNP_SOCKET(("%s: close %d\n", __FUNCTION__, s));
		...
		hc->buf = cmsMem_alloc(HTTP_BUFSIZE, ALLOC_ZEROIZE);
		if(hc->buf == NULL){
			UPNP_ERROR(("cmsMem_alloc failed \n"));
			cmsMem_free(hc);
			close(s);
...
void http_receive(caction_t flag, struct http_connection *c, void *arg)
{
	...
	if ((c->maxbytes - c->bytes_recvd) < HTTP_BUF_LOWATER) {
	    ...
	    c->buf = cmsMem_realloc(c->buf, c->maxbytes);
	    assert(c->buf != NULL);
	}
...
void http_response(UFILE *up, http_error_t error_code, const char *body, int body_len)
{
    ...
    if (write(ufileno(up), (char *) body, body_len) == -1) 
       UPNP_ERROR(("write error %d:bodyptr=%p,body_len=%d\n", errno, body, body_len));
}
...
static void process_subscribe(UFILE *up, char *fname, char *msg)
{
    ...
    if ((psvc = find_svc_by_url(fname)) == NULL) {
	UPNP_INFORM(("Cannot subscribe - service \"%s\" not found.\n", fname));
	http_error( up, HTTP_NOT_FOUND );
    } else {
