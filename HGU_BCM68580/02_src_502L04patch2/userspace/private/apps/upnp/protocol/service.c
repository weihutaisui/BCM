...
PService init_service(PServiceTemplate svctmpl, PDevice pdev,int dynInstId)
{
    PService psvc;
    PFSVCINIT func;
    int i, table_size;
    ...
    psvc = (PService) cmsMem_alloc(sizeof(Service), ALLOC_ZEROIZE);
    if(psvc == NULL){
    ...
    psvc->vars = (StateVar *) cmsMem_alloc(table_size, ALLOC_ZEROIZE);
    if(psvc->vars == NULL){
       UPNP_ERROR(("cmsMem_alloc failed \n"));
       cmsMem_free(psvc);
       return NULL;
    ...
    if ( strstr ( wanif, "ppp" ) != NULL ) {
        if ( strcmp ( svctmpl->name, "WANIPConnection:1" ) == 0 ) {
            return psvc;
        }
    } else {
        if ( strcmp (svctmpl->name, "WANPPPConnection:1" ) == 0 ) {
            return psvc;
        }
    }
...
{
    UPNP_INFORM(("Action \"%s\" not implemented, svc \"%s\"\r\n", ac->name, psvc->template->name));

    soap_error( up, SOAP_ACTION_FAILED );
