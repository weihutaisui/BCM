                 ...
                 timer_t *         pTimer    /* ptr to return value */)
{
    ...
    if (clock_id != CLOCK_REALTIME) {
       syslog(LOG_ERR,"timer_create can only support clock id CLOCK_REALTIME");
       exit(1);
    }

    if (evp != NULL) {
       if (evp->sigev_notify != SIGEV_SIGNAL || evp->sigev_signo != SIGALRM) {
          syslog(LOG_ERR,"timer_create can only support signalled alarms using SIGALRM");
          exit(1);
       }
    }
...
int timer_delete(timer_t timerid /* timer ID */)
{
    ...
    if (event->flags & TFLAG_DELETED) {
       syslog(LOG_DEBUG,"Cannot delete a deleted event");
       return 1;
    }
...
#endif
    if (event->next) {
       syslog(LOG_DEBUG,"calling timer_settime with a timer that is already on the queue.");
    }
...
static void print_event_queue(void)
{
    ...
    for (event = event_queue; event; event = event->next) {
       printf("#%d (0x%" PRIxPTR ")->0x%" PRIxPTR ": \t%d sec %d usec\t%p\n", 
              i++, (uintptr_t) event, (uintptr_t) event->next, (int) event->it_value.tv_sec, (int) event->it_value.tv_usec, event->func);
       if (i > g_maxevents) {
          printf("...(giving up)\n");
          break;
       }
