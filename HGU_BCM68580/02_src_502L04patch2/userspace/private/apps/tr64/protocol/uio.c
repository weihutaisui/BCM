...
int uprintf(UFILE *up, char *fmt, ...)
{
   ...
   {
      if (up->flags & UIO_SOCKRST)
      { 
         UPNP_ERROR(("uprintf : remote side already reset the tcp connection\n"));
      }
      else
      {
         if(ferror(up->io.fp)==0)
         {
            n = vfprintf(up->io.fp, fmt, ap);
            if (n > 0) 
            {
               up->bytes_written += n;
               if(up->bytes_written >1400)
               {
                  up->bytes_written = 0;
                  uflush(up);                
               }
            ...
            {
               UPNP_ERROR(("uprintf: vfprintf errno[%d]\n", errno));    
               UPNP_ERROR(("uprintf: remote side reset the tcp connection\n"));  
               up->flags |= UIO_SOCKRST;
            }
         }
         else
         {
            UPNP_ERROR(("uprintf: remote side reset the tcp connection errno[%d]\n", errno));    
            if(errno == ECONNRESET)
            {
               up->flags |= UIO_SOCKRST;
               UPNP_ERROR(("uprintf: remote side reset the tcp connection\n"));    
...
void uclose(UFILE *up)
{
   if (up->flags & UIO_FILE) {
      if (up->flags & UIO_SOCKRST)
      {
         UPNP_ERROR(("uclose: remote side already reset the tcp connection\n"));    
      }
      else
      {
         fclose(up->io.fp);
...
void uflush(UFILE *up)
{
   if (up->flags & UIO_FILE) 
   {
      if (up->flags & UIO_SOCKRST)
      {    
         UPNP_ERROR(("uflush : remote side already reset the tcp connection\n"));
      }
      else
      {
         int ret = 0;
         ...
         if(ret==0) 
         {
            if(fflush(up->io.fp)!=0)
            {
               up->flags |= UIO_SOCKRST;
               UPNP_ERROR(("uflush : remote side reset the tcp connection\n"));
               return;
            }
         }
         else
