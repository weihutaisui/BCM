 ...
 */
#ifdef BUILD_STATIC
int gmac_main(int argc, char **argv)
#else
...
#endif
{
    ...
    while ( argc && nExitCode == 0 )
    {
        ...
        {
            nExitCode = GMACCTL_INVALID_COMMAND;
...
{
    int nRet = GMACCTL_SUCCESS, nNumOptInfo = 0, nArgType = 0;
    ...
    *pnArgNext = 0;
    ...
    do
    {
        ...
        case ARG_TYPE_OPTION:
            ...
            {
                nRet = GMACCTL_INVALID_NUMBER_OF_OPTIONS;
    ...
    } while ( (nRet == GMACCTL_SUCCESS) && (nArgType!=ARG_TYPE_COMMAND) );

    if ( nRet == GMACCTL_SUCCESS )
