...
void processSampleSetStatusChanged_igd(CmsMsgHeader *msg)
{
   ...
   SampleSetObject *pSmpsetObj = NULL;
   ...
   while (!found && (cmsObj_getNext(MDMOID_SAMPLE_SET, &iidStack, (void **) &pSmpsetObj)) == CMSRET_SUCCESS)
   {
      int id = PEEK_INSTANCE_ID(&iidStack); 
      if ((UINT32) id == msg->wordData)
      {
