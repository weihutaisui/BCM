...
void processRequestPppChange_dev2(const CmsMsgHeader *msg)
{
   ...
   {
      if (!cmsUtl_strcmp(pppintf->name, pppIfName))
   	{
   	   found = TRUE;
   	   break;
      }
   ...
   if (msg->wordData == USER_REQUEST_CONNECTION_DOWN && 
      !cmsUtl_strcmp(pppintf->connectionStatus, MDMVS_CONNECTED))
   {
   
      CMSMEM_REPLACE_STRING(pppintf->X_BROADCOM_COM_UserRequest, MDMVS_DOWN);
      change = TRUE;
   }
   else if (msg->wordData == USER_REQUEST_CONNECTION_UP && 
      !cmsUtl_strcmp(pppintf->connectionStatus, MDMVS_DISCONNECTED))
   {
      /* for user request pppd up if  in the "Disconnected" connectionStatus */
      CMSMEM_REPLACE_STRING(pppintf->X_BROADCOM_COM_UserRequest, MDMVS_UP);
      change = TRUE;
   }
   ...
   else
   {
      cmsLog_debug("No need to change. user request=%d, connectStatus=%s", 
         msg->wordData, pppintf->X_BROADCOM_COM_UserRequest);
   }      
