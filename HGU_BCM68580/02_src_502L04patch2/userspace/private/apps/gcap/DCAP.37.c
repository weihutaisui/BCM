...
{
    printf("Usage: DCAP.37 <ON/[OFF]> [-M] [-r] [-h]\n\
Set DUT to operate in Preferred NC mode.\n\
...
GCAP_GEN int DCAP_37_main(int argc, char **argv)
{
    int ret;
    void *ctx;
   ...
   for (i=1; i < argc; i++)
   {
    ...
    {
        ret = moca_set_preferred_nc(ctx, 0);
    }
    else
    {
        ret = moca_set_preferred_nc(ctx, 1);
    }
