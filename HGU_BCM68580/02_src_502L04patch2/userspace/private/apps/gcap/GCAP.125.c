...
{
    printf("Usage: GCAP.125 <mask> [-i iface] [-h]\n\
...
GCAP_GEN int GCAP_125_main(int argc, char **argv)
{
    int ret = 0;
    int rtrPrioMask = 0;
    ...
    if (argc < 2)
    {
        printf( "Error!  Missing parameter - mask\n");
        return(-2);
    }
   ...
   for (i=1; i < argc; i++)
   {
    ...
    if (rtrPrioMask > 0xF)
    {
       printf( "Error! Invalid retransmission bit mask. Valid values are 0-15.\n");
       return(-3);
    }
