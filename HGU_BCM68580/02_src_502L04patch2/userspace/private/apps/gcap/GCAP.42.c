...
{
    printf("Usage: GCAP.42 [<[ON]/OFF>] [GARPL_Threshold] [RLAPM_value] [-r] [-M] [-h]\n\
...
GCAP_GEN int GCAP_42_main(int argc, char **argv)
{
    int ret=0;
    int i;
    void *ctx;
   ...
   else 
   {
       ...
       for (i=0;i<66;i++)
       {
          if (i%8 == 0)
             printf("\n");
#if defined(STANDALONE)
          printf ("[%3d=%4d]", i, rlapmtbl.rlapmtable[i]/2);
#else
          printf ("[%3d=%4.01f]", i, rlapmtbl.rlapmtable[i]/2.0);
#endif
       }
       printf("\n");
    } 
