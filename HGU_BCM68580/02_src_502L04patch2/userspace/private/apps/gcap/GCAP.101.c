...
{
    printf("Usage: GCAP.101 <Per mode> [-M] [-h]\n\
...
GCAP_GEN int GCAP_101_main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        printf( "Error!  Missing parameter - Per mode\n");
        return(-2);
    }
   ...
   for (i=1; i < argc; i++)
   {
    ...
    if (per_mode_str != NULL)
    {
        ...
        {
           printf( "Error!  Illegal value for <Per mode>. Must be one of 'NPER' or 'VLPER'.\n");
           moca_close(ctx);
           return(-3);
        }
    ...
    {
       moca_close(ctx);
       printf( "Error!  Invalid parameter - Per mode\n");
       return(-4);
    }
