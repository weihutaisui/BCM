...
static int VersionHandler( void );
...
static COMMAND_INFO g_Cmds[] = {
     ...
     {"stop", {NULL}, (FN_COMMAND_HANDLER)StopHandler},
...
#ifdef SUPPORT_SELT
     {"selt", {"--start", "--stop", "--status", "--steps", "--cfg", NULL}, (FN_COMMAND_HANDLER)SeltHandler},
#endif
#ifdef NTR_SUPPORT
     {"ntr", {"--start", "--stop", NULL}, NTRHandler},
#endif
     {"--version", {NULL}, (FN_COMMAND_HANDLER)VersionHandler},
     {"--help", {NULL}, (FN_COMMAND_HANDLER)HelpHandler},
     {"", {NULL}, NULL}
};
...
#ifdef CONFIG_VDSL_SUPPORTED
static char* QnToString1(SINT32 val, int q, int ipartlen)
{
   static   char  str1[32];
   SINT32        iPart;
   ...
   if (val < 0) {
      ...
      if (0 == iPart) {
         if(ipartlen==5)
         	sprintf(str1," ");
         sprintf(str1,"  -0.%1d", (Qn2DecF(val,q)+500)/1000);
         return str1;
      }
   }
   else
      iPart = val >> q;
   if(ipartlen==5)
      sprintf( str1, "%5d.%1d", iPart, (Qn2DecF(val,q)+500)/1000);
   else sprintf( str1, "%4d.%1d", iPart, (Qn2DecF(val,q)+500)/1000);
   return str1;
}
#endif
/***************************************************************************
...
static int ParseCfgParam (POPTION_INFO pOptions, int nNumOptions, int bInit, adslCfgProfile *pAdslCfg, int *pUp)
{
   ...
   vdslProfileMask |= kGfastProfileMask;
#else
   vdslProfileMask |= kGfastProfileMask1;
#endif
#endif
...
static int StartHandler(unsigned char lineId, POPTION_INFO pOptions, int nNumOptions )
{
   ...
   if (nRet == BCMADSL_STATUS_SUCCESS) 
   {
      cmsRet = xdslCtl_Initialize(lineId, NULL, NULL, &adslCfg );
   }
#endif
...
static void ProfileShow(unsigned char lineId)
{
	...
	printf("\tSOS:\t\t%s\n", (pAdslCfg->vdslCfgFlagsValue & kVdslSOSDisable)? "Off": "On");
#endif
#ifndef ANNEX_C
...
static void PrintAdslCfg(unsigned char lineId)
{
	...
	printf("\n"
		...
		adsCfg.maxDelay ? (unsigned int) adsCfg.maxDelay & 0x7FFFFFFF : -1,
#ifdef CONFIG_VDSL_SUPPORTED
...
static int InfoHandler(unsigned char lineId, POPTION_INFO pOptions, int nNumOptions )
{
	...
	if(adslMib.adslPhys.adslLDCompleted) {
		...
		else if((UINT32)-1 == adslMib.adslPhys.adslLDCompleted)
			printf("Loop Diagnostic failed!\n");
		else if((UINT32)-2 == adslMib.adslPhys.adslLDCompleted)
...
static int GetSnrClampShape(char *ppStr, char *snrClampShape, int snrClampSize)
{
    ...
    do {
        ...
        fprintf( stderr, "\t\tskipped some spaces, now at 0x%x, offset=%d\n", *p, p-ppStr);

        if (!GetInteger(&p, &n1,0))
        {
                fprintf( stderr, "\t\tcannot get an integer from 0x%x, n1=%d, offset=%d\n", *p, n1,p-ppStr);
                break;
        }
        p = SkipWhitespace(p);
		if ('-' == *p) {
            ...
            fprintf( stderr, "\t\tBP %d - %d, lastBpIndex=%d, offset=%d\n", n1, n2, lastBpIndex,p-ppStr);
            if (n1>lastBpIndex) {
    ...
    fprintf( stderr, "\t\treturning %d elements after processing %d char\n", i0, p-ppStr );
	...
	while ((0 == nRet) && (nNumOptions-- > 0)) {
		...
		if (0 == strcmp(pOptions->pszOptName, "--start")) {
            ...
            if ((pOptions->pszParms[0][0] == '0') &&
                ...
                printf("SELT config: 0x%x   steps: 0x%x\n",seltData.seltCfg, seltData.seltSteps);
            else
            {
				nRet = ADSLCTL_INVALID_PARAMETER;
				break;
