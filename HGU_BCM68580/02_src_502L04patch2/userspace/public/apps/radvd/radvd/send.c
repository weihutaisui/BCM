...
send_ra(struct Interface *iface, struct in6_addr *dest)
{
	...
	while(prefix)
	{
		if( prefix->enabled && (!prefix->DecrementLifetimesFlag || prefix->curr_validlft > 0) )
		{
			...
			if (iface->cease_adv && prefix->DeprecatePrefixFlag) {
				...
				if (prefix->DecrementLifetimesFlag) {
					uint32_t curr_validlft = prefix->curr_validlft;
					...
					if (prefix->AdvValidLifetime == curr_validlft)
						return 0;
