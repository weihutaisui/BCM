...
// Get angle between two vectors (in radians such that -PI < angle <= PI)
        ...
        if (self.p.adjustSizes) {
          ...
          while (i < nodes.length && i < self.state.index + sInt) {
            ...
            if (!fixed && n.fa2) {
              ...
              // reduce shaking: if the new force is opposite of the old one, adjust
              // the movement to be slower by a random amount.   This will allow the
              // node to eventually converge to the ideal location.
              var absAngle = Math.abs(_getAngle( {x:n.fa2.old_dx,y:n.fa2.old_dy}, {x:n.fa2.dx,y:n.fa2.dy} ));

              if (absAngle > Math.PI * 0.70) {
                 factor = Math.max(0, factor-(Math.random()*0.01+0.005));
              }
              else {
                 // add a tiny bit of friction to reduce longer shakes
                 factor = Math.max(factor-0.0005, 0);
              }
              
              var _dx=n.fa2.dx * factor;
              var _dy=n.fa2.dy * factor;
              n.x += _dx;
              n.y += _dy;
            }
          }
...
sigma.publicPrototype.startForceAtlas2 = function(it) {
    if (!this.forceatlas2) {
    ...
    var atlas = this.forceatlas2;

    this.addGenerator('forceatlas2', it ? function() {
        for (var i = 0; i < it; i++)
            atlas.atomicGo();
    } : this.forceatlas2.atomicGo, function() {
        return true;
    });
};
