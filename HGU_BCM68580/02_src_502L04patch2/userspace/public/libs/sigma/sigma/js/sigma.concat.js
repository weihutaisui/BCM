...
sigma.classes.EventDispatcher = function() {
  ...
  function dispatch(type, content) {
    ...
    /**
    * Returns true if has one of the events binding
    */
  function has(events) {
      var eArray = typeof events == 'string' ? events.split(' ') : events;
...
function Graph() {
  ...
  function checkHover(mX, mY) {
    ...
    self.nodes.forEach(function(node) {
      ...
      var dX = Math.abs(node['displayX'] - mX);
      var dY = Math.abs(node['displayY'] - mY);
      var s = node['displaySize'];

      var oldH = node['hover'];
      var newH = dX < s && dY < s && (dX * dX + dY * dY) < s*s;
    ...
    if (self.has("overedges downedges"))
        checkEdgeHover(mX, mY, over.concat(on));
  ...
  function _dist2(v, w) { return _sqr(v.x - w.x) + _sqr(v.y - w.y) }
  function _distToSegmentSquared(p, v, w) {
    ...
    return _dist2(p, { x: v.x + t * (w.x - v.x),
                       y: v.y + t * (w.y - v.y) });
  }



  function checkEdgeHover(mX, mY, nodesOver) {
      var over = [], out = [];
      self.edges.forEach(function(edge) {
          ...
          if (nodesOver.indexOf(edge.source.id) > -1 || nodesOver.indexOf(edge.target.id) > -1) {
              edge['hover'] = false;
              return;
          }

          var x1 = edge.source.displayX, y1 = edge.source.displayY, x2 = edge.target.displayX, y2 = edge.target.displayY;
          var dX = x1 - x2;
          var dY = y1 - y2;

          var oldH = edge['hover'];
          var newH = _distToSegmentSquared({x:mX,y:mY}, {x:x1,y:y1}, {x:x2,y:y2}) < 3*3;
          ...
          if (oldH && !newH) {
              ...
              edge['mouseX'] = mX;
              edge['mouseY'] = mY;
              //console.log("over edge label '" + edge.label + "' distance: " + s + ". source " + edge.source.label + ", target " + edge.target.label);
          }
      });
  ...
  function iterNodes(fun, ids) {
    ...
    if (!a || a.length == 0 || !a[0]) return self;
...
function MouseCaptor(dom) {
  ...
  function startDrag() {
    ...
    var targeted = self.graph.graph.nodes.filter(function(n) {
        return !!n['hover'];
    }).map(function(n) {
        return n;
    });
    dnode = targeted.length ? targeted[0] : null;
  ...
  function drag() {
      if (dnode) {
          var n = dnode;
          var x = (self.mouseX - self.stageX) / self.ratio;
          var y = (self.mouseY - self.stageY) / self.ratio
          var g = self.graph.graph;
          n.x = (x - self.graph.width / 2) / g.scale + g.xMid;
          n.y = (y - self.graph.height / 2) / g.scale + g.yMid;
          self.dispatch('dragnode');
      }
      else {
  ...
  sigma.publicPrototype.draggingNode = function() {
      return !!dnode;
  };

  sigma.publicPrototype.zoom = function(zoomIn) {
      startInterpolate(
        self.graph.width / 2,
        self.graph.height / 2,
        self.ratio * (zoomIn ?
 ...
 * @this {Plotter}
 */
function Plotter(nodesCtx, edgesCtx, labelsCtx, edgeLabelsCtx, hoverCtx, graph, w, h) {
  sigma.classes.Cascade.call(this);
  ...
  function task_drawEdgeLabels() {
    var c = graph.edges.length;
    var s, t, i = 0;
    var ctx = edgeLabelsCtx;

    while ( (i++ < self.p.edgesSpeed) && (self.currentEdgeLabelIndex < c) ) {
      ...
      if (e['hidden'] ||
        ...
        var p1 = {};
        p1.x = e['source']['displayX'];
        p1.y = e['source']['displayY'];
        var p2 = {};
        p2.x = e['target']['displayX'];
        p2.y = e['target']['displayY'];      

        drawEdgeLabel(ctx,e['label'],p1,p2, "#a0a0a0");
        p1.x--;
        p1.y--;
        p2.x--;
        p2.y--;
        drawEdgeLabel(ctx,e['label'],p1,p2, "#101010");
  ...
  function drawNode(node) {
    ...
    if (self.p.borderSize > 0) {
        ctx.fillStyle = node['borderColor'] || self.p.borderColor;
        ctx.beginPath();
  ...
  function drawLabel(node) {
      ...
      if (node['displaySize'] >= self.p.labelThreshold || node['forceLabel']) {
          ...
          drawNodeLabel(ctx, node, fontSize, true);
      }
      return self;
  };

  function drawNodeLabel(ctx, node, fontSize, fill) {
      var tooWide = ctx.fillStyle == (node['color'] || self.p.defaultNodeColor);
      if (!tooWide) {
          var lines = getLines(ctx, node['label'], node['displaySize'] - fontSize / 3);
          for (var i = 0, w = 2 * node['displaySize']; i < lines.length; i++)
      ...
      if (tooWide) {
          // figure out if right has enough space
          var w = ctx.measureText(node['label']).width;
          var x = Math.round(node['displayX'] + node['displaySize'] * 1.5);
          var y = Math.round(node['displayY'] + 0.5 * fontSize + 3);
          var right = true;
          for (var i = 0, l = graph.nodes.length; i < l; i++) {
              var n = graph.nodes[i];
              if (n.id != node.id && n.displayX - n.displaySize < x + w && x < n.displayX + n.displaySize
                    && n.displayY - n.displaySize < y && y - fontSize < n.displayY + n.displaySize) {
                  right = false;
                  break;
              }
          }
          if (right && x + w < width)
              fill && ctx.fillText(node['label'], x, y);
          else {
              var left = true;
              x = Math.round(node['displayX'] - node['displaySize'] * 1.5);
              for (var i = 0, l = graph.nodes.length; i < l; i++) {
                  var n = graph.nodes[i];
                  if (n.id != node.id && n.displayX + n.displaySize > x - w && n.displayX - n.displaySize < x
                        && n.displayY - n.displaySize < y && y - fontSize < n.displayY + n.displaySize) {
                      left = false;
                      break;
                  }
              ...
              if (left)
                  fill && ctx.fillText(node['label'], x - w, y);
              else if (x + w < width) {
                  var edges = graph.edges.filter(function(e) { return e.target.id == node.id || e.source.id == node.id });
                  if (edges.length == 1) {
                      if (fill) {
                          var source = edges[0].source.id == node.id ? edges[0].target : edges[0].source;
                          var dx = node.displayX - source.displayX;
                          var dy = node.displayY - source.displayY;
                          var angle = Math.atan2(dy, dx);
                          // Keep text upright
                          var upsideDown = false;
                          if (angle < -Math.PI / 1.67 || angle > Math.PI / 1.67) {
                              angle -= Math.PI;
                              upsideDown = true;
                          }
                          ctx.save();
                          ...
                          ctx.rotate(angle);
                          ctx.fillText(node['label'], upsideDown ? -w - node['displaySize'] * 1.5 : node['displaySize'] * 1.5, fontSize / 3);
                          ctx.restore();
                      }
                  }
                  else
      ...
      function drawTexts(lines) {
          for (var i = 0, y = 1 - lines.length / 2 ; i < lines.length; i++) {
              var w = ctx.measureText(lines[i]).width;
              ctx.fillText(
                lines[i],
                Math.round(node['displayX'] - w / 2),
                Math.round(node['displayY'] + (y + i) * fontSize)
              );
          }
      }
  };
  ...
  function drawHoverNode(node) {
      ...
      //);
      drawNodeLabel(ctx, node, fontSize, false);
      return self;
  };
  ...
  function isOnScreen(node) {
	  if (isNaN(node['x']))
		  node['x'] = node['displayX'] = isNaN(node['old_x']) ? Math.random() : node['old_x'] ;
	  if (isNaN(node['y']))
		  node['y'] = node['displayY'] = isNaN(node['old_y']) ? Math.random() : node['old_y'] ;
  ...
  this.task_drawEdgeLabels = task_drawEdgeLabels;
...
function Sigma(root, id) {
  ...
  // Interaction listeners:
    ...
    targeted = self.graph.edges.filter(function(n) {
        return !!n['hover'];
      }).map(function(n) {
        return n.id;
      });
  ...
  function draw(nodes, edges, labels, safe) {
    ...
    if (e) {
        if (e > 1) {
            while (self.plotter.task_drawEdgeLabels()) {}
        } else {
            if (previous) {
              sigma.chronos.queueTask(
                self.plotter.task_drawEdgeLabels,
                'edgeLabel_' + self.id,
                previous
              );
            } else {
...
function SigmaPublic(sigmaInstance) {
  ...
  // Events
  s.bind('downnodes upnodes downedges upedges downgraph upgraph dragnode', function(e) {
...
sigma.chronos = new (function() {
   ...
   */
  function insertFrame(callback, delay) {
  ...
  function onTaskEnded(e) {
    if (generators[e['content'].taskName] != undefined) {
      ...
      if (getGeneratorsCount(true) == 0) {
        insertFrame(startGenerators, frameTime);
      }
    }
  };
