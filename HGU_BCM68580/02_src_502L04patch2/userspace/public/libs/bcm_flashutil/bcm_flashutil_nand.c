    ...
    for(offset=0;search_complete == 0 && offset < MAX_NVRAM_MIRROR_LOOKUP_OFFSET;offset += 2048) {
        ...
        else
        {
            ...
            if(strncmp((const char *)buff, NVRAM_DATA_SIGN, strlen(NVRAM_DATA_SIGN)) == 0) {
                //check nvram crc
                printf("NVRAM_MIRROR SCAN: NVRAM back up found at address %" PRIx64 " \n", offset);
                memcpy(nv, buff+strlen(NVRAM_DATA_SIGN), sizeof(NVRAM_DATA));
                crc=nv->ulCheckSum;
                nv->ulCheckSum = 0;  
                if (crc == getCrc32((unsigned char *)nv, sizeof(NVRAM_DATA),CRC32_INIT_VALUE)) {
                      search_complete=1;
                }
                nv->ulCheckSum=crc;
            }
...
*/
static void nvram_data_mirror_search(char *hunt_ptr, int size_to_search, PNVRAM_DATA pnd)
{
    char *saved_hunt_ptr=hunt_ptr;
    int nvram_mirr_info_idx=0, fd;
    NVRAM_MIRROR_INFO nvram_mirror_info;
    PNVRAM_DATA nv;
    uint32_t crc;
    int status=0;
    ...
    if(hunt_ptr != NULL)
    {
        memset(&nvram_mirror_info,'\0',sizeof(NVRAM_MIRROR_INFO));
        nvram_mirror_info.active_idx=0; // always make the primary as active
        nvram_mirror_info.offset[nvram_mirr_info_idx]=0x10580;
        nvram_mirror_info.validity[nvram_mirr_info_idx]=1;
        nvram_mirror_info.write_fail_count[nvram_mirr_info_idx]=0;
        nvram_mirr_info_idx++;

        fd=open(NVRAM_DATA_MIRROR_INFO, O_CREAT|O_WRONLY, 0644);

        if(fd)
        {
            while(size_to_search > 2048)
            {
                if(strncmp(hunt_ptr, NVRAM_DATA_SIGN, strlen(NVRAM_DATA_SIGN)) == 0)
                {
                    nv=(PNVRAM_DATA)(hunt_ptr+strlen(NVRAM_DATA_SIGN));
                    /* Recalculate the nvramData CRC. */
                    crc=nv->ulCheckSum;
                    nv->ulCheckSum = 0;
                    nv->ulCheckSum = crc_getCrc32((unsigned char *)nv, sizeof(NVRAM_DATA), CRC32_INIT_VALUE);
                ...
                if(status == -1)
                    printf("error writing %s file\n", NVRAM_DATA_MIRROR_INFO);
                sync();
                close(fd);
        }
    }
}
int handleCferom(mtd_info_t *mtd, char *image_ptr,unsigned int image_size, unsigned int wfiFlags, void *inMemNvramData)
{
    int nvramXferSize;
    ...
    if(get_flash_type() == FLASH_IFC_NAND || get_flash_type() == FLASH_IFC_SPINAND ) {
        nvram_data_mirror_search(image_ptr, image_size, pnd);
    }
    return 0;
}
