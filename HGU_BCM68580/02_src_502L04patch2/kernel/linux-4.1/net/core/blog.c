...
int blog_pptp_rcv( struct fkbuff *fkb_p, uint32_t h_proto, uint32_t *rcv_pktSeq) 
{
    ...
    if (ip_p != NULL) 
    {
        ...
        if ( ip_p->proto == BLOG_IPPROTO_GRE ) 
        {
            ...
            ofst += 2;  //key len and key ID must existed

            h_proto = *(uint16_t*) (grehdr_p + 1);
            ...
            payload = (char *)(grehdr_p + ofst);
 
            if ((payload[0] == PPP_ALLSTATIONS) && (payload[1] == PPP_UI))
            {
                blog_print( "\nPPTP with address & control\n" );
                goto PPTP_NORMAL;
            }

            pppInfo= ntohs(*(uint16_t*)(grehdr_p + ofst));
            h_proto = *(uint16_t*)(grehdr_p + ofst + 1);
            ...
            switch (h_proto)
            {
                ...
                default:
                    if(pppInfo == BLOG_PPTP_NOAC_PPPINFO)
                        {
                            blog_print( "\nRX PPTP pppInfo 0x%04x\n", pppInfo );
                            goto PPTP_NORMAL;
                        }
                blog_print( "\nABORT UNKNOWN RX PPTP pppInfo 0x%04x\n", pppInfo );
                return BLOG_PPTP_ENCRYPTED;                   /* compressed data, continue in stack */
           }
