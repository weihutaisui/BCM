...
static int otpcfg(ui_cmdline_t *cmd,int argc,char *argv[])
{
   ...
   else
   {
      ...
      else if ((! strcmp(pFirstArg, "set")) && (! strcmp(pSecondArg, "mode")) && (! strcmp(pThirdArg, "secure")))
      {
         ...
         {
             res = read_potp(OTP_CPU_CORE_CFG_ROW, &otpRowData);
             if ((res == 0) && (! (otpRowData & 0x68360)))
             {
                 printf(" ---------- Chip OTP is not burn\n");
                 return res;
             } /* check board type and debug 6836 */
         } /* check board type 6836 */
#endif
