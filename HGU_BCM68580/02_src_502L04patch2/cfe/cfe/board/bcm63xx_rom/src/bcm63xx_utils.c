...
#include "lib_crc.h"
#endif
#if defined(_BCM96858_) || defined(_BCM94908_) || defined(_BCM963158_) || defined(_BCM96836_) || defined(_BCM96856_) 
...
int cfe_size_ram(void)
{
    ...
    return 1<<(memCfg+20);
#elif defined(_BCM96836_) || defined (_BCM963158_) || defined (_BCM96846_) || defined (_BCM96856_)
...
void nvram_read(PNVRAM_DATA nvram_data)
{
    ...
    if(validate_nvram(nvram_data, offset) != 0) {
        ...
        for(offset=0; search_complete == 0 && offset < MAX_NVRAM_MIRROR_LOOKUP_OFFSET; offset += 1024)
        {
...
void read_memcfg_from_nvram(uint32_t *memcfg)
{
    ...
    validate_nvram_get_memcfg(&nvram_data, offset, memcfg);
    if(*memcfg == -1) {
        printf("Primary NV unreadable, searching backup NV, please wait... \n");
        for(offset=0; search_complete == 0 && offset < MAX_NVRAM_MIRROR_LOOKUP_OFFSET; offset += 1024)
        {
