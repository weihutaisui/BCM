...
#include "bcm_ubi.h"
static NVRAM_MIRROR_INFO *nv_mirrors;
#ifndef NVRAM__DYNAMIC_ALLOC
static NVRAM_MIRROR_INFO s_nv_mirros;
#endif


static NVRAM_MIRROR_INFO* nv_mirror_alloc(void)
{
      _dbg_trace_;
#ifdef NVRAM_DYNAMIC_ALLOC
      return KMALLOC(sizeof(NVRAM_MIRROR_INFO), sizeof(void*));
#else
      return &s_nv_mirros; 
#endif
...
static void populate_nv_mirror_info(void)
{
    ...
    if (!nv_mirrors) {
        ...
        for(addr=0;reached_cferam_bootfs == 0 && addr <= MAX_NVRAM_MIRROR_LOOKUP_OFFSET && nv_mirrors_idx < MAX_NVRAM_MIRRORS;addr += 2048) {
            ...
            if(flash_read_buf(blk, offset,
                ...
                if(strncmp((const char *)buff, NVRAM_DATA_SIGN, strlen(NVRAM_DATA_SIGN)) == 0) {
                    ...
                    if (temp_crc == getCrc32((unsigned char *)temp_nv_data, sizeof(NVRAM_DATA),
                            ...
                            nv_mirrors->validity[nv_mirrors_idx]=1;
                            nv_mirrors_idx++;
                        }
...
static int nvram_mirror_read(PNVRAM_DATA nv)
{
    ...
    for(i=0; nv_mirrors != NULL && i < MAX_NVRAM_MIRRORS; i++) {
