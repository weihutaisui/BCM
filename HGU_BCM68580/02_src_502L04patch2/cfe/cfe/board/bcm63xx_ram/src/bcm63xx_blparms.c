...
extern void reserve_astra_memory(void);
...
void blparms_init(void)
{
    ...
    xparms_init();
#if defined (CFG_DT)
    dtb_init((void*)CFG_DTB_ADDRESS, CFG_DTB_MAX_SIZE);
#endif
}
...
void dtb_install(void)
{
    ...
    dtb_set_memory(cfe_get_sdram_size());
    
    set_reserved_memory();

    /* all the secondary cpus has the same release address */

#if defined (_BCM96858_) || defined (_BCM94908_) || defined (_BCM96836_) || defined(_BCM963158_) || defined(_BCM96856_)
    ...
    NVRAM_COPY_TO(&nvramData);
        ...
        dtb_set_chosen_root(get_emmc_chosen_root());
        ...
        if(bootCfeVersion != NULL)
        {
                ...
                printf("Appending CFE version to dtb, ret:%d\n",
                       dtb_append_data_blob(BRCM_CFEVER_PROP,
                        (char*)bootCfeVersion, cfe_ver_size));
        }
#endif

 /* Pass NVRAM in dtb to kernel in binary blob format */ 
    if(dtb_getprop(dtb_get_fdt(), "/", BRCM_NVRAM_PROP, &len) == NULL)
    {
        printf("Appending NVRAM to dtb, ret:%d\n",
            dtb_append_data_blob(BRCM_NVRAM_PROP,
...
#if (INC_ASTRA==1)
    /* Upadte Linux device tree to reserve memory for Astra */
    reserve_astra_memory();
#endif
...
extern unsigned long fmw_arg0;
    ...
    dtb_prepare(DTB_PREPARE_FDT, (unsigned char*)0x2000000, 4096);
#else
