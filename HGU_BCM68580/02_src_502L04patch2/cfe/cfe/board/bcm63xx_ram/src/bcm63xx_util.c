...
#if defined(_BCM963138_) || defined(_BCM963148_) || defined(_BCM96858_) || defined(_BCM94908_) || defined(_BCM96836_)
...
#elif defined (_BCM94908_) || defined (_BCM96858_) || defined(_BCM96836_) || defined(_BCM963158_) || \
        defined(_BCM96846_) || defined(_BCM96856_)
...
/* Loads and copies image to the cache aligned ddr buffer */
int cfe_load_image(const char* fname, void* ddr_addr, unsigned long max_size)
{
     unsigned char *data = 0;
     unsigned int size = 0;

     if ((long)ddr_addr & (CFE_CACHE_FLUSH_RANGE -1)) {
          printf("%s: Destination memory not aligned\n", __func__, fname);
          return -1;
     }

     // load pmc rom image from bootfs
     if ( cfe_fs_fetch_file(fname, strlen(fname), &data, &size) == 0 && size <= max_size ) {
          // copy to the aligned buffer
          memcpy(ddr_addr, data, size);
          _cfe_flushcache(CFE_CACHE_FLUSH_RANGE, ddr_addr, ddr_addr + size);
          // No need to free the memory allocated by cfe_fs_fetch_file. The memory used in
          // cfe_fs_fetch_file was taken from general purpose FREE memory area - not from heap.
          return 0;
     }
     return -1;
}
