...
{
    tc_to_queue_drv_priv_t *tbl = (tc_to_queue_drv_priv_t *)bdmf_obj_data(mo);
    tc_to_queue_t *tmp_entry, *list_entry = NULL;
    tc_to_queue_t *entry = NULL;
    ...
    {
        for (inst = rdpa_if_lan0; inst < rdpa_if__number_of; inst++)
        {
            if (tc_to_queue_linked_port[inst] == tbl->index)
            {
                rc = rdpa_port_get(inst, &obj);
                if (rc < 0)
                {
                    BDMF_TRACE_ERR("failed to get port object with index of %d", (int)inst);
                    goto exit;
                }
                rc = rc ? rc : rdpa_tc_to_queue_set_single_entry_ex(mo, 1, obj, index, queue, 1);
                if (rc)
                {
                    BDMF_TRACE_ERR("tc_to_queue_set_single_entry");
                    goto exit;
                }
                bdmf_put(obj);
                obj = NULL;
    ...
    else
    {
        ...
        if (rc)
        {
            BDMF_TRACE_ERR("Can't reallocate %d sized tables for all tconts , failed on tcont %s",
                (int)tbl->size, bdmf_attr_get_enum_text_hlp(&rdpa_if_enum_table, inst));
            goto exit;
        }
        for (inst = 0; inst < RDPA_MAX_TCONT; inst++)
        {
            if (tc_to_queue_linked_tcont_llid[inst] == tbl->index)
            {
                ...
                if (rc < 0)
                {
                    ...
                    if (rc < 0)
                    {
                        BDMF_TRACE_ERR("failed to get tcont/llid object with index of %d", (int)inst);
                        goto exit;
                    }
                }
                ...
                if (rc)
                {
                    BDMF_TRACE_ERR("Can't link tc %d queue %d to tcont %s , rdd_error = %d\n",
                        (int)index, (int)(queue), bdmf_attr_get_enum_text_hlp(&rdpa_if_enum_table, inst), rc);
                    goto exit;
                }
    ...
    const char *link_attrs)
{
    ...
    if (tbl->dir == rdpa_dir_us)
    {
        ...
        if (other->drv == rdpa_port_drv() && !rdpa_is_gbe_mode() && !rdpa_is_car_mode())
        {
            BDMF_TRACE_RET_OBJ(BDMF_ERR_PARM, mo,
