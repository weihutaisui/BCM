...
{
    .type_name = "configuration_type", .help = "Configuration type - ACL/FLOW/QOS/IP_FLOW",
    .values = {
...
#else
    {rdpa_if_ssid0, offsetof(rdd_ic_context_t, ds_pci_vlan_cmd)},
#endif
...
static rdpa_if rdpa_lan_wlan_egress_ifs[] =
{
    rdpa_if_lan0,
    rdpa_if_lan1,
    rdpa_if_lan2,
    rdpa_if_lan3,
    rdpa_if_lan4, /* XXX: Comply to IPTV map */
#ifdef XRDP
    rdpa_if_wlan0,
    rdpa_if_wlan1,
    rdpa_if_wlan2
#else
    rdpa_if_ssid0 /* rdpa_if_ssid0 should represent all WLAN PCI ports */
#endif
};
...
static bdmf_error_t configure_rdd_flow(struct bdmf_object *mo, rdpa_ic_info_t  *info, bdmf_index index)
{
    ...
    if (priv->dir == rdpa_dir_us && (priv->cfg.field_mask & RDPA_IC_MASK_INGRESS_PORT))
    {
        if (rdpa_is_fttdp_mode())
#ifdef XRDP
            key.ingress_port = rdpa_if_to_rdd_vport(info->key.ingress_port, 0);		/* Vport */
#else
...
static int delete_rdd_flow(struct bdmf_object *mo, rdpa_ic_key_t *entry_key, int index, struct ingress_classifier *rule)
{
    ...
    if (priv->dir == rdpa_dir_us && (priv->cfg.field_mask & RDPA_IC_MASK_INGRESS_PORT))
    {
        if (rdpa_is_fttdp_mode())
#ifdef XRDP
            entry_key->ingress_port = rdpa_if_to_rdd_vport(entry_key->ingress_port, 0);		/* Vport */
#else
    ...
    uint32_t size)
{
    ...
    rc = classification_ctx_index_get(priv->dir, 0, &idx);
    if (rc < 0)
    ...
    if (priv->cfg.type == RDPA_IC_TYPE_GENERIC_FILTER)
        RDD_BYTE_1_BITS_WRITE_G(1, RDD_FORCE_IC_IP_FLOW_ADDRESS_ARR, 0);
#endif
...
#ifdef XRDP
    if ((priv->cfg.type == RDPA_IC_TYPE_GENERIC_FILTER) && !priv->num_flows)
        RDD_BYTE_1_BITS_WRITE_G(0, RDD_FORCE_IC_IP_FLOW_ADDRESS_ARR, 0);
#endif
    ...
    void *val, uint32_t size)
{
    ...
    if (priv->cfg.type == RDPA_IC_TYPE_ACL || priv->dir == rdpa_dir_us)
        return BDMF_ERR_NOENT;
...
static int ingress_class_port_action_get_next(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index *index)
{
    ...
    for (i = first_flow; i < RDD_DS_IC_CONTEXT_TABLE_SIZE; i++)
    {
        ...
        {
            for (j = first_port; j < ARRAY_LENGTH(rdpa_lan_wlan_egress_ifs); j++)
            {
                if (has_per_port_vlan_action(i, rdpa_lan_wlan_egress_ifs[j]))
                {
                    key->port = rdpa_lan_wlan_egress_ifs[j];
                    key->flow = i;
                    return 0;
                }
            }
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    if (cfg->type == RDPA_IC_TYPE_ACL)
        BDMF_TRACE_RET(BDMF_ERR_PARM, "ACL is not supported\n");
#endif
    ...
    if (((priv->dir == rdpa_dir_us) && (priv->cfg.field_mask & RDPA_IC_MASK_GEM_FLOW)) ||
        ((priv->dir == rdpa_dir_ds) && (priv->cfg.field_mask & RDPA_IC_MASK_INGRESS_PORT)))
        return BDMF_ERR_INVALID_OP;
...
struct bdmf_aggr_type classification_key_type = {
    ...
    .fields = (struct bdmf_attr[])
    {
        ...
        {.name = "generic_mask", .help = "Generic rule mask", .size = sizeof(uint32_t),
