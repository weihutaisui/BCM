...
static int xtmchannel_post_init(struct bdmf_object *mo)
{
    ...
    if (priv->egress_tm)
    {
        rc = _rdpa_egress_tm_channel_set(priv->egress_tm, mo, rdpa_wan_dsl_channel_base() + priv->index);
        rc = rc ? rc : _rdpa_egress_tm_orl_prty_set(priv->egress_tm, priv->orl_prty);
        if (priv->enable)
            rc = rc ? rc : _rdpa_egress_tm_enable_set(priv->egress_tm, 1);
        if (rc)
...
static void xtmchannel_destroy(struct bdmf_object *mo)
{
    ...
    /* Scheduler cleanup */
    if (priv->egress_tm)
        _rdpa_egress_tm_channel_set(priv->egress_tm, NULL, rdpa_wan_dsl_channel_base() + priv->index);
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    if (mo->state == bdmf_state_active)
    {
        if (egress_tm)
        {
            rc = _rdpa_egress_tm_channel_set(egress_tm, mo, rdpa_wan_dsl_channel_base() + xtmchannel->index);
        ...
        {
            rc = _rdpa_egress_tm_channel_set(xtmchannel->egress_tm, NULL, rdpa_wan_dsl_channel_base() + xtmchannel->index);
        }
        if (rc < 0)
            return rc;
    }
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    {
        rc = _rdpa_egress_tm_enable_set(xtmchannel->egress_tm, enable);
        if (rc < 0)
            return rc;
    }
    ...
    int ptm_bonding)
{
    ...
    if (cfg_xtmflow)
    {
        rc = rdpa_us_wan_flow_config(xtmflow, rdpa_wan_dsl_channel_base() + xtmchannel_index,
...
int rdpa_xtm_flow_id_to_channel_id(int xtm_flow, int *channel_id)
{
    ...
    BUG_ON((unsigned)xtm_flow >= RDPA_MAX_XTMFLOW);
