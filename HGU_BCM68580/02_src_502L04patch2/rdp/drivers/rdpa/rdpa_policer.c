...
static int policer_attr_stat_write(struct bdmf_object *mo, struct bdmf_attr *ad,
    bdmf_index index, const void *val, uint32_t size)
{
    policer_drv_priv_t *policer = (policer_drv_priv_t *)bdmf_obj_data(mo);

    return policer_attr_stat_write_ex(policer);
}
...
static struct bdmf_attr policer_attrs[] = {
    ...
    { .name = "stat", .help = "Statistics",
        ...
        .read = policer_attr_stat_read, .write = policer_attr_stat_write
    },
