...
{
    int16_t channel_id;                                   /* Channel index in RDD (wan channel / tcont / lan emac) */
...
#if 0
static inline int _rdpa_egress_tm_dir_channel_queue_to_rdd_tc_check(rdpa_traffic_dir dir, int channel, uint32_t queue_id,
    int *rc_id, int *queue, int *tc)
{
...
static inline int _rdpa_egress_tm_wan_flow_queue_to_rdd_tc_check(int wan_flow, uint32_t queue_id, int *channel,
    int *rc_id, int *queue, int *tc)
{
#if defined(DSL_63138) || defined(DSL_63148)
    if (wan_flow == GBE_WAN_FLOW_ID)
    ...
    else
    {
      ...
      if (rc)
         return rc;
      *channel += rdpa_wan_dsl_channel_base();
    }
#elif defined(WL4908)
    *channel = RDD_WAN_CHANNEL_0;
#elif defined(BCM63158)
    *channel = 0; /* TBD. BCM63158 need to implement */
#endif

    return _rdpa_egress_tm_rc_queue_id_tc_check_by_dir_channel_queue(rdpa_dir_us, *channel, queue_id, rc_id, queue, tc);
}

static inline int _rdpa_egress_tm_wan_flow_queue_to_rdd(int wan_flow, uint32_t queue_id, int *channel, int *rc_id,
    int *queue)
{
#if defined(BCM63158)
   if (rdpa_is_gbe_mode())
   ...
   else if (rdpa_is_gpon_or_xgpon_mode()) 
   {
      ...
      rc = rdpa_gem_flow_id_to_tcont_id(wan_flow, channel);
      if (rc)
         return rc;
   }
   else
#elif defined(__OREN__) || defined(XRDP)
   if (rdpa_is_gpon_or_xgpon_mode())
   {
    ...
    return __rdpa_egress_tm_dir_channel_queue_to_rdd(rdpa_dir_us, channel, queue_id, rc_id, queue, NULL);
}
...
{
    return _rdpa_egress_tm_rc_queue_id_tc_check_by_dir_channel_queue(rdpa_dir_ds, _rdpa_port_channel_no_lock(port), queue_id, rc_id, queue, tc);
}
...
{
    return _rdpa_egress_tm_rc_queue_id_tc_check_by_dir_channel_queue(dir, channel, queue_id, rc_id, queue, tc);
}
