...
typedef struct
{
    ...
    bdmf_boolean enable; /* enable/disable */
    ...
    /* 1 if queue id is auto-assigned */
    bdmf_boolean queue_id_auto_assigned[RDPA_MAX_EGRESS_QUEUES];
...
void rdpa_rdd_tx_queue_disable(struct bdmf_object *mo, tm_queue_hash_entry_t *qentry,
    pd_offload_ddr_queue_t *ddr_cfg);
...
/*
 * The following functions are not supported on 6858 [yes].
 * Mapped 1x1 with existing RDD implementation for now
 */

bdmf_error_t rdpa_rdd_ds_service_queue_overall_rate_limiter_enable(bdmf_boolean enable);
bdmf_error_t rdpa_rdd_ds_tm_service_queue_cfg(uint16_t queue_id, uint16_t pkt_threshold,
    rdd_rate_limiter_t rate_limiter, rdd_queue_profile_id_t profile_id);
bdmf_error_t rdpa_rdd_ds_service_queue_rate_limiter_cfg(rdd_rate_limiter_t rate_limiter,
    rdd_rate_limit_params_t *budget);
bdmf_error_t rdpa_rdd_service_queue_pm_counters_get(uint32_t service_queue, rdd_service_queue_pm_counters_t  *counters);
#ifdef XRDP
...
/* Extra definitions needed for succesful build */
