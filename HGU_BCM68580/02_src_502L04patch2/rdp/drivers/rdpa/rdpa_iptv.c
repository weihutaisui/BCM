...
uint32_t accumulate_iptv_filter_counter;
...
static int iptv_pre_init(struct bdmf_object *mo)
{
    ...
    accumulate_iptv_filter_counter = 0;
    return 0;
}
...
static void iptv_table_reset(void)
{
    ...
    while (rc != BDMF_ERR_NO_MORE)
    {
        rc = _iptv_attr_channel_get_next(&channel_index, &entry);
        if (rc)
            continue;

        rc = rdpa_iptv_channel_rdd_remove(channel_index, entry, 1, 0);
    }
...
static int iptv_attr_stat_write(struct bdmf_object *mo, struct bdmf_attr *ad,
    bdmf_index index, const void *val, uint32_t size)
{
    return rdpa_iptv_stat_write_ex(ad, index, val, size);
}
...
static struct bdmf_attr iptv_attrs[] =
{
    ...
    { .name = "iptv_stat", .help = "IPTV global statistics",
        ...
        .read = iptv_attr_stat_read, .write = iptv_attr_stat_write
    },
