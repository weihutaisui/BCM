...
static inline bdmf_boolean llid_has_dedicated_data_channel(llid_drv_priv_t *llid)
{
...
int llid_set_l1_channel(llid_drv_priv_t *llid)
{
    ...
    {
        llid->channels[0] = epon_l2_l1_alloc[llid->index].l1_channel_control;
        epon_l1_channels[llid->channels[0]] = llid->index;
        
        if (llid_has_dedicated_data_channel(llid))
        {
            llid->channels[1] = epon_l2_l1_alloc[llid->index].l1_channel_data;
    ...
    const int16_t *channels)
{
    ...
    case rdpa_epon_dpoe:
        /* fall through */
    case rdpa_epon_bcm:    
        rc = _rdpa_egress_tm_channel_set(tm_obj, owner, *channels);
        break;
    case rdpa_epon_ctc:
        /* fall through */
    case rdpa_epon_cuc:
        /* fall through */
    default:
        rc = _rdpa_egress_tm_channel_group_set(tm_obj, owner, group_id, 
...
static int llid_control_enable_set(struct bdmf_object *mo, bdmf_boolean enable)
{
    ...
    if (!llid_has_dedicated_data_channel(llid))
        ...
        return _rdpa_egress_tm_enable_set(llid->control_egress_tm, enable);
}
...
static int llid_data_enable_set(struct bdmf_object *mo, bdmf_boolean enable)
{
             ...
             rdpa_is_car_mode())
    {
        ...
        rc = _rdpa_egress_tm_enable_set(llid->data_egress_tm, enable);
...
static int llid_post_init(struct bdmf_object *mo)
{
    ...
    if (llid->control_egress_tm)
    {
        ...
        {
            rc = rc ? rc : _rdpa_egress_tm_enable_set(llid->control_egress_tm, 
                1);
        }
        if (rc)
        {
    ...
    if (llid->data_egress_tm && llid_has_dedicated_data_channel(llid))
    {
        ...
        if (llid->data_enable)
            rc = rc ? rc : _rdpa_egress_tm_enable_set(llid->data_egress_tm, 1);
        if (rc)
        {
    ...
    rc = rc ? rc : _cfg_ds_gem_flow_hw(1, llid->index, 
        0, rdpa_flow_dest_eth, rdpa_discard_prty_low, llid->ds_def_flow);
...
static void llid_destroy(struct bdmf_object *mo)
{
    ...
    _cfg_ds_gem_flow_hw(0, llid->index, 0, rdpa_flow_dest_none, 
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
        ...
        llid_has_dedicated_data_channel(llid))
    {
        ...
        if (egress_tm)
        {
            ...
            if (llid->data_enable)
            {
                rc = rc ? rc : _rdpa_egress_tm_enable_set(egress_tm, 1);
            ...
            if (rc)
            {
                ...
                llid_egress_tm_set(llid->data_egress_tm, mo, llid->index, 
                    llid->num_channels, &llid->channels[1]);
                _rdpa_egress_tm_enable_set(llid->data_egress_tm, 
                    llid->data_enable);
            }
        }
        if (rc < 0)
        ...
        if (egress_tm)
        {
            ...
            if (rc)
            {
                ...
                llid_egress_tm_set(llid->control_egress_tm, mo, llid->index, 1,
                    llid->channels);
                _rdpa_egress_tm_enable_set(llid->control_egress_tm,
                    llid->control_enable);
            }
        }
        if (rc < 0)
...
{
    return &epon_l2_l1_alloc[l1_queue];
}
