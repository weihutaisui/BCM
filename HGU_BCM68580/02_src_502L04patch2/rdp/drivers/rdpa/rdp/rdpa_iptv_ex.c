...
extern uint32_t accumulate_iptv_filter_counter;
...
int rdpa_iptv_stat_read_ex(struct bdmf_attr *ad, bdmf_index index, void *val, uint32_t size)
{
    ...
    accumulate_iptv_filter_counter += iptv_filter_counter;
    stat->rx_valid_pkt = port_counters.rx_valid;
    stat->discard_pkt = port_counters.bridge_filtered_packets + accumulate_iptv_filter_counter +
