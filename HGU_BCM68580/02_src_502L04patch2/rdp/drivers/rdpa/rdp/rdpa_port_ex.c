...
int rdpa_vlan_update_bridge_arl_lkp_method_ex(struct bdmf_object *vlan_obj, int arl_lkp_method)
{
    return BDMF_ERR_OK;
}
...
int port_stat_read(struct bdmf_object *mo, void *val, uint32_t size)
{
    int rc = 0;
    ...
    case rdpa_if_switch:
        accumulate_port_stat[port->index].rx_crc_error_pkt += bbh_rx_counters.crc_error;
        accumulate_port_stat[port->index].rx_discard_1 += bbh_rx_counters.no_bpm_bn_error;
        accumulate_port_stat[port->index].rx_discard_2 += bbh_rx_counters.no_sbpm_sbn_error;
        accumulate_port_stat[port->index].rx_discard_max_length += bbh_rx_counters.too_long_error;
        accumulate_port_stat[port->index].rx_discard_min_length += bbh_rx_counters.too_short_error;
        accumulate_port_stat[port->index].bbh_drop_1 += bbh_rx_counters.no_dma_cd_error;
        accumulate_port_stat[port->index].bbh_drop_2 += bbh_rx_counters.no_sdma_cd_error;
        accumulate_port_stat[port->index].bbh_drop_3 += bbh_rx_counters.runner_congestion;
        accumulate_port_stat[port->index].rx_valid_pkt += bbh_rx_counters.incoming_packets;
        accumulate_port_stat[port->index].tx_valid_pkt += bbh_tx_counters.tx_packets_from_ddr + bbh_tx_counters.tx_packets_from_sram;
#if !defined(DSL_63138) && !defined(DSL_63148) && !defined(WL4908)
        accumulate_port_stat[port->index].tx_discard = rdd_port_counters.bridge_tx_packets_discard;
        accumulate_port_stat[port->index].discard_pkt = rdd_port_counters.bridge_filtered_packets;
#else
        if (rdpa_if_is_lan(port->index))
        {
            accumulate_port_stat[port->index].rx_valid_pkt += lan_rx_packet;
            accumulate_port_stat[port->index].tx_valid_pkt += lan_tx_packet;
            accumulate_port_stat[port->index].tx_discard   += lan_tx_discard;
        }
        else
        {
            /* switch */
            accumulate_port_stat[port->index].tx_discard += bbh_tx_counters.dropped_pd;
        }
#endif
        break;
    ...
    case rdpa_if_wan0:
        ...
        accumulate_port_stat[port->index].rx_discard_2 += bbh_rx_counters.no_sbpm_sbn_error;
#if !defined(DSL_63138) && !defined(DSL_63148) && !defined(WL4908)
        accumulate_port_stat[port->index].rx_crc_error_pkt = (uint32_t)crc_counter;
        accumulate_port_stat[port->index].rx_valid_pkt = rdd_port_counters.rx_valid;
        accumulate_port_stat[port->index].tx_valid_pkt = rdd_port_counters.tx_valid;
        accumulate_port_stat[port->index].discard_pkt = rdd_port_counters.bridge_filtered_packets;
        accumulate_port_stat[port->index].tx_discard = rdd_port_counters.bridge_tx_packets_discard;
#else
        accumulate_port_stat[port->index].rx_crc_error_pkt += bbh_rx_counters.crc_error;
        ...
        accumulate_port_stat[port->index].rx_discard_1 += subnet_stat.rx_dropped_packet;
        accumulate_port_stat[port->index].tx_discard += subnet_stat.tx_dropped_packet;
        }
#endif
        break;
    ...
    case rdpa_if_wan1:
        ...
        accumulate_port_stat[port->index].rx_crc_error_pkt += bbh_rx_counters.crc_error;
        accumulate_port_stat[port->index].rx_valid_pkt += bbh_rx_counters.incoming_packets;
        accumulate_port_stat[port->index].tx_valid_pkt += bbh_tx_counters.tx_packets_from_ddr;
    ...
    case rdpa_if_wlan1:
        {
            ...
            {
                accumulate_port_stat[port->index].rx_valid_pkt = cpu_stat.rx.passed.packets - port->host_stat.rx.passed.packets;
                accumulate_port_stat[port->index].rx_discard_1 = cpu_stat.rx.discarded.packets - port->host_stat.rx.discarded.packets;
                accumulate_port_stat[port->index].tx_valid_pkt = cpu_stat.tx.passed.packets - port->host_stat.tx.passed.packets;
                accumulate_port_stat[port->index].tx_discard = cpu_stat.tx.discarded.packets - port->host_stat.tx.discarded.packets;
...
{
#if !defined(DSL_63138) && !defined(DSL_63148) && !defined(WL4908)/* TODO */
