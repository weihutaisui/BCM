    ...
    const rdpa_fdb_data_t *data, rdd_mac_params_t *mac_params)
{
    ...
    /* XXX: This should move to RDP. For XRSP, need to get find rdpa_if by emac, and from rdpa_if get dedicated vport */
...
int bridge_set_fw_elig_ex(struct bdmf_object *mo, rdpa_if port, rdpa_ports new_mask)
{
    ...
    const rdpa_fdb_data_t *data)
{
    ...
    /* STATIC MAC -> rdd_mac_entry_add() should be called */
    ...
    if (!is_mcast)
    {
        ...
        if (mac_params.bridge_port != new_mac_params.bridge_port)
        {
            if (!is_new)
            {
                rdpa_port_inc_dec_num_sa(
                    rdpa_rdd_bridge_port_to_if(mac_params.bridge_port,
                    RDPA_WIFI_SSID_INVALID), 0);
            ...
            /* Check that max_sa on the new port is not exceeded */
            rc = rdpa_port_inc_dec_num_sa(
                rdpa_rdd_bridge_port_to_if(new_mac_params.bridge_port,
                RDPA_WIFI_SSID_INVALID), 1);
            if (rc)
            {
                /* Failed to increment SA. Most likely max_sa is exceeded */
                goto exit;
            }
        }
    }
    ...
    if (rdd_rc)
    {
        ...
        if (!is_mcast && (mac_params.bridge_port != new_mac_params.bridge_port))
        {
            rdpa_port_inc_dec_num_sa(
                rdpa_rdd_bridge_port_to_if(new_mac_params.bridge_port,
                RDPA_WIFI_SSID_INVALID), 0);
...
void default_bridge_cfg_ex(struct bdmf_object *mo)
{
}
