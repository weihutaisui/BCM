...
{
    uint32_t length;
    int rdd_rc, channel = 0, rc_id, priority, tc_id;
    ...
    */
    ...
    /* tc check will modify the 6bit tc value to 1 bit is_high_prio */
    tc_id = extra_info.tc_id;
    rdd_rc = _rdpa_egress_tm_wan_flow_queue_to_rdd_tc_check(wan_flow, egress_queue, &channel, &rc_id, &priority, &tc_id);
    if (rdd_rc)
    {
...
{
    uint32_t length;
    int rdd_rc, rc_id, priority, tc_id;
    ...
    rdd_rc = rdd_cpu_tx_write_eth_packet_from_abs_address(sysb, length,
                                                           rdd_emac, egress_queue, extra_info);
    /* NOTE : Pay attention to the error handling and skb_free() code below */
...
#endif /*defined(DSL_63138) || defined(DSL_63148) || defined(WL4908)*/
