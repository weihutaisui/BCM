...
static int _ip_class_read_rdd_ip_flow(uint32_t index, rdpa_ip_flow_info_t *info)
{
    ...
    if ((rdd_ip_flow_ctx.conn_dir == rdpa_dir_us) && (rdd_ip_flow_ctx.actions_vector & rdpa_fc_action_l2gre_tunnel))
    {
#if defined(OREN)
        info->result.gre_tunnel_idx = rdd_ip_flow_ctx.gre_tunnel_index;
#else
        info->result.gre_tunnel_idx = 0;
#endif
    }
...
#ifndef LEGACY_RDP
    rc = rdd_ip_flow_delete(ip_flow_module, index);
#else
...
void remove_all_flows(struct bdmf_object *mo)
{
    ...
    {
        rc = rdd_context_entry_get(i, NULL);
        if (rc)
            continue;

        rc = ip_class_attr_flow_delete(mo, NULL, i);
        if (rc)
            bdmf_trace("IP flow deletion failed, error=%d\n", rc);
    }
}
    ...
    bdmf_index index, void *val, uint32_t size)
{
    ...
    /* read the ip flow data from the RDD */
    return _ip_class_read_rdd_ip_flow(index, info);
}
...
#ifndef LEGACY_RDP
    rc = rdd_context_entry_modify(index, &rdd_ip_flow_ctx);
#else
    rc = rdd_context_entry_modify(&rdd_ip_flow_ctx, index);
#endif
    if (rc)
    ...
    uint32_t size)
{
    ...
    tunnel_enable = (rdd_ip_flow.context_entry.actions_vector & (rdpa_fc_action_dslite_tunnel | rdpa_fc_action_l2gre_tunnel)) ? 1 : 0;

    if (info->key.dir == rdpa_dir_ds && tunnel_enable && ds_lite_flows_num == 0 && l2gre_flows_num == 0)
#if !defined(LEGACY_RDP) || defined(__OREN__)
        rdd_tunnels_parsing_enable(&ds_tunnels_parsing, 1);
#else
        rdd_ds_lite_enable(1);
#endif
...
#if !defined(LEGACY_RDP) || defined(__OREN__)
        if (tunnel_enable && (ds_lite_flows_num == 0) && (l2gre_flows_num == 0))
            rdd_tunnels_parsing_enable(&ds_tunnels_parsing, 0);
#else
        if (tunnel_enable && (ds_lite_flows_num == 0))
            rdd_ds_lite_enable(0);
#endif
    ...
    if (rdd_ip_flow.context_entry.actions_vector & rdpa_fc_action_l2gre_tunnel)
    {
        ++l2gre_flows_num;

        if (info->key.dir == rdpa_dir_us)
            ip_class->gre_tunnels[info->result.gre_tunnel_idx].ref_cnt++;
    }
    ...
    void *val, uint32_t size)
{
    ...
    return rdd_ip_flow_find(ip_flow_module, &info->key, (uint32_t *)index);
#else
    ...
    rc = rdd_connection_entry_search(&rdd_ip_flow, info->key.dir, (uint32_t *)index);
    if (rc)
        return BDMF_ERR_NOENT;
...
int ip_flow_can_change_on_fly_params_ex(const rdpa_ip_flow_info_t *const info, rdd_fc_context_t *rdd_ip_flow_ctx)
{
    ...
    if (rdd_ip_flow_ctx->egress_port != rdpa_if_to_rdd_bridge_port(info->result.port, &wifi_ssid))
        return BDMF_ERR_PARM;

    if (rdd_ip_flow_ctx->wifi_ssid != wifi_ssid)
        return BDMF_ERR_PARM;
    if (rdd_ip_flow_ctx->l2_hdr_offset != info->result.l2_header_offset + RDD_PACKET_HEADROOM_OFFSET)
        return BDMF_ERR_PARM;

    if ((info->key.dir == rdpa_dir_us) && (info->result.action_vec & rdpa_fc_action_dslite_tunnel))
    {
        int v6_subnet_idx;
...
void ip_class_prepare_new_rdd_ip_flow_params_ex(rdpa_ip_flow_info_t *const info, rdd_fc_context_t *rdd_ip_flow_ctx)
{
    ...
    rdd_ip_flow_ctx->egress_port =  rdpa_if_to_rdd_bridge_port(info->result.port, &wifi_ssid);
    rdd_ip_flow_ctx->wifi_ssid = wifi_ssid;

    if (!rdpa_if_is_wifi(info->result.port))
        rdd_ip_flow_ctx->wan_flow_index = info->result.wan_flow;
...
void ip_class_rdd_ip_flow_cpu_vport_cfg_ex(rdpa_ip_flow_info_t *const info, rdd_fc_context_t *rdd_ip_flow_ctx)
{
    ...
    if (info->result.wfd.nic_ucast.is_wfd)
    {
        ...
        rdd_ip_flow_ctx->wfd.nic_ucast.wfd_idx = info->result.queue_id;
    }
    else
    {
