    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    rdd_dhd_helper_flow_ring_flush(dhd_helper->radio_idx, flow_ring_id);
    return 0;
}
...
{    
    ...
    return rdd_dhd_rx_post_init(dhd_helper->radio_idx, &dhd_helper->init_cfg) ? BDMF_ERR_NOMEM : 0;
}
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    return rdd_dhd_rx_post_uninit(dhd_helper->radio_idx, &dhd_helper->init_cfg) ? BDMF_ERR_NOMEM : 0;
}
