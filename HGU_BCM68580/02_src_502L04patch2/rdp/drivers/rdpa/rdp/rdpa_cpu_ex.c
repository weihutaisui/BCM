...
#endif
            cpu_tx_args.direction.us.wan_channel =
                _rdd_wan_channel(cpu_tx_args.direction.us.wan_channel);
...
#else
    cpu_tx_args.direction.us.is_spdsrvc = info->is_spdsvc_setup_packet;
#endif
#endif
...
/* "l4_dst_port_to_reason" attribute add callback */
int cpu_attr_l4_dst_port_to_reason_add_ex(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index *index,
    const void *val, uint32_t size)
{
#if defined(DSL_63138) || defined(DSL_63148) || defined(WL4908)
    cpu_drv_priv_t *cpu_data = (cpu_drv_priv_t *)bdmf_obj_data(mo);
    rdpa_l4_dst_port_to_reason_cfg_t *info = (rdpa_l4_dst_port_to_reason_cfg_t *)val;
    int rc;

    if (cpu_data->index != rdpa_cpu_host)
        return BDMF_ERR_NOT_SUPPORTED;

    rc = rdd_l4_dst_port_add(info->is_static, info->is_tcp, info->l4_dst_port, info->reason, (uint32_t *)index);
...
/* "l4_dst_port_to_reason" attribute read callback */
    ...
    uint32_t size)
{
    ...
    rdd_l4_dst_port_read((uint32_t)index, &info->is_static, &info->is_tcp, &info->l4_dst_port, &info->reason, &info->refcnt);
...
/* "l4_dst_port_to_reason" attribute delete callback */
int cpu_attr_l4_dst_port_to_reason_delete_ex(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index index)
{
    ...
    rdd_l4_dst_port_delete((uint32_t)index);
...
/* "l4_dst_port_to_reason" attribute find callback */
    ...
    void *val, uint32_t size)
{
    ...
    return rdd_l4_dst_port_find(info->is_tcp, info->l4_dst_port, (uint32_t *)index);
#else
    return BDMF_ERR_NOT_SUPPORTED;
#endif
}
...
int rdpa_cpu_packet_get(rdpa_cpu_port port, bdmf_index queue, rdpa_cpu_rx_info_t *info)
{
    ...
    {
#if !(defined(DSL_63138) || defined(DSL_63148) || defined(WL4908) || defined(XRDP))
