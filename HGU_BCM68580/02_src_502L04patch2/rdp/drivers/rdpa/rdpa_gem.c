...
static int gem_attr_stat_write(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index index, const void *val,
    uint32_t size)
{
    return gem_attr_stat_write_ex(mo, ad, index, val, size);
}
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
   ...
   if (cfg == NULL)
    ...
    {
        if (priv->index > 31 && (cfg->destination != rdpa_flow_dest_eth ||
...
static struct bdmf_attr gem_attrs[] = {
    ...
    { .name = "stat", .help = "GEM statistics",
        ...
        .read = gem_attr_stat_read, .write = gem_attr_stat_write
    },
    BDMF_ATTR_LAST
};
