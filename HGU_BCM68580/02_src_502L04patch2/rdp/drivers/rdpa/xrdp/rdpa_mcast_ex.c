...
#if (RDPA_MCAST_MAX_FLOWS != RDD_IPTV_DDR_CONTEXT_TABLE_SIZE)
#error "RDPA_MCAST_MAX_FLOWS != RDD_IPTV_DDR_CONTEXT_TABLE_SIZE"
#endif
...
static uint32_t flow_index_to_channel_index_g[RDPA_MCAST_MAX_FLOWS];

static int __channel_index_get(bdmf_index flow_index, uint32_t *channel_index)
{
    if (flow_index >= RDPA_MCAST_MAX_FLOWS ||
        flow_index_to_channel_index_g[flow_index] == BDMF_INDEX_UNASSIGNED)
    {
        return BDMF_ERR_NOENT;
    }

    *channel_index = flow_index_to_channel_index_g[flow_index];

    return BDMF_ERR_OK;
}

static int __flow_index_get(uint32_t channel_index, bdmf_index *flow_index)
{
    bdmf_index i;

    for (i = 0; i < RDPA_MCAST_MAX_FLOWS; ++i)
    {
        if (flow_index_to_channel_index_g[i] == channel_index)
        {
            *flow_index = i;
...
static int __flow_index_add(uint32_t channel_index, bdmf_index *flow_index)
{
    bdmf_index i;

    if (__flow_index_get(channel_index, flow_index) == BDMF_ERR_OK)
    {
        BDMF_TRACE_ERR("Duplicate Multicast Flow: channel_index 0x%08X, flow_index %lu\n",
                       channel_index, *flow_index);
    ...
    for (i = 0; i < RDPA_MCAST_MAX_FLOWS; ++i)
    {
        if (flow_index_to_channel_index_g[i] == BDMF_INDEX_UNASSIGNED)
        {
            flow_index_to_channel_index_g[i] = channel_index;
...
static int __flow_index_delete(bdmf_index flow_index, uint32_t *channel_index)
{
    if (flow_index >= RDPA_MCAST_MAX_FLOWS ||
        flow_index_to_channel_index_g[flow_index] == BDMF_INDEX_UNASSIGNED)
    {
        BDMF_TRACE_ERR("Invalid Multicast Flow Index: %lu\n", flow_index);
...
static void __flow_index_init(void)
{
    int i;

    for (i = 0; i < RDPA_MCAST_MAX_FLOWS; ++i)
    {
        flow_index_to_channel_index_g[i] = BDMF_INDEX_UNASSIGNED;
    }
}
                                               ...
                                               rdd_mcast_flow_t *rdd_mcast_flow)
{
    ...
    rdd_mcast_flow->context.number_of_ports = rdd_iptv_entry->replications + 1;
    rdd_mcast_flow->context.port_mask = IPTV_TO_MCAST_VPORT_MASK(rdd_iptv_entry->egress_port_vector);
    rdd_mcast_flow->context.wlan_mcast_clients = 0;
...
int rdpa_mcast_rdd_context_get_ex(bdmf_index index, rdd_mcast_flow_t *rdd_mcast_flow)
{
    ...
    rc = __channel_index_get(index, &channel_index);
    if (rc)
    {
        return rc;
    }
...
int rdpa_mcast_rdd_flow_add_ex(bdmf_index *index, rdd_mcast_flow_t *rdd_mcast_flow)
{
    ...
    rc = __flow_index_add(channel_index, index);
...
{
    uint32_t channel_index = BDMF_INDEX_UNASSIGNED;
    int rc;

    rc = __flow_index_delete(index, &channel_index);
    if (rc)
    {
        return rc;
    }
...
int rdpa_mcast_rdd_flow_find_ex(bdmf_index *index, rdd_mcast_flow_t *rdd_mcast_flow)
{
    ...
    return __flow_index_get(channel_index, index);
}
                                             ...
                                             rdd_mcast_port_context_t *rdd_port_context)
{
    ...
    rdd_port_context_u32 = entry->context;
                                             ...
                                             rdd_mcast_port_context_t *rdd_port_context)
{
    ...
    rc = rdpa_mcast_rdd_vport_to_rdpa_if_ex(rdd_vport, &rdpa_port);
    if (rc)
    {
        BDMF_TRACE_RET(rc, "Could not rdpa_mcast_rdd_vport_to_rdpa_if_ex: %d\n", rdd_vport);
    }

    rc = _rdpa_egress_tm_lan_port_queue_to_rdd(rdpa_port, rdd_port_context->queue, &rc_id, &queue);
    if (rc)
    {
        BDMF_TRACE_RET(rc, "DS egress queue %u is not configured\n", rdd_port_context->queue);
    }

    rdd_port_context->queue = queue;

    rdd_port_context_u32 = *((uint32_t *)(rdd_port_context));
    entry->context = swap4bytes(rdd_port_context_u32);
