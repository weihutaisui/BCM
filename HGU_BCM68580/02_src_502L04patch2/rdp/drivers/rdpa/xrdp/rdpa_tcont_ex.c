...
int tcont_tx_bbh_flush_status_get(uint8_t tcont_id, bdmf_boolean *bbh_flush_done_p)
{
    ...
    /* 2. Set PD Empty Select re-direct adress - read only the first 32 tconts */
    ...
    /* 3. Read PD Empty Select */

    rc = ag_drv_bbh_tx_debug_counters_swrddata_get(BBH_TX_WAN_ID, &swrddata);
    if (rc)
    {
       BDMF_TRACE_RET(BDMF_ERR_INTERNAL, "Failed to read flush status due to bbh driver error %d\n", rc);
    }

    *bbh_flush_done_p = swrddata & (1 << tcont_id) ? 1 : 0;
