 ...
 * HASH table */
int profile_map_update_port_and_vid0(rdpa_if port_idx, uint8_t profile)
{
    ...
    rc = port_update_hash_port_and_vid0(port, rdpa_vlan_ingress_filter_profile_modify_cb, &profile);
...
static int profile_map_entry_add(uint8_t profile, struct bdmf_object *owner_obj)
{
    ...
    if (owner_obj->drv == rdpa_port_drv())
    {
        ...
        rdd_ingress_filter_vport_to_profile_set(vport, profile);
#ifdef CONFIG_RNR_BRIDGE
        rc = profile_map_update_port_and_vid0(port->index, profile);
        if (rc)
        {
...
static int profile_map_entry_del(uint8_t profile,  struct bdmf_object *owner_obj)
{
    ...
    if (entry->owner_obj->drv == rdpa_port_drv())
    {
        ...
        if (!found)
        {
            BDMF_TRACE_ERR("Attempt to remove unexisting profile mapping, profile %d, port %s\n", profile,
                bdmf_attr_get_enum_text_hlp(&rdpa_if_enum_table, port->index));
            return BDMF_ERR_NOENT;
        }

        rdd_ingress_filter_vport_to_profile_set(vport, INVALID_PROFILE_IDX);
#ifdef CONFIG_RNR_BRIDGE
        rc = profile_map_update_port_and_vid0(port->index, INVALID_PROFILE_IDX);
        if (rc)
        {
    ...
    else if (entry->owner_obj->drv == rdpa_vlan_drv())
    {
        ...
        if (!found)
        {
            BDMF_TRACE_ERR("Attempt to remove unexisting profile mapping, profile %d, VLAN %s\n", profile, vlan->name);
            return BDMF_ERR_NOENT;
        }
