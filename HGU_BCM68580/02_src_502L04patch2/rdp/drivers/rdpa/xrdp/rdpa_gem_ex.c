...
/* GEM counters shadow */
static uint32_t rx_gem_counters[RX_FLOW_COUNTERS_NUM][3] = {};
static uint32_t tx_gem_counters[TX_FLOW_COUNTERS_NUM][2] = {};
...
void remove_def_flow(gem_drv_priv_t *gem)
{
    ...
    rdpa_cntr_id_dealloc(TCAM_IPTV_DEF_CNTR_GROUP_ID, DEF_FLOW_CNTR_SUB_GROUP_ID, gem->index);
    ...
    bdmf_index ds_def_flow)
{
    ...
    {
        rdpa_cntr_id_dealloc(RX_FLOW_CNTR_GROUP_ID, NONE_CNTR_SUB_GROUP_ID, gem_flow);
        rdd_rx_flow_del(gem_flow);
        return rc;
    }

    rdpa_cntr_id_alloc(RX_FLOW_CNTR_GROUP_ID, &cntr_id);
    rdd_rx_flow_cfg(gem_flow, destination, RDD_WAN0_VPORT, cntr_id);
#endif
    return rc;
}
    ...
    bdmf_boolean calc_crc, bdmf_boolean encrpt)
{
    ...
    if (cfg_gem)
    {
        ...
        rdd_tm_flow_cntr_cfg(gem_flow, cntr_id);
        rdd_tx_flow_enable(gem_flow, rdpa_dir_us, 1);
    }
    else
    {
        rdpa_cntr_id_dealloc(TX_FLOW_CNTR_GROUP_ID, NONE_CNTR_SUB_GROUP_ID, gem_flow);
        rdd_tm_flow_cntr_cfg(gem_flow, TX_FLOW_CNTR_GROUP_INVLID_CNTR);
        rdd_tx_flow_enable(gem_flow, rdpa_dir_us, 0);
    }
    return 0;
}
...
int gem_attr_stat_read_ex(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index index, void *val, uint32_t size)
{
    ...
    rx_gem_counters[cntr_id][0] += cntr_arr[0];
    rx_gem_counters[cntr_id][1] += cntr_arr[1];
    rx_gem_counters[cntr_id][2] += cntr_arr[2];
    stat->rx_packets = rx_gem_counters[cntr_id][0];
    stat->rx_bytes = rx_gem_counters[cntr_id][1];
    stat->rx_packets_discard = rx_gem_counters[cntr_id][2];

    /* TX counters */
    cntr_id = rdd_tm_flow_cntr_id_get(gem->index);
    rc = drv_cntr_counter_read(TX_FLOW_CNTR_GROUP_ID, cntr_id, cntr_arr);
    if (rc)
        BDMF_TRACE_RET(BDMF_ERR_INTERNAL, "Could not read CNTR flow counters for GEM flow %ld. err: %d\n", gem->index, rc);

    /* Keep counters values for accumulate_gem_stat */
    tx_gem_counters[cntr_id][0] += cntr_arr[0];
    tx_gem_counters[cntr_id][1] += cntr_arr[1];
    stat->tx_packets = tx_gem_counters[cntr_id][0];
    stat->tx_bytes = tx_gem_counters[cntr_id][1];
    ...
    uint32_t size)
{
    ...
    cntr_id = rdd_rx_flow_cntr_id_get(gem->index);
    rx_gem_counters[cntr_id][0] = 0;
    rx_gem_counters[cntr_id][1] = 0;
    rx_gem_counters[cntr_id][2] = 0;

    cntr_id = rdd_tm_flow_cntr_id_get(gem->index);
    tx_gem_counters[cntr_id][0] = 0;
    tx_gem_counters[cntr_id][1] = 0;
