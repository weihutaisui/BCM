...
void rdpa_cntr_id_alloc(uint32_t group_id, uint32_t *cntr_id)
{
    ...
    /* Each time before allocate counter - his value is  cleared */
    rc = drv_cntr_counter_clr(group_id, *cntr_id);
    if (rc)
        BDMF_TRACE_ERR("Cannot clear counter %d for Group: %d ; error %d\n", *cntr_id, group_id, rc);
}
...
void rdpa_cntr_id_dealloc(cntr_group_id_t group_id, cntr_sub_group_id_t sub_group, uint32_t entry_idx)
{
    ...
    case TX_FLOW_CNTR_GROUP_ID:
        ...
        break;
    case TCAM_IPTV_DEF_CNTR_GROUP_ID:
        ...
        */
            ...
            cntr_id = TCAM_IPTV_DEF_CNTR_GROUP_INVLID_CNTR;

        if (cntr_id == TCAM_IPTV_DEF_CNTR_GROUP_INVLID_CNTR)
        {
