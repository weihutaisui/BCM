...
static int tcont_post_init(struct bdmf_object *mo)
{
    ...
    if (priv->egress_tm)
    {
        rc = _rdpa_egress_tm_channel_set(priv->egress_tm, mo, priv->index);
...
static void tcont_destroy(struct bdmf_object *mo)
{
    ...
    /* Scheduler cleanup */
    if (priv->egress_tm)
        _rdpa_egress_tm_channel_set(priv->egress_tm, NULL, priv->index);
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    if (mo->state != bdmf_state_init && egress_tm != tcont->egress_tm)
    {
        if (egress_tm)
        {
            rc = _rdpa_egress_tm_channel_set(egress_tm, mo, tcont->index);
            rc = rc ? rc : _rdpa_egress_tm_orl_prty_set(egress_tm, tcont->orl_prty);
            if (tcont->enable)
                rc = rc ? rc : _rdpa_egress_tm_enable_set(egress_tm, 1);
            if (rc)
                _rdpa_egress_tm_channel_set(egress_tm, NULL, tcont->index);
        }
        else
        {
            rc = _rdpa_egress_tm_channel_set(tcont->egress_tm, NULL, tcont->index);
        }
        if (rc < 0)
            return rc;
    }
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    {
        rc = _rdpa_egress_tm_enable_set(tcont->egress_tm, enable);
        if (rc < 0)
            return rc;
    }
