...
const bdmf_attr_enum_table_t rdpa_cpu_reason_enum_table = {
    ...
    .values = {
        ...
        {"cpu_mirror", rdpa_cpu_rx_reason_cpu_mirroring},
        {"mac_spoofing", rdpa_cpu_rx_reason_mac_spoofing},
        {"queue0", rdpa_cpu_rx_reason_direct_queue_0},
        {"queue1", rdpa_cpu_rx_reason_direct_queue_1},
        {"queue2", rdpa_cpu_rx_reason_direct_queue_2},
        {"queue3", rdpa_cpu_rx_reason_direct_queue_3},
        {"queue4", rdpa_cpu_rx_reason_direct_queue_4},
        {"queue5", rdpa_cpu_rx_reason_direct_queue_5},
        {"queue6", rdpa_cpu_rx_reason_direct_queue_6},
        {"queue7", rdpa_cpu_rx_reason_direct_queue_7},
        ...
        {"etype_cfm", rdpa_cpu_rx_reason_etype_801_1ag_cfm},
...
/*  l4_dst_port_to_reason_cfg aggregate type */
struct bdmf_aggr_type l4_dst_port_to_reason_cfg_type = {
    .name = "l4_dst_port_to_reason_cfg", .struct_name = "rdpa_l4_dst_port_to_reason_cfg_t",
    .help = "l4_dst_port_to_reason mapping",
    .fields = (struct bdmf_attr[]) {
        { .name = "is_tcp", .help = "TCP or UDP",
          .type = bdmf_attr_boolean, .size = sizeof(bdmf_boolean),
            .offset = offsetof(rdpa_l4_dst_port_to_reason_cfg_t, is_tcp),
        },
        { .name = "l4_dst_port", .help = "dst port",
            .type = bdmf_attr_number,
            .size = sizeof(uint16_t),
            .offset = offsetof(rdpa_l4_dst_port_to_reason_cfg_t, l4_dst_port),
        },
        { .name = "reason", .help = "Trap reason", .type = bdmf_attr_enum,
          .size = sizeof(rdpa_cpu_reason),
          .offset = offsetof(rdpa_l4_dst_port_to_reason_cfg_t, reason),
          .ts.enum_table = &rdpa_cpu_reason_enum_table
        },
        { .name = "is_static", .help = "static or dynamic",
          .type = bdmf_attr_boolean, .size = sizeof(bdmf_boolean),
            .offset = offsetof(rdpa_l4_dst_port_to_reason_cfg_t, is_static),
        },
        { .name = "refcnt", .help = "reference count", .type = bdmf_attr_number,
            .size = sizeof(uint8_t),
            .offset = offsetof(rdpa_l4_dst_port_to_reason_cfg_t, refcnt),
        },
        BDMF_ATTR_LAST
    }
};
DECLARE_BDMF_AGGREGATE_TYPE(l4_dst_port_to_reason_cfg_type);
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
    ...
    /* Configure RDD */
    {
            ...
            if (rxc.size)
            {
                if (!rxc.ring_head)
                {
                    rc = rc ? rc : rdp_cpu_ring_create_ring_ex(rdd_ring_idx, rxc.type, rxc.size,
                        (bdmf_phys_addr_t *)&rxc.ring_head, &rw_idx_addr, BCM_PKTBUF_SIZE, NULL);
        ...
        rdd_ring_init(rdd_ring_idx, rxc.type, (bdmf_phys_addr_t)(uintptr_t)rxc.ring_head, rxc.size,
            size_of_entry, rdd_ring_idx, write_idx
#ifdef XRDP
             , rw_idx_addr
#endif
             );
    ...
    struct bdmf_attr *ad, bdmf_index index, void *val, uint32_t size)
{
...

    {
        ...
        rdd_error = rdd_cpu_rx_meter_drop_counter_get(dindex->index,
            dindex->dir, &drop_count);
        if (rdd_error)
        {
            BDMF_TRACE_RET_OBJ(BDMF_ERR_PARM, mo,
                "rdd_cpu_rx_meter_drop_counter_get failed. rc=%d\n", rdd_error);
        }
        *stat = drop_count;
    }
#else
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
    ...
    if (meter_cfg->sir < RDPA_CPU_METER_MIN_SR ||
    ...
    if (meter_cfg->burst_size < meter_cfg->sir)
    {
        BDMF_TRACE_RET_OBJ(BDMF_ERR_PARM, mo, "Burst size %d is invalid. Expected greater or equal to SiR\n",
            meter_cfg->burst_size);
    }
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
    ...
    /* flush in RDD */
    {
        ...
        rdd_ring_init(rdd_ring_idx, cpu_data->rxq_cfg[index].type,
            ...
            cpu_data->rxq_cfg[index].size, size_of_entry, rdd_ring_idx, write_idx
#ifdef XRDP
             , 0
#endif
             );
...
static struct bdmf_attr cpu_attrs[] = {
    ...
    { .name = "l4_dst_port_to_reason", .help = "TCP/UDP dst port to reason mapping",
        .type = bdmf_attr_aggregate,
        .ts.enum_table = &rdpa_cpu_reason_enum_table,
        .ts.aggr_type_name = "l4_dst_port_to_reason_cfg", .array_size = 256,
#ifdef XRDP
        .flags = BDMF_ATTR_READ | BDMF_ATTR_WRITE | BDMF_ATTR_CONFIG | BDMF_ATTR_DEPRECATED,
#else
        .flags = BDMF_ATTR_READ | BDMF_ATTR_WRITE | BDMF_ATTR_CONFIG,
#endif
        .add = cpu_attr_l4_dst_port_to_reason_add_ex, .read = cpu_attr_l4_dst_port_to_reason_read_ex,
        .del = cpu_attr_l4_dst_port_to_reason_delete_ex, .find = cpu_attr_l4_dst_port_to_reason_find_ex
    },
