...
static int vlan_pre_init(struct bdmf_object *mo)
{
    ...
    if (!rdpa_if_is_lan(port->index) && !rdpa_if_is_wan(port->index))
    {
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    rdpa_if port_index = rdpa_port_index_get(mo->owner, &port_index);
    bdmf_boolean vid_enable;
    int rc = 0;
    int i;
    BDMF_TRACE_DBG("port=%d\n", port_index);
    if (!rdpa_if_is_wan(port_index))
    ...
    vid_enable = (vlan->discard_prty == port->tm_cfg.discard_prty) ? 0 : 1;
    
    for (i = 0; i < RDPA_MAX_VLANS; i++)
    {
        if (vlan->vids[i].vid == BDMF_INDEX_UNASSIGNED)
            continue;
        rc = rc ? rc : vlan_update_priority_vids(vlan->vids[i].vid, vid_enable);
    }
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    if (i >= 0)
    {
        ...
        if (rdpa_if_is_wan(port_index) && (vlan->discard_prty != port->tm_cfg.discard_prty))
            vlan_update_priority_vids(vlan->vids[i].vid, 0);
    ...
    else
    {
        ...
        if (rdpa_if_is_wan(port_index) && i > 0 && syscfg->switching_mode == rdpa_vlan_aware_switching)
        ...
        if (rdpa_if_is_wan(port_index) && (vlan->discard_prty != port->tm_cfg.discard_prty))
            vlan_update_priority_vids(vlan->vids[i].vid, 1);
    }
