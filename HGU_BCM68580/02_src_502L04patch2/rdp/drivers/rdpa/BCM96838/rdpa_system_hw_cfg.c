...
extern int runner_reserved_memory_get(uint32_t *tm_base_addr,
    uint32_t *mc_base_addr, uint32_t *tm_size);
...
void rdp_get_init_params(rdp_init_params *init_params)
{
    uint32_t tm_size, buffers_amount;

    init_params->bpm_buffer_size = rdpa_mtu_to_bpm_buffer_size(RDPA_MTU);
    
    runner_reserved_memory_get(&init_params->runner_tm_base_addr,
        &init_params->runner_mc_base_addr, &tm_size);
