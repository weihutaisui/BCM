    ...
    void *tx_post_descr)
{
    ...
    pkt_ptr = (uint32_t *)(((uint8_t *)RDD_RSV_PHYS_TO_VIRT(data_buf_addr)) - 14);
...
static int dhd_sim_dongle_tx_post_isr(int irq, void *isr_priv)
{
    ...
    if (!dhd_sim_data->enabled)
    {
        /* Radio is disabled, don't react on new packets in any TX Post ring */
    ...
    {
        GET_ADDR_HIGH_LOW(addr_hi, addr_lo, RDD_RSV_VIRT_TO_PHYS((void *)wr_ptr));
        ...
        {            
            ...
            GET_ADDR_HIGH_LOW(addr_hi, addr_lo, RDD_RSV_VIRT_TO_PHYS(tx_post_descr));            
            ...
            i = (i + 1) & ~DHD_SIM_TX_POST_FLOW_RING_SIZE; /* Wrap around */
            *rd_ptr = i;
        }
    }
    return BDMF_IRQ_HANDLED;
...
static int dhd_sim_resources_alloc(dhd_sim_drv_priv_t *dhd_sim_data)
{
    ...
    for (i = 0; i < MAX_TX_POST_FLOW_RINGS; i++)
    {
        ...
        _dhd_sim_resource_alloc(&dhd_sim_data->tx_post_flow_ring_arr[i],
            tx_post_desc_size * DHD_SIM_TX_POST_FLOW_RING_SIZE, NULL, 0);
        if (!dhd_sim_data->tx_post_flow_ring_arr[i])
            goto err;

        GET_ADDR_HIGH_LOW(addr_hi, addr_lo, RDD_RSV_VIRT_TO_PHYS(dhd_sim_data->tx_post_flow_ring_arr[i]));
    ...
    {           
        case FR_FORMAT_WI_WI64: /* Legacy Work Item */
        {
            ...
            *rx_post_rd_ptr = (rx_post_rd_idx);
            ...
            data_buf_ptr = RDD_RSV_PHYS_TO_VIRT((((uint64_t)(((RDD_DHD_RX_POST_DESCRIPTOR_DTS *)rx_post_descr)->data_buf_addr_hi) << 32) | 
        ...
        {                           
            ...
            *rx_post_rd_ptr = (rx_post_rd_idx);
            ...
            /* Prepare RX Complete descriptor, copy data to FPM, request_id serves as FPM BN */
            data_buf_ptr = RDD_RSV_PHYS_TO_VIRT((uint64_t) ((((RDD_DHD_RX_POST_DESCRIPTOR_CWI32_DTS *)rx_post_descr)->data_buf_addr_low)) + data_offset);
            memcpy(data_buf_ptr, val, size); 
