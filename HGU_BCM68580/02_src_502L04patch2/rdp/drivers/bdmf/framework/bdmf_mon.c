	...
	int nzstat, int max_prints, int format, int print_level, int *print_obj_name)
{
    ...
    else
    {
        ...
        /* if index type is bdmf_attr_object need to make sure ref counter is back down */ 
        if ((nprints >= max_prints) && (rc != BDMF_ERR_NO_MORE) && (attr->index_type == bdmf_attr_object))
            bdmf_put((bdmf_object_handle)j);
