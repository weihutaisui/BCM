...
static int user_ioctl_attr_impl_gen(codegen_parm_t *parm, const struct bdmf_attr *attr, bdmf_func_type func_type)
{
    ...
    int is_read = (func_type==func_read || func_type==func_find);
    char func_suffix_comment[CODEGEN_FUNC_SUFFIX_SIZE], func_suffix[CODEGEN_FUNC_SUFFIX_SIZE];
    char attr_val_type_no_pointer[MAX_STR] = "";
    char attr_index_type_no_pointer[MAX_STR] ="";
    int attr_name_len;
    int idx_name_len;
    int is_pointer;
    ...
    strcpy(attr_index_type_no_pointer, attr_index_type);
    ...
    strcpy(attr_val_type_no_pointer, attr_val_type);
...
static void user_obj_get_ioctl_gen(codegen_parm_t *parm)
{
    ...
    fprintf(hf, "&pa->mo)))\n"
            "\t{\n"
            "\t\tBDMF_TRACE_DBG(\"%s_%s_get failed ret: %s\\n\", pa->ret);\n",
            parm->name_prefix, parm->drv->name, "\%d");
    fprintf(hf, "\t}\n\n"
            "\treturn 0;\n"
            "}\n\n");
}
...
static int is_attr_contain_buffer(const struct bdmf_attr *attr)
{
    const struct bdmf_attr *fields = attr->aggr_type->fields;

    while (fields->name)
    {
        if (fields->type == bdmf_attr_buffer)
            return 1;
        else if (fields->type == bdmf_attr_aggregate)
        {
             if (is_attr_contain_buffer(fields))
                 return 1;
        }
        ++fields;
    }
...
static void user_ioctl_attr_case_gen(codegen_parm_t *parm)
{
    ...
    while(attr && attr->name)
    {
             ...
             (attr->type==bdmf_attr_aggregate)) )
        {
            ...
            if (attr->type==bdmf_attr_aggregate && is_attr_contain_buffer(attr))
            {
                ++attr;
                continue;
            }
...
static int user_ioctl_impl_gen(codegen_parm_t *parm)
{
    ...
    fprintf(hf, "\t\tdefault:\n"
            "\t\t\tBDMF_TRACE_ERR(\"no such ioctl cmd: %s\\n\", op);\n"
            "\t\t\tret = EINVAL;\n"
            "\t\t}\n\n"
            "\treturn ret;\n"
            "}\n",
                                ...
                                int is_idx_pointer)
{
    ...
    fprintf(hf, "{\n"
            ...
            func_name);
    if(is_idx_pointer)
        fprintf(hf, "\tpa.ai_ptr = (uint32_t)ai_;\n");
    else
                              ...
                              int is_idx_pointer)
{
    ...
    else if (func_type==func_write)
    {
        ...
        if (attr->type == bdmf_attr_string)
        {
            fprintf(hf, "\tpa.ptr = (uint64_t)(long)%s_;\n", attr->name);
        }
        else if (attr->type == bdmf_attr_object)
    ...
    else
    {
        ...
        {
            if(_bdmf_mon_codegen_is_numeric(attr))
                fprintf(hf, "\tpa.parm = (uint64_t)(long)%s_;\n", attr->name);
            else
                fprintf(hf, "\tpa.ptr = (uint32_t)%s_;\n", attr->name);
...
static int user_func_api_attr_impl_gen(codegen_parm_t *parm, const struct bdmf_attr *attr, bdmf_func_type func_type, char *ioctl_str)
{
    ...
    fprintf(hf, "\n\tfd = open(RDPA_USR_DEV_NAME, O_RDWR);\n"
            ...
            "\tif (ret)\n"
            "\t{\n"
            "\t\trdpa_usr_error(\"ioctl failed, ret=%s\\n\", ret);\n"
            "\t\tclose(fd);\n"
            "\t\treturn ret;\n"
            "\t}\n\n"
            "\tclose(fd);\n"
...
static int user_obj_get_gen(codegen_parm_t *parm, char *ioctl_str)
{
    ...
    if (nkeys > 1 || is_string)
        fprintf(hf, "\tpa.ptr = (uint32_t)(long)%s;\n\n", parm->keyname);
    else if (nkeys == 1)
    ...
    fprintf(hf, "\tif (ret)\n"
                "\t{\n"
                "\t\trdpa_usr_error(\"ioctl failed, Errno[%s] ret=%s\\n\", strerror(errno), ret);\n"
                "\t\tclose(fd);\n"
                "\t\treturn ret;\n"
                "\t}\n\n"
                "\t*%s_obj = pa.mo;\n"
...
static int bdmf_codegen_pygen_add_imports(codegen_parm_t *p)
{
    ...
    rc = rc ? rc : bdmf_codegen_pygen_add_import(p, NULL, "rdpa_object");
...
static const char *bdmf_codegen_pygen_mangle_class_name(const char *name, int is_type)
{
    ...
    while ((p=strchr(pname, '_')))
    {
        ...
        strncat(mangled_name, part, sizeof(mangled_name));
        pname = p+1;
    }
...
const int bdmf_codegen_pygen_print_attr_type_name(codegen_parm_t *p, const struct bdmf_attr *a, int is_index)
{
...
{
    int rc = 0;
    rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sif type(%s) != %s:\n",
        tabs, value, bdmf_codegen_pygen_mangle_class_name(ag->name, 0));
    rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, BDMF_TAB "%sraise value_error(str(type(self)), '%s', str(%s))\n",
    ...
    const char *index,const char *value, const char *tabs)
{
    ...
    if (value)
    {
        ...
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_enum('%s_value', %s)\n",
                tabs, mname, value);
        }
        else if (a->type == bdmf_attr_enum_mask)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_enum_mask('%s_value', %s)\n",
                tabs, mname, value);
        }
        else if (a->type == bdmf_attr_aggregate)
        {
            rc = rc ? rc : bdmf_codegen_pygen_add_validate_aggr(p, a, a->aggr_type, value, tabs);
        }
        else if (a->type == bdmf_attr_ipv4_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ipv4_addr('%s_value', %s)\n",
                tabs, mname, value);
        }
        else if (a->type == bdmf_attr_ipv6_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ipv6_addr('%s_value', %s)\n",
                tabs, mname, value);
        }
        else if (a->type == bdmf_attr_ip_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ip_addr('%s_value', %s)\n",
                tabs, mname, value);
        }
        else if (a->type == bdmf_attr_ether_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ether_addr('%s_value', %s)\n",
                tabs, mname, value);
        }
    }
    if (index)
    {
        ...
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_enum('%s_index', %s)\n",
                tabs, mname, index);
        }
        else if (a->index_type == bdmf_attr_aggregate)
        {
            rc = rc ? rc : bdmf_codegen_pygen_add_validate_aggr(p, a, a->index_aggr_type, index, tabs);
        }
        else if (a->index_type == bdmf_attr_ipv4_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ipv4_addr('%s_index', %s)\n",
                tabs, mname, index);
        }
        else if (a->index_type == bdmf_attr_ipv6_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ipv6_addr('%s_index', %s)\n",
                tabs, mname, index);
        }
        else if (a->index_type == bdmf_attr_ip_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ip_addr('%s_index', %s)\n",
                tabs, mname, index);
        }
        else if (a->index_type == bdmf_attr_ether_addr)
        {
            rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, "%sself.validate_ether_addr('%s_index', %s)\n",
                tabs, mname, value);
        }
    }
    return rc;
    ...
    uint32_t filter, uint32_t not_filter, int is_type)
{
    ...
    rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, BDMF_TAB2 "delim=''\n");
    for (a = aa; a->name; ++a)
    {
    ...
    const char *suffix, const struct bdmf_attr *aa, int is_type)
{
    ...
    if (is_type)
    {
        rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, BDMF_TAB2 "super(%s%s, self).__init__('%s%s', session)\n",
...
static int bdmf_codegen_pygen_type(codegen_parm_t *p, bdmf_type_handle ot)
{
    ...
    if (t->aattr)
    {
        ...
        rc = bdmf_codegen_pygen_class(p, t->name, t->description, "", t->aattr, 1);
        if (p->hf != NULL)
        {
            fclose(p->hf);
            p->hf = NULL;
