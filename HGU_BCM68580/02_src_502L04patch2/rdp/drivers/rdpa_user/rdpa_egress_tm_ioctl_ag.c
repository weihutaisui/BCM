...
static int rdpa_user_egress_tm_enable_get(rdpa_ioctl_cmd_t *pa)
{
	bdmf_boolean parm;

	BDMF_TRACE_DBG("inside egress_tm_user_enable_get\n");

	if ((pa->ret = rdpa_egress_tm_enable_get(pa->mo, &parm)))
	{
		BDMF_TRACE_DBG("rdpa_egress_tm_enable_get failed, ret:%d\n", pa->ret);
	}
...
static int rdpa_user_egress_tm_enable_set(rdpa_ioctl_cmd_t *pa)
{

	BDMF_TRACE_DBG("inside egress_tm_user_enable_set\n");

	if ((pa->ret = rdpa_egress_tm_enable_set(pa->mo, (bdmf_boolean)(long)pa->parm)))
	{
		BDMF_TRACE_DBG("rdpa_egress_tm_enable_set failed, ret:%d\n", pa->ret);
	}
...
static int rdpa_user_egress_tm_queue_flush_set(rdpa_ioctl_cmd_t *pa)
{
	rdpa_tm_queue_index_t  ai;

	BDMF_TRACE_DBG("inside egress_tm_user_queue_flush_set\n");
	...
	if ((pa->ret = rdpa_egress_tm_queue_flush_set(pa->mo, &ai, (bdmf_boolean)(long)pa->parm)))
	{
		BDMF_TRACE_DBG("rdpa_egress_tm_queue_flush_set failed, ret:%d\n", pa->ret);
	}
...
long rdpa_egress_tm_ag_ioctl(unsigned int op, rdpa_ioctl_cmd_t *pa)
{
	...
	switch (op){
		...
		case RDPA_EGRESS_TM_ENABLE_GET:
			ret = rdpa_user_egress_tm_enable_get(pa);
			break;

		case RDPA_EGRESS_TM_ENABLE_SET:
			ret = rdpa_user_egress_tm_enable_set(pa);
			break;
		...
		case RDPA_EGRESS_TM_QUEUE_FLUSH_SET:
			ret = rdpa_user_egress_tm_queue_flush_set(pa);
			break;
