...
/*            e4h: 112-byte5h: 128-byte6h: 144-byte7h: 160-byte                                   */
...
/*                          1h: Enable variable context lengthReset value is 0x0.                 */
...
/*                          hat key length is reduced by 4 bit0h: Disable variable context length */
/*                          1h: Enable variable context lengthReset value is 0x0.                 */
/* key_len_tbl7:  - Table 7 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl7:  - Table 7 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl6:  - Table 6 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl6:  - Table 6 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl5:  - Table 5 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl5:  - Table 5 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl4:  - Table 4 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl4:  - Table 4 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl3:  - Table 3 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl3:  - Table 3 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl2:  - Table 2 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl2:  - Table 2 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl1:  - Table 1 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl1:  - Table 1 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/* key_len_tbl0:  - Table 0 length of the key.0h: 16-byte key;1h: 32-byte key;                    */
/* non_cacheable_tbl0:  - Table 0 Non Cacheable.0h: Cacheable.lookup is fetched from DDR and cach */
/*                     ed (i.e., internal NAT table is updated).1h: Non Cacheable.lookup is fetch */
/*                     ed from DDR and not cached (i.e., internal NAT table is notupdated) and se */
/*                     ssion hit count and byte count values are only updated in DDR;             */
/**************************************************************************************************/
