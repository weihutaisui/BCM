...
bbh_to_dma_x_t  g_bbh_to_dma_x[BBH_ID_NUM] = { 
                                             ...
                                             {BBH_ID_DSL, DMA0_ID}};
bbh_to_dma_x_t  g_bbh_to_sdma_x[BBH_ID_NUM] = { 
                                             {BBH_ID_0, SDMA0_ID},
                                             {BBH_ID_1, SDMA0_ID},
                                             {BBH_ID_2, SDMA0_ID},
                                             {BBH_ID_PON, SDMA1_ID},
...
uint8_t g_bbh_buff_num[NUM_OF_DMA][NUM_OF_PERIPHERALS_PER_DMA] = { {5, 5, 5, 5, 4, 4, 4 } };

bb_source_t g_dma_bb_source[BBH_ID_NUM] = {
                                             {BB_ID_RX_BBH_0, BB_ID_TX_LAN},
                                             {BB_ID_RX_BBH_1, BBH_ID_NULL},
                                             {BB_ID_RX_BBH_2, BBH_ID_NULL},
                                             {BB_ID_RX_PON, BB_ID_TX_PON},
...
urgent_threhold_t g_dma_urgent_threshold[BBH_ID_NUM][2] = {
         ...
         { {DMA0_U_THRESH_IN_BBH_ID_DSL_VALUE, DMA0_U_THRESH_OUT_BBH_ID_DSL_VALUE},
           {SDMA0_U_THRESH_IN_BBH_ID_DSL_VALUE, SDMA0_U_THRESH_OUT_BBH_ID_DSL_VALUE} } };
...
strict_priority_t g_dma_strict_priority[BBH_ID_NUM][2] = {
         ...
         { {DMA0_STRICT_PRI_RX_BBH_ID_DSL_VALUE, DMA0_STRICT_PRI_TX_BBH_ID_DSL_VALUE},
           {SDMA0_STRICT_PRI_RX_BBH_ID_DSL_VALUE, SDMA0_STRICT_PRI_TX_BBH_ID_DSL_VALUE} } };
...
rr_weight_t g_dma_rr_weight[BBH_ID_NUM][2] = {
         ...
         { {DMA0_RR_WEIGHT_RX_BBH_ID_DSL_VALUE, DMA0_RR_WEIGHT_TX_BBH_ID_DSL_VALUE},
           {SDMA0_RR_WEIGHT_RX_BBH_ID_DSL_VALUE, SDMA0_RR_WEIGHT_TX_BBH_ID_DSL_VALUE} } };
...
static int bbh_profiles_init(void)
{
    ...
    for (bbh_id = 0; bbh_id < BBH_ID_NUM; bbh_id++)
    {
        ...
        /* Note that when the TX tables are indexed later by using a TX BBH ID, it may need to be adjusted
           since the TX BBH WAN IDs do not match the RX BBH IDs. */
...
static int bbh_rx_dsl_init(int wan_bbh)
{
    ...
    /* BBH-RX mactype init */
    //rc = ag_drv_bbh_rx_mac_mode_set(BBH_ID_DSL, 0, 0, 1) ; To confirm why DSL mac mode does not work????
    rc = ag_drv_bbh_rx_mac_mode_set(BBH_ID_DSL, 0, 0, 0) ;
...
static int sbpm_init(void)
{
    ...
    // The following 3 sets of values are default register values
    // and should likely be deleted.  They are here because HW sim was also 
    // programming these values.  After everything is up and running
    // delete and see if it still works :-)

    bdmf_trace("%s  %d  %d\n", __FUNCTION__, __LINE__, rc);

    rc = rc ? rc : ag_drv_sbpm_regs_sbpm_sp_bbh_low_set(  0x90000000 );
    rc = rc ? rc : ag_drv_sbpm_regs_sbpm_sp_bbh_high_set( 0x0FFC000B );

    rc = rc ? rc : ag_drv_sbpm_regs_sbpm_sp_rnr_low_set(  0x0000003F );
    rc = rc ? rc : ag_drv_sbpm_regs_sbpm_sp_rnr_high_set( 0x00000000 );
    
    rc = rc ? rc : ag_drv_sbpm_regs_sbpm_ug_map_low_set(  0x7c000000 );
    rc = rc ? rc : ag_drv_sbpm_regs_sbpm_ug_map_high_set( 0x0ad80001 );
...
static int bbh_tx_lan_init(void)
{
    ...
    /* Note that these bbh_tx tables are index based on the RX id's so the TX index here needs to be adjusted */
    /* So, use the RX_ID instead of tx id here to index the tables */
    config.dma_cfg = &(g_bbh_tx_dma_profile->bbh_tx_dma_cfg[BBH_ID_FIRST]);
    config.sdma_cfg = &(g_bbh_tx_sdma_profile->bbh_tx_sdma_cfg[BBH_ID_FIRST]);
    config.ddr_cfg = &(g_bbh_tx_ddr_profile->bbh_tx_ddr_cfg[BBH_ID_FIRST]);
...
static int bbh_tx_wan_init(void)
{
    ...
    for (bbh_id = BBH_TX_ID_AE10; bbh_id < BBH_TX_ID_NUM; bbh_id++)
    {
        ...
        config.msg_rnr_cfg[0].tcont_addr = IMAGE_3_REPORT_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
        config.msg_rnr_cfg[0].ptr_addr = IMAGE_3_BBH_TX_EGRESS_REPORT_COUNTER_TABLE_ADDRESS >> 3;
        config.msg_rnr_cfg[0].task_number = IMAGE_3_US_TM_REPORTING_THREAD_NUMBER;
        config.src_id.fpmsrc = BB_ID_FPM;
        config.src_id.sbpmsrc = BB_ID_SBPM;
        config.src_id.msgrnrsrc = get_runner_idx(reporting_runner_image);

        /* Note that these bbh_tx tables are index based on the RX id's so the TX index here needs to be adjusted */
        /* So, adjust the bbh_id to get the rx_bbh_id used for indexing these initialization tables */
        rx_bbh_id = bbh_id + (BBH_ID_PON - BBH_TX_ID_PON);
        config.dma_cfg = &(g_bbh_tx_dma_profile->bbh_tx_dma_cfg[rx_bbh_id]);
        config.sdma_cfg = &(g_bbh_tx_sdma_profile->bbh_tx_sdma_cfg[rx_bbh_id]);
        config.ddr_cfg = &(g_bbh_tx_ddr_profile->bbh_tx_ddr_cfg[rx_bbh_id]);
...
static int bbh_tx_pon_init(int wantype)
{
    ...
    /* NOTE That indices on these tables are based on the RX ID not the TX id. */
...
static int dispatcher_reorder_init(void)
{
    ...
    for (core_index = 0; core_index < GROUPED_EN_SEGMENTS_NUM; core_index++)
    {
        ...
        {
            dispatcher_reorder_rnr_group_init(&cfg, 1, core_index, (1<< IMAGE_0_DS_TM_WAN_DIRECT_THREAD_NUMBER), (IMAGE_0_DIRECT_PROCESSING_PD_TABLE_ADDRESS >> 3), 0);
        }
    }
...
uint32_t data_path_init_fiber(int wan_bbh)
{
    ...
    /* TODO!! need to create a logic to notify what QM queues are used for GPON.
     * As for now, we only support 1 WAN type, so this is ok */
...
uint32_t data_path_init_gbe(rdpa_emac wan_emac)
{
    ...
    /* Assume wan_emac type has been validated */
    /* QM mactype init */
    rc = qm_wantype_init(rdpa_emac_to_bbh_mac_type_tx[wan_emac]);
