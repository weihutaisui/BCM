...
#else
       if (hit_count)
           rc = ag_drv_natc_eng_hit_count_get(eng_idx, (uint32_t *)hit_count);
       if (byte_count)
       {
            rc = rc ? rc : ag_drv_natc_eng_byte_count_get(eng_idx, (uint32_t *)byte_count);
            if (hit_count)
            {
                *byte_count = *byte_count << (NATC_NUM_OF_BITS_IN_BYTE_COUNT - 32);
                *byte_count |= (*hit_count >> NATC_NUM_OF_BITS_IN_HIT_COUNT);
                *hit_count &= ((1UL<< NATC_NUM_OF_BITS_IN_HIT_COUNT) - 1);
            }
        }
#endif	   
    }
