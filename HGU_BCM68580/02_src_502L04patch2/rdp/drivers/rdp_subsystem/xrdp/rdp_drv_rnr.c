...
#define DA_FILTER_NO_MASK_FIRST_ID 2
#define DA_FILTER_NO_MASK_LAST_ID  DRV_PARSER_DA_FILTER_NUM
...
bdmf_error_t _drv_rnr_quad_parser_da_filter_without_mask_set(rnr_quad_id_e quad_id,
...
bdmf_error_t drv_rnr_quad_parser_da_filter_without_mask_set(drv_rnr_da_flt_grp_t da_filt_grp, uint8_t sw_filter_index,
    uint8_t *mac_address, bdmf_boolean enable)
{
    static uint8_t sw_filter_to_hw_filter[DA_MAC_FILTER_GROUP_SIZE][DA_FILTER_NO_MASK_LAST_ID - DA_FILTER_NO_MASK_FIRST_ID] = {};
    uint8_t quad_id, hw_filter_index;
    bdmf_error_t rc = 0;
    rnr_quad_da_filter_valid da_filt_valid;
    if (!enable)
    {
        if (sw_filter_to_hw_filter[da_filt_grp][sw_filter_index])
        {
            for (quad_id = 0; !rc && quad_id < NUM_OF_RNR_QUAD; quad_id++)
            {
                rc = rc ? rc : drv_rnr_quad_parser_da_filter_valid_cfg(quad_id, sw_filter_to_hw_filter[da_filt_grp][sw_filter_index], 0);
            }
            sw_filter_to_hw_filter[da_filt_grp][sw_filter_index] = 0;
        }
        return rc;
    }
    ...
    //find available HW filter index
    ag_drv_rnr_quad_da_filter_valid_get(0, &da_filt_valid);
    for (hw_filter_index = DA_FILTER_NO_MASK_FIRST_ID; hw_filter_index < 8; hw_filter_index++)
        if (!(*(((bdmf_boolean *)(&da_filt_valid.da_filt0_valid)) + hw_filter_index)))
            break;
    if (hw_filter_index == 8)
        return BDMF_ERR_NORES;

    for (quad_id = 0; !rc && quad_id < NUM_OF_RNR_QUAD; quad_id++)
    {
        rc = rc ? rc :  drv_rnr_quad_parser_da_filter_valid_cfg(quad_id, hw_filter_index, 1);
        rc = rc ? rc :  _drv_rnr_quad_parser_da_filter_without_mask_set(quad_id, hw_filter_index, mac_address);
    }
    if (!rc)
        sw_filter_to_hw_filter[da_filt_grp][sw_filter_index] = hw_filter_index;
    return rc;
}
...
{
    bdmf_error_t rc = 0;
    uint8_t qtag_nest_0, qtag_nest_1;
