...
int drv_dis_reor_queues_init()
{
    ...
    rc = rc ? rc : ag_drv_dsptchr_congestion_glbl_congstn_set(0, &congs_init);
    rc = rc ? rc : ag_drv_dsptchr_congestion_total_egrs_congstn_set(0, &congs_init);

    congs_init.frst_lvl = DIS_REOR_CONGESTION_THRESHOLD - NUM_OF_PROCESSING_TASKS;
    congs_init.scnd_lvl = DIS_REOR_CONGESTION_THRESHOLD;
