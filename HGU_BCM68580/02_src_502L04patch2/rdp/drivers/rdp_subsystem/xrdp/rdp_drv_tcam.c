...
#define RDP_TCAM_RULE_PRIORITY_REMAP(pri) ((RDP_TCAM_MAX_PRIORITIES - 1) - pri)
...
static bdmf_error_t _drv_tcam_rule_find_by_key_mask_pri(const rdp_tcam_priority pri, const rdp_tcam_key_area_t *key,
    ...
    const rdp_tcam_context_t               *context)
{
    ...
    if (pri >= RDP_TCAM_MAX_PRIORITIES)
        return BDMF_ERR_PARM;

    priority = RDP_TCAM_RULE_PRIORITY_REMAP(pri);
    ...
    const rdp_tcam_key_area_t              *mask)
{
    ...
    /* Find TCAM entry */
    err =  _drv_tcam_rule_find_by_key_mask_pri(pri, key, mask, &rule_index);
    if (err != BDMF_ERR_OK)
        return err;
...
static bdmf_error_t _drv_tcam_rule_find_by_key_mask_pri(
    const rdp_tcam_priority                pri,
    const rdp_tcam_key_area_t              *key,
    const rdp_tcam_key_area_t              *mask,
    uint16_t                               *rule_index)
{
    ...
    /* Calculate number of flows with higher priority */
    for (i = 0; i < priority; i++)
    {
        start_entry += tcam_priority_count[i];
    }
    ...
    for (entry = start_entry; (entry < (start_entry + tcam_priority_count[priority])) && !is_match; entry++)
    {
    ...
    uint16_t  *rule_index)
{
    ...
    /* Find TCAM entry */
    return _drv_tcam_rule_find_by_key_mask_pri(priority, key, mask, rule_index);
}
...
static int _tcam_cli_delete_handler(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    err = drv_tcam_rule_delete(priority, &key, &mask);
