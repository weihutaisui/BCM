...
static int bbh_tx_pon_init(int wantype)
{
    ...
    if (config.mac_type == MAC_TYPE_EPON)
    {
        ...
        config.rnr_cfg[0].tcont_addr = IMAGE_0_BBH_TX_EPON_QUEUE_ID_TABLE_ADDRESS >> 3;
        ...
        config.sts_rnr_cfg[0].tcont_addr = IMAGE_0_US_TM_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
    ...
    else
    {
        ...
        config.rnr_cfg[0].tcont_addr = IMAGE_0_US_TM_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
        config.rnr_cfg[0].skb_addr = 0;
...
static int bbh_tx_init(void)
{
    ...
    if (p_dpi_cfg->bbh_id_gbe_wan != BBH_ID_NULL)
    {
        ...
        config.rnr_cfg[1].tcont_addr = IMAGE_0_US_TM_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
...
static int dispatcher_reorder_init(void)
{
    ...
    /* VIQ for epon tm - 12 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(us_tm_runner_image), ((IMAGE_0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) |
        (IMAGE_0_TM_WAN_THREAD_NUMBER << 12)), dsptchr_viq_dest_reor, dsptchr_viq_non_delayed, 
        DISP_REOR_VIQ_EPON_TM, DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);

    /* VIQ for CPU_TX egress - 13 */
    ...
    /* VIQ for flush US - 15 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(us_tm_runner_image) /* us/ds - same runner */, (IMAGE_0_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) |
        (IMAGE_0_TM_FLUSH_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_TM_FLUSH,
        DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);

    /* VIQ for TX Mirroring - Initialized for US - 16 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(us_tm_runner_image), 
        ((IMAGE_0_TX_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_0_TM_WAN_THREAD_NUMBER << 12)),
         dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_TX_MIRRORING,
         DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);
    
    /* VIQ for CPU_RX_copy - 17 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(cpu_rx_runner_image), (IMAGE_1_CPU_RX_COPY_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 |
        IMAGE_1_PROCESSING_CPU_RX_COPY_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_CPU_RX_COPY,
        DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);
    
#ifdef CONFIG_DHD_RUNNER
    /* VIQ for DHD_RX_COMPLETE_0 - 18 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(dhd_complete_runner_image), (IMAGE_1_DHD_RX_COMPLETE_0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 |
        IMAGE_1_PROCESSING_DHD_RX_COMPLETE_0_THREAD_NUMBER << 12), dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_RX_COMPLETE_0,
        DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);

    /* VIQ for DHD Mcast - 19 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(dhd_tx_post_runner_image), (IMAGE_2_DHD_MCAST_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 |
        IMAGE_2_IMAGE_2_DHD_MCAST_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_MCAST,
        DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);
#endif   
    ...
    for (core_index = 0; core_index < GROUPED_EN_SEGMENTS_NUM; core_index++)
    {
        ...
        {
            dispatcher_reorder_rnr_group_init(&cfg, 1, core_index, (1<< IMAGE_1_PROCESSING_WAN_DIRECT_THREAD_NUMBER), offsetof(RDD_PACKET_BUFFER_DTS, pd) >> 3, (sizeof(RDD_PACKET_BUFFER_DTS) >> 3));
        }
    }
...
#ifndef _CFE_
    cfg.rnr_grp_num = 2;
    cfg.dsptchr_rnr_group_list[0].queues_mask = 0x003F | /* all bbh viq except wan-bbh exclusive viq */
        (1 << DISP_REOR_VIQ_CPU_TX_FORWARD);
#ifdef CONFIG_DHD_RUNNER
    cfg.dsptchr_rnr_group_list[0].queues_mask |=  (1 << DISP_REOR_VIQ_DHD_RX_COMPLETE_0);
#endif        
    cfg.dsptchr_rnr_group_list[1].queues_mask = 0x800; /* wan-bbh exclusive viq 11 */
#else
...
static int qm_init(void)
{
    ...
    rnr_group_cfg.rnr_task = IMAGE_1_PROCESSING_CPU_RX_UPDATE_FIFO_THREAD_NUMBER;
    ...
    rnr_group_cfg.rnr_task = IMAGE_1_PROCESSING_CPU_RX_COPY_UPDATE_FIFO_THREAD_NUMBER;
    ...
    /* TBD - to free one of the groups dhd mcast - group 8 (dhd mcast fifo task) */    
#ifdef UNDEF    
...
int data_path_init(dpi_params_t *dpi_params)
{
    ...
    int rc; 
    rc = drv_rnr_quad_ubus_decode_wnd_cfg(0, 0, g_board_size_power_of_2, DECODE_CFG_PID_B53, IS_DDR_COHERENT);
    if (rc < 0)
    {
