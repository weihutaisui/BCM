...
typedef uint32_t rdpa_flow_control_traffic; /**< Mask of \ref rdpa_fc_traffic_fields flow control traffic types */

/** Flow control configuration parameters */
typedef struct {
    uint32_t rate; /**< Ingress limit rate [bits/sec] . \RDP_LIMITED */
    uint32_t mbs; /**< Maximal burst size in bytes . \RDP_LIMITED */
    uint32_t threshold; /**< Flow control threshold . \RDP_LIMITED */
    bdmf_mac_t src_address; /**< Flow control mac source address . \RDP_LIMITED */
    rdpa_flow_control_traffic traffic_types; /**< Flow control traffic types bitmask . \XRDP_LIMITED */
    bdmf_object_handle policer; /**< Referenced policer for flow control values . \XRDP_LIMITED */
} rdpa_port_flow_ctrl_t;

/* Actions of the optional flow control traffic types */
typedef enum
{
    RDPA_FLOW_CONTROL_BROADCAST,
    RDPA_FLOW_CONTROL_MULTICAST,
    RDPA_FLOW_CONTROL_UNKNOWN_DA,    
    RDPA_FLOW_CONTROL_ALL_TRAFFIC
} rdpa_fc_traffic_value;

/** IP classification bypass modes */
typedef enum
{
    RDPA_FLOW_CONTROL_MASK_BROADCAST = (1 << RDPA_FLOW_CONTROL_BROADCAST), /**< In this mode, broadcast packets are rate limited */
    RDPA_FLOW_CONTROL_MASK_MULTICAST = (1 << RDPA_FLOW_CONTROL_MULTICAST), /**< In this mode, multicast packets are rate limited */
    RDPA_FLOW_CONTROL_MASK_UNKNOWN_DA = (1 << RDPA_FLOW_CONTROL_UNKNOWN_DA), /**< In this mode, unknown_da packets are rate limited */    
    RDPA_FLOW_CONTROL_MASK_ALL_TRAFFIC = (1 << RDPA_FLOW_CONTROL_ALL_TRAFFIC), /**< In this mode, all traffic are rate limited */
} rdpa_fc_traffic_fields;
