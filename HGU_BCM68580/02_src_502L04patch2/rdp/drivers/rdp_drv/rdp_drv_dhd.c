...
int rdp_drv_rx_post_init(uint32_t radio_idx, rdpa_dhd_init_cfg_t *init_cfg)
{
    ...
    for (i = 0, write_ptr = (uintptr_t)init_cfg->rx_post_flow_ring_base_addr; i < DHD_RX_POST_FLOW_RING_SIZE - 1; i++)
    {
    ...
    *ring_info->wr_idx_addr = (DHD_RX_POST_FLOW_RING_SIZE - 1);    
...
{    
    ...
    cntr_id = COUNTER_DHD_TX_DROP_0_SSID_0 + radio_idx*16 + ssid;
...
static void 
rdp_drv_dhd_cpu_tx_set_packet_descriptor(const rdpa_dhd_tx_post_info_t *info, void *buffer, void* data, RDD_DHD_CPU_QM_DESCRIPTOR_DTS *cpu_tx_descriptor, int16_t free_index, uint16_t pkt_length)
{
    bdmf_phys_addr_t phy_addr;
...
int rdp_drv_dhd_helper_dhd_complete_message_get(rdpa_dhd_complete_data_t *dhd_complete_info)
{
    ...
    {      
       ...
       drv_cntr_counter_read(CPU_RX_CNTR_GROUP_ID, COUNTER_CPU_GROUP_WRITE_PTR_DHD_0 + dhd_complete_info->radio_idx, cntr_arr);
    ...
    RDD_CPU_RING_DESCRIPTOR_READ_IDX_WRITE_G(descriptor->shadow_read_idx, RDD_DHD_CPU_RECYCLE_RING_DESCRIPTOR_TABLE_ADDRESS_ARR, dhd_complete_info->radio_idx);
