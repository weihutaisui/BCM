...
exclude=" \
  ...
  --exclude=$imp/router/bshared/include \
  --exclude=$imp/router/bshared/src \
  --exclude=$imp/usbdev \
  --exclude=$imp/tools \
  --exclude=$imp/router_rules*.mk \
  --exclude=$imp/router-filelist*.txt \
  --exclude=$imp_org/main/components/apps/wbd/cli \
  --exclude=$imp_org/main/components/apps/wbd/master \
  --exclude=$imp_org/main/components/apps/wbd/shared \
  --exclude=$imp_org/main/components/apps/wbd/slave \
"
...
if  [ $cus == "1" ]; then
  ...
  if  [ -e  wlan-router.$rver/$ver/src/wlan-all-src-$ver.tgz ]; then
     echo "Add BSD/LLTD/WBD executable files to wlan-all-src-$ver.tgz"
     ...
     tar --append --ignore-failed-read \
         --file=wlan-router.$rver/$ver/src/wlan-all-src-$ver.tar \
             bcmdrivers/broadcom/net/wl/$imp_org/main/src/router/bsd/bsd*
     tar --append --ignore-failed-read \
         --file=wlan-router.$rver/$ver/src/wlan-all-src-$ver.tar \
             bcmdrivers/broadcom/net/wl/$imp_org/main/src/router/lltd/lld2d*
     tar --append --ignore-failed-read \
         --file=wlan-router.$rver/$ver/src/wlan-all-src-$ver.tar \
             bcmdrivers/broadcom/net/wl/$imp_org/main/src/router/bshared/prebuilt/libbshared*
     # must do for building customer/release builds (bshared/src should not exist in the tarballs)
     ...
     tar --append --ignore-failed-read \
         --file=wlan-router.$rver/$ver/src/wlan-all-src-$ver.tar \
             bcmdrivers/broadcom/net/wl/$imp_org/main/components/apps/wbd/prebuilt \
             bcmdrivers/broadcom/net/wl/$imp_org/main/components/apps/wbd/www
