...
#if defined(CONFIG_BCM96836) && !defined(BCM_WFD)
...
uint32 chainidx_bitmap[WLAN_DEVICE_MAX]={0}; /* per phy interface */
...
void wl_txchainhandler_complete(int wl_radio_idx)
{
	...
	while ((chainIdx = get_32bitmap_pos(chainidx_bitmap[wl_radio_idx])) != -1) {
		...
		while ((fifo = get_16bitmap_pos(pt->prio_bitmap)) != -1) {
			...
			wl_write_to_chain_table(pktci, pt->idx, pChain->chead, pChain->ctail, fifo);
...
#if defined(BCM_WFD)
{
	...
	case PKTC_TBL_SET_STA_ASSOC:
		/* param0 is STA addr */
		pt = (wl_pktc_tbl_t *)PKTC_TBL_FN_LOOKUP(pktc_tbl, (uint8_t*)param0);
		if (pt != NULL) {
			pt->sta_assoc = param1;
		}
		return 0;
