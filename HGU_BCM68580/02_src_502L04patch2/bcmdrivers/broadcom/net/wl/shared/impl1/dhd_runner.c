	...
	dhd_runner_dma_buf_t buf_type, dhd_dma_buf_t *dma_buf)
{
	...
	if (buf_type & ATTACH_DMA_INDX_BUF) {
	    ...
	    if (ringid >= BCMPCIE_COMMON_MSGRINGS) { /* DHD_IS_FLOWRING */
	        ...
	        dma_buf->len = ntohs(rdpa_cache->items) * dhd_hlp->txpost_ring_cfg.size;
	...
	bool force_dhd)
{
	...
	ring_sz = flowmgr->profile->items[wme_ac] * flowmgr->dhd_hlp->txpost_ring_cfg.size;
	    ...
	    case H2R_MSGRING_FORMAT_NOTIF:
	            ...
	            (uint8)arg1, (uint32)arg2));
	        {
	            ...
	            switch (ring) {
	                ...
	                case BCMPCIE_D2H_MSGRING_CONTROL_COMPLETE:
	                    /* Runner does not need to registers control subn/cmplt rings */
	                    DHD_ERROR(("dhd%d_rnr: MSG Ring [%d] not supported by Runner\n",
	                ...
	                default:
	                    ...
	                    /*
	                     * Applying the same format for other ring types due to
	                     * runner limitation
	                     */
	                    dhd_hlp->rxpost_ring_cfg.type = type;
	                    dhd_hlp->txsts_ring_cfg.type = type;
	             ...
	             */
	            if (type && (dhd_hlp->rnr_sup_feat.msgringformat == 0)) {
	                DHD_ERROR(("dhd%d_rnr: Ring [%d] format [%d] not supported by Runner\n",
	                    dhd_hlp->dhd->unit, ring, type));
	            ...
	            if (dhd_hlp->txpost_ring_cfg.type != dhd_hlp->rnr_en_feat.msgringformat) {
	                dhd_hlp->dhd_init_cfg.flow_ring_format = type;
	                dhd_hlp->rnr_en_feat.msgringformat = type;
	                RLOG("dhd%d_rnr MSG Ring format set to %d",
	                    dhd_hlp->dhd->unit, type);
	            }
#endif /* RNR_DHD_HLPR_MSGRINGFRMT */
	        }
	        break;
...
dhd_runner_profile_init(dhd_pub_t *dhdp)
{
	...
	RLOG("%s: N+M profile = %1d %02d:%04d %02d:%04d %02d:%04d %02d:%04d %02d:%04d",
...
dhd_runner_policy_init(dhd_pub_t *dhdp)
{
	...
	memset(buff, 0, sizeof(buff));
	length = dhd_runner_key_get(dhdp->unit, DHD_RNR_KEY_POLICY, buff,
	    sizeof(buff));
	...
	int bufflen)
{
	...
	/* Status */
	bcm_bprintf(b, "\nDHD Runner: \n");
	bcm_bprintf(b, "  Status     : %s %s %s %s %s %s\n",
