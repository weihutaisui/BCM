 ...
 *
 * $Id: emf_linux.c 710528 2017-07-13 05:17:00Z $
 */
...
static void *
emf_find_ifp_by_name(emf_struct_t *emf, emf_info_t **emfi, uint8 *ifname)
{
	emf_info_t *emfi_ptr = *emfi;
	emf_iflist_t *iflist_ptr;

	ASSERT(ifname != NULL);

	/* Lock is already held by the callers */
	/* Search for interface in emf instance passed */
	for (iflist_ptr = emfi_ptr->iflist_head; iflist_ptr != NULL;
		iflist_ptr = iflist_ptr->next) {

		if (!strncmp(iflist_ptr->if_name, ifname, IFNAMSIZ)) {
			return (iflist_ptr->if_ptr);
		}
	}

	for (emfi_ptr = emf->list_head; emfi_ptr != NULL; emfi_ptr = emfi_ptr->next) {
		for (iflist_ptr = emfi_ptr->iflist_head; iflist_ptr != NULL;
			iflist_ptr = iflist_ptr->next) {
			if (!strncmp(iflist_ptr->if_name, ifname, IFNAMSIZ)) {
				*emfi = emfi_ptr;
...
emf_iflist_add(emf_struct_t *emf, emf_info_t *emfi, struct net_device *if_ptr)
{
	...
	if (emf_iflist_find(emf, emfi, if_ptr, &prev) == NULL)
	{
		...
		ptr->if_ptr = if_ptr;
		strncpy(ptr->if_name, if_ptr->name, IFNAMSIZ);
		ptr->if_name[IFNAMSIZ - 1] = '\0';
...
emf_cfg_request_process(emf_cfg_request_t *cfg)
{
	...
	switch (cfg->command_id)
	{
		...
		case EMFCFG_CMD_IF_DEL:
			...
			if (if_ptr == NULL)
			{
				/* Interface(Netdevice) might be deleted in linux stack.
				 * Search in emf database and delete interface.
				 */
				if (cfg->command_id == EMFCFG_CMD_IF_DEL) {
					if_ptr = emf_find_ifp_by_name(emf, &emfi, if_cfg->if_name);
