...
#ifndef _NGPON_COMMON_H_
#define _NGPON_COMMON_H_
...
{
	int addr_idx;
	for (addr_idx = 0; addr_idx < RU_NGPON_BLOCKS[block_idx]->addr_count; addr_idx++)
	{
		pr_info("block %s[%d] address = 0x%16lx\n", RU_NGPON_BLOCKS[block_idx]->name, addr_idx,
			RU_NGPON_BLOCKS[block_idx]->addr[addr_idx]);
	}
}
