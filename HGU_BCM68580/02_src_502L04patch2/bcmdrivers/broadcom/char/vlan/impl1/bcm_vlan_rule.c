                                  ...
                                  int cmdIndex)
{
    ...
    BCM_VLAN_FLOW_DEBUG("cmdIndex %d, tableEntry->cmdCount %ld",
                          ...
                          int isMulticast)
{
    ...
    for(i=0; i<BCM_VLAN_MAX_TAGS; ++i)
    {
        ...
        if((flowVlanTag->tciMask & BCM_VLAN_FLOW_COPY_PBITS) &&
           (!BCM_VLAN_IS_DONT_CARE(txVlanTag->pbits)))
        {
            ret = -EINVAL;
                          ...
                          struct net_device *txVlanDev)
{
    ...
    ret = createBlogRules(blog_p, &flowRuleTables);
