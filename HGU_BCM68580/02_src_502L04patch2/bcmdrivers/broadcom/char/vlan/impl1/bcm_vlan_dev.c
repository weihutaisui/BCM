...
#ifdef CONFIG_BLOG
static void bcmVlan_addBlogStats(struct rtnl_link_stats64 *dStats64_p,
...
static void bcmVlan_devCollectBlogStats(struct net_device * dev_p,
                                        struct rtnl_link_stats64 *dStats64_p)
{
    ...
    blog_unlock();
    /* Add the running blog stats */
    bcmVlan_addBlogStats(dStats64_p, &bStats);
    return;
}
...
void bcmVlan_devUpdateStats(struct net_device * dev_p, BlogStats_t *blogStats_p)
{
    ...
    /* Add the pushed blog stats to device counters */
    bcmVlan_addBlogStats(dStats_p, blogStats_p);
...
#ifdef CONFIG_BLOG
    bcmVlan_devCollectBlogStats(dev_p, dStats64_p);
#endif /* CONFIG_BLOG */
