...
static void add_l2_commands(rdpa_ip_flow_info_t *ip_flow, Blog_t *blog_p)
{
        ...
        && l2_header_need_llc_snap(blog_p->rnr.radio_idx))
    {
        ...
        ip_flow->result.action_vec |= rdpa_fc_action_llc_snap_set_len;
    }
#endif

    ip_flow->result.l2_header_number_of_tags = blog_p->vtag_num;
    ip_flow->result.l2_header_offset = blog_p->rx.length - blog_p->tx.length - bcmtaglen - l2_header_offset;
    ip_flow->result.l2_header_size = blog_p->tx.length + bcmtaglen;
    memcpy(&ip_flow->result.l2_header[l2_header_offset], blog_p->tx.l2hdr, bcmtaglocation);
    memcpy(&ip_flow->result.l2_header[l2_header_offset + bcmtaglocation + bcmtaglen], blog_p->tx.l2hdr + bcmtaglocation,
    ...
    if (is_ext_switch_port)
    {
        ...
        memcpy(&ip_flow->result.l2_header[l2_header_offset + bcmtaglocation], &bcmtag, bcmtaglen);

        protoDebug("<bcm chnl %d, bcmtag.dst %x>\n", blog_p->tx.info.channel, bcmtag.dst_port);
    }

#if defined(XRDP) && !defined(G9991)
    if (l2_header_offset) /* Need to add LLC Snap */
    {
        l2_header_insert_llc_snap(ip_flow->result.l2_header, blog_p);
        ip_flow->result.l2_header_size += L2_DOT11_LLC_SNAP_HDR_LEN;
    }
#endif
    ...
    else if (blog_p->tx.info.phyHdrType == BLOG_NETXLPHY)
    {
        ...
        ip_flow->result.wl_accel_type = RDPA_WL_ACCEL_WFD;
        ip_flow->result.wl_metadata = 0;
        ip_flow->result.wfd.nic_ucast.is_wfd = 1;
    }
}
