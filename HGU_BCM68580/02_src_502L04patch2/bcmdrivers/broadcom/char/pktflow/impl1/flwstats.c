...
#define BCM_FLWSTATS_POLL_INTERVAL_IN_SEC      1
#define BCM_FLWSTATS_GRANULARITY_IN_SEC        30
...
#if 0
int pushFlwStatsToDPI_Stub(uint32_t flwIdx, FlwStIf_t *flwStIf_p)
{
    printk("flwIdx %d bytes %lu packets %lu TS %lu\n", flwIdx, flwStIf_p->rx_bytes, flwStIf_p->rx_packets, flwStIf_p->pollTS_ms);
    return 0;
}
#endif
static int getDeltaFlwStats(uint32_t flwIdx, 
                            FlwStatsFc_t *flwStFc_p, 
                            FlwStats_t *deltaSt_p)
{
#if 0
    //Debug
    FlwStIf_t tmpFlwStIf;
#endif
    ...
    //printk("delta rx_p %ld rx_b %ld flwstats rx_p %ld rx_b %ld\n", 
    //deltaSt->rx_packets, deltaSt->rx_bytes, flwSt->rx_packets, 
    //flwSt->rx_bytes);
...
#if 0
    //Debug
    printk("getDeltaFlwStats() ");
    tmpFlwStIf.rx_packets = prevSt[flwIdx].rx_packets;
    tmpFlwStIf.rx_bytes   = prevSt[flwIdx].rx_bytes;
    tmpFlwStIf.pollTS_ms  = prevSt[flwIdx].pollTS_ms;
    flwStIf_request(FLWSTIF_REQ_PUSH, flwIdx, &tmpFlwStIf);
#endif	
...
static int flwStatsThread(void *thread_data)
{
    ...
    numFlwsPerPoll = maxFlwIdx / BCM_FLWSTATS_GRANULARITY_IN_SEC;
...
poll_delay:
        ...
        schedule_timeout(BCM_FLWSTATS_POLL_INTERVAL_IN_SEC * CONFIG_HZ);
    }
    return 0;
}

int flwStatsPushCallbk(uint32_t flwIdx, unsigned long ublog_p, unsigned long nReset)
{
    Blog_t *blog_p = (Blog_t *)ublog_p;
    FlwStatsFc_t flwStFc;
#ifdef CONFIG_BCM_DPI_MODULE
    FlwStIf_t    flwStIf;
#endif

    //No BLOG_LOCK needed as this function is called from fcache context

    memset(&flwStFc, 0, sizeof(flwStFc));
    if (getFlwStatsFn(flwIdx, (unsigned long)&blog_p, (unsigned long)&flwStFc) == 0)
    {
       // printk("flwStatsEvictCallbk flwIdx %d RxPkts %lu RxBytes %llu 
       // blog_p %p\n", flwIdx, flwSt.rx_packets, flwSt.rx_bytes, blog_p);
        flwStFc.pollTS_ms = jiffies_to_msecs(jiffies);
        updateStatsTable(flwIdx, blog_p, &flwStFc);
    }

#ifdef CONFIG_BCM_DPI_MODULE
    flwStIf.rx_bytes = flwStFc.rx_bytes;
    flwStIf.rx_packets = flwStFc.rx_packets;
    flwStIf.pollTS_ms  = flwStFc.pollTS_ms;

#if 0
    //Debug
    printk("pushFlwStatsToDPI() ");
#endif

    //Push Stats to DPI
    flwStIf_request(FLWSTIF_REQ_PUSH, (void *)blog_p->ct_p[BLOG_CT_PLD],
                    (unsigned long)blog_p->ct_p[BLOG_CT_DEL], (uint32_t)blog_p->nf_dir_pld, 
                    blog_p->nf_dir_del, &flwStIf);
#endif
...
/*
 *---------------------------------------------------------------------------
 * Function Name: flwStatsGetByFlwIdx
 * Description  : Return flwstats based on Flow Idx. Used by DPI module.
 * Returns      : None
 *---------------------------------------------------------------------------
 */
int flwStatsGetByFlwIdx(uint32_t flwIdx, FlwStIf_t *pFlwStIf)
{
    FlowKey_t key = {};
    Blog_t *blog_p;
    FlwStatsFc_t flwStatsFc;
    ...
    /* Is the flwIdx within valid range? */
    if(key.id.fc.self >= fcache_max_ent())
    {
    ...
    /* Protect against flowstats thread call to get stats */
    BLOG_LOCK_BH();
    if (getFlwStatsFn(key.id.fc.self, (unsigned long)&blog_p, (unsigned long)&flwStatsFc) != 0)
    {
        printk("getFlwStatsFn failed\n");
    }
    BLOG_UNLOCK_BH();

    /* We have a valid query.  Copy stats info to the return structure */
    pFlwStIf->rx_packets = flwStatsFc.rx_packets;
    pFlwStIf->rx_bytes   = flwStatsFc.rx_bytes;		
    pFlwStIf->pollTS_ms  = jiffies_to_msecs(jiffies);
...
int flwStatsInit(void)
{
    ...
    fc_flwstats_bind(&getFlwStatsFn, (HOOK3PARM)flwStatsPushCallbk);

    flwStIf_bind(flwStatsGetByFlwIdx, NULL);

#if 0	
    //Debug
    flwStIf_bind(flwStatsGetByFlwIdx, pushFlwStatsToDPI_Stub);
#endif
