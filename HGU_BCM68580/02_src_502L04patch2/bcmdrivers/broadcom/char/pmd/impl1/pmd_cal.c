...
int set_invalid(pmd_calibration_param cal_param)
{
    ...
    switch (cal_param)
    {
        ...
        case pmd_sat_neg:
            ...
            break;
        case pmd_edge_rate:
            calibration_file.edge_rate.valid[cal_index] = 0;
            break;
        case pmd_preemphasis_weight:
            calibration_file.preemphasis_weight.valid[cal_index] = 0;
            break;
        case pmd_preemphasis_delay:
            calibration_file.preemphasis_delay.valid[cal_index] = 0;
            break;
        case pmd_duty_cycle:
...
int pmd_cal_param_set(pmd_calibration_param cal_param, int32_t val, uint16_t set_index)
{
    ...
    case pmd_sat_neg:		
        ...
        break;
    case pmd_edge_rate:
        calibration_file.edge_rate.val[set_index] = val_word;
        calibration_file.edge_rate.valid[set_index] = 1;
        break;
    case pmd_preemphasis_weight:
        calibration_file.preemphasis_weight.val[set_index] = val_word;
        calibration_file.preemphasis_weight.valid[set_index] = 1;
        break;
    case pmd_preemphasis_delay:
        calibration_file.preemphasis_delay.val[set_index] = val;
        calibration_file.preemphasis_delay.valid[set_index] = 1;
        break;
    case pmd_duty_cycle:
...
int pmd_cal_param_get(pmd_calibration_param cal_param, int32_t *val, uint16_t get_index)
{
    ...
    case pmd_sat_neg:				
        *val = calibration_file.sat_neg.val;
        ...
        break;
    case pmd_edge_rate:
        *val = calibration_file.edge_rate.val[get_index];
        valid = calibration_file.edge_rate.valid[get_index];
        break;
    case pmd_preemphasis_weight:
        *val = calibration_file.preemphasis_weight.val[get_index];
        valid = calibration_file.preemphasis_weight.valid[get_index];
        break;
    case pmd_preemphasis_delay:
        *val = calibration_file.preemphasis_delay.val[get_index];
        valid = calibration_file.preemphasis_delay.valid[get_index];
        break;
    case pmd_duty_cycle:
...
int pmd_cal_is_config_valid(pmd_calibration_param *pmd_cal_param_valid, uint8_t size)
{
    ...
    while (index < size)
    {
        ...
        {
            case pmd_file_watermark:
                if (!(calibration_file.watermark.valid))
                    goto exit_error_print;
                break;
            case pmd_file_version:
                if (!(calibration_file.version.valid))
                    goto exit_error_print;
                break;
            case pmd_level_0_dac:
                if (!(calibration_file.level_0_dac.valid))
                    goto exit_error_print;
                break;
            case pmd_level_1_dac:
                if (!(calibration_file.level_1_dac.valid))
                    goto exit_error_print;
                break;
            case pmd_bias:
                if (!(calibration_file.bias.valid))
                    goto exit_error_print;
                break;
            case pmd_mod:
                if (!(calibration_file.mod.valid))
                    goto exit_error_print;
                break;
            case pmd_apd:
                if (!(calibration_file.apd.valid))
                    goto exit_error_print;
                break;
            case pmd_mpd_config:
                if (!(calibration_file.mpd_config.valid))
                    goto exit_error_print;
                break;
            case pmd_mpd_gains:
                if (!(calibration_file.mpd_gains.valid))
                    goto exit_error_print;
                break;
            case pmd_apdoi_ctrl:
                if (!(calibration_file.apdoi_ctrl.valid))
                    goto exit_error_print;
                break;
            case pmd_rssi_a:
                if (!(calibration_file.rssi_a_cal.valid))
                    goto exit_error_print;
                break;
            case pmd_rssi_b:
                if (!(calibration_file.rssi_b_cal.valid))
                    goto exit_error_print;
                break;
            case pmd_rssi_c:
                if (!(calibration_file.rssi_c_cal.valid))
                    goto exit_error_print;
                break;
            case pmd_temp_0:
                if (!(calibration_file.temp_0.valid))
                    goto exit_error_print;
                break;
            case pmd_temp_coff_h:
                if (!(calibration_file.coff_h.valid))
                    goto exit_error_print;
                break;
            case pmd_temp_coff_l:
                if (!(calibration_file.coff_l.valid))
                    goto exit_error_print;
                break;
            case pmd_esc_thr:
                if (!(calibration_file.esc_th.valid))
                    goto exit_error_print;
                break;
            case pmd_rogue_thr:
                if (!(calibration_file.rogue_th.valid))
                    goto exit_error_print;
                break;
            case pmd_avg_level_0_dac:
                if (!(calibration_file.avg_level_0_dac.valid))
                    goto exit_error_print;
                break;
            case pmd_avg_level_1_dac:
                if (!(calibration_file.avg_level_1_dac.valid))
                    goto exit_error_print;
                break;
            case pmd_dacrange:
                if (!(calibration_file.dacrange.valid))
                    goto exit_error_print;
                break;
            case pmd_los_thr:
                if (!(calibration_file.los_thr.valid))
                    goto exit_error_print;
                break;
            case pmd_sat_pos:
                if (!(calibration_file.sat_pos.valid))
                    goto exit_error_print;
                break;
            case pmd_sat_neg:
                if (!(calibration_file.sat_neg.valid))
                    goto exit_error_print;
                break;
            case pmd_edge_rate:
                if (!(calibration_file.edge_rate.valid[cal_index]))
                    goto exit_error_print;
                break;
            case pmd_preemphasis_weight:
                if (!(calibration_file.preemphasis_weight.valid[cal_index]))
                    goto exit_error_print;
                break;
            case pmd_preemphasis_delay:
                if (!(calibration_file.preemphasis_delay.valid[cal_index]))
                    goto exit_error_print;
                break;
            case pmd_duty_cycle:
                if (!(calibration_file.duty_cycle.valid))
                    goto exit_error_print;
                break;
            case pmd_mpd_calibctrl:
                if (!(calibration_file.calibctrl.valid))
                    goto exit_error_print;
                break;
            case pmd_tx_power:
                if (!(calibration_file.tx_power.valid))
                    goto exit_error_print;
                break;
            case pmd_bias0:
                if (!(calibration_file.bias0.valid))
                    goto exit_error_print;
                break;
            case pmd_temp_offset:
                if (!(calibration_file.temp_offset.valid))
                    goto exit_error_print;
                break;
            case pmd_bias_delta_i:
                if (!(calibration_file.bias_delta_i.valid))
                    goto exit_error_print;
                break;
            case pmd_slope_efficiency:
                if (!(calibration_file.slope_efficiency.valid))
                    goto exit_error_print;
                break;
            default:
                BCM_LOG_ERROR(BCM_LOG_ID_PMD, "Read of calibration param number %d not supported \n",(uint32_t)pmd_cal_param_valid[index]);
                return -1;
                break;
        }
        index++;
...
int pmd_cal_file_init(void)
{
    ...
    switch (pmd_board_type)
    {
        case pmd_xgpon1_10_2_board:
        {
            cal_index = US_2G_INDEX;
            break;
        }
        case pmd_epon_2_1_board:
        case pmd_gpon_2_1_board:
        case pmd_epon_1_1_board:
        case pmd_gbe_1_1_board:
        case pmd_epon_10_1_board:
        {
        ...
        default:
        {
            printk("pmd: unknown board_type (%d)in cal init", pmd_board_type);
            break;
        }
    }
