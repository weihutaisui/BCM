...
UINT32 extBondingDbgFlag = 0;	//TO DO: Remove after debugging
#endif
...
extern void AdslCoreSetTime(UINT32 timeMs);
...
#ifdef SUPPORT_DSL_BONDING
INT32			adslCoreXmtGain [MAX_DSL_LINE] = {ADSL_XMT_GAIN_AUTO, ADSL_XMT_GAIN_AUTO};
Bool			adslCoreXmtGainChanged[MAX_DSL_LINE] = {AC_FALSE, AC_FALSE};
#else
INT32			adslCoreXmtGain	[MAX_DSL_LINE] = {ADSL_XMT_GAIN_AUTO};
...
#endif
#ifdef SUPPORT_DSL_BONDING
UINT32			adslCoreHsModeSwitchTime[MAX_DSL_LINE] = {0, 0};
#else
UINT32			adslCoreHsModeSwitchTime[MAX_DSL_LINE] = {0};
#endif
...
#else
uintptr_t		gPhyPCAddr = HOST_LMEM_BASE + 0x5B0;
#endif
...
struct {
	UINT32		rcvCtrl, rcvAddr, rcvPtr;
	UINT32		xmtCtrl, xmtAddr, xmtPtr;
} adslCoreDiagState;
...
UINT32			bkupSdramSize = 0;
UINT32			bkupLmemSize = 0;
...
void BcmXdslCoreSendStatToExtBondDev(struct sk_buff *skbDiag, UINT32 cmd, char *buf0, int len0, char *buf1, int len1);
...
void BcmXdslDiagStatSaveLocal(UINT32 cmd, char *statStr, int n, char *p1, int n1);
...
UINT32 BcmAdslCoreStatusSnooper(dslStatusStruct *status, char *pBuf, int len);
//void BcmAdslCoreAfeTestStatus(dslStatusStruct *status);
void BcmAdslCoreSendBuffer(UINT32 statusCode, uchar *bufPtr, UINT32 bufLen);
void BcmAdslCoreSendDmaBuffers(UINT32 statusCode, int bufNum);
...
void BcmAdslCoreDebugSendCommand(char *fileName, ushort cmd, UINT32 offset, UINT32 len, char *bufAddr);
...
static ulong	spinLockNestedCallEnter[NR_CPUS ] = {0};
static ulong	spinLockCalledEnter[NR_CPUS ] = {0};
static ulong	spinLockNestedCallExit[NR_CPUS ] = {0};
static ulong	spinLockCalledExit[NR_CPUS ] = {0};
#endif
...
static UINT32 flattenCmdBuf[CMD_BUF_SIZE>>2];
static UINT32 xdslStatusBuffer[STAT_BUF_SIZE>>2];
...
void BcmXdslCoreProcessStatFromExtBondDev(EXT_BONDING_COMM_BufferDesc *pBufDesc, void *devContext)
{
	...
	switch(logCommmand) {
		...
		case statusInfoData-1:
			...
			int unflattenLen = UnflattenStatus((UINT32 *)(pBuf+1), &status);
...
void BcmXdslCoreProcessCmdFromExtBondDev(EXT_BONDING_COMM_BufferDesc *pBufDesc, void *devContext)
{
	...
	UnflattenCommand((UINT32 *)EXT_BONDING_COMM_getData(pBufDesc), &cmd);
...
void BcmXdslCoreSendStatToExtBondDev(struct sk_buff *skbDiag, UINT32 cmd, char *buf0, int len0, char *buf1, int len1)
{
	...
	if(statusInfoData == pLogProtHdr->logCommmand) {
		UINT32 *pStatCode = (UINT32 *)(pLogProtHdr+1);
...
typedef struct _xdslSearchEnt {
	...
	UINT32	searchTime;	/* time used so far */
	UINT32	rxAtten;		/* Rx attenuation */
} xdslSearchEnt;
...
LOCAL void BcmXdslCoreSetDefaultMediaType(INT32 bcmMediaType)
{
...
void BcmXdslCoreMediaSearchSM(xdslMediaSearchEvent mediaSearchEvent, UINT32 param)
{
	xdslMedia curMedia;
		...
		case MEDIASEARCH_DONE_S:
			switch(mediaSearchEvent) {
				...
				case MEDIASEARCH_RXLNATTNAVG_E:
					...
					BCMOS_EVENT_LOG((KERN_CRIT TEXT("%s"), str));
#ifdef CONFIG_BCM963268
					if(!ADSL_PHY_SUPPORT(kAdslPhyBonding) && ((UINT32)-1 != MEDIA_RX_LNATTNAVG(curMedia)) && (param > (MEDIA_RX_LNATTNAVG(curMedia) + 640))) {
...
AC_BOOL BcmXdslCoreProcessMediaSearchCfgCmd(UINT32 mediaSrchCfg, AC_BOOL phySwitchOnly)
{
	int newVal, curVal;
...
*/
LOCAL void BcmXdslDiagFirstSplitStatSaveLocal(UINT32 cmd, int totalLen, void *statStr, int n, void *p1, int n1)
{
#ifdef SUPPORT_DSL_BONDING
	UINT32 lineId;
#endif
...
#else
Boolean IsValidAfeId(UINT32 afeId)
{
	UINT32  n;
...
void BcmXdslCoreSendAfeInfo(int toPhy)
{
	...
	(defined(CONFIG_BCM963381) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM963158))
...
{
	dslStatusStruct status;
	INT32 *pPhyInfo = (INT32 *)AdslCoreGetPhyInfo();
#ifdef ADSLDRV_LITTLE_ENDIAN
	adslPhyInfo phyInfo;
	BlockLongMoveReverse(4, pPhyInfo, (INT32 *)&phyInfo);	/* sdramPageAddr - sdramPhyImageAddr */
	BlockShortMoveReverse(4,(short *)(pPhyInfo+4), (short *)(((INT32 *)&phyInfo)+4));	/* fwType - mnVerNum */
	BlockLongMoveReverse(4, pPhyInfo+7, ((INT32 *)&phyInfo)+7);	/* features[0 -3] */
	pPhyInfo = (INT32 *)&phyInfo;
#endif
		...
		)
	{
		...
		if ((tMs > noStatTimeout) && (tPingMs > 1000)) {
			...
			if (tMs > (noStatTimeout + 5000)) {
				...
				for(i = 0; i < 20; i++) {
					BcmAdslCoreDiagWriteStatusString(0, "PC = 0x%08x\n", ADSL_ENDIAN_CONV_INT32(*(volatile UINT32 *)gPhyPCAddr)); // (HOST_LMEM_BASE + 0x5E8)
...
#ifdef _NOOS
UINT32 BcmAdslCoreCalibrate(void)
{
...
ulong BcmAdslCoreCalibrate(void)
{
	...
	//AdslDrvPrintf(TEXT("%s: Start calibrating adslCoreCyclesPerMs\n"), __FUNCTION__);
#else
	//AdslDrvPrintf(TEXT("%s: Start calibrating adslCoreCyclesPerMs\n"), __FUNCTION__);
	bcmOsGetTime(&tick1);
	do {
...
#ifndef _NOOS
void BcmAdslCoreSetWdTimer(UINT32 timeUs)
{
#ifdef CONFIG_BCM963158
	WDTIMER0->WatchDogDefCount = timeUs * (FPERIPH/1000000);
...
void __BcmAdslCoreStart(int diagDataMap, Bool bRestoreImage)
{
	...
	gPhyPCAddr = (uintptr_t)HOST_LMEM_BASE + 0x5B0;
	adslCoreResetPending = AC_FALSE;
	bcmOsGetTime(&statLastTick);
...
UINT32 BcmAdslCoreStatusSnooper(dslStatusStruct *status, char *pBuf, int len)
{
	...
	switch (statusCode) {
		...
		case kDslExceptionStatus:
			...
			sp = pStackPtr;
#else
			sp = (UINT32*) status->param.dslException.sp;
#endif
			for (i = 0; i < 28; i += 4)
			...
			sp = (UINT32*) status->param.dslException.argv;
			...
			sp = (UINT32 *)ADSL_ADDR_TO_HOST(spAddr);
			stackSize = 64;
#else
			sp = (UINT32 *) status->param.dslException.stackPtr;
		...
		case kDslConnectInfoStatus:
			...
			switch (status->param.dslConnectInfo.code) {
				...
				case kDslATUAvgLoopAttenuationInfo:
					BcmXdslCoreMediaSearchSM(MEDIASEARCH_RXLNATTNAVG_E, (UINT32)val << 4);
					break;
#if 0 && defined(CONFIG_BCM963268)
				case kDslFramingModeInfo:
					if(ADSL_PHY_SUPPORT(kAdslPhyBonding) && (val & kAtmHeaderCompression)) {
						INT32 otherBcmMediaType = (0 == lineId) ? BCM_MEDIATYPE_EXTERNALAFE: BCM_MEDIATYPE_INTERNALAFE;
		...
		case kDslTrainingStatus:
			...
			switch (status->param.dslTrainingInfo.code) {
				...
				{
#ifdef CONFIG_BCM_DSL_GFAST
					INT32 modCfg = adslCoreCfgProfile[0].adslAnnexAParam;
...
***************************************************************************/
void *BcmCoreAllocLowMem(UINT32 size)
{
		...
		if ((lmAddrPhys & 0x00200000) | ((lmAddrPhys+size) & 0x00200000)) {
			...
			for (i = 0; i < PAD_BLK_NUM; i++) {
			  ...
			  DiagStrPrintf(0,DIAG_DSL_CLIENT, "dma_alloc_coherent[%d]: p=0x%08lX, sz=%d, pAddr=0x%08lX\n", i, 
			    (UINT32) padAddr[i], PAD_BLK_SIZE, (UINT32) padAddrPhys[i]);
			  if (NULL == padAddr[i]) {
				i--;
				break;
			  }
...
#endif
#ifdef PRINT_INTR_STATUS
	volatile UINT32 *pAdslEnum = (UINT32 *) XDSL_ENUM_BASE;
#endif
	adslCoreIntrCnt++;

#if defined(PRINT_INTR_STATUS)
	printk("Intr 1: IntrCnt=%lu sch=%d INTSTATUS=0x%08X INTMASK_I=0x%08X\n",
		adslCoreIntrCnt, dpcScheduled, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
...
#endif
#if defined(PRINT_INTR_STATUS)
	printk("Intr 2: IntrCnt=%lu sch=%d INTSTATUS=0x%08X INTMASK_I=0x%08X\n",
		adslCoreIntrCnt, dpcScheduled, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
#ifdef LINUX_KERNEL_2_6
	return IRQ_HANDLED;
...
#endif
#if defined(PRINT_INTR_STATUS)
	printk("Dpc 1: sch=%d INTSTATUS=0x%08X INTMASK=0x%08X\n",
		dpcScheduled, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
...
#if defined(PRINT_INTR_STATUS)
		printk("Dpc 11: INTSTATUS=0x%08X INTMASK=0x%08X\n", pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
		bcmOsSemGive(syncPhyMipsSemId);
#if !defined(__ECOS)
		dpcScheduled=AC_FALSE;
...
#if defined(PRINT_INTR_STATUS)
		printk("Dpc 12: INTSTATUS=0x%08X INTMASK=0x%08X\n", pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
#if defined(CONFIG_SMP) && defined(__KERNEL__)
		BcmCoreDpcSyncExit();
#endif
#if !defined(BCM_CORE_TEST) && (defined(__KERNEL__) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0))) && !defined(KERNEL_UNMASK_DSL_INTR) && !defined(CONFIG_ARM) && !defined(CONFIG_ARM64)
		BcmHalInterruptEnable(INTERRUPT_ID_ADSL);
#endif
		return;
	}
...
#endif
#if defined(PRINT_INTR_STATUS)
	printk("Dpc 2: sch=%d INTSTATUS=0x%08X INTMASK=0x%08X\n",
		dpcScheduled, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
}
#endif /* !_NOOS */
...
int BcmAdslCoreLogSaveAdsl2Capabilities(INT32 *cmdData, g992p3DataPumpCapabilities *pCap)
{
	cmdData[0]  = (INT32)pCap->rcvNTREnabled;
	cmdData[1]  = (INT32)pCap->shortInitEnabled;
	cmdData[2]  = (INT32)pCap->diagnosticsModeEnabled;
	cmdData[3]  = (INT32)pCap->featureSpectrum;
	cmdData[4]  = (INT32)pCap->featureOverhead;
	cmdData[5]  = (INT32)pCap->featureTPS_TC[0];
	cmdData[6]  = (INT32)pCap->featurePMS_TC[0];
	cmdData[7]  = (INT32)pCap->featureTPS_TC[1];
	cmdData[8]  = (INT32)pCap->featurePMS_TC[1];
	cmdData[9]  = (INT32)pCap->featureTPS_TC[2];
	cmdData[10] = (INT32)pCap->featurePMS_TC[2];
	cmdData[11] = (INT32)pCap->featureTPS_TC[3];
	cmdData[12] = (INT32)pCap->featurePMS_TC[3];
	cmdData[13] = (INT32)pCap->readsl2Upstream;
	cmdData[14] = (INT32)pCap->readsl2Downstream;
	cmdData[15] = (INT32)pCap->sizeIDFT;
	cmdData[16] = (INT32)pCap->fillIFFT;
	cmdData[17] = (INT32)pCap->minDownOverheadDataRate;
	cmdData[18] = (INT32)pCap->minUpOverheadDataRate;
	cmdData[19] = (INT32)pCap->maxDownATM_TPSTC;
	cmdData[20] = (INT32)pCap->maxUpATM_TPSTC;
	cmdData[21] = (INT32)pCap->minDownATM_TPS_TC[0];
	cmdData[22] = (INT32)pCap->maxDownATM_TPS_TC[0];
	cmdData[23] = (INT32)pCap->minRevDownATM_TPS_TC[0];
	cmdData[24] = (INT32)pCap->maxDelayDownATM_TPS_TC[0];
	cmdData[25] = (INT32)pCap->maxErrorDownATM_TPS_TC[0];
	cmdData[26] = (INT32)pCap->minINPDownATM_TPS_TC[0];
	cmdData[27] = (INT32)pCap->minUpATM_TPS_TC[0];
	cmdData[28] = (INT32)pCap->maxUpATM_TPS_TC[0];
	cmdData[29] = (INT32)pCap->minRevUpATM_TPS_TC[0];
	cmdData[30] = (INT32)pCap->maxDelayUpATM_TPS_TC[0];
	cmdData[31] = (INT32)pCap->maxErrorUpATM_TPS_TC[0];
	cmdData[32] = (INT32)pCap->minINPUpATM_TPS_TC[0];
	cmdData[33] = (INT32)pCap->maxDownPMS_TC_Latency[0];
	cmdData[34] = (INT32)pCap->maxUpPMS_TC_Latency[0];
	return 35;
}
#endif

#ifdef G993
int BcmAdslCoreLogSaveVdsl2Capabilities(INT32 *cmdData, g993p2DataPumpCapabilities *pCap)
{
	cmdData[0]  = (INT32)pCap->verId;
	cmdData[1]  = (INT32)pCap->size;
	cmdData[2]  = (INT32)pCap->profileSel;
	cmdData[3]  = (INT32)pCap->maskUS0;
	cmdData[4]  = (INT32)pCap->cfgFlags;
	cmdData[5]  = (INT32)pCap->maskEU;
	cmdData[6]  = (INT32)pCap->maskADLU;
	return 7;
}
#endif
...
{
	INT32	cmdData[120];	/* Worse case was 100 ulongs */
	int		n;
...
UINT32 BcmXdslCoreGetPhyExtraCfg(unsigned char lineId, unsigned char phyExtraCfgNum)
{
...
void BcmXdslCoreSetPhyExtraCfg(unsigned char lineId, unsigned char phyExtraCfgNum, UINT32 phyExtraCfg)
{
	dslExtraCfgCommand  *pExtraCfg;
...
void BcmXdslCoreClearPhyExtraCfg(unsigned char lineId, unsigned char phyExtraCfgNum, UINT32 phyExtraCfg)
{
	dslExtraCfgCommand  *pExtraCfg;
...
void BcmXdslCoreSendCmd(unsigned char lineId, UINT32 cmd, UINT32 value)
{
...
#if defined(CONFIG_SMP) && defined(__KERNEL__)
	printk("%s: NR_CPUS = %d\n", __FUNCTION__, NR_CPUS);	//TO DO: TONY - Remove after debugging
	spin_lock_init(&gSpinLock);
	for(i=0; i<NR_CPUS; i++) {
		spinCount[i ] = 0;
#ifdef SPINLOCK_CALL_STAT
		spinLockNestedCallEnter[i] = 0;
		spinLockCalledEnter[i ] = 0;
		spinLockNestedCallExit[i] = 0;
		spinLockCalledExit[i] = 0;
#endif
	}
#endif
...
#if defined(PRINT_INTR_STATUS)
	AdslDrvPrintf(TEXT("%s 1: INTSTATUS_I=0x%08X INTMASK_I=0x%08X\n"),
		__FUNCTION__, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
	/* Clear pending status interrupt, and enable it */
#ifdef CONFIG_BCM963158
	tmp = pAdslEnum[ADSL_Core2HostMsg];
...
#endif
#if defined(PRINT_INTR_STATUS)
	AdslDrvPrintf(TEXT("%s 2: INTSTATUS_I=0x%08X INTMASK_I=0x%08X\n"),
		__FUNCTION__, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
#ifndef XDSL_DRV_STATUS_POLLING
	pAdslEnum[ADSL_INTMASK_I] |= MSGINT_MASK_BIT;
#if defined(PRINT_INTR_STATUS)
	AdslDrvPrintf(TEXT("%s 3: INTSTATUS_I=0x%08X INTMASK_I=0x%08X\n"),
		__FUNCTION__, pAdslEnum[ADSL_INTSTATUS_I], pAdslEnum[ADSL_INTMASK_I]);
#endif
#if defined(__KERNEL__)
	bcmOsDpcEnqueue(irqDpcId);
...
void BcmAdslCoreStartBERT(unsigned char lineId, UINT32 totalBits)
{
	dslCommandStruct	cmd;
#ifdef SUPPORT_DSL_BONDING
...
void BcmAdslCoreBertStartEx(unsigned char lineId, UINT32 bertSec)
{
	BcmCoreDpcSyncEnter();
...
extern UINT32 gFastTestModeReq;
#endif
...
#ifdef  ADSLDRV_LITTLE_ENDIAN
			BlockLongMoveReverse(5, (INT32 *)cmd.param.dslClearEocMsg.dataPtr, (INT32 *)cmd.param.dslClearEocMsg.dataPtr);
#endif
			AdslCoreCommandHandler(&cmd);
			gSharedMemAllocFromUserContext=0;
...
void BcmAdslCoreSetTestExecutionDelay(unsigned char lineId, int testMode, UINT32 value)
{
	dslCommandStruct	cmd;
...
Bool BcmAdslCoreG997SendData(unsigned char lineId, int eocMsgType, void *buf, int len)
{
	...
	BcmCoreDpcSyncEnter();
	bRet = AdslCoreG997SendData(lineId, eocMsgType, buf, len);
	BcmCoreDpcSyncExit();
	return bRet;
}
...
void BcmAdslCoreNotify(unsigned char lineId, INT32 acEvent)
{
...
#if 0
void AddVcTest(UINT32 vci)
{
	BcmAdslCoreAtmAddVc(vci >> 16, vci & 0xFFFF);
}

void DelVcTest(UINT32 vci)
{
...
void BcmAdslCoreDiagWriteStatus(dslStatusStruct *status, char *pBuf, int len)
{
	...
	switch (DSL_STATUS_CODE(status->code)) {
	  case kDslExceptionStatus:
		...
		sp = (UINT32*) status->param.dslException.sp;
#endif
		sp = (UINT32 *) (uintptr_t)ADSL_ENDIAN_CONV_UINT32(sp[28]);
#ifdef FLATTEN_ADDR_ADJUST
		if (((uintptr_t)sp & 0x50000000) == 0x50000000)
			sp = (UINT32 *) ((uintptr_t)sp & ~0x40000000);
		sp = (UINT32 *)(ADSL_ADDR_TO_HOST((uintptr_t)sp));
#endif
		p1 = (void *) sp;
		n1 = 64 * sizeof(INT32);
		}
	  ...
	  case kDslConnectInfoStatus:
		{
		UINT32	bufLen = status->param.dslConnectInfo.value;
...
static INT32 f2DecI(INT32 val, int q)
{
   return (val/q);
}

static INT32 f2DecF(INT32 val, int q)
{
   int      sn = val >> 31;
   return (((val ^ sn) - sn) % q);
}
...
#endif
extern void DiagUnflattenStatus1(INT32 *pFlatStatus, int statusLen, void *pStat, void *pOutBuf, BOOL diagClientLE);
...
void BcmAdslCoreDiagStatusSnooper(dslStatusStruct *status, char *pBuf, int len)
{
	...
	parsedBuf[0] = 0;
	DiagUnflattenStatus1((INT32 *)pBuf, len, (void *)status, parsedBuf, 0);
	parsedStrLen = StatusParser((modemStatusStruct *)status, parsedBuf);
	if(parsedStrLen > 0) {
		...
		case LOG_CMD_MIB_GET1:
		{
			...
			if (kAdslMibStatusNoObject != res) {
				DiagWriteString(lineId, DIAG_DSL_CLIENT, "LOG_CMD_MIB_GET1: filename=%s dataLen=%d 0x%X 0x%X 0x%X\n",
					pName, size,*(UINT32*)res,*((UINT32*)res+1),*((UINT32*)res+2));
...
_cmd_reset:
		...
		case LOG_CMD_TEST_DATA:
			adslCoreAlwaysReset = (0 != *(INT32*) pCmdData) ? AC_TRUE : AC_FALSE;
			break;
		...
		case LOG_CMD_BERT_EX:
			BcmAdslCoreBertStartEx(lineId, *(INT32*)pCmdData);
			break;
#ifdef SUPPORT_XDSLDRV_GDB
		case LOG_CMD_GDB:
		{
		...
		case (kDslIdleXmtCmd - kFirstDslCommandCode):
			cmd.param.value = *(INT32*)pCmdData;
			/* fall through */
        ...
        case (kDslSeltConfiguration - kFirstDslCommandCode):
            {
                ...
                {
                    pMibInfo->selt.seltCfg = *(UINT32 *)pCmdData;
		...
		case (kDslPLNControlCmd - kFirstDslCommandCode):
			...
			cmd.param.dslPlnSpec.plnCmd = ((INT32*)pCmdData)[0];
			if (kDslPLNControlStart == cmd.param.dslPlnSpec.plnCmd) {
				cmd.param.dslPlnSpec.mgnDescreaseLevelPerBin = ((INT32*)pCmdData)[1];
				cmd.param.dslPlnSpec.mgnDescreaseLevelBand   = ((INT32*)pCmdData)[2];
			}
			gSharedMemAllocFromUserContext=1;
			if (kDslPLNControlDefineInpBinTable == cmd.param.dslPlnSpec.plnCmd) {
				cmd.param.dslPlnSpec.nInpBin = ((UINT32*)pCmdData)[3];
			...
			if (kDslPLNControlDefineItaBinTable == cmd.param.dslPlnSpec.plnCmd) {
				cmd.param.dslPlnSpec.nItaBin = ((INT32*)(uintptr_t)pCmdData)[5];
...
void BcmAdslCoreAfeTestInit(afeTestData *pAfe, void *pSdram, UINT32 size)
{
#ifdef CONFIG_ARM64
...
void BcmAdslCoreAfeTestAck (afeTestData	*pAfeData, UINT32 frNum, UINT32 frRcv)
{
	dslStatusStruct		status;
...
void BcmAdslCoreResetPhy(int copyImage)
{
	...
	if(!copyImage) {
		...
		pSdramImageAddr = AdslCoreSetSdramImageAddr(ADSL_ENDIAN_CONV_INT32(((UINT32*)HOST_LMEM_BASE)[2]), ADSL_ENDIAN_CONV_INT32(((UINT32*)HOST_LMEM_BASE)[3]), bkupSdramSize);
...
void BcmAdslCoreAfeTestMsg(void *pMsg)
{
	...
	if ( ((UINT32) -1 == pCmd->param.dslAfeTestSpec.afeParamPtr) &&
		 ((UINT32) -1 == pCmd->param.dslAfeTestSpec.imagePtr) ) {
		...
		switch (pCmd->param.dslAfeTestSpec.type) {
			...
			case kDslAfeTestLoadMemDump:
				...
				if((TRUE == pBkupImageEnable) && (0 != afeParam.size) && (0 != afeImage.size)) {
					UINT32 combineImageLen = afeParam.size+afeImage.size;
	...
	if (pCmd->param.dslAfeTestSpec.afeParamSize != 0) {
...
#endif
		pAfeData = &afeParam;
		frNum = (UINT32)(uintptr_t) (pCmd->param.dslAfeTestSpec.afeParamPtr) & 0xFF;
		...
		if (NULL == pAfeData->pSdram) {
			pAfeData->pSdram = AdslCoreSetSdramImageAddr(ADSL_ENDIAN_CONV_INT32(((UINT32*)HOST_LMEM_BASE)[2]), ADSL_ENDIAN_CONV_INT32(((UINT32*)HOST_LMEM_BASE)[3]), pAfeData->size);
			if(NULL == pAfeData->pSdram) {
		...
		frNum = (UINT32) (uintptr_t)(pCmd->param.dslAfeTestSpec.imagePtr) & 0xFF;
...
void BcmAdslCoreSendBuffer(UINT32 statusCode, uchar *bufPtr, UINT32 bufLen)
{
	UINT32				n, frCnt, dataCnt, dataSize;
...
#endif
UINT32 BcmAdslCoreGetConfiguredMod(uchar lineId)
{
...
void  BcmAdslCoreDebugReadFile(char *fileName, UINT32 readCmd, uchar *bufPtr, UINT32 bufLen);
void  __BcmAdslCoreDebugReadFileCached(char *fileName, UINT32 readCmd, uchar *bufPtr, UINT32 bufLen);
void  BcmAdslCoreDebugReadFileCached(char *fileName, UINT32 readCmd, uchar *bufPtr, UINT32 bufLen);
#ifdef READ_FILE_CACHE_SIZE
void  (*bcmDebugReadFileFunc)(char *fName, UINT32 rcmd, uchar *pBuf, UINT32 bLen) = BcmAdslCoreDebugReadFileCached;
#else
void  (*bcmDebugReadFileFunc)(char *fName, UINT32 rcmd, uchar *pBuf, UINT32 bLen) = BcmAdslCoreDebugReadFile;
#endif

char	*dirFileName = NULL;
UINT32	dirFileOffset = 0;
...
void BcmAdslCoreDebugWriteFile(char *fileName, char cmd, uchar *bufPtr, UINT32 bufLen)
{
	BcmAdslCoreDebugSendCommand(fileName, cmd, 0, bufLen, bufPtr);
	BcmAdslCoreSendBuffer(kDslDataAvailStatus, bufPtr, bufLen);
}

void BcmAdslCoreDebugCompleteCommand(UINT32 rdSize)
{
	char	strBuf[16];
	UINT32	monitorId, reqAddr, reqSize;
	...
	pAdslLmem[MEM_MONITOR_ID] ^= (UINT32) -1;
}
...
void __BcmAdslCoreDebugReadFile(char *fileName, UINT32 readCmd, uchar *bufPtr, UINT32 bufLen, void *fnComplPtr)
{
	UINT32	offset = 0;
...
void *BcmAdslCoreDebugCacheAlloc(UINT32 *pBufSize)
{
	void	*pMem;
	UINT32	bufSize = *pBufSize;
...
UINT32 BcmAdslCoreDebugReadCacheContig(uchar *bufPtr, UINT32 bufLen)
{
	UINT32		n;
...
void BcmAdslCoreDebugReadFileCached(char *fileName, UINT32 readCmd, uchar *bufPtr, UINT32 bufLen)
{
	if (NULL == cacheFileName) {
		...
		BcmAdslCoreDiagWriteStatusString(0, "AllocateCache: ptr=0x%X, size=%d", (UINT32) cacheBufPtr, cacheSize);
		if (NULL == cacheBufPtr) {
...
void BcmAdslCoreProcessTestCommand(void)
{
	...
	char	*pName;
	uchar	*pBuf;
	UINT32	cmdId, param1, param2, param3, param4;
	char	buf[1000];
	int	len;
#ifdef PHY_BLOCK_TEST
	uchar	cmd;
...
_write_file:
		...
		case GLOBAL_EVENT_PR_MSG:
			...
			if (1 == adslNewImg) {
				...
				volatile UINT32	*pAdslEnum = (UINT32 *) XDSL_ENUM_BASE;
#if defined(CONFIG_BCM963268) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963381) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM963158)
#if defined(CONFIG_BCM963158)
...
LOCAL void BcmAdslCoreProfileTimerFn(UINT32 arg);
