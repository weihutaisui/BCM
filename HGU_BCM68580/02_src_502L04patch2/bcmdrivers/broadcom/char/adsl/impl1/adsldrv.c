...
extern void XdslCoreSetAhifState(unsigned char lineId, UINT32 state, UINT32 event);
...
static int dslThread(void * data)
{
    ...
    while (atomic_read(&dslPollLock) > 0) {
        timeOutInJiffies = wait_event_interruptible_timeout(dslThreadWaitQue, (false != BcmXdslDrvReqEventPending()), timeOutInJiffies);
        if(timeOutInJiffies > 1) {
            //printk("***%s: timeOutInJiffies=%ld\n", __FUNCTION__, timeOutInJiffies);
