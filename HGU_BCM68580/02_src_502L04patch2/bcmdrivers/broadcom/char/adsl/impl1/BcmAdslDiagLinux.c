...
struct sk_buff * GetSkb(dslDrvSkbPool *skbDev, int len)
{
	...
	if (skbDiag != NULL) {
        ...
        skbDiag->dev = skbDev->netDev;
	}
	else {
		if(len < MIN_DAIG_DATA_LEN)
			totalLen += (MIN_DAIG_DATA_LEN - len);
		if( NULL != (skbDiag = alloc_skb (totalLen+skbDev->dataAlignMask, GFP_ATOMIC)) ) {
			skbDev->extraSkb++;
			skbRef = ppskb[0];
			skbDiag->data = DIAG_DATA_ALIGN(skbDiag->head, skbDev->dataAlignMask);
			skbDiag->dev = skbDev->netDev;
			skbDiag->protocol = htons(eth_type_trans (skbDiag, skbDev->netDev));
			memcpy(skbDiag->data, DIAG_DATA_ALIGN(skbRef->head, skbDev->dataAlignMask), skbDev->frameHeaderLen);
			skbDiag->len = skbDev->frameHeaderLen;
#ifndef DIAG_ALLOC_SKB_DYNAMICALLY
			printk("* %s: len=%d extraSkb=%d\n", __FUNCTION__, len, skbDev->extraSkb);
#endif
		}
	}
...
extern void BcmXdslCoreSendStatToExtBondDev(struct sk_buff *skbDiag, UINT32 cmd, char *buf0, int len0, char *buf1, int len1);
...
int __DiagWriteData(struct sk_buff *skbDiag, UINT32 cmd, char *buf0, int len0, char *buf1, int len1)
{
	diagSockFrame		*dd;
	int					n,totalBufLen;
	UINT32				lineId, clientType;
...
static struct in_ifaddr * BcmAdslCoreGetIntf(struct net_device	*dev, char *pDevName, UINT32 ipAddr)
{
...
BcmAdslCoreInitNetDev(PADSL_DIAG pAdslDiag, int port, struct sk_buff **ppskb, char *devname)
{
	...
	printk ("Drv%sCmd: CONNECT map=0x%X, logTime=%d, gwIpAddr=%s, dstPort=%d\n",
		devname, (UINT32)pAdslDiag->diagMap&0xffff, pAdslDiag->logTime,
	...
	for (n = 0; n < nDevNames; n++) {
		...
		if(NULL != (ifa = BcmAdslCoreGetIntf(dev, diagDevNames[n], (UINT32)pAdslDiag->srvIpAddr)))
			break;
	}
	...
	if (NULL == ifa) {
		...
		for (n = 0; n < nDevNames; n++) {
			...
			if (NULL == dev)
				continue;
			if(NULL != (ifa = BcmAdslCoreGetIntf(dev, diagDevNames[n], (UINT32)pAdslDiag->gwIpAddr)))
				break;
		}
	}
...
LOCAL UINT32 sndCnt = 0;
LOCAL UINT32 failCnt = 0, prevFailSndCnt = 0, continousFailcnt=0;
...
LOCAL UINT32	gInterval = 10;

LOCAL void BcmAdslCoreTestCmdStart(UINT32 interval)
{
