...
Public	void
BlockLongSubtract (int size, INT32* src1Ptr, INT32* src2Ptr, INT32* dstPtr)
	/****************************************************************************/
	/*	This function perfoms block suburaction of two short type arrays		*/	
	/****************************************************************************/
	{
	INT32	*srcEndPtr = src1Ptr + size;
...
Public	void
BlockShort2LongMove (int size, short* srcPtr, INT32* dstPtr)
	/****************************************************************************/
	/*	This function perfoms block suburaction of two short type arrays		*/	
	/****************************************************************************/
	{
...
BlockShortScale (int size, short scaleFactor, int shift, short* srcPtr, short* dstPtr)
	...
	do
		{
		*dstPtr++ = ( ((INT32)*srcPtr++) * (INT32)scaleFactor) >> shift;
		} while (srcPtr != srcEndPtr);
	}
#endif
...
Public	void
BlockLongFill (int size, INT32 pattern, INT32* dstPtr)
	/****************************************************************************/
	/*	This function fills a block with a constant								*/	
	/****************************************************************************/
	{
	INT32	*dstEndPtr = dstPtr + size;
...
Public	void
BlockLongClear (int size, INT32* dstPtr)
	/****************************************************************************/
	/*	This function fills a block with a constant								*/	
	/****************************************************************************/
	{
...
Public	void
BlockComplexLongFill(int size, INT32 xFill, INT32 yFill, ComplexLong *dstPtr)
	{
...
#if !defined(CONFIG_ARM) && !defined(CONFIG_ARM64)
Public	void
BlockLongMove (int size, INT32* srcPtr, INT32* dstPtr)
	/****************************************************************************/
...
BlockShortInterpolate (int size, short scaleFactor, int shift, short* src1Ptr, short* src2Ptr, short* dstPtr)
	...
	do
		{
		*dstPtr = *src1Ptr + (((INT32)(*src2Ptr - *src1Ptr) * scaleFactor) >> shift);
		src1Ptr++;
		src2Ptr++;
		dstPtr++;
...
BlockReal2ComplexMult	(int size,
	...
	do
		{
		dstPtr->x = ((INT32)(*src1Ptr) * (INT32)(src2Ptr->x)) >> 14;
		dstPtr->y = ((INT32)(*src1Ptr) * (INT32)(src2Ptr->y)) >> 14;
		src1Ptr++;
		src2Ptr++;
		dstPtr++;
		} while (src1Ptr != src1EndPtr);
...
BlockComplexMultLongAcc	(int size,
	...
	do
		{
		dstPtr->x += ( (INT32)(src1Ptr->x) * (INT32)(src2Ptr->x) -
		               (INT32)(src1Ptr->y) * (INT32)(src2Ptr->y) ) >> shift;
		dstPtr->y += ( (INT32)(src1Ptr->x) * (INT32)(src2Ptr->y) +
		               (INT32)(src1Ptr->y) * (INT32)(src2Ptr->x) ) >> shift;
...
BlockComplexMult	(int size,
	...
	do
		{
		shortComplexTemp.x = (	(INT32)(src1Ptr->x) * (INT32)(src2Ptr->x)
		             		  -	(INT32)(src1Ptr->y) * (INT32)(src2Ptr->y) ) >> 14;
		shortComplexTemp.y = (	(INT32)(src1Ptr->x) * (INT32)(src2Ptr->y)
		             		  +	(INT32)(src1Ptr->y) * (INT32)(src2Ptr->x) ) >> 14;
...
BlockComplexConjigateMult	(int size,
	...
	do
		...
		shortComplexTemp.x = (	(INT32)(src1Ptr->x) * (INT32)(src2Ptr->x)
		             		  +	(INT32)(src1Ptr->y) * (INT32)(src2Ptr->y) ) >> 14;
		shortComplexTemp.y = (	(INT32)(src1Ptr->y) * (INT32)(src2Ptr->x)
		             		  -	(INT32)(src1Ptr->x) * (INT32)(src2Ptr->y) ) >> 14;
		*dstPtr = shortComplexTemp;
		src1Ptr++;
		src2Ptr++;
		dstPtr++;
...
BlockReal2ComplexMacc	(int size,
	...
	do
		{
		sum.x += ((INT32)(*src1Ptr) * (INT32)(src2Ptr->x));
		sum.y += ((INT32)(*src1Ptr) * (INT32)(src2Ptr->y));
		src1Ptr++;
		src2Ptr++;
		} while (src1Ptr != src1EndPtr);
...
BlockPower (int size, short* srcPtr)
	...
	do
		{
		power1 = VLAddLong(power1, (INT32)srcPtr[0] * (INT32)srcPtr[0]);
#ifdef BLOCKPOWER_40BIT_SINGLE_ACCUMULATOR
		power1 = VLAddLong(power1, (INT32)srcPtr[1] * (INT32)srcPtr[1]);
#else
		power2 = VLAddLong(power2, (INT32)srcPtr[1] * (INT32)srcPtr[1]);
#endif
		srcPtr += 2;
...
BlockPower (int size, short* srcPtr)
	...
	do
		{
		power += ((INT32)(*srcPtr) >> 2) * ((INT32)(*srcPtr) >> 2);
...
BlockFullPower (int size, short* srcPtr)
	...
	do
		{
		power += ((INT32)(*srcPtr)) * ((INT32)(*srcPtr));
...
BlockCorrelate (int size, short* src1Ptr, short* src2Ptr)
	...
	do
		{
		correlation += ((INT32)(*(src1Ptr + 0)) >> 2) * ((INT32)(*(src2Ptr + 0)) >> 2);
		correlation += ((INT32)(*(src1Ptr + 1)) >> 2) * ((INT32)(*(src2Ptr + 1)) >> 2);
		src1Ptr += 2;
		src2Ptr += 2;
		} while (src1Ptr != src1EndPtr);
 ...
 ******************************************************************************/
	...
	do
		...
		do
			{
			tempX = ( (
				(INT32)(scalePtr->x) * (INT32)(srcPtr->x) -
				(INT32)(scalePtr->y) * (INT32)(srcPtr->y) + rnd) >> scaleShift);
			tempY = ( (
				(INT32)(scalePtr->x) * (INT32)(srcPtr->y) +
				(INT32)(scalePtr->y) * (INT32)(srcPtr->x) + rnd) >> scaleShift);

/* This overflow may produce unpredictable results depending on */
/* different C compilers and hardware platforms */

			WARN( tempX == (INT32)((short)tempX) );
			WARN( tempY == (INT32)((short)tempY) );
 ...
 ******************************************************************************/
	...
	do
		...
		do
			...
			tempX = (((INT32)(*scalePtr) * (INT32)(srcPtr->x)) >> scaleShift);
			tempY = (((INT32)(*scalePtr) * (INT32)(srcPtr->y)) >> scaleShift);
 ...
 ****************************************************************************/
	...
	do
		...
		do
			{
			cx  += (INT32)(bp1->x) * (INT32)(bp2->x)
			     + (INT32)(bp1->y) * (INT32)(bp2->y);
			cy  += (INT32)(bp1->x) * (INT32)(bp2->y)
			     - (INT32)(bp1->y) * (INT32)(bp2->x);
			bp1 += symSize;
			bp2 += symSize;
			} while (bp2 != bp3);
		...
		if(gainShift1 >= 0)
			{
			cx += (INT32)((UINT32)coefLowPtr->x << gainShift1);
			cy += (INT32)((UINT32)coefLowPtr->y << gainShift1);
...
BlockCplxLongConjigateMultCplxShort	(int size,
	...
	INT32		tempX0, tempX1, tempY0, tempY1, temp0, temp1;
	...
	do
		...
		temp0 = tempX0 * (INT32)(src2Ptr->x) + tempY0 * (INT32)(src2Ptr->y);
		temp1 = tempX1 * (INT32)(src2Ptr->x) + tempY1 * (INT32)(src2Ptr->y);
		dstPtr->x = (temp0 >> 12) + (temp1 << 3);
		
		temp0 = tempY0 * (INT32)(src2Ptr->x) - tempX0 * (INT32)(src2Ptr->y);
		temp1 = tempY1 * (INT32)(src2Ptr->x) - tempX1 * (INT32)(src2Ptr->y);
