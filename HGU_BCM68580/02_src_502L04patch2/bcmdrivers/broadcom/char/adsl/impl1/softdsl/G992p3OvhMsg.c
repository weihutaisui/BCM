...
Private Boolean G992p3OvhMsgRcvProcessCmd(void *gDslVars, dslFrameBuffer *pBuf, uchar *pData, UINT32 cmdLen, dslFrame *pFrame);
...
Private short log2Int (UINT32 x)
{
    short k=0;
    UINT32 y=1;
    while(y<x) {y<<=1;k++;}
...
static int FrameDataCopy(void *gDslVars, dslFrameBuffer *pBuf, int firstBufOffset, UINT32 destLen, uchar *pDest)
{
...
extern int BcmAdslCoreDiagWriteStatusData(UINT32 cmd, char *buf0, int len0, char *buf1, int len1);

static void G992p3OvhMsgPrintAssembledSeg(uchar *pData, int dataLen)
{
	dslStatusStruct	status;
	UINT32			cmd = statusInfoData;
...
static uchar G992p3OvhMsgGetSC(void *gDslVars, dslFrameBuffer *pBuf, UINT32 frameLen)
{
	uchar	*pData;
	int		len;
	UINT32	totalLen=0;
...
static UINT32 GetMultiFrameLen(void *gDslVars, DListHeader *pFrameList)
{
	void		*p;
	dslFrame	*pFrame;
	UINT32	len=0;
...
static int MultiFrameDataCopy(void *gDslVars, DListHeader *pFrameList, int firstBufDataOffset, UINT32 destLen, uchar *pDest)
{
	void				*p;
	dslFrame			*pFrame;
	dslFrameBuffer	*pBuf;
...
Private void G992p3OvhMsgInitPollCmd(void *gDslVars, int id, uchar *cmd, UINT32 len, UINT32 period, UINT32 flags)
{
...
Public void G992p3OvhMsgUpdateTimer(void *gDslVars, UINT32 timeMs)
{
...
Public void G992p3OvhMsgTimer(void *gDslVars)
{
    ...
    if ((globalVar.timeMs - globalVar.timeCmdOut) > 1000) {
      if (globalVar.cmdTryCnt >= 3) {
        ...
        if (G992p3FrameIsBitSet(gDslVars, globalVar.lastTxCmdFrame, kFramePollCmd)
          && ((UINT32) -1 != globalVar.cmdNum && !(globalVar.pollCmd[globalVar.cmdNum].cmdFlags & kPollCmdStayActive))) {
  ...
  if (((globalVar.timeMs - globalVar.cmdPollingStamp) > 5000) && ((globalVar.timeMs - globalVar.timeCmdOut) > 200)) {
    ...
    if (((UINT32) -1 != cmd) && G992p3OvhMsgXmtSendCmd(gDslVars, pCmd->cmd,pCmd->len, NULL,0, true,true)) {
...
#define	CHECK_CMD_RSP(cmdCode)	if (((UINT32) -1 == globalVar.cmdNum) ||((cmdCode) != pCmd->cmd[kG992p3CmdCode])) return true;
#define	CHECK_SUBCMD_RSP(subCmdCode)	if ( (subCmdCode) != pCmd->cmd[kG992p3CmdSubCode] ) return true;

Private Boolean G992p3OvhMsgRcvProcessRsp(void *gDslVars, dslFrameBuffer *pBuf, uchar *pData, UINT32 rspLen)
{
	long	mibLen;
	adslMibInfo	*pMib;
	g992p3PollCmdStruct	*pCmd = &globalVar.pollCmd[globalVar.cmdNum];
...
#endif
					{
						...
						if(XdslMibIsGinpActive(gDslVars, US_DIRECTION)) {
							...
							if(*pMsg != pMib->xdslInfo.dirInfo[US_DIRECTION].lpInfo[path1].delay) {
								...
								dslCmd.param.value = (INT32)pMib->xdslInfo.dirInfo[US_DIRECTION].lpInfo[path1].delay;
								(*globalVar.cmdHandlerPtr)(gDslVars, &dslCmd);
							}
							pMsg+=1;
						}
...
Private void G992p3CopyOemData(uchar *pDest, UINT32 dstLen, uchar *pSrc, UINT32 srcLen)
{
  if (dstLen <= srcLen)
...
void G992p3GetOemData(void *gDslVars, int paramId, uchar *pDest, UINT32 dstLen, uchar *pSrc, UINT32 srcLen)
{
  dslStatusStruct     status;
  int           len;
    ...
    *rLen=4;
    ...
    for (j=0;j<pMib->dsNegBandPlanDiscovery.noOfToneGroups;j++)
    {
        for(;i<=stopSCG;i++) {
            ...
            { 
                hlog = (UINT32) (((6*16-pHlogInfo[i*g])*5) >> 3);
                if (hlog > 0x3FC)
                        hlog=0x3FF;
            }
    ...
    *rLen+=2+2*(stopSCG-startSCG+1);
    ...
    {   
        for(;i<=stopSCG;i++) {
            ...
            { 
                qln = (UINT32) ((-pQlnInfo[i*g] - 23*16) >> 3);
    ...
    *rLen+=2+stopSCG-startSCG+1;
    ...
    for (j=0;j<pMib->dsNegBandPlan.noOfToneGroups;j++)
    {
        for(;i<=stopSCG;i++) {
            ...
            { 
                snr = (UINT32) ((pSnrInfo[i*g] + 32*16) >> 3);
...
Private Boolean G992p3OvhMsgRcvProcessCmd(void *gDslVars, dslFrameBuffer *pBuf, uchar *pData, UINT32 cmdLen, dslFrame *pFrame)
{
	...
	__SoftDslPrintf(gDslVars, "G992p3OvhMsgRcvProcessCmd: pFrame = 0x%x, pBuf = 0x%x, pData %x %x %x, cmdLen = %d",
		0, (INT32)pFrame, (INT32)pBuf, pData[kG992p3CmdCode], pData[kG992p3CmdSubCode], pData[kG992p3CmdSubCode+1], (int)cmdLen);
#endif
	switch (pData[kG992p3CmdCode]) {
							...
							case kG992p3OvhMsgCmdPMDQLineNoise:
							{
								...
								for (j=0;j<pMib->dsNegBandPlanDiscovery.noOfToneGroups;j++)
								{
									for(;i<=stopSCG;i++) {
										...
										{
											qln = (UINT32) ((-pQlnInfo[i*g] - x*16) >> 3);
											pMsg[i-startSCG]=(qln > 254) ? 255 : (uchar)qln;
										}
									}
								}
		...
		case kG992p3OvhMsgCmdOLR:
		{
			...
			switch (cmd.param.dslOLRRequest.msgType) {
				...
				case kG992p3OvhMsgCmdOLRReq4:
					...
					if ((cmd.param.dslOLRRequest.nCarrs != 0) && (cmdLen > (UINT32)(pMsg-pData))) {
						int pri;
...
Boolean G992p3OvhMsgRcvProcessSegAck(void *gDslVars, dslFrameBuffer *pBuf, uchar *pData, UINT32 cmdLen)
{
	...
	uchar segFlagAndId)
{
	...
	DiagWriteString(gLineId(gDslVars), DIAG_DSL_CLIENT, "%s: pFrame=0x%x pBuf=0x%x, pData=0x%x, len=%d, CmdOrRsp = %s, CmdCode = %x\n",
		__FUNCTION__, (INT32)pFrame, (INT32)pBuf, (INT32)pData, len, (kG992p3Cmd == cmdOrRsp )? "CMD":"RSP", pData[kG992p3CmdCode]);
#endif
...
Public void G992p3OvhMsgReturnFrame(void *gDslVars, void *pVc, ulong mid, dslFrame *pFrame)
{
  ...
  uchar       *pData;
  int         len;
  UINT32       val = G992p3GetFrameInfoVal(gDslVars, pFrame);
...
Public void INMReadCntrResponse(void *gDslVars)
{
	...
	UINT32 *pPLNDurationHist, *pPLNIntrArvlHist;
	...
	pPLNDurationHist = (UINT32 *)AdslMibGetObjectValue(gDslVars,OidStr,sizeof(OidStr),NULL,&dataLen);
	pMsg=&globalVar.txRspBuf[kG992p3CmdSubCode+1];
	for(i=0;i<17;i++)
	{
		val=(UINT32)pPLNDurationHist[i];
		WriteCnt32(pMsg,val);
		pMsg+=4;
	}
	OidStr[1]=kOidAdslPrivPLNIntrArvlHist;
	pPLNIntrArvlHist = (UINT32 *)AdslMibGetObjectValue(gDslVars,OidStr,sizeof(OidStr),NULL,&dataLen);
	for(i=0;i<8;i++)
	{
		val=(UINT32)pPLNIntrArvlHist[i];
		WriteCnt32(pMsg,val);
		pMsg+=4;
	}
...
Public void PLNReadResponses(void *gDslVars, char PLNTableName)
{
  ...
  switch(PLNTableName)
    ...
    case kDslImpulseNoiseDurationTableLongPtr:{
            int j;UINT32 *pPLNDurationHist;long dataLen;ushort val;
      uchar OidStr[] = { kOidAdslPrivate, kOidAdslPrivPLNDurationHist };
      pPLNDurationHist = (UINT32 *)AdslMibGetObjectValue(gDslVars,OidStr,sizeof(OidStr),NULL,&dataLen);
    ...
    case kDslImpulseNoiseTimeTableLongPtr:{
              int j;UINT32 *pPLNIntrArvlHist;ushort val;long dataLen;
    ...
    case kDslReceivedEocCommand:
        if( (kDslVdslSraReqSnd == status->param.dslClearEocMsg.msgId) ||
            ...
            if(G992p3OvhMsgXmtSendOLRCmd(gDslVars,
                                        olrCmd, (UINT32)olrLen,
                                        (uchar *)status->param.dslClearEocMsg.dataPtr,
                                        (UINT32)status->param.dslClearEocMsg.msgType & kDslClearEocMsgLengthMask,
                                        true)) {
                mibLen = sizeof(adslMibInfo);
                pMib = (void *) AdslMibGetObjectValue (gDslVars, NULL, 0, NULL, &mibLen);
