...
typedef struct {
  UINT32		tPCR0;					/* CLP = 0, time between cells in us */
  UINT32		tPCR;					/* CLP = 0+1 */
  UINT32		tolPCR;					/* tolerance for PCR in us */

  UINT32		tSCR0;					/* CLP = 0 */
  UINT32		tSCR;					/* CLP = 0+1 */
  UINT32		tolSCR;					/* tolerance for SCR in us */
...
#define	kAtmMidEntireVc		((UINT32) -1)
...
typedef	void (*atmStatusHandler) (void *gDslVars, UINT32 statusCode, ...);
...
typedef struct _atmPhyCounters {
	ushort						id;
	ushort						bertStatus;
	UINT32					bertCellTotal;
	UINT32					bertCellCnt;
	UINT32					bertBitErrors;
