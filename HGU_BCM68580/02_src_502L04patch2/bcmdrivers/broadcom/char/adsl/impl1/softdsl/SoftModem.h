...
#define ALWAYS_LONG_ALIGN() INT32 MacroPaste(ALIGNMENT,__LINE__);
...
typedef INT32	dialerStatusCode;
...
typedef INT32	A8TStatusCode;
#define kA8TFinished				0

typedef INT32	callerIDStatusCode;
...
typedef INT32	callerIDErrorCode;
...
typedef INT32	connectionInfoCode;
#define kRTDelay			1
...
typedef INT32	trainingProgressCode;
...
typedef INT32	lapmTakedownReason;
...
typedef INT32	lapmParameterCode;
#define kLapmXmtK			0
#define kLapmRcvK			1
...
typedef INT32	lapmTestResultCode;
...
typedef INT32	v42bisStatusCode;
...
typedef INT32	mnpTakedownReason;
...
typedef INT32	mnpParameterCode;
...
typedef INT32	v70TakedownReason;
...
typedef INT32	v70ParameterCode;
...
typedef INT32	v70ErrorCode;
#define kV70NoError					0			
#define kV70BadFrame				1			

typedef INT32	audioStatusCode;
...
#ifndef ADSL_MODEM
typedef INT32	modemCommandCode;
#endif
...
typedef INT32 v250IdStringCode;
#define kGMIString						1
#define kGMMString						2
#define kGMRString						3
#define kGSNString						4
...
typedef INT32	kCallProgressParameterCode;
...
typedef	INT32	framerParameterCode;
...
typedef	void	(SM_DECL *timerHandlerType)			(void *gDslVars, INT32);
...
typedef	bitMap	(SM_DECL *signalDetectorType)	(void *gDslVars, int, INT32, INT32*);
...
typedef struct
	...
	INT32	nominalVariance;	/* the signal variance which gives 1e-2 BLER Q10 */
...
typedef struct
	...
	INT32			rateThresholdAdjustment;	/* dB Q4	*/
...
typedef	struct	SoftwareModemCommandParameters
	...
	union
		{
		INT32				xmtGain;
		UINT32				hybridDelayQ4ms;
		INT32				modemSampleRate;
		INT32				timeInMs;
		INT32				state;
		INT32				freq;
		...
		struct
			...
			bitMap				setup;
			directionType		direction;
			INT32				fill[2]; /* need to match linkLayerSpec */
			rxDataHandlerType	rxDataHandlerPtr;
			txDataHandlerType	txDataHandlerPtr;
			} framerSpec;
		struct
		...
		struct
			{
			UINT32			maxTones;					/* maximum number of simultaneous tones to detect */
			UINT32			allowableVariance;			/* maximum cumulative variance in the eight interpolated frequencies */
			UINT32			totalPowerThreshold;		/* ignore complete block if power less than this */
			UINT32			powerShiftThreshold;		/* ignore a bin if its power is less than (totalPowerValue >> powerShiftThreshold) */
			UINT32			toneMatchThresholdHz;		/* tones within +/- this many Hz of original tone are considered the same tone */
			UINT32			binSeparation;				/* ignore tones with a spacing of less than this */
			UINT32			outsideFreqDeviation;		/* an individual value in the interpolated array can be up to this many Hz outside of the expected angle range */
			} multiToneSpec;
		struct
			{
			uchar				*dialString;	/* nil limited string for DTMF dialing sequence	*/
			INT32				pulseBreakTime, 
								pulseMakeTime, 
								pulseInterDigitTime,
								toneDigitTime, 
								toneInterDigitTime, 
...
#define	kSAM8bitSYNHuntEnabled		((UINT32)1 << kSAMTransparentIdleTypeShift)
#define	kSAM16bitSYNHuntEnabled		((UINT32)2 << kSAMTransparentIdleTypeShift)

/* <framed_idle> */
#define	kSAMSendFlagsOnIdle			0
#define	kSAMSendMarksOnIdle			((UINT32)1 << kSAMFramedIdleTypeShift)

/* <framed_un_ov> */
#define	kSAMAbortOnUnderrun			0
#define	kSAMFlagsOnUnderrun			((UINT32)1 << kSAMFramedOverrunActionShift)

/* <hd_auto> */
#define	kSAMHalfDuplexNoAuto		0
#define	kSAMHalfDuplexAuto			((UINT32)1 << kSAMHalfDuplexModeShift)


/* <crc_type> */
#define	kSAMNoCRC					0
#define	kSAM16bitCRC				((UINT32)1 << kSAMCRCTypeShift) 
#define	kSAM32bitCRC				((UINT32)2 << kSAMCRCTypeShift) 
				
/* <nrzi_en> */
#define	kSAMNRZIDisabled			0
#define	kSAMNRZIEnabled				((UINT32)1 << kSAMNRZIEnabledShift)
...
extern void		SM_DECL SoftModemTimer			(INT32 timeQ24ms);
 ...
 * Internal functions
 */
extern	INT32	SM_DECL SoftModemGetDCOffset(void);
extern	void	SM_DECL SoftModemDisableDCOffsetTracking(void);
extern	void	SM_DECL SoftModemEnableDCOffsetTracking(void);
extern	INT32	SM_DECL SoftModemGetRcvPower(void);
extern	UINT32	SM_DECL SoftModemGetHybridDelay(void);
