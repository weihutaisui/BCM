...
#define	SB_CONV_LONG(x)		(ADSL_ENDIAN_CONV_INT32((UINT32)x))
#else
#define	SB_CONV_LONG(x)		x
#endif

#define	StretchBufferGetSize(pSB)		(SB_CONV_LONG((pSB)->pEnd) - SB_CONV_LONG((pSB)->pStart))
#define	StretchBufferGetStartPtr(pSB)	((uintptr_t)(UINT32) SB_CONV_LONG((pSB)->pStart))
#define	StretchBufferGetReadPtr(pSB)	((uintptr_t)(UINT32) SB_CONV_LONG((pSB)->pRead))
#define	StretchBufferGetWritePtr(pSB)	((uintptr_t)(UINT32) SB_CONV_LONG((pSB)->pWrite))
...
static __inline int _StretchBufferGetReadAvailTotal(stretchBufferStruct *fBuf, INT32 rdPtr)
{
	INT32 wrPtr = SB_CONV_LONG(fBuf->pWrite);
...
{
	INT32 wrPtr = SB_CONV_LONG(fBuf->pWrite);
	INT32 rdPtr = SB_CONV_LONG(fBuf->pRead);
	return( ((wrPtr - rdPtr) >= 0) ? wrPtr - rdPtr: SB_CONV_LONG(fBuf->pStretchEnd) - rdPtr );
}
static __inline int _StretchBufferGetWriteAvail(stretchBufferStruct *fBuf, INT32 rdPtr)
{
	INT32 wrPtr = SB_CONV_LONG(fBuf->pWrite);
