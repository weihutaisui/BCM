...
#define ReadCnt16(pData)  \
  (((UINT32) ((uchar *)(pData))[0] << 8) + ((uchar *)(pData))[1])
#define ReadCnt32(pData)          \
  (((UINT32) ((uchar *)(pData))[0] << 24) + \
  ((UINT32) ((uchar *)(pData))[1] << 16) + \
  ((UINT32) ((uchar *)(pData))[2] << 8)  + \
  ((uchar *)(pData))[3])

INT32    secElapsedInDay;
...
extern UINT32 BcmAdslCoreGetConfiguredMod(uchar lineId);
#ifdef SUPPORT_SELT
...
Private int SM_DECL AdslMibNotifyIdle (void *gDslVars, UINT32 event)
{
    return 0;
}
...
Private UINT32 UpdateAttnEtr(void *gDslVars, UINT32 attRate, UINT32 ndr, UINT32 etr)
{
	UINT32 attnEtr = ndr != 0 ? (((etr << 8)/ndr)*attRate) >> 8 : attRate;
...
Public void XdslMibUpdateAttnEtr(void *gDslVars, int dir)
{
	...
	if (DS_DIRECTION == dir)
	  globalVar.adslMib.xdslPhys.attnETR = UpdateAttnEtr(gDslVars, globalVar.adslMib.xdslPhys.adslCurrAttainableRate/1000,
	    globalVar.adslMib.xdslInfo.dirInfo[0].lpInfo[0].dataRate, globalVar.adslMib.xdslInfo.dirInfo[0].lpInfo[0].etrRate);
	else
	  globalVar.adslMib.xdslAtucPhys.attnETR = UpdateAttnEtr(gDslVars, globalVar.adslMib.xdslAtucPhys.adslCurrAttainableRate/1000,
	    globalVar.adslMib.xdslInfo.dirInfo[1].lpInfo[0].dataRate, globalVar.adslMib.xdslInfo.dirInfo[1].lpInfo[0].etrRate);
#endif
}

#define kTwoThirdQ15      ((INT32)21845)   /* 2/3 in Q15 */
#define kFourFifthQ15     ((INT32)26214)   /* 4/5 in Q15 */
#define kTenLog10TwoQ12     ((INT32)12330)   /* 10*log10(2) in Q12 */
#define kTenLog10EQ12     ((INT32)17789)   /* 10*log10(e) in Q12 */
Public short
UtilQ0LinearToQ4dB (UINT32 x)
  {
  int   k;
  INT32  y, z, v2, v3, v4, u;
  /*long  v5;*/
...
Private void AdslMibSetConnectionInfo(void *gDslVars, G992CodingParams *param, INT32 code, INT32 val, adslConnectionInfo *pConInfo)
{
...
Private void Adsl2MibSetConnectionInfo(void *gDslVars, G992p3CodingParams *param, INT32 code, INT32 val, adsl2ConnectionInfo *pConInfo)
{
...
Private UINT32 AdslMibShowtimeSFErrors(UINT32 *curCnts, UINT32 *oldCnts)
{
    return (curCnts[kG992ShowtimeSuperFramesRcvdWrong] - oldCnts[kG992ShowtimeSuperFramesRcvdWrong]);
}

Private UINT32 AdslMibShowtimeRSErrors(UINT32 *curCnts, UINT32 *oldCnts)
{
    return (curCnts[kG992ShowtimeRSCodewordsRcvedCorrectable] - oldCnts[kG992ShowtimeRSCodewordsRcvedCorrectable]);
}

Private Boolean AdslMibShowtimeDataError(UINT32 *curCnts, UINT32 *oldCnts)
{
    return 
...
Private void XdslMibCaptureCntrs(void *gDslVars, int captureSrc)
{
  ...
  if(XdslMibIsGfastMod(gDslVars) && !pInhibitCntrs->inhibitCntrCapture) {
    int pathId = XdslMibGetCurrentMappedPathId(gDslVars, DS_DIRECTION);
...
do {                                                                \
    if (!state) {                                                   \
        state = true;                                               \
        if(!XdslMibIsGfastMod(gDslVars) || (false == globalVar.uasOnSes)) {  \
          IncPerfCounterVar(&globalVar.adslMib.adslPerfData, cnt);  \
          INC_STAT_HIST_24HR_CNT(cnt);                              \
...
Private void AdslMibConnectionStatUpdate (void *gDslVars, UINT32 *cntOld, UINT32 *cntNew, int pathId)
{
...
Private void AdslMibUpdateShowtimeErrors(void *gDslVars, UINT32 nErr, int pathId)
{
...
Private void AdslMibUpdateShowtimeRSErrors(void *gDslVars, UINT32 nErr)
{
...
Private void AdslMibUpdateIntervalCounters (void *gDslVars, UINT32 sec)
{
    adslMibInfo     *pMib = &globalVar.adslMib;
    INT32    secElapsed, i, n;
...
#endif
    UINT32 n = globalVar.adslMib.xdslStat[0].fireStat.status;
    UINT32 n0 = globalVar.adslMib.xdslStat[0].ginpStat.status;
    UINT32 n1 = globalVar.adslMib.xdslStat[MAX_LP_NUM-1].ginpStat.status;
#ifdef CONFIG_VDSL_SUPPORTED
...
Public void AdslMibTimer(void *gDslVars, UINT32 timeMs)
{
    UINT32   sec;
    int path0, path1;
    ...
    if(globalVar.txShowtimeTime>3000 && globalVar.txUpdateStatFlag==1 )
    {
        ...
        if(sec > globalVar.secElapsedShTm+1)
        {
            ...
            if(XdslMibIsXdsl2Mod(gDslVars))
            {
                ...
                var64 |= 1000;
#endif
                globalVar.adslMib.xdslStat[path0].xmtStat.cntRS = (UINT32)var64;
                if(!XdslMibIsGinpActive(gDslVars, US_DIRECTION) ||
                    ...
                    globalVar.adslMib.xdslStat[path0].xmtStat.cntSF = (UINT32)var64;
                }
                ...
                if(globalVar.adslMib.lp2TxActive)
                {
                    ...
                    var64 |= 1000;
#endif
                    globalVar.adslMib.xdslStat[path1].xmtStat.cntRS = (UINT32)var64;
                    if(!XdslMibIsGinpActive(gDslVars, US_DIRECTION) ||
                        ...
                        globalVar.adslMib.xdslStat[path1].xmtStat.cntSF = (UINT32)var64;
                    }
                }
            }
            else
            {
                ...
                globalVar.adslMib.adslStat.xmtStat.cntSF = (UINT32)var64;
            }
        }
    }
...
Private void AdslMibSetBitAndGain(void *gDslVars, int tblIdx, void *buf, int bufBgSize, Boolean bShared)
{
    ...
    for (i = 0; i < bufBgSize; i++) {
        if (!bShared || (0 == (globalVar.bitAlloc[tblIdx + i] | globalVar.gain[tblIdx + i]))) {
            ...
            valGi=(bufPtr[i << 1] >> 4) | ((UINT32)bufPtr[(i << 1) + 1] << 4);
...
Private int AdslMibGetAdsl2Mode(void *gDslVars, UINT32 mask)
{
    return globalVar.adslMib.xdslInfo.xdslMode & mask;
}

Private void AdslMibSetAdsl2Mode(void *gDslVars, UINT32 mask, int mod)
{
    globalVar.adslMib.xdslInfo.xdslMode = (globalVar.adslMib.xdslInfo.xdslMode & ~mask) | mod;
}

Private void XdslMibSetXdslMode(void *gDslVars, UINT32 mask, int mod)
{
    globalVar.adslMib.xdslInfo.xdslMode = (globalVar.adslMib.xdslInfo.xdslMode & ~mask) | mod;
}
...
Private INT32 AdslMibGetTotalNumOfLoadedTones(void *gDslVars, unsigned char dir)
{
    INT32 totalNumOfLoadedTones;
...
Public void AdslMibUpdateACTPSD(void *gDslVars, INT32 ACTATP, unsigned char dir)
{    
    INT32 *pActPsd, nLoadedTones;
...
{
    UINT32 per, rsRate, den, UG;
    
    xdslFramingInfo   *p = &globalVar.adslMib.xdslInfo.dirInfo[dir].lpInfo[pathId];
    UG = p->U*p->G;
    den = (p->M*p->L);
    per =(den) ? (((2*p->T*((UINT32)p->M*(p->B[0]+1)+p->R)*UG) << 8)/den) : 1;
    if(0 == per)
        per = 1;

    den = (8*p->N);
    rsRate = (den) ? ((UINT32)p->L*4000)/den : 0;
...
Private void AdslMibCalcVdslPERp(void *gDslVars, int pathId, int dir)
{
   ...
   if (kVdslProfile30a == globalVar.adslMib.xdslInfo.vdsl2Profile)
       den <<= 1;
   per = (den) ? (((UINT32)(2*pXdslFramingParam->T * pXdslFramingParam->U * pXdslFramingParam->N) << 8)/den) : 1; /* Q8 */
   if(0 == per)
   per = 1;
  ...
  switch (DSL_STATUS_CODE(status->code)) {
    ...
    case kDslDspControlStatus:
      switch(status->param.dslConnectInfo.code)
      {
        ...
        {
          UINT32 *pLong = (UINT32 *)status->param.dslConnectInfo.buffPtr;
    ...
    case kDslTrainingStatus:
      ...
      switch (status->param.dslTrainingInfo.code) {
...
#if defined(CONFIG_BCM_DSL_GFAST)
            if(XdslMibIsGfastMod(gDslVars)) {
                if(ADSL_PHY_SUPPORT(kAdslPhyGfast212a) && (kGfastProfile212a == globalVar.adslMib.xdslInfo.vdsl2Profile)) {
        ...
        case kDslFinishedG994p1:
            if((UINT32)-1 == globalVar.adslMib.adslPhys.adslLDCompleted)
                globalVar.adslMib.adslPhys.adslLDCompleted = (UINT32)-2;
        ...
        case kG992MessageExchangeRcvInfo:
            if (kDslG992p2ReceivedMsg2 == globalVar.g992MsgType) {
                ...
                if (XdslMibIsXdsl2Mod(gDslVars)) {
                    ...
                    val -= 7;
                    n    = 7;
                    globalVar.adslMib.adslAtucPhys.adslCurrAtn = (((UINT32) p[1] & 0x3) << 8) | p[0];
                    globalVar.adslMib.adslAtucPhys.adslCurrSnrMgn = ((INT32) p[5] << 8) | p[4];
                    rate = ((UINT32) p[9] << 24) | ((UINT32) p[8] << 16) | ((UINT32) p[7] << 8) | p[6];
                    globalVar.adslMib.adslAtucPhys.adslCurrAttainableRate = ActualRate(rate);

                    rate = ((INT32) p[11] << 8) | p[10];
                ...
                switch (*p) {
                  case 0x11:
                    ...
                    n = ((UINT32) p[13] << 24) | ((UINT32) p[12] << 16) | ((UINT32) p[11] << 8) | p[10];
        ...
        case kG992ShowtimeMonitoringStatus:
          {
...
#ifdef  ADSLDRV_LITTLE_ENDIAN
            UINT32 dstCounters[DSL_COUNTERS_MAX];
            ...
            if (XdslMibIsDataCarryingPath(gDslVars, pathId, DS_DIRECTION)) {
               ...
               if (AdslMibShowtimeDataError(counters, (UINT32 *)&globalVar.shtCounters2lp[pathId])) {
#ifdef CONFIG_BCM_DSL_GFAST
                   if(!globalVar.adslMib.fastRetrainActive && !G992p3OvhMsgIsL3RspPending(gDslVars))
#endif
                   {
                   UINT32   nErr = AdslMibShowtimeSFErrors(counters, (UINT32 *)&globalVar.shtCounters2lp[pathId]);
               ...
               n = AdslMibShowtimeRSErrors(counters, (UINT32 *)&globalVar.shtCounters2lp[pathId]);
               if (n != 0)
            ...
            AdslMibConnectionStatUpdate (gDslVars, (UINT32 *)&globalVar.shtCounters2lp[pathId], counters, pathId);
    ...
    case kAtmStatus:
      ...
      switch (status->param.atmStatus.code) {
          ...
          case kAtmStatCounters:
              ...
              INT32 *pSrc = (INT32 *)(ADSL_ADDR_TO_HOST(status->param.atmStatus.param.value));
              BlockShortMoveReverse(2, (short *)pSrc, (short *)p);	/* id and bertStatus(first 2 ushort) */
              BlockLongMoveReverse((sizeof(atmPhyCounters)>>2)-1, pSrc+1, (INT32 *)p+1);
#else
    ...
    case kDslReceivedEocCommand:
      switch(status->param.dslClearEocMsg.msgId) {
        ...
        case kDslShowtimeSnrMarginHdrQ8:
        {
          ...
          INT32 avgSNRMargin = ADSL_ENDIAN_CONV_INT32(pSnrMarginInfoType->avgSNRMargin) >> 4;  /* Q8  -->  Q4 */
        ...
        case kGinpMonitoringCounters:
        {
          ...
          BlockLongMoveReverse(sizeof(GinpCounters) >> 2, (INT32 *)pGinpCounter, (INT32 *)&ginpCounter);
        ...
        case kGfastEocMonitoringCounters:
        {
          ...
          BlockLongMoveReverse(sizeof(gfastEocCounters) >> 2, (INT32 *)pEocCounters, (INT32 *)&eocCounters);
        ...
        case kDslGfastVectoringEocSegments:
        {
          ...
          BlockLongMoveReverse(sizeof(GfastTxVectorFBEocSegment) >> 2, (INT32 *)pVectorFBCounters, (INT32 *)&vectorFBCounters);
        ...
        case kDsl993p2FramerDeframerDs:
        {
          ...
          INT32 msgId = status->param.dslClearEocMsg.msgId;
        ...
        {
          INT32 val = ADSL_ENDIAN_CONV_INT32(*(INT32 *)status->param.dslClearEocMsg.dataPtr);
#ifdef USE_TRAINING_ATTNDR
          INT32 newAttndr = ActualRate(val*1000);
          INT32 halfLastAttndr = ((globalVar.adslMib.xdslPhys.adslCurrAttainableRate)>>11); /* Hz to kHz and divide by 2 (div by 2048) */
          pathId = XdslMibGetCurrentMappedPathId(gDslVars, DS_DIRECTION);
      ...
      case kDslNtrCounters:
      {
         ...
         BlockLongMoveReverse(ntrCntLen>>2, (INT32 *)pNtr, (INT32 *)&globalVar.adslMib.ntrCnt);
      }
         break;
#endif
#ifdef CONFIG_VDSL_SUPPORTED
      ...
      case kDsl993p2TestHlin:
          ...
          if(n > nMaxToneAllow)
             n = nMaxToneAllow;
          BlockLongMoveReverse(n<<1,(INT32 *)(status->param.dslClearEocMsg.dataPtr),(INT32 *)globalVar.uer);
      break;
#ifdef SUPPORT_SELT
      ...
      {
        UINT32 val = ADSL_ENDIAN_CONV_INT32(*(UINT32 *)status->param.dslClearEocMsg.dataPtr);
        globalVar.bandPlanType= val;
        break;
      }
      case kDsl993p2MaxRate:
      {
        UINT32 *pUlong = (UINT32 *) status->param.dslClearEocMsg.dataPtr;
...
Public void AdslMibBertStartEx(void *gDslVars, UINT32 bertSec)
{
    ...
    globalVar.adslMib.adslBertStatus.bertSecCur = (UINT32)-1;
}
...
Private void AdslMibAdd64 (cnt64 *pCnt64, UINT32 num)
{
    UINT32   n;
...
Public UINT32 AdslMibBertContinueEx(void *gDslVars, UINT32 totalBits, UINT32 errBits)
{
...
Public void AdslMibSetShowtimeMargin(void *gDslVars, INT32 showtimeMargin)
{
