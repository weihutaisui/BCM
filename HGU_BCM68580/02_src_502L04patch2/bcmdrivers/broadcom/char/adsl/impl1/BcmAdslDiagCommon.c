...
extern void BcmAdslCoreSetWdTimer(UINT32 timeUs);

extern Bool BcmXdslDiagStatSaveLocalIsActive(void);
extern void BcmXdslDiagStatSaveLocal(UINT32 cmd, char *statStr, int n, char *p1, int n1);
extern void BcmAdslDiagEnablePrintStatCmd(void);

void DiagWriteDataCont(UINT32 cmd, char *buf0, int len0, char *buf1, int len1);
...
int DiagWriteData(UINT32  cmd, char *buf0, int len0, char *buf1, int len1);
...
void DiagsDeregisterClient(UINT32 clientType)
{
...
int DevSkbSendDiagsPacket(dslDrvSkbPool *skbDev, UINT32 cmd, dslDrvFragment *pFrag, int nFrag)
{
...
void DiagWriteStatus(void *stat, char *pBuf, int len)
{
	...
	static	INT32	statStrBuf[4096/4];
	char			*statStr, *p1;
	UINT32		cmd, clientType, statusCode;
	INT32			n,n1;
	uchar			lineId;
		...
		case kDslPrintfStatusSaveLocalOnly:
		{
			...
			if((statusCode == kDslPrintfStatus) && (0 != status->param.dslPrintfMsg.argNum)) {
				BlockLongMoveReverse(status->param.dslPrintfMsg.argNum,
					(INT32 *)status->param.dslPrintfMsg.argPtr,
					(INT32 *)status->param.dslPrintfMsg.argPtr);
			}
#endif
#if defined(CONFIG_ARM64)
			if (kDslPrintfStatus == statusCode) {
			  if (0xFF != *status->param.dslPrintfMsg.fmt) {
			    UINT32 *pArg   = status->param.dslPrintfMsg.argPtr;
...
void DiagWriteStatusShort(UINT32 lineId, UINT32 clientType, UINT32 code, UINT32 value)
{
...
void DiagWriteStatusLong(UINT32 lineId, UINT32 clientType, UINT32  msgId, void *ptr, UINT32 len, UINT32  flags)
{
	dslStatusStruct   status;
...
void DiagOpenFile(UINT32 lineId, UINT32 clientType, char *fname)
{
	dslStatusStruct	  status;
...
void DiagDumpData(UINT32 lineId, UINT32 clientType, void *ptr, UINT32 len, UINT32  flags)
{
	dslStatusStruct	  status;
	status.code = kDslReceivedEocCommand | (lineId << DSL_LINE_SHIFT) | (clientType << DIAG_TYPE_SHIFT);
...
void DiagWriteStringV(UINT32 lineId, UINT32 clientType, const char *fmt, void *ap)
{
	dslStatusStruct 	status;
...
void __DiagStrPrintf(UINT32 lineId, UINT32 clientType, const char *fmt, int fmtLen, int argNum, ...)
{
	...
	{
		int i;
		UINT32 *pParam64, *pData32 = (UINT32 *)arg;
...
int BcmAdslCoreDiagWriteStatusData(UINT32 cmd, char *buf0, int len0, char *buf1, int len1)
{
...
void BcmAdslCoreDiagWriteLog(logDataCode logData, ...)
{
	...
	switch	(logData) {
		case	commandInfoData:
			...
			n = va_arg(ap, INT32);
			for (i = 0; i < n ; i++)
				logDataPtr += sprintf(logDataPtr, "%d ", pCmdData[i]);
			logDataPtr += sprintf(logDataPtr, "\n");
			break;
		case (inputSignalData - 2):
			datum = va_arg(ap, INT32);
...
char *ConvertToDottedIpAddr(UINT32 ipAddr, char *buf)
{
	if(NULL != buf) {
		sprintf(buf,"%d.%d.%d.%d",
			(int)((ipAddr>>24)&0xFF),
...
void BcmCoreDiagZeroCopyStatBufRecycle(struct sk_buff *skb, unsigned long context, UINT32 flags)
{
...
diagZeroCopyAppCtrlType * BcmCoreDiagZeroCopyStatAppInit(unsigned char lineId, unsigned char logCmd, struct sk_buff *model, int bufSize, int numOfBuf, int dataAlignMask, int frameHeaderLength)
{
	...
	bufSize = (bufSize+3) & ~3;
	bufSize += sizeof(UINT32);	/* Reserve sizeof(UINT32) for adslDmaDesc->flags */
#ifdef CONFIG_ARM64
	alignSbkLen = (bufSize + 0xf) & ~0xf;
	alignSbkLen += sizeof(struct skb_shared_info);
	alignSbkLen = (alignSbkLen + 0xf) & ~0xf;
	printk("%s: bufSize=%d alignSbkLen=%d sizeof(struct skb_shared_info)=%ld\n", __FUNCTION__, bufSize, alignSbkLen, sizeof(struct skb_shared_info));
...
#else
	pDrvSkbPool= DevSkbAllocate(model, bufSize, numOfBuf, 0, 0, dataAlignMask, frameHeaderLength, dmaZone);
	if(NULL == pDrvSkbPool) {
		printk("%s: Failed to allocate skbPools\n", __FUNCTION__);
		BcmCoreDpcSyncExit();
		return NULL;
...
#endif
	pDrvSkbPool->skbLengh -= sizeof(UINT32);
#ifdef CONFIG_ARM64
	...
	for(i = 0; i < DIAG_ZEROCOPY_NBUF_MAX(pAppCtrl); i++) {
		...
		UINT32 *pFlag = (UINT32 *)DIAG_ZEROCOPY_PAD_PTR(skb, pAppCtrl);
		...
		pAppCtrl->pDescRing[i].flags = (UINT32)UNCACHED(ADSL_ENDIAN_CONV_UINT32(SDRAM_ADDR_TO_ADSL(pFlag)));
		pAppCtrl->pDescRing[i].addr = (UINT32)UNCACHED(ADSL_ENDIAN_CONV_UINT32(SDRAM_ADDR_TO_ADSL(dd->diagData)));
		*pFlag = 0;
		Cache_Flush_Len(skb->data, frameHeaderLength);
		Cache_Flush_Len(pFlag, sizeof(UINT32));
	}
...
void BcmAdslCoreDiagStartLog_1(unsigned char lineId, UINT32 map, UINT32 time)
{
	...
	if(NULL != diagCtrl.pEyeDataAppCtrl[lineId]) {
		...
		if(0==diagCtrl.diagDataMap[lineId])
			printk("%s: map=0x%X time=%d pAppCtrl=0x%p bufIndex=%d wrCnt=%u ovrCnt=%u wrErrCnt=%u maxLpCnt=%u\n",
				__FUNCTION__, map, time,
				pAppCtrl, pAppCtrl->bufIndex, pAppCtrl->wrCnt,
...
void BcmAdslCoreDiagStartLog_2(unsigned char lineId, UINT32 map, UINT32 time)
{
	dslCommandStruct	cmd;
#ifdef SUPPORT_DSL_BONDING
	cmd.command = kDslDiagSetupCmd | (lineId << DSL_LINE_SHIFT);
...
void BcmCoreDiagZeroCopyReturnTxDoneBuffers(diagZeroCopyAppCtrlType *pAppCtrl)
{
	...
	if(loop > 13) {
		DiagWriteString(0, DIAG_DSL_CLIENT, "bufIndex=%d bufIndexTxDone=%d wrCnt=%u ovrCnt=%u wrErrCnt=%u retSkbNum=%u\n",
...
static void BcmAdslCoreDebugReadMem(unsigned char lineId, DiagDebugData *pDbgCmd)
{
	...
	*(short *)pDiagFrame->diagHdr.logProtoId = *(short *) LOG_PROTO_ID;
	...
	if(CMDID_READ_MEM_CHIPID == pDbgCmd->cmdId) {
		...
		*(UINT32 *)pData = ADSL_ENDIAN_CONV_INT32(PERF->RevID);
		*((UINT32 *)pData+1) = ADSL_ENDIAN_CONV_INT32(PERF->RevID >> CHIP_ID_SHIFT);
	...
	pDbgCmd->param1 = (UINT32)(uintptr_t)pMem;
	pMemEnd = pMem + pDbgCmd->param2;
	pDbgCmd->param2 = (UINT32)(uintptr_t)pMemEnd;
	frCnt = 0;
	while (pMem != pMemEnd) {
		n = pMemEnd - pMem;
		if (n > dataSize)
		...
		pDbgRsp->param1 = ADSL_ENDIAN_CONV_INT32((UINT32)(uintptr_t)pMem);
...
void *BcmAdslAllocPhyMem(UINT32 size)
{
	void		*p;
...
{
	UINT32	origDiagMap, map, clientType;
	int		dstPort;
	diagSockFrame	*dd;
	...
	dstPort = ((UINT32) pAdslDiag->diagMap) >> 16;
	map = (UINT32) pAdslDiag->diagMap & 0xFFFF;
	clientType = (UINT32) (((pAdslDiag->diagCmd & DIAG_TYPE_CMD_MASK) >> DIAG_TYPE_CMD_SHIFT) & 0x3);
		...
		case LOG_CMD_CONNECT:
			...
			if (pAdslDiag->diagMap & DIAG_DATA_GDB_ID) {
				...
				if (0 == diagLockSession) {
					pAdslDiag->diagMap = (int)((UINT32)pAdslDiag->diagMap & ~((UINT32)DIAG_LOCK_SESSION));
			...
			switch (pDbgCmd->cmd) {
				...
				case DIAG_DEBUG_CMD_DUMPBUF_CFG:
					...
					if (0 == pDbgCmd->param1) {
					  ...
					  printk("DIAG_DEBUG_CMD_DUMPBUF_CFG: Buffer allocated ptr=0x%p size=%d\n", dumpBuf.pBuf, dumpBuf.bufSize);
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
					  dumpBuf.pHostAddr = (void *) ADSL_ENDIAN_CONV_INT32(((UINT32)dumpBuf.pBuf));
#endif
					  dumpBuf.bufSize |= (pDbgCmd->param2 & 7) << 29;
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
					  pBufTmp = dumpBuf.pBuf;
					  dumpBuf.pBuf = (void *) ADSL_ENDIAN_CONV_INT32(SDRAM_ADDR_TO_ADSL(dumpBuf.pBuf));
				...
				{
					UINT32	*pAddr = (UINT32 *)BcmAdslCoreDiagAddrConv(pDbgCmd->param1);
				...
				case DIAG_DEBUG_CMD_WRITE_FILE:
				{
					...
					pDbgCmd->param1 = (UINT32)(uintptr_t)pData;
				...
				{
					int res, (*pExecFunc) (UINT32 param) = (void *)(uintptr_t)(pDbgCmd->param1);
...
UINT32 BcmXdslDiagGetSrvIpAddr(void)
{
	UINT32 res = 0;
	if(NULL != diagCtrl.skbModel) {
