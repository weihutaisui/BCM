...
static void serdes_init (pon_params_t* pon_params)
{
    ...
    uint16_t tx_pll_fracn_ndiv    = 0x0;
    ...
    {
#if !defined (CONFIG_BCM963158)
        RU_FIELD_WRITE(0, MISC, 2, CFGACTIVEETHERNET2P5 , 1); //  cfgActiveEthernet2P5.set(1); // Always use the divideBy1 clock
#endif
        ...
        //WAN_INTERFACE_SELECT @63158 = 1: 1G AE; 2. 2.5G AE; 3: 10G AE; 4: GPON; 5-7: Reserve.
        if ((mode == EPON_10_10) || (mode == AE_10_10))
        ...
        else if ((mode == EPON_1_1) || (mode == AE_1_1))
            RU_FIELD_WRITE(0, MISC, 3, CHOP(CR_XGWAN_TOP_WAN_MISC_WAN_CFG_, WAN_INTERFACE_SELECT ),  1);
#endif
    }
...
// ########################################################## 
    ...
    writePONSerdesReg(0x0800, 0xd080, wr_serdes, 0xffff);



    /* #(*) TX Phase Interpolator Control 0 (0xD070)	*/
    writePONSerdesReg( (DEVID_1 | LANE_BRDCST)  , 0xd070,0x2003,0xffff);
    ...
    // *********************************************************************************
    writePONSerdesReg(0x0800, 0xD154, 0x2000, 0x6000);   /* # [13] core_dp_s_rstb = 1 */
    if (pll_use == TWO_PLLS)
        writePONSerdesReg(0x0900, 0xD154, 0x2000, 0x6000);
    ...
    /* Configure DIG_COM_TOP_USER_CONTROL  (0xd0f4) */
    writePONSerdesReg((DEVID_1 | LANE_BRDCST),DIG_COM_TOP_USER_CONTROL_0 , 0x2000, 0x2000);
    ...
    writePONSerdesReg(0x0800,0xD0A0,0x0000,0x00c0);
    ...
    //#===============================================================
    if ((mode == EPON_10_1) && (optics_type == BP_GPON_OPTICS_TYPE_PMD))
    {
        ...
        writePONSerdesReg(0x800, 0xd0e3, 0x0001, 0x0001);
    ...
    } // END if ((mode == EPON_10_1)&&(optics_type == BP_GPON_OPTICS_TYPE_PMD))
    //#===============================================================
