...
static void wan_init(pon_params_t* pon_params)
{
    ...
    if ((mode ==XGPON_10_2 )&&(optics_type == BP_GPON_OPTICS_TYPE_PMD))
    {   
...
static void serdes_init (pon_params_t* pon_params)
{
    ...
    writePONSerdesReg( (DEVID_1|LANE_0|serdes_PLL_0), 0xD0B9, 0x0000, 0x0004); // PLL0: Power-Up. ams_pll_pwrdn[bit5] = 0 
    if (pll_use == TWO_PLLS)
        writePONSerdesReg( (DEVID_1|LANE_0|serdes_PLL_1), 0xD0B9, 0x0000, 0x0004); // PLL1: Power-Up. ams_pll_pwrdn[bit5] = 0 
    else
        writePONSerdesReg( (DEVID_1|LANE_0|serdes_PLL_1), 0xD0B9, 0x0004, 0x0004); // PLL1: Power-Off. ams_pll_pwrdn[bit5] = 1
    ...
    { //(*) GPON OS mode
        ...
        //# [3]   = ams_pll_fp3_rh   = 1
        writePONSerdesReg(0x0800,0xd0b1,0x0038,0x00f8);
        writePONSerdesReg(0x0800,0xd0b2,0x0001,0x0001);
        writePONSerdesReg(0x0800,0xd0b4,0x5077,0xffff);
        writePONSerdesReg(0x0800,0xd0b6,0x0001,0x0021);
        writePONSerdesReg(0x0800,0xd0b7,0x432c,0xffff);
        writePONSerdesReg(0x0800,0xd0b8,0x8c70,0xffff); // [14]pll_ditheren = 0
    }
    //#===================================================================================================
    else if (mode == XGPON_10_2)
    { //(*) XGPON OS mode w PLL Ref-Clock=50MHz. 
        ...
        // # [7]   = ams_pll_vco_div4 = 0
        // # [6]   = ams_pll_vco_div2 = 0
        writePONSerdesReg(0x0800,0xd0b1,0x0000,0x00c0);
        writePONSerdesReg(0x0900,0xd0b1,0x0000,0x00c0);

        writePONSerdesReg(0x0800,0xd0b4,0x5077,0xffff);  // pll_kvh_force = '01'
        writePONSerdesReg(0x0900,0xd0b4,0x5077,0xffff);

        writePONSerdesReg(0x0800,0xd0b7,0x432c,0xffff);
        writePONSerdesReg(0x0900,0xd0b7,0x432c,0xffff);

        writePONSerdesReg(0x0800,0xd0b8,0x8c70,0xffff);  // [14]pll_ditheren = 0
        writePONSerdesReg(0x0900,0xd0b8,0x8c70,0xffff);
    }
    //#===================================================================================================
    else if (mode == NGPON_10_10)
    { //(*) NGPON2(10/10) Full-Rate, TX rate =9.95328G, RX rate = 9.95328G
        //////////////////////////////////////////////////////////////////////////////////////
        writePONSerdesReg(0x0800, 0xd0b1, 0x0000, 0x00c0);  // pll_vco_div4[7]= pll_vco_div2[6] = 0
        writePONSerdesReg(0x0800, 0xd0b4, 0x1000, 0x3000);  // [13:12] = ams_pll_kvh_force = '01'
        writePONSerdesReg(0x0800, 0xd0b4, 0x0000, 0x0300);  // [09:08] = pll_2rx_clkbw     = '00'
        writePONSerdesReg(0x0800, 0xd0b6, 0x0000, 0x0001);  // Mario.C: Do not use 0xD0B6[bit0]=test_fracn_en. It is only a test mode.
        ...
        if (pll_refclk== PADREF)
        {
            ...
            // writePONSerdesReg(0x0800, 0xd0b4, 0x0000, 0x4000); // default: Integer(fullrate)   --> bit[14]force_kvh_bw=0
            writePONSerdesReg(0x0800, 0xd0b4, 0x4000, 0x4000); // (*) Improved jitter: FraqN(fullrate)--> bit[14]force_kvh_bw=1
            writePONSerdesReg(0x0800, 0xd0b4, 0x2000, 0x3000); // (*) Improved jitter:  pll_kvh_force = 2
            writePONSerdesReg(0x0800, 0xd0b7, 0x0000, 0xffff); // pll_fracn_div_l[15:00] = lower 16 bits of fracn_div[17:16]=0
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x0003); // pll_fracn_div_h[01:00] = upper 2MSB bits of fracn_div[17:16]=0
            writePONSerdesReg(0x0800, 0xd0b8, 0x0008, 0x0008); // pll_fracn_bypass[3] = 0
            writePONSerdesReg(0x0800, 0xd0b8, 0x0400, 0x3ff0); // pll_fracn_ndiv_int[13:04] = 0x40
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x4000); // pll_ditheren [14]   = 0
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x8000); // pll_fracn_sel[15]   = 0
            writePONSerdesReg(0x0800, 0xd0b1, 0x0000, 0x0030); // pll_fp3_ctrl [05:04]= 0
            writePONSerdesReg(0x0800, 0xd0b1, 0x0000, 0x0008); // pll_fp3_rh   [03]   = 0
            writePONSerdesReg(0x0800, 0xd0b2, 0x0000, 0x0001);
        }
        else
        {
            ...
            // pll_2rx_clkbw     = '00'        --> 0xd0b4 [9:8]   = 2'b00
            //////////////////////////////////////////////////////////////////////////////////////
            writePONSerdesReg(0x0800, 0xd0b4, 0x4000, 0x4000); // FraqN(fullrate)--> bit[14]force_kvh_bw=1
            writePONSerdesReg(0x0800, 0xd0b4, 0x2000, 0x3000); // (*) Improved jitter:  pll_kvh_force = 2
            writePONSerdesReg(0x0800, 0xd0b7, 0x432c, 0xffff); // pll_fracn_div_l[15:00]
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x0003); // pll_fracn_div_h[01:00]
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x0008); // pll_fracn_bypass[3] = 0
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x0004);
            writePONSerdesReg(0x0800, 0xd0b8, 0x0c70, 0x3ff0); // pll_fracn_ndiv_int[13:04] = 0xc7
            writePONSerdesReg(0x0800, 0xd0b8, 0x0000, 0x4000); // pll_ditheren [14]   = 0
            writePONSerdesReg(0x0800, 0xd0b8, 0x8000, 0x8000); // pll_fracn_sel[15]   = 1
            writePONSerdesReg(0x0800, 0xd0b1, 0x0030, 0x0030); // pll_fp3_ctrl [05:04]= 3
            writePONSerdesReg(0x0800, 0xd0b1, 0x0008, 0x0008); // pll_fp3_rh   [03]   = 1
            writePONSerdesReg(0x0800, 0xd0b2, 0x0001, 0x0001);
        }
    }
    ...
    if (pll_use == TWO_PLLS)
        writePONSerdesReg(0x0800,0xd08d,0x0002,0x0003); // PLL#1  as clock for RX lane
    else // pll== ONE_PLL
        writePONSerdesReg(0x0800,0xd08d,0x0000,0x0003); // PLL#0 as clock for RX lane


    //(*) PLL charge pump settings
    //#===============================================================

    writePONSerdesReg(0x0800,0xd0b2,0x000a,0x001e); // pll_iqp [04:01] = 0x5 = default

#if defined (CONFIG_BCM96858) // New 10G settings for Sequoia B0
    if ((mode == XGPON_10_2)||(mode == NGPON_10_10))
    {
        //  0xD0B2[bit0]=en_HRz<1> =1,  0xD0B0[bit11]=en_HRz<0>=1  -->>> en_HRz = 6[kOhm]
        writePONSerdesReg(0x0800,0xd0b2,0x0001,0x0001);
        writePONSerdesReg(0x0800,0xd0b0,0x0800,0x0800);

        writePONSerdesReg(0x0900,0xd0b2,0x0001,0x0001);
        writePONSerdesReg(0x0900,0xd0b0,0x0800,0x0800);

        // set PLL-Current-ChargePump=ipq[bit4:1]= 0x0 -->>> 50[uA]
        writePONSerdesReg(0x0800,0xd0b2,0x0000,0x001e);
        writePONSerdesReg(0x0900,0xd0b2,0x0000,0x001e);
    }
#endif
    //#===============================================================
    ...
    writePONSerdesReg(0x0800, 0xD050, rx_tx_rate_ratio, 0x0007);
    ...
    if (mode ==GPON_2_1 )
        writePONSerdesReg(0x0800, 0xd080,0xC074,0xffff); //tx_osr_mode = 7, rx_osr_mode = 4

    else if (mode ==XGPON_10_2 )
        writePONSerdesReg(0x0800, 0xd080,0xC040,0xffff); //tx_osr_mode = 4, rx_osr_mode = 0

    else if (mode == NGPON_10_10)
        writePONSerdesReg(0x0800, 0xd080,0xC000,0xffff); //tx_osr_mode = 0, rx_osr_mode = 0


    /* #(*) TX Phase Interpolator Control 0 (0xD070)	*/
    writePONSerdesReg(0x0800, 0xd070,0x5003,0xffff);
    ...
    writePONSerdesReg(0x0800, 0xD002, 0x02B2, 0xFFFF);  // 0x2b2=690[dec]
    ...
    writePONSerdesReg(0x0800, 0xD003, 0x00f0, 0x00F0); // fieldSet( "pattern_sel" , f );  //prev.value = 0x3


    // (*)  Analog = VGA, and PF programming
    writePONSerdesReg(0x0800, 0xD041, 0x0007, 0x000F); // _set_rx_pf_main(7);
    writePONSerdesReg(0x0800, 0xD042, 0x0003, 0x0007); // _set_rx_pf2(3);
    writePONSerdesReg(0x0800, 0xD02B, 0x0000, 0x3E00); // _set_rx_vga(32);
    ...
    // OverSample mode
    writePONSerdesReg(0x0800,0xd07c,0x0000,0x0100); // tx_lbe4_0[bit8]=0
    ...
    if (mode ==GPON_2_1 )
    {
        ...
        writePONSerdesReg(0x0800,0xd0a1,0x006c,0x007f);
        writePONSerdesReg(0x0800,0xd092,0x0004,0x0007);
    }
    else if (mode ==XGPON_10_2 )
    {
        //(*) TX : LBE & MAC clocks control  (OverSample mode)
        writePONSerdesReg(0x0800,0xd0a1,0x0026,0x007f);   // bit[0]   = ams_tx_tx_wclk4_en = 0
        // bit[6:4] = ams_tx_tx_pon_mac_ctrl = 2 = tx_pon_mac clock division control
        // (*) RX : MAC clock control
        writePONSerdesReg(0x0800, 0xD092, 0xA804, 0xFFFF);// bit[2:0] = ams_rx_rx_pon_mac_ctrl = 4 = rx_pon_mac clock division control
    }
    else if (mode == NGPON_10_10)
    {
        //(*) TX : LBE & MAC clocks control  (OverSample mode)
        writePONSerdesReg(0x0800,0xd0a1,0x0000,0x0001); // bit[0]   = ams_tx_tx_wclk4_en = 0
        writePONSerdesReg(0x0800,0xd0a1,0x000c,0x000e); // bit[3:1] = tx_sync_e_ctrl = 6
        writePONSerdesReg(0x0800,0xd0a1,0x0060,0x0070); // bit[6:4] = ams_tx_tx_pon_mac_ctrl = 6 = tx_pon_mac clock division control
        // (*) RX : MAC clock control
        writePONSerdesReg(0x0800,0xd092,0xab04,0xffff); // bit[2:0] = ams_rx_rx_pon_mac_ctrl = 4 = rx_pon_mac clock division control
    }
    ...
    writePONSerdesReg(0x0800, 0xD154, 0x2000, 0x2000);   /* # [13] core_dp_s_rstb = 1 */
    if (pll_use == TWO_PLLS)
    ...
    writePONSerdesReg(0x0800,0xd010,0x0200,0x0200);
    ...
    rd_serdes=readPONSerdesReg(0x0800, 0xd01a);  // RX_PMD_LOCK[0xD01A] , bit[0] = 1
    __logDebug("  RX_DSC_LOCK =0x%08x. ",rd_serdes );

    rd_serdes = readPONSerdesReg(0x0800,0xd01e); // # Get DSC_STATE [0xD0E1] bits[15:11] = 9 = Done
    ...
    { 
        ...
        writePONSerdesReg(0x0800, 0xD040, 0x0080, 0x0080);
        writePONSerdesReg(0x0800, 0xD041, 0x0008, 0x000F);
        writePONSerdesReg(0x0800, 0xD042, 0x0000, 0x0007);
    }
...
void pon_drv_gpon_init(serdes_wan_type_t wan_type , int pll50mhz)
{
    ...
    switch (wan_type)
    {
        ...
        default:
            __logError("\n Undefined PON type!!! Using GPON as default\n");
            break;
