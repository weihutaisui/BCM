    ...
    if (PonCfgDbGetUpRate() == LaserRate10G)
        {
          ...
          PonMgrRdpaLlid8021AESet(link,true);
          
          PonMgrRdpaLlidSciSet(link,true);
          EponSetAesTx (1UL << link, EponAesExplicitSci);
        ...
        PonMgrRdpaLlid8021AESet(link,true);
        PonMgrRdpaLlidSciSet(link,true);
        /*if AesTx set, there is no traffic can be sent.but if not set, 10M traffic can be send.*/
        //EponSetAesTx (1UL << link, EponAesExplicitSci);
