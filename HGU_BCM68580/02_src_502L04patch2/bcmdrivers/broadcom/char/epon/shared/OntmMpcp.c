...
#define MaxMpcpDownTime           10 //10S
static U8 mpcpDownTimeCnt[TkOnuMaxBiDirLlids] = {0};
...
BOOL g_MpcpRegisterEn = TRUE; /* the flag for enable/disable MPCP reigster process */
...
BOOL OntPutAssociateMcastInService (LinkIndex link, PhyLlid assignedPort, LinkIndex *mcastLink)
    ...
    __logDebug("assigned Link: %d, mPhyLLID: 0x%x\n", link, assignedPort);
  
    // This link was not already assigned to a Port.
    if (mpcpCfg.biDirLinks[link].associatedUniDirLink == TkOnuInvalidLlid)
        {
        // try to find if Port is already assigned
        for (LinkIdx = TkOnuFirstRxOnlyLlid; LinkIdx <= TkOnuLastRxOnlyLlid; LinkIdx++)
            {
            if (PonMgrGetPhyLlid(LinkIdx) == assignedPort)
                {
                isfound = TRUE;
                break;
                }
        ...
        if (!isfound)
            ...
            for (LinkIdx = TkOnuFirstRxOnlyLlid; LinkIdx <= TkOnuLastRxOnlyLlid; LinkIdx++)
                {
                if (mpcpCfg.uniDirLinks[LinkIdx].useCount == 0)
                    {
                    isfound = TRUE;
                    break;
                    }
            ...
            if((!isfound) && (TkOnuNumBiDirLlids > mpcpCfg.numToRegister))
                {
            ...
            if  ((LinkIdx <= TkOnuLastRxOnlyLlid) && 
                //shared LLID is installed by other link.
                (mpcpCfg.uniDirLinks[LinkIdx].useCount != 0))
                {
                //just only update the local data.
                *mcastLink = LinkIdx;
                mpcpCfg.biDirLinks[link].associatedUniDirLink = *mcastLink;
                mpcpCfg.uniDirLinks[LinkIdx].useCount++; 
                return TRUE;
                }
            }

        // If no free spot, return right away.
        if ((!isfound) || (mpcpCfg.uniDirLinks[LinkIdx].useCount != 0))
            {
            __logError("No free spot!\n");
            return FALSE;
            }

        // If we are still here, that means everything is in order.
        mpcpCfg.biDirLinks[link].associatedUniDirLink = LinkIdx;
        mpcpCfg.uniDirLinks[LinkIdx].useCount++;

        // Update rules TODO
        // Update hardware. Don't check use count to save code.
        PonMgrCreateLink (LinkIdx, assignedPort);
        *mcastLink = LinkIdx;
        return TRUE;
        }
    else
        {
        LinkIdx = mpcpCfg.biDirLinks[link].associatedUniDirLink;
        ...
        if (PonMgrGetPhyLlid(LinkIdx) == assignedPort)
            ...
            *mcastLink = LinkIdx;
            ...
            {
            __logError("Associate uLink(%d) with mPhyLLID(0x%x) fail! Already associated with mPhyLLID(0x%x)\n", link, assignedPort, PonMgrGetPhyLlid(LinkIdx));
            return FALSE;
            }
        }
...
BOOL OntPutStandaloneMcastInService (PhyLlid assignedPort, LinkIndex *mcastLink)
    ...
    __logDebug("assigned mPhyLLID: 0x%x\n", assignedPort);
    
    // try to find if Port is already assigned
    for (LinkIdx = TkOnuDynStartRxOnlyLlid; LinkIdx <= TkOnuDynEndRxOnlyLlid; LinkIdx++)
        {
        if (PonMgrGetPhyLlid(LinkIdx) == assignedPort)
            {
            *mcastLink = LinkIdx;
            __logError("ERR! mPhyLLID(0x%x) already assigned\n", assignedPort);
            return FALSE;
            }
        }
    ...
    for (LinkIdx = TkOnuDynStartRxOnlyLlid; LinkIdx <= TkOnuDynEndRxOnlyLlid; LinkIdx++)
        ...
        if (mpcpCfg.uniDirLinks[LinkIdx].useCount == 0)
            {
            mpcpCfg.uniDirLinks[LinkIdx].useCount = 1;
            PonMgrCreateLink (LinkIdx, assignedPort);
            *mcastLink = LinkIdx;
            return TRUE;
            }
        }
    if (TkOnuNumBiDirLlids > mpcpCfg.numToRegister)
        {
#ifdef TkOnuFullFeatureLlids
        if(mpcpCfg.biDirLinks[TkOnuFirstRxOnlyLlid-1].state == rdpa_epon_link_unregistered)
            {
            ...
            PonMgrCreateLink (TkOnuFirstRxOnlyLlid, assignedPort);
...
////////////////////////////////////////////////////////////////////////////////
static
int OntPutMcastInService (rdpa_epon_assign_mcast_op op, LinkIndex link, PhyLlid assignedPort, LinkIndex *mcastLinkIdx)
    {
    BOOL rc = FALSE;
    LinkIndex mcastLink = TkOnuInvalidLlid;
    BDMF_MATTR(gem_attrs, rdpa_gem_drv());
    rdpa_gem_flow_ds_cfg_t ds_cfg = {};
    bdmf_object_handle gem;
    
    if (rdpa_epon_mcast_associate_set == op)
        {
        rc = OntPutAssociateMcastInService(link, assignedPort, &mcastLink);
        }
    else if(rdpa_epon_mcast_standalone_set == op)
        {
        rc = OntPutStandaloneMcastInService(assignedPort, &mcastLink);
        }
    else
        {
        __logError("Unknow rdpa_epon_assign_mcast_op=%d\n", op);
        return -1;
        }
    ...
    if(rc)
        ...
        if(mpcpCfg.uniDirLinks[mcastLink].useCount == 1)
            {
            rc = rdpa_gem_index_set(gem_attrs, mcastLink);
            ...
            if (rc < 0)
                {
                __logError( "Failed to create downstream runner port\n");
                return -1;
                }
            }
        }
        ...
        {
        __logError("PutMcastInService fail!\n");
        return -1;
        }

        *mcastLinkIdx = mcastLink;
        return 0;
    } // OntPutMcastInService

static
BOOL OntPutAssociateMcastOutOfService (LinkIndex link, PhyLlid assignedPort, LinkIndex* Multilink)
    {
    LinkIndex  LinkIdx;
    PhyLlid existPort;
    BOOL toRemoveResource = FALSE;
    
    __logDebug("De-assigned Link: %d, mPhyLLID: 0x%x\n", link, assignedPort);
    
    if(link < TkOnuMaxBiDirLlids)
        {
        LinkIdx = mpcpCfg.biDirLinks[link].associatedUniDirLink;
        if (LinkIdx != TkOnuInvalidLlid)
            {
            BOOL toRemove = FALSE;

            existPort = PonMgrGetPhyLlid(LinkIdx);
            ...
            if (toRemove)
                {
                mpcpCfg.biDirLinks[link].associatedUniDirLink = TkOnuInvalidLlid;
                mpcpCfg.uniDirLinks[LinkIdx].useCount--;
        ...
        *Multilink = LinkIdx;  
        return toRemoveResource;
    }

static
BOOL OntPutStandaloneMcastOutOfService (PhyLlid assignedPort, LinkIndex* Multilink)
    {
    LinkIndex  LinkIdx;
    PhyLlid existPort;
    BOOL toRemoveResource = FALSE;
    
    __logDebug("De-assigned mPhyLLID: 0x%x\n", assignedPort);
...
#ifdef TkOnuFullFeatureLlids
        if(TRUE == toRemoveResource && TkOnuDynStartRxOnlyLlid == LinkIdx 
            && TkOnuNumRxOnlyLlids!= TkOnuRsvNumRxOnlyLlids)
            {
            TkOnuFirstRxOnlyLlid++;
            }
#endif
...
////////////////////////////////////////////////////////////////////////////////
static
int OntPutMcastOutOfService (rdpa_epon_assign_mcast_op op, LinkIndex link, PhyLlid assignedPort, LinkIndex *mcastLinkIdx)
    {
    int rc;
    BOOL toRemoveResource = FALSE;
    LinkIndex Multilink = TkOnuInvalidLlid;
    bdmf_object_handle gem;
    
    if (rdpa_epon_mcast_associate_set == op)
        {
        toRemoveResource = OntPutAssociateMcastOutOfService(link, assignedPort, &Multilink);
        }
    else if(rdpa_epon_mcast_standalone_set == op)
        {
        toRemoveResource = OntPutStandaloneMcastOutOfService(assignedPort, &Multilink);
        }
    else
        {
        __logError("Unknow rdpa_epon_assign_mcast_op=%d\n", op);
    ...
    if(toRemoveResource)
        {
        rc = rdpa_gem_get(Multilink, &gem);
        if(!rc)
            {
            bdmf_put(gem);
            bdmf_destroy(gem);
            }
        PonMgrDestroyLink(Multilink);
        // Update rules TODO
        //UpdateMcastLinkRule(TkOnuFirstRxOnlyLlid+uniDirLink,FALSE);
        }
    else
        {
        __logNotice("Ignore: De-assign link=%d, PhyLlid=0x%x. Resource busy\n", link, assignedPort);
        }

        *mcastLinkIdx = Multilink;
        return 0;
...
void PutLinkInService (LinkIndex link)
    ...
    mcast_link.ulink_map = EPON_ASSOCIATE_LINK_IDX;
    ...
    if(!isCarMode() && !OntDirDefLinkState(link))
...
void PutLinkOutOfService (LinkIndex link)
    ...
    if(mpcpCfg.biDirLinks[link].associatedUniDirLink != TkOnuInvalidLlid)
        {
        (void)OntPutMcastOutOfService (rdpa_epon_mcast_associate_set, link,
            PonMgrGetPhyLlid(mpcpCfg.biDirLinks[link].associatedUniDirLink), &mcastLinkIdx);
        }
    ...
    if (mpcpCfg.numRegistered == 0)
        ...
        PonMgrCreateLink (TkOnuDefaultRxOnlyLlid, GetBroadcastLlid());
...
void HandleMpcpDiscovery (const MpcpGateMsg * gateMsg)
    ...
    if (rdpa_epon_link_registering == mpcpCfg.biDirLinks[currentLink].state)
        ...
        if (SynchronizeMpcp (gateMsg))
            {
            static BOOL isFirstDiscGate = TRUE;
            if (isFirstDiscGate) /* don't response the first one Disc.Gate, just sync MAC */
                {
                MpcpDebug (MpcpDebugStd, ("MPCP: Ignore 1st Disc.Gate\n"));
                isFirstDiscGate = FALSE;
                return;
                }
...
void HandleMpcp (U8  *msg)
    ...
    if (!g_MpcpRegisterEn)
        {
            __logNotice("Mpcp Registration disabled!\n");
            return;
        }
        ...
        switch (mpcpCfg.biDirLinks[link].state)
            ...
            case rdpa_epon_link_in_service:
                ...
                if (!PonMgrLinkAlive (link))
                    ...
                    // Normally OAM stack would reset link. 
                    // If we lost this msg, link would be stuck in this state. 
                    // We use timeout to recover it.
                    mpcpDownTimeCnt[link]++;
                    if (mpcpDownTimeCnt[link] > MaxMpcpDownTime)
                        {
                        MpcpFatal();
                        }
                    }
...
void OntmMpcpInit (void)
    ...
    for (biDirLink = 0; biDirLink < TkOnuNumTxLlids; biDirLink++)
        {
        mpcpCfg.biDirLinks[biDirLink].associatedUniDirLink = TkOnuInvalidLlid;
    ...
    if(EponGetMacMode() == EPON_MAC_NORMAL_MODE)
        ...
        for (linIdx = 0; linIdx < TkOnuNumBiDirLlids; linIdx++)            
            PutLinkOutOfService (linIdx);

        OntmMpcpAdjustNumLinks(PonCfgDbGetLinkNum());
        }

    pmd_dev_assign_pon_stack_callback(epon_set_pmd_fb_done);
...
BOOL OntmMpcpAssignMcast (rdpa_epon_assign_mcast_op op, LinkIndex unicast_link_id, 
    ...
    BOOL  result = FALSE;
    int rc = -1;
    LinkIndex mcastLinkIdx = TkOnuInvalidLlid;

    if (mcast_link->enable)
        {
        rc = OntPutMcastInService (op, unicast_link_id, mcast_link->llid, &mcastLinkIdx);
        }
    else
        {
        rc = OntPutMcastOutOfService (op, unicast_link_id, mcast_link->llid, &mcastLinkIdx);
        }

    if(!rc)
        {
        *mlink_idx = mcastLinkIdx;
        result = TRUE;
        }
...
void OntmMpcpCmdInfo (void)
    ...
    for (argc = TkOnuFirstRxOnlyLlid; argc <= TkOnuLastRxOnlyLlid; argc++)
        {
