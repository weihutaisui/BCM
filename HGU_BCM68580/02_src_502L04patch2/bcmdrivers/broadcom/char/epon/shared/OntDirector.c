...
////////////////////////////////////////////////////////////////////////////////
//extern
int OntDirGlobalShaperSet(U32 rate)
    {
    U32 l1Map = 0;
    ...
    return (OntDirShaperSet (l1Map,rate,OnuShaperDefBurstSize,&globalShaperIndex) ? 0 : -1);
    }
...
int OntFlushLinkSet(LinkIndex link, BOOL enable)
    {
    
    spin_lock_bh(&ontmgr_spinlock);	
    if (enable)
        PonMgrFlushL1L2(link);
    spin_unlock_bh(&ontmgr_spinlock);

    return 0;
    }

EXPORT_SYMBOL(OntFlushLinkSet);
#endif
...
////////////////////////////////////////////////////////////////////////////////
/// \brief Get link index according to LLID value 
///
///
/// \return
/// None
////////////////////////////////////////////////////////////////////////////////
//extern
BOOL OntDirLinkIndexGet(PhyLlid llid, LinkIndex *linkIndex)
    {
    BOOL retVal = TRUE;
    LinkIndex i;
    PhyLlid llid_loop;
    ...
    *linkIndex = InvalidMcastIdx;
    for(i = 0; i < TkOnuNumTotalLlids; i++)
        {
        OntDirLLIDGet(i, &llid_loop);
        if(llid_loop == llid)
            {
            *linkIndex = i;
            break;
...
BOOL OntDirEponLinkLoopbackGet(LinkIndex link, U8 *loopback)
    ...
    if (rdpa_port_get(rdpa_wan_type_to_if(rdpa_wan_epon), &port_obj) != 0)
        return FALSE;
...
void OntDirInit(void)
    ...
    if (macMode == EPON_MAC_NORMAL_MODE)
        {
        //10G FEC enable by default
        for (i = 0; i < TkOnuFirstRxOnlyLlid; i++)   
            {
            PonMgrSetDefaultFec(i, FecApp10G);
            }
        }
#endif
    }
