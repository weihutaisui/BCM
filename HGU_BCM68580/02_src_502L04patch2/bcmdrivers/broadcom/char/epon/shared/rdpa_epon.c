...
typedef struct
{
    ...
    rdpa_epon_mcast_link_t mcast_link[RDPA_EPON_MAX_PHY_LLID];
    rdpa_epon_fec_enable_t fec_enable[RDPA_EPON_MAX_PHY_LLID];
} epon_drv_priv_t;
    ...
    struct bdmf_attr *ad, bdmf_index index, void *val, uint32_t size)
{
    ...
    if(rc)
        BDMF_TRACE_RET(BDMF_ERR_INTERNAL,"epon stack returned registered links: %u\n",
           *(unsigned char*)val);
...
int (*p_PonMgrLinkFlushSet)(LinkIndex link, BOOL enable);
EXPORT_SYMBOL(p_PonMgrLinkFlushSet);

static int epon_attr_link_flush_write(struct bdmf_object *mo,
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
    int rc = BDMF_ERR_OK;

    rc =  p_PonMgrLinkFlushSet(index,*(bdmf_boolean*)val);

    BDMF_TRACE_RET(rc,
        "Flushing link %ld, to state %u\n", index, *(bdmf_boolean*)val);
}
...
BOOL (*p_OntmMpcpAssignMcast) (rdpa_epon_assign_mcast_op op, LinkIndex unicast_link_id, 
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
    ...
    if (EPON_ASSOCIATE_LINK_IDX == mcast_link->ulink_map) /* ASSOCIATE */
    {
        if (p_OntmMpcpAssignMcast(rdpa_epon_mcast_associate_set, index, mcast_link, &multi_index))
        {      
            epon_priv->mcast_link[multi_index].enable = mcast_link->enable;
            if (mcast_link->enable)
            {
                epon_priv->mcast_link[multi_index].llid = mcast_link->llid;
                epon_priv->mcast_link[multi_index].ulink_map |= (1 << index);
            }
            else
            {
                epon_priv->mcast_link[multi_index].ulink_map &= ~(1 << index);
                if (0 == epon_priv->mcast_link[multi_index].ulink_map)
                    epon_priv->mcast_link[multi_index].llid = 0;
            }
        }
        else
            return -1;
    ...
    {
        if (p_OntmMpcpAssignMcast(rdpa_epon_mcast_standalone_set, index, mcast_link, &multi_index))
        {
            epon_priv->mcast_link[multi_index].enable = mcast_link->enable;
            if (mcast_link->enable)
                epon_priv->mcast_link[multi_index].llid = mcast_link->llid;
...
struct bdmf_aggr_type rdpa_epon_mcast_link_type = {
    ...
    .fields = (struct bdmf_attr[]) {
        ...
        { .name = "ulink_map", .help = "Physical Uni-link map which mapped to Multi-link",
            .type = bdmf_attr_number, .size = sizeof(uint32_t),
            .offset = offsetof(rdpa_epon_mcast_link_t, ulink_map)
        },
    ...
    struct bdmf_attr *ad, bdmf_index index, const void *val, uint32_t size)
{
    ...
    BDMF_TRACE_INFO("llid %lu: ds %u, us %u\n", index, fec_enable->ds, fec_enable->us);
...
static struct bdmf_attr epon_attrs[] =
{
    ...
    { .name = "link_enable", .help = "Enable epon link",
        ...
        .array_size = RDPA_EPON_MAX_PHY_LLID
    },
    { .name = "link_flush", .help = "Flush epon link",
        .flags = BDMF_ATTR_WRITE | BDMF_ATTR_CONFIG,
        .type = bdmf_attr_boolean, 
        .write = epon_attr_link_flush_write,
        .array_size = RDPA_EPON_MAX_PHY_LLID
    },
