...
static int compat_get_l2cpstats_parms(PortL2cpStatsParaCompat   * ioctlParms32, PortL2cpStats  *portL2cpStats)
{
    int err = 0;
    
    portL2cpStats->port = ioctlParms32->port;
    portL2cpStats->l2cpStats = (L2cpStats *)ioctlParms32->l2cpStats;
   
    return err;     
}

static int compat_put_l2cpstats_parms(PortL2cpStatsParaCompat  * ioctlParms32, PortL2cpStats   *portL2cpStats)
{
    int err = 0;

    ioctlParms32->port = portL2cpStats->port;
    ioctlParms32->l2cpStats = (compat_uptr_t)portL2cpStats->l2cpStats;
...
long EponIoctlAcessCompat(unsigned int cmd, unsigned long arg)
{
        ...
        (ioctlFuncs[cmdnr].fun != NULL))
    {
        ...
        switch (cmd) {
            ...
            break;
            case BCMEPONPortL2cpStats:
            {
                err = compat_get_l2cpstats_parms(&EArgCompat.eponparm.portL2cpStats, &EArg.eponparm.portL2cpStats);
                ret = (*ioctlFuncs[cmdnr].fun) (&EArg);
                err = compat_put_l2cpstats_parms(&EArgCompat.eponparm.portL2cpStats, &EArg.eponparm.portL2cpStats);
            }
            default:
            {
