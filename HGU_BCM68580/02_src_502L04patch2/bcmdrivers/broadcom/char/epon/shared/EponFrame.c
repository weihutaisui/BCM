...
BOOL EponMpcpReceive(void)
    ...
    while(1)
        ...
        ethHeader = (EthernetFrame*)((U8 *)info.data + info.data_offset);
        if(is_link_local_ether_addr(ethHeader->da.u8))
        {
            statsPon[EponBiDnL2cpFramesRx] ++; 
            statsPon[EponBiDnL2cpOctetsRx] += info.size;
        }
...
void EponMpcpQueueOpen(void)
    ...
    rc = rdpa_system_cpu_reason_to_tc_set(system_obj, rdpa_cpu_rx_reason_omci, RDPA_MPCP_CPU_RX_QUEUE_ID);
...
int EponUsrLinkSend(U8 link, U8 *pbuf, U32 size, BOOL skb)
    ...
    if (skb)
        {
            ...
            daPtr = PNBUFF_2_SKBUFF(pbuf)->data;
        }   
    else
        {
    ...
    if(is_link_local_ether_addr(daPtr))
	    {
            statsPon[EponBiUpL2cpFramesTx] ++; 
            statsPon[EponBiUpL2cpOctetsTx] += size;
	    }
...
static int EponOamHandle(rdpa_cpu_rx_info_t *info)
    {
        ...
        sysb->mark = SKBMARK_SET_PORT(sysb->mark, info->reason_data);
        ...
        if (oam_dev != NULL)
            ...
            /* EAP handle */
                ...
                if (dev == NULL)
                    ...
                    if(is_link_local_ether_addr(((EthernetFrame *)dataBuf)->da.u8))
                    {
                        statsPon[EponBiDnL2cpFramesDiscarded] ++; 
                        statsPon[EponBiDnL2cpOctetsDiscarded] += len;
                    }
...
void EponOamQueueOpen(void)
    ...
    /* Map reason UDEF_1 to special TC, and TC to RDPA_OAM_CPU_RX_QUEUE_ID.
       For simplicity, use TC = RDPA_OAM_CPU_RX_QUEUE_ID */
    rc = rdpa_system_cpu_reason_to_tc_set(system_obj, rdpa_cpu_rx_reason_oam, RDPA_OAM_CPU_RX_QUEUE_ID);
    rc = rc ? rc : rdpa_cpu_tc_to_rxq_set(oam_obj, RDPA_OAM_CPU_RX_QUEUE_ID, RDPA_OAM_CPU_RX_QUEUE_ID);
    if (rc < 0)
        {
        BCM_LOG_ERROR(BCM_LOG_ID_EPON, "Error (%d) configuraing TC to CPU reason and queue", rc);
...
void EponDyingGaspIfInfo(void)
    {
    dg_info.method = rdpa_cpu_tx_port;
    dg_info.port = rdpa_wan_type_to_if(rdpa_wan_epon);
    dg_info.cpu_port = rdpa_cpu_host;
    dg_info.x.wan.flow = OntUserGetLinkFlow(0, 101); /* dependent on oam_mode */
