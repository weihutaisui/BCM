...
#define L2CP_STATS_DEBUG                     0

#ifdef CONFIG_BCM96858
#define STATS_L2CP_MAX_PORTS 9 //8 UNIs + 1 Epon0
#else
#define STATS_L2CP_MAX_PORTS 5 //4 UNIs + 1 Epon0
#endif
...
// L2CP statistics for PON and Ether ports
L2cpStats statsPortL2cpStats[STATS_L2CP_MAX_PORTS] = {};
HookDevice hookDevice[STATS_L2CP_MAX_PORTS] = {};
...
U64 BULK statsLink[TkOnuNumTotalLlids][EponLinkHwStatCount];
...
StatThd BULK statsLinkThd[TkOnuNumTotalLlids][EponLinkHwStatCount];
...
void CollectLinkRx ( LinkIndex BULK lnk, U8 statStart, U8 statEnd )
    ...
    for (stat = statStart; stat <= statEnd; ++stat)
        {
        EponReadLinkStat(lnk, (EponLinkStatId)stat, &dbuf, FALSE);
...
void CollectLinkRxOnly (U8 lnk)
    ...
    // stats collected directly
    //downstream
    if (lnk < TkOnuFirstRxOnlyLlid)
        return;        
    
    for (stat = EponDnTotalBytesRx; stat <= EponDnOamBytesRx; ++stat)
        {
        EponReadLinkStat (lnk,stat,&dbuf, FALSE);
...
void CollectWanStats (void)
    ...
    bdmf_lock();
    rc = rdpa_port_get(rdpa_wan_type_to_if(rdpa_wan_epon), &port_obj);
    if (rc)
        goto unlock_exit;
...
void StatsClearEponLinkStats (U8 linkStart, U8 linkEnd)
    ...
    for(link = linkStart; link < linkEnd; ++link)
        ...
        for (stat = EponBiDnTotalBytesRx;
            ...
            {
            U64 buf;
            EponReadLinkStat(link,stat, &buf, FALSE);
            if (((EponBiUpTotalBytesTx + stat)) <= EponBiUpUnicastFramesTx)
                {
                EponReadLinkStat(link, (EponBiUpTotalBytesTx + stat), &buf, FALSE);
                }
            }
        }
...
////////////////////////////////////////////////////////////////////////////////
/// StatsGetL2cp:  Get port L2cp stat
///
 // Parameters:
/// \param port     UNI port
/// 
...
/// TRUE if stat is supported, FALSE otherwise
////////////////////////////////////////////////////////////////////////////////
void StatsGetL2cp (U8 port, L2cpStats * dst)
    {
    memcpy(dst, &statsPortL2cpStats[port], sizeof(L2cpStats));
    }

////////////////////////////////////////////////////////////////////////////////
/// StatsClearL2cp:  clear port L2cp stat
///
 // Parameters:
/// \param port     UNI port
/// 
...
/// TRUE if stat is supported, FALSE otherwise
////////////////////////////////////////////////////////////////////////////////
void StatsClearL2cp (U8 port)
    {
    memset(&statsPortL2cpStats[port],0, sizeof(L2cpStats));
    }
...
void StatsClearAll (void)
   ...
   memset (statsPortL2cpStats, 0, sizeof(statsPortL2cpStats));
...
int findDevInCashe(const struct nf_hook_state *state,unsigned int hooknum)
{
    int i;
    int ifidx,ifindex;
    int devIdex = -1; 
    ...
    if(hooknum == NF_BR_LOCAL_IN)
        ...
        ifname = (char *)state->in->name;
        }
    else
        {
        ifindex = state->out->ifindex;
        ifname = (char *)state->out->name;
        }

    for (i = 0; i < STATS_L2CP_MAX_PORTS; i++)
        {
        if(hookDevice[i].ifindex == ifindex)//find dev in the cashe
            { 
            return i;
            }
        }

    //did't find the dev in cashe, store it
    if (strncmp(ifname, "epon0", 4) == 0)
        {
        hookDevice[0].ifindex = ifindex;//epon0 stored in hookDevice[0]
        devIdex = 0;
        }
    else if (strncmp(ifname, "eth", 3) == 0)// DPOE don't care wifi and oam 
        {
        sscanf(ifname, "eth%d.", &ifidx);

        hookDevice[ifidx+1].ifindex = ifindex;//ethx stored from hookDevice[1]
        devIdex = ifidx+1;
        }
...
unsigned int hook_func(const struct nf_hook_ops *ops, 
    ...
    if (is_link_local_ether_addr(mac_header->h_dest))
        {
        int devIndex = findDevInCashe(state,ops->hooknum);

        if(devIndex < 0)
            return NF_ACCEPT;
        
        if (ops->hooknum == NF_BR_LOCAL_IN)
            {
                if(!devIndex)
                    {
                    statsPon[EponBiDnL2cpFramesRx] ++; 
                    statsPon[EponBiDnL2cpOctetsRx] += skb->len + 14; // eth header
                    }
                else
                    {
                    statsPortL2cpStats[devIndex - 1].rxL2cpFrames++;
                    statsPortL2cpStats[devIndex - 1].rxL2cpBytes += skb->len + 14; // eth header
                    }
            }    
        else
            {
                if(!devIndex)        
                    {                
                    statsPon[EponBiUpL2cpFramesTx] ++; 
                    statsPon[EponBiUpL2cpOctetsTx] += skb->len + 14; // eth header
                    }
                else
                    {
                    statsPortL2cpStats[devIndex - 1].txL2cpFrames++;
                    statsPortL2cpStats[devIndex - 1].txL2cpBytes += skb->len + 14; // eth header
                    }
            }
            ...
            {
            int i;
            printk("=====================L2CP Statistics===================\n");
            printk("%10s%15s%15s%15s%15s\n","Interface","Rx Frame","Rx Bytes","Tx Frame","Tx Bytes");
            printk("%-10s%15lld%15lld%15lld%15lld\n", "epon0", 
                statsPon[EponBiDnL2cpFramesRx], statsPon[EponBiDnL2cpOctetsRx],
                statsPon[EponBiUpL2cpFramesTx], statsPon[EponBiUpL2cpOctetsTx]);
            for (i = 0; i < STATS_L2CP_MAX_PORTS - 1; i++)
                {
                    printk("%3s%-7d%15lld%15lld%15lld%15lld\n", "eth",i, 
                    statsPortL2cpStats[i].rxL2cpFrames, statsPortL2cpStats[i].rxL2cpBytes,
                    statsPortL2cpStats[i].txL2cpFrames, statsPortL2cpStats[i].txL2cpBytes);
                }
            printk("-------------------------------------------------------\n");    
            }
#endif
...
static int __init hook_init(void)
    {
    nfhoIn.hook = (nf_hookfn *)hook_func;
    nfhoIn.hooknum = NF_BR_LOCAL_IN;
    nfhoIn.pf = PF_BRIDGE;
    nfhoIn.priority = NF_BR_PRI_FIRST;

    nf_register_hook(&nfhoIn);
    
    nfhoOut.hook = (nf_hookfn *)hook_func;
    nfhoOut.hooknum = NF_BR_LOCAL_OUT;
    nfhoOut.pf = PF_BRIDGE;
    nfhoOut.priority = NF_BR_PRI_FIRST;
...
void StatsInit (void)
    ...
    //init L2cp stats for Dpoe
    if (oam_mode == rdpa_epon_dpoe)
        {
        int i = 0;
        
        for (i = 0; i < STATS_L2CP_MAX_PORTS; i++)
            {
            hookDevice[i].ifindex = -1;
            }
