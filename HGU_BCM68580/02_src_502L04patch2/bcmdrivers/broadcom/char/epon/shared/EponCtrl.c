...
int EponCfg10gFecAutoDet(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;
    BOOL enable = (BOOL)(EArg->eponparm.fec10gAutoDet);
    
    DumpData(DebugIoctl,
        "EponCfg10gFecAutoDet",(U8 *)EArg,sizeof(EponCtlParamt));
    
    if (EArg->ope == EponSetOpe)
    {
        PonMgr10gFecAutoDetSet(enable);
    }
    else
    {
        EArg->eponparm.fec10gAutoDet = (U8)PonMgr10gFecAutoDetGet();
    }
    
    return ret;       
}

int EponCfg10gFecSwitchOnce(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;
    
    DumpData(DebugIoctl,
        "EponCfg10gFecSwitchOnce",(U8 *)EArg,sizeof(EponCtlParamt));
    
    if (PonMgr10gFecAutoDetGet() == FALSE)
        PonMgr10gFecAutoSwitch();
    else
        printk("10G FEC auto detection must be disabled first \n");
...
int EponCfgGetLinkIndex(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;

    DumpData(DebugIoctl,"EponCfgGetLinkIndex",(U8 *)EArg,sizeof(EponCtlParamt));

    if (EArg->ope == EponGetOpe)
    {
        if(!OntDirLinkIndexGet(EArg->eponparm.llid.LLID, &EArg->eponparm.llid.link))
        {
            ret = EponSTATUSNOTFOUND;
        }
    }
...
int EponCfgAssignMcast(EponCtlParamt* EArg)
{
    ...
    if (EArg->ope == EponSetOpe)
    {
        if (ActDelAllMcastLlid == assignMcast->flags) /* Delete all, only apply to standalone mode */
        {
            uint32_t multi_index;
            for (multi_index = 0; multi_index < RDPA_EPON_MAX_PHY_LLID; multi_index++)
            {
                rc = rdpa_epon_mcast_link_get(epon_obj, multi_index, &mcast_link);
                if (rc) 
        ...
        else /* add/delete one*/
        {
            ...
            mcast_link.ulink_map = assignMcast->idxMcast; /* use "ulink_map" as standaone/associate flag in SET operation */
            rc = rdpa_epon_mcast_link_set(epon_obj, assignMcast->link, &mcast_link);
            if (rc) 
                goto epn_ioctl_cfg_mcast_err_2;
        }
    ...
    {
        uint32_t multi_index;
        if (ActGetMcastLlidByMcastIdx == assignMcast->flags) /* Get phyLlid by idxMcast */
        {
            assignMcast->assignedPort = InvalidPhyLlid;
            rc = rdpa_epon_mcast_link_get(epon_obj, assignMcast->idxMcast, &mcast_link);
            if (rc) 
                goto epn_ioctl_cfg_mcast_err_2;
        ...
        else if (ActGetMcastIdxByMcastLlid == assignMcast->flags) /* get idxMcast by phyLlid*/
        {
            assignMcast->idxMcast = InvalidMcastIdx;
            for (multi_index = 0; multi_index < RDPA_EPON_MAX_PHY_LLID; multi_index++)
            {
                rc = rdpa_epon_mcast_link_get(epon_obj, multi_index, &mcast_link);
                if (rc) 
                ...
                if(assignMcast->assignedPort == mcast_link.llid)
                {
                    assignMcast->idxMcast = multi_index;
                    break;
                }
            }
        }
        else if (ActGetMcastLlidAndIdxByUcastIdx == assignMcast->flags) /* get idxMcast and phyLlid by ucastIdx*/
        {
            #define IS_IDX_IN_MAP(idx, map) ((map & (1 << idx)) >> idx)
            ...
            for (multi_index = 0; multi_index < RDPA_EPON_MAX_PHY_LLID; multi_index++)
            {
                ...
                if (IS_IDX_IN_MAP(assignMcast->link, mcast_link.ulink_map))
                {
                    assignMcast->assignedPort = mcast_link.llid;
                    assignMcast->idxMcast = multi_index;
                    break;
...
int EponCfgClk1ppsCompTrans(EponCtlParamt* EArg)
{
    ...
    {
        ClkTransGetConfig(&EArg->eponparm.clkTransCfg);
    }
...
int EponCfgClockTransferTime(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;
        
    DumpData(DebugIoctl,"EponCfgClockTransferTime",(U8 *)EArg,sizeof(EponCtlParamt));
    
    if (EArg->ope == EponSetOpe)
    {
        //1. Send MPCP Clock
        ClkTrans1ppsTickCallback(EArg->eponparm.clkTransportTime.mpcpRefClock);

        //2. Tod string
        ClkTransTodProcess(&EArg->eponparm.clkTransportTime.strTod[0], EArg->eponparm.clkTransportTime.todLen);
    }
    else
    {
        U8 length = 0;
        ...
        //1. Fill Mpcp Clock
        EArg->eponparm.clkTransportTime.mpcpRefClock = ClkGetCurrentMpcpRefClock();

        //2. Fill Tod String
        ClkGetCurrentTodString(&EArg->eponparm.clkTransportTime.strTod[0], &length);
        EArg->eponparm.clkTransportTime.todLen = length;
    }
   
    return ret; 
}

int EponCfgClockPropagationParameter(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;
        
    DumpData(DebugIoctl,"EponCfgClockPropagationParameter",(U8 *)EArg,sizeof(EponCtlParamt));
    
    if (EArg->ope == EponSetOpe)
    {
        ClkTransSetPropagationParameter(&EArg->eponparm.clkPropagationParameter);
    }
    else
    {
        ClkTransGetPropagationParameter(&EArg->eponparm.clkPropagationParameter);
    }
...
int EponL2cpStats(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;
    U8 port =(U8)EArg->eponparm.portL2cpStats.port;
    L2cpStats l2cpStatsTmp;
    DumpData(DebugIoctl,
        "EponCfgFailSafe",(U8 *)EArg,sizeof(EponCtlParamt));
    ...
    {
        StatsGetL2cp(port,&l2cpStatsTmp);
		 
        copy_to_user((void *)(EArg->eponparm.portL2cpStats.l2cpStats), 
            		(void *)&l2cpStatsTmp,sizeof(L2cpStats));
    }
	
    return ret;       
}

int EponCfgGetMaxPhyLlidNum(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;

    DumpData(DebugIoctl,"EponCfgGetMaxPhyLlidNum",(U8 *)EArg,sizeof(EponCtlParamt));

    if (EArg->ope == EponGetOpe)
    {
        EArg->eponparm.maxPhyLlidNum = RDPA_EPON_MAX_PHY_LLID;
    }

    return ret;
}

int EponCfgMpcpRegisterEn(EponCtlParamt* EArg)
{
    int ret = EponSTATUSSUCCESS;    
    
    DumpData(DebugIoctl,
        "EponCfgMpcpRegisterEn",(U8 *)EArg,sizeof(EponCtlParamt));
    
    if (EArg->ope == EponSetOpe)
    {
        g_MpcpRegisterEn = (BOOL)(EArg->eponparm.MpcpRegisterEn);
    }
    else
    {
        EArg->eponparm.MpcpRegisterEn = g_MpcpRegisterEn;
    }
...
EponIoctlHandle_t  ioctlFuncs[] =  //IOTCL function handle
{
    ...
    {BCMEPONCfgClkTransferTime, EponCfgClockTransferTime},
    {BCMEPONCfgClkPropagationParameter, EponCfgClockPropagationParameter},
#endif
#ifdef CONFIG_EPON_10G_SUPPORT
    {BCMEPONCfg10gFecAutoDet, EponCfg10gFecAutoDet},
    {BCMEPONCfg10gFecSWOnce, EponCfg10gFecSwitchOnce},
#endif

    {BCMEPONDumpEpnInfo, EponCfgDumpEpnInfo},
    {BCMEPONPortL2cpStats, EponL2cpStats},
    {BCMEPONGetLinkIndex, EponCfgGetLinkIndex},
    {BCMEPONGetMaxPhyLlidNum, EponCfgGetMaxPhyLlidNum},
    {BCMEPONCfgMpcpRegisterEn, EponCfgMpcpRegisterEn}
};
