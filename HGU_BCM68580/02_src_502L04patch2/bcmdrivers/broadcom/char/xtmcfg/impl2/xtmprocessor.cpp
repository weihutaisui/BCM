...
/* No selection necessary for 63158. BBH is the only option */
   ...
   XtmOsPrintf ("bcmxtmcfg: xDSL G.Fast Mode = %s \n", (m_ulxDSLMode==kXdslModGfast)? "ENABLED" : "DISABLED") ;
            ...
            while( (pConn = pXtmProcessor->m_ConnTable.Enum( &idx )) != NULL ) {
               ...
               if(pConn->Connected()) {
                  if (pConn->SetLinkInfo( (ulPortMask & PORT_TO_PORTID(pXtmProcessor->m_ulTrafficMonitorPort)),
                           &Li, MAX_INTERFACES, NULL, pXtmProcessor->m_ulxDSLMode) != XTMSTS_SUCCESS) {
                     XtmOsPrintf("(%s): Failed to reprogram the SAR for this Non-Bonded mode on Port1\n",__FUNCTION__);
                  }
               }
            }
         }
...
void XTM_PROCESSOR::UpdateLinkStatus ( UINT32 ulPhysPort,  PXTM_INTERFACE_LINK_INFO pLinkInfo )
{
   ...
   if ( (pLinkInfo->ulLinkState == LINK_UP) &&
      ...
      if (ret != 0)
         ...
         MapXtmOsPrintf ("bcmxtmcfg: DS xDSL G.inp Mode = %s \n", (m_ulxDSLDSGinpMode==1)? "ENABLED" : "DISABLED") ;

         m_ulxDSLMode = MibInfo1.adslConnection.modType ;
         MapXtmOsPrintf ("bcmxtmcfg: xDSL G.Fast Mode = %s \n", (m_ulxDSLMode==kXdslModGfast)? "ENABLED" : "DISABLED") ;
         MapXtmOsPrintf ("bcmxtmcfg: xDSL ADSL Mode = %s \n", (m_ulxDSLMode<kVdslModVdsl2)? "ENABLED" : "DISABLED") ;
      }
   }
}
    ...
    PXTM_INTERFACE_LINK_INFO pLinkInfo, UINT32 timerExpired )
{
   ...
   XtmOsPrintf ("(%s) - port = %d, state = %d ulTrafficType = %d\n",__FUNCTION__, ulPhysPort, pLinkInfo->ulLinkState, ulTrafficType) ;
   ...
   if (pLinkInfo->ulLinkState == LINK_UP) {
      if ((timerStopped) && (m_ulPrevRelatedUpEvtValid == 0)) {
         ...
         MapXtmOsPrintf ("bcmxtmcfg: PendEvt1 LI, P=%d, S=%d \n", ulPhysPort, pLinkInfo->ulLinkState) ;
         goto _End ;
      }
   }
   else {
      if ((!timerStopped) || (m_ulPendingEvtValid[ulPhysPort] == 1)) {
         ...
         memcpy (&m_PendingEvtLinkInfo[ulPhysPort], pLinkInfo, sizeof (XTM_INTERFACE_LINK_INFO)) ;
         MapXtmOsPrintf ("bcmxtmcfg: PendEvt2 LI, P=%d, S=%d \n", ulPhysPort, pLinkInfo->ulLinkState) ;
         goto _End ;
      }
      else {
         m_ulPrevRelatedUpEvtValid = 0;
    ...
    //XtmOsPrintf("%s:ulPhysPort[%x] m_ulTrafficMonitorPort[%x]\n",__FUNCTION__,ulPhysPort,m_ulTrafficMonitorPort);
    /* Is this ATM traffic? */
    if ((ulTrafficType == TRAFFIC_TYPE_ATM) || (ulTrafficType == TRAFFIC_TYPE_ATM_BONDED)) {
        ...
        if (m_InitParms.bondConfig.sConfig.autoSenseAtm == BC_ATM_AUTO_SENSE_ENABLE) {
            ...
            XtmOsPrintf ("bcmxtmcfg: AutoSenseATM is not enabled. Link Traffic type from PHY=%lu \n",
                      ulTrafficType) ;
            // Correct the port number to the physical port for Non-Bonding
            // builds.
            m_ulTrafficMonitorPort = ulPhysPort;
        }
    } /* if (atm types) */
                ...
                (ulTrafficType == TRAFFIC_TYPE_ATM ? "ATM" :
                 (ulTrafficType == TRAFFIC_TYPE_PTM ? "PTM" :
                  (ulTrafficType == TRAFFIC_TYPE_PTM_BONDED ? "PTM_BONDED" :
                   (ulTrafficType == TRAFFIC_TYPE_ATM_BONDED ? "ATM_BONDED" : 
                    (ulTrafficType == TRAFFIC_TYPE_PTM_RAW ? "PTM_RAW" : "No Service"))))),
                ulTrafficType) ;
				...
				if( ulPhysPort < MAX_INTERFACES )
				{
					...
					while( (pConn = m_ConnTable.Enum( &i )) != NULL )
					{
                        ...
                        m_LogPhyPortmap[0].PhysicalPortId = PHY_PORTID_0;
								...
								if(pConn->SetCfg(&ConnCfg)!= XTMSTS_SUCCESS)
									XtmOsPrintf("(%s)Failed to reconfigure the connection configuration for bonded case and PORT 0 LINK up\n",__FUNCTION__);
							}
                     //XtmOsPrintf("Calling SetLinkInfo 1\n");
							bxStatus = pConn->SetLinkInfo( (ulLinkMask & ulPortMask),
                                    pLinkInfo, ulBondingPhysPort, pOtherLinkInfo, m_ulxDSLMode);
                                                  ...
                                                  char *pszNetworkDeviceName )
{
    ...
    if( pConn )
    {
        ...
        if (ulLinkMask != 0)
        {
           ...
           pConn->SetLinkInfo( ulLinkMask, &Li, ulBondingPort, pOtherLi, m_ulxDSLMode) ;
        }
        bxStatus = XTMSTS_SUCCESS;
    }
    else
