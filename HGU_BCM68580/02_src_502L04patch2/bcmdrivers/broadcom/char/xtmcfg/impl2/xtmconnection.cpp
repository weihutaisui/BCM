...
#if defined(CONFIG_BCM963148) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963158) 
            pCfg->TransmitQParms[i].usSize = XTMCFG_QUEUE_DROP_THRESHOLD ;
#endif
        }
                                           ...
                                           UINT32 ulxDSLMode)
{
   ...
   //XtmOsPrintf("%s:Enter\n",__FUNCTION__);
    if( m_hDev != INVALID_HANDLE )
    {
        ...
        {
            if( (*m_pfnXtmrtReq) (m_hDev, XTMRT_CMD_LINK_STATUS_CHANGED, &Lsc) == 0 )
            {
                XtmOsRequestLock(m_ulConnSem);
                if(pendingConnected)
                {
...
BCMXTM_STATUS XTM_CONNECTION::DeleteNetworkDevice( void )
{
   ...
   //XtmOsPrintf("%s:Enter\n",__FUNCTION__);
    if( m_ulConnected )
    {
        ...
        bxStatus = SetLinkInfo( 0x0, &LinkInfo, 0, NULL, m_ulxDSLMode );
        if(bxStatus != XTMSTS_SUCCESS)
           return bxStatus;
    }
...
BCMXTM_STATUS XTM_CONNECTION::UnreserveOneTxQueue( PXTMRT_TRANSMIT_QUEUE_ID pTxQId )
{
    ...
    else
    {
            ...
            ( ulIsPtm && pInfo->ulMpQueueCount == 0) )
        {
            ...
            XP_REGS->ulSstMpPriArb03 &= ~(SST_MP_ARB_ALG_PLVLN_MASK<<(8 * ulIdx));
#else
...
#if defined(CONFIG_BCM963158)
                XP_REGS->ulSstMpPriArb03 |=  ((1<<ulSubPriority)<<(8 * ulMpIdx));
#else
 ...
 ***************************************************************************/
void XTM_CONNECTION:: WaitForTxComplete (void)
{
   unsigned int j1,k ;
   ...
   waitLoop = (m_ulxDSLMode<kVdslModVdsl2) ? 0x7ffff : 0x7fff ;
