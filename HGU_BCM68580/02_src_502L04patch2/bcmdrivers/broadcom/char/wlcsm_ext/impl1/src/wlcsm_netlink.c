...
wlcsm_nl_rcv_skb(struct sk_buff *skb)
{
    ...
    if (skb->len >= NLMSG_SPACE(0)) {
        ...
        case WLCSM_MSG_NVRAM_GETALL: {
            int len=0,retlen=0,maxAllowedSize=0;
            char *bigbuffer=NULL;
            char *tempptr=NULL;
            ...
            if(bigbuffer) {
                ...
                retlen=wlcsm_nvram_getall(bigbuffer,len);
                ...
                do {
                    if(len>maxAllowedSize) {
                        wlcsm_sendup_response(msg_hdr->type,bigbuffer,maxAllowedSize,nlh->nlmsg_pid,0);
                        bigbuffer+=maxAllowedSize;
