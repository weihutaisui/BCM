...
static int merlin_direct_txfir_set(E_MERLIN_LANE lane_index, merlin_txfir_t *txfir)
{
    uint16_t reg_val = 0;
...
static int merlin_direct_txfir_get(E_MERLIN_LANE lane_index, merlin_txfir_t *txfir)
{
    uint16_t reg_val = 0;
    ...
    MERLIN_READ_REG(1, lane_index, 0xd110, MASK_BITS_BETWEEN(0, 3), 0, &txfir->pre);
    MERLIN_READ_REG(1, lane_index, 0xd111, MASK_BITS_BETWEEN(0, 5), 0, &txfir->main);
    MERLIN_READ_REG(1, lane_index, 0xd110, MASK_BITS_BETWEEN(5, 9), 5, &txfir->post1);
    MERLIN_READ_REG(1, lane_index, 0xd111, MASK_BITS_BETWEEN(7, 10), 7, &txfir->post2);
...
static int  merlin_direct_ioctl(E_MERLIN_LANE lane_index, merlin_command_t cmd, merlin_control_t *data)
{
    ...
    switch(cmd)
    {
        ...
        case MERLIN_CMD_LOOPBACK_SET:
            ...
            break;
        case MERLIN_CMD_FIR_SET:
            ret = merlin_direct_txfir_set(lane_index, &data->txfir);
            break;
        case MERLIN_CMD_FIR_GET:
            ret = merlin_direct_txfir_get(lane_index, &data->txfir);
            break;
        default:
