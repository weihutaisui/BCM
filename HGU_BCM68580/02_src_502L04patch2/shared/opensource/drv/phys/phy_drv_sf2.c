...
static int ethsw_cl45phy_set_speed(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    phy_dev_caps_get(phy_dev, &caps);  
    if (phy_dev->cascade_prev) {
        phy_dev_caps_get(phy_dev->cascade_prev, &caps_parent);
        caps &= caps_parent;
    }
...
static int ethsw_cl45phy_get_caps(phy_dev_t *phy_dev, uint32_t *caps)
{
    ...
    *caps = PHY_CAP_AUTONEG;
    if (v16 & CL45_REG_CAP_2P5G_5G)
        *caps |= PHY_CAP_2500;
    if (v16 & CL45_REG_CAP_100MB_T)
        *caps |= PHY_CAP_100_FULL|PHY_CAP_100_HALF;
    if (v16 & CL45_REG_CAP_1GB_T)
        *caps |= PHY_CAP_1000_FULL|PHY_CAP_1000_HALF;
    if (v16 & CL45_REG_CAP_10GB_T)
        *caps |= PHY_CAP_10000;
